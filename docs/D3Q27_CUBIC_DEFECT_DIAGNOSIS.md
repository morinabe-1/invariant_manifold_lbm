...
**最終結果 — 2026-09-08:** 全960 caseの保存後監査で `passed / accepted`。
全10 validity・H1/H2/H3と独立192 caseとの全対応が通過した。元の3次数不合格・81半減不達・37悪化を再現した診断であり、元Q012gの棄却は保持する。
末尾の最終結果節に封印、全件数、未取得の元終了codeと別processの再監査、主張範囲を記録した。以下の事前登録条件は変更していない。
...
### mainの33³全320 caseの保存後監査 — 2026-09-08

同じPID 35240／handle 31939の33³も全96方向・320 caseを完了した。
192 profile、元1,920 field・640欠陥、全holdout64 caseのexact記録、全96 childのSHA/header/bodyを監査した。
H1/H2/H3・Gramは全て通過し、独立workerに対応する全21方向・64 caseのprimary科学的recordも厳密一致した。
mainはprimaryのみなのでgrid内の二方式vector比較は0、独立側との対応は登録subsetの64 caseに限る。
17³と合わせた監査済みmainは640 case、worker対応は128 caseであり、全960 caseの完了や全replay192 caseとの対応照合完了ではない。

main gridのnormalized SHA256:
`ba2976127868ae834dbe17f183b34ddf8ef0bc0d0ea1cbd3579ae43cece9adff`。
grid digest:
`6620536a216d25d44c20e3180b830de501d913b25e074ed4d68794f62fbf3789`。
全方向childのsealはこの外部封印済みgridに保存した。source digestは`c7001e41…`のまま不変である。

全64一般方向の元／予測fitを独立OLS式で照合し、旧H4の64成功を保持した。
傾き差最大`0.00017371130414334246`は`.01`以内、最小振幅比の相対差最大`0.00039331366732360686`は1%以内だった。
全64 holdoutのP9 vector相対誤差は`1e-3`以内で、欠陥比の半減／悪化判定も旧記録と一致した。
`.008`の32 caseは旧H5成功、`.032`の32 caseは旧H5不合格を維持した。
`.032`では全32方向が欠陥半減に不達で、方向2/4/7/16/23の5件は三次が二次より悪かった。
最大の方向16は元比`1.316172134164264`、予測`1.3161721341163626`である。

独立subsetに含まれない方向23・`.032`の元比`1.1349501394251245`も、予測`1.1349501393864596`で再現した。
同じW3欠陥の四次までの切断予測はvector相対誤差`0.412617433263631`、六次まででは`9.259700967342776e-5`、
九次まででは`1.1068671250188703e-9`だった。高次寄与を含めた診断であり、leading vectorだけの十分性や新しい高次chartではない。
先頭係数norm同士の比からは`1.0046251188932527`となるが、これは記述量であり、実欠陥比や受理gateを置き換えない。

全4,160切断Gram norm²を補償和でも照合した。全640 sampleの再構成差最大はholdout方向9・degree2・`.008`のPhi再構成で
`1.5381118046148136e-14`、対応する元floorは`1.4882165908113665e-12`だった。他の全再構成比較も各caseの元floor以内である。
[main N33の保存後検査](../tests/test_q012g3_cubic_defect_main_n33_artifact.py)の14件が警告error化で通過した（33.74秒）。
全coverage・child・worker対応・OLS・holdout・Gram・summaryに加え、方向23削除、旧H5／比書換え、末尾C9欠落、tail／末尾座標改変を拒否した。
main N17の13件とworker N33の11件も通過した（49.24秒）。計38件を検証し、Ruff・整形・compileallも通過した。
追加artifact検査は正式177対照に組み込まず、稼働中のscientific sourceを変更していない。
同じmainは65³へ進んでいる。全960 case・全worker対応・終了時入力/source・最終親監査まではQ012g3全体を未判定とする。
元Q012gの棄却を保持し、悪化の再現を悪化の解消とせず、SSM存在、連続球・grid-uniform半径、TT優位性にも一般化しない。

### mainの65³・登録反例34の保存後監査 — 2026-09-08

同じPID 35240／handle 31939で一般方向34の全4振幅が保存された後、別途監査した。
2 profile、元24 field・8欠陥、H1/H3・Gramは全て通過し、封印済みworkerの同じ方向の全primary科学的recordと厳密一致した。
main側はprimaryだけなので、この4 case内での二方式vector比較は0である。独立側の73比較は先に検証済みのworkerの範囲であり、今回追加計算した件数ではない。
main方向artifactのnormalized SHA256は
`49f71e876eb782e43086700fc777792df55d25303a9dd936588e4b44cbab6341`。
source digestは`c7001e41…`のまま不変である。

元三次傾き`4.103841695537577`を九次予測`4.103894349187252`で再現した。
最小振幅比は元`0.0801557831453807`、予測`0.08014626737054589`である。
独立OLS式で元／予測fitを照合し、H3を満たす一方で旧H4の不合格も正しく保持した。
同じW3欠陥の四次までの予測傾きは`3.9999999998584626`、五次まで`4.077688458854748`、六次まで`4.1038942623554435`だった。
有限振幅窓での高次寄与による傾き超過の再現であり、W4/W6/W9の構築、漸近次数の変更、G3単独への原因帰属ではない。

[main反例34の保存後検査](../tests/test_q012g3_cubic_defect_main_counterexample34_artifact.py)の追加10件と、既存worker反例34・65³一般方向の19件、計29件が警告error化で通過した（8.00秒）。
全52切断Gram norm²の補償和、旧判定の成功への書換え、C9／末尾case欠落、末尾座標改変の拒否を含む。
これは65³の4 caseの部分監査であり、同格子の全320 caseやQ012g3全体の受理ではない。
正式177対照へ追加せず、同じmainの残る反例51/52・全方向・全worker対応・終了時照合・最終親監査を継続する。

### 最終結果 — main全960 caseの保存後監査（2026-09-08）

65³の全96方向・320 caseと最終親が保存された。11:17 JST時点でPID 35240の不在とhandle 31939の消失を確認した。
元のexit codeと`final_saved_audit` consoleは取得できなかったため、正常終了を推測しない。
別processで同じ`audit_document(..., worker=False)`を保存された最終親に対して実行し、全件成功・exit 0を確認した。
この検証は物理計算の再起動や既存artifactの上書きではない。

全三格子・288方向・960 case・576 profile、元5,760 field・1,920欠陥を全件監査した。
全288方向childのheader/body/SHA、三gridの全保存後audit、最終親のdigest、入力・sourceの開始前後と現在の一致を確認した。
holdout全192 case・1,152 fieldのexact保存量記録もQ012g1と一致した。一般方向の追加保存量gateとはしない。
保存された177人工対照はexit 0・177 passed（19.64秒）で、対照sourceの封印も一致した。
独立workerの全192 case・126 profile・4,599 vector比較を全監査し、全63方向・192 caseのprimary科学的recordがmainと厳密一致した。
main自身はprimaryのみなので、main内の独立vector比較数は0である。全960 caseが第二式で独立再計算されたと読まない。

外部封印（normalized SHA256）:

- main最終親: `40d5bbbee9a3a6c4c4330062a1eada7422567e478f2ae06b2c9851e409423a91`
- main evidence digest: `0a1288a65eb9211f26be17478b8425c39cff7481011c5ef2010081b6b698eee6`
- main65³ grid: `9f2fa6e80d3e2fcd9afe109fd6da8b0e9ea3b2c29bf1ec419b56299ced07b53a`
- main65³ grid digest: `5b7b7eb4225ba2f41102a692504665af1704c793337b2800efef136c2e0e9900`

main17³/33³、worker最終親、全scientific sourceの外部封印は前節の値から不変である。

全10 validity・H1/H2/H3が通り、**Q012g3の診断を `passed / accepted` とする**。
旧Q012gの不合格は次のとおり全件再現した。

| 格子 | main case | 旧H4の不合格方向 | 旧欠陥半減不達case | 旧三次悪化case |
|---|---:|---:|---:|---:|
| 17³ | 320 | 0 | 0 | 0 |
| 33³ | 320 | 0 | 32 | 5 |
| 65³ | 320 | 3 | 49 | 32 |
...
半減不達・悪化は全192 holdoutの欠陥比だけの判定であり、旧H5 compositeを新たに受理した結果ではない。
旧H5の成功数は17³/33³/65³で64/32/0のままである。
65³の`.008`では17/32 caseが半減不達、`.032`では32/32 caseが二次より悪い。
旧H4反例51は元傾き4.102353999806372→予測4.10242111001829、反例52は元比0.10292984357429616→予測0.10292376971677472だった。
方向52は傾きが区間内であり、方向34/51と同じ傾き超過としてまとめない。

全192 generic fitを独立OLS式で再計算した。元との傾き差最大は0.00024180368661497198、最小振幅比の相対差最大は0.0005470464888657247で、登録上限内だった。
全12,480切断Gram norm²を補償和で確認した。
全1,920 degree別sampleの再構成差最大は65³・一般方向46・degree3・`.002`のPhi再構成4.244566574097749e-14で、元floor4.1140092794350784e-12以内だった。
他の全case・全比較も各々の元floor以内である。

最悪例65³・holdout方向21・`.032`の旧欠陥比5.834017983080117を、九次予測5.834017984274533で再現した。
同じW3欠陥の四次までの予測はvector相対誤差約91.70%、六次まででは6.58493e-5、九次まででは約3.28809e-10だった。
四次項だけで全欠陥を説明せず、五次以上の寄与・符号付き干渉・有理式の残りを区別する。
方向0・`.008`の先頭係数比による半減誤分類もmainで再現した。先頭norm比を全欠陥比のgateに置き換えない。

[main全体の保存後検査](../tests/test_q012g3_cubic_defect_main_artifact.py)の18件が警告error化で通過した（169.49秒）。
fresh入力chain、全child、全worker対応、OLS、Gram、旧不合格、C9／tail／末尾座標の改変拒否を含む。
既存main二格子・反例34・worker65³一般方向の47検査も73.81秒で通過した。計65件を確認し、Ruff・整形・compileallも通過した。
追加artifact検査は正式177対照に含めず、封印した科学的実装も編集していない。

今回閉じた問いは、**既存W2/W3の不変性欠陥を再構成し、登録した次数・有限振幅失敗を再現できるか**である。
元特殊288 case・null fitは本問の対象外のまま保存し、元Q012g全1,248 caseの `passed / rejected` は変更しない。
新しい高次chart、SSM存在・一意性、連続球、grid-uniform実用半径、長時間trajectory、TT優位性は未認証である。
次は高次補正の必要性・四次構築の可否と費用を事前評価し、自然Fourier sparseのW2/W3をbaselineとして保持する。
診断の受理だけで高次chartを自動的に成功扱いしない。
