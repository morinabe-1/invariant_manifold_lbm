...
**封印結果: passed / accepted。** 全三格子・246,480組・578,760列、11 validity gateと2仮説が通過した。
全結果・保存後監査・適用範囲は末尾に記す。以下の事前登録と途中の観測記録は履歴として保持する。
...
## 全三格子の封印結果 — passed / accepted

主計算は同じsource・processのまま正常終了した。全11 validity gate、H1/H2とも通過し、
本問の有限次数の数値preflightを受理する。途中の二格子だけで行った判定ではない。

| 指標 | 17³ | 33³ | 65³ | 登録上限 |
|---|---:|---:|---:|---:|
| 完了block triple | 82,160 | 82,160 | 82,160 | 全数 |
| 完了Taylor column | 192,920 | 192,920 | 192,920 | 全数 |
| candidate不合格 | 0 | 0 | 0 | 0 |
| 元float64外部残差の不合格 | 0 | 0 | 17 | 別列で保持 |
| R64ベクトルのexact normでの不合格 | 0 | 0 | 17 | 別列で保持 |
| condition最大 | 39414.9040331 | 500430.0734275 | 7246918.3125850 | 1e8 |
| exact相対外部残差最大（表示値） | 1.39634703561228e-13 | 2.0521811084050202e-12 | 3.079011002599421e-11 | 1e-10 |
| full-population相対残差最大 | 6.990591908581969e-13 | 9.368784158656308e-12 | 1.1699491177173276e-10 | 1e-9 |
| 固定葉・構造のscaled誤差最大 | 4.4591966249596965e-15 | 4.667724332247319e-15 | 4.576149665470201e-15 | 5e-12 |
| H3共役誤差最大 | 1.1147904145972829e-12 | 1.3635071648500198e-11 | 8.808028117046897e-11 | 1e-8 |
| forcing共役誤差最大 | 3.500196505837025e-16 | 3.61003281690433e-16 | 3.2160412964769017e-16 | 1e-8 |
| R3共役誤差最大 | 9.899056296961976e-16 | 9.94418790020395e-16 | 1.633987968714588e-15 | 1e-8 |
| 独立物理forcing相対誤差最大 | 3.990206655849577e-15 | 1.0179528866498556e-14 | 2.8019891843487965e-14 | 1e-8 |
| raw三次応答norm最大 | 2700.4656950 | 22509.4451252 | 176601.8717057 | 診断値 |

exact残差の表はexact分母での近似表示値である。保存分母・exact分母・十進定数の全3 gateは
二乗normの有理数比較で全246,480組が通過した。R128格納値との照合も全件で登録`1e-24`以内。
大きいconditionとraw応答normを保存し、この三点だけからgrid-uniform性や漸近指数を主張しない。
normはraw三次微分・orthonormal Fourier global-l2規約で、Taylor fiberの格納値や局所振幅と混同しない。

### 残った元の17件と独立性

元float64の不合格は全て65³の`shear/shear/shear`で、ordinalは次の17個である。

```text
903, 2205, 17786, 18794, 19190, 19208, 32608, 33385, 33511,
33718, 45627, 50764, 54758, 55169, 63622, 69288, 71731
```

全件が封印したQ012f1のpaired/refined選択行に一致し、Q012f1aのexact proofも同一だった。
R64のnormだけをexactにしても不合格で、残差積和をexactにすると同じ元分母でも通る。
新たな未選択のfloat64不合格やcandidate不合格はなかった。元Q012f/Q012f1の棄却は保持する。
SVDの例外時代替は65³のordinal `45729, 55289`の2件で、両方通過した。
これは正常終了した残差失敗を救済するためのbackend切替ではない。

全9,243二次pairをfreshに再構築・再検証し、全三次forcingと各格子8方向の独立物理式が通過した。
固定324組/格子・計972組の別process再計算では、入力、SVD参考値、解・応答、固定3回補正history、
GMP有理数残差のproofが主計算と全件一致した。全246,480組の別process再実行ではない。
H3への事後的共役投影や、虚部を捨てた後の合格判定は行っていない。

31実装テストと16artifactテスト、ruff、format checkは通過した。16件の保存後監査は、
全246,480行の有理数判定・集計と全578,760列の座標対応・hash・波数・共役対応を覆う。
各格子のexact残差・condition・full残差の最悪例をfresh/GMPで再構築し、全24方向の物理forcingも再計算した。
source sealと過去の棄却を確認し、workerを1件欠落させてdigestを再計算しても受理しないことを検証した。

```powershell
python -m pytest tests/test_d3q27_refined_cubic.py tests/test_d3q27_refined_cubic_artifact.py -q
```
...
| 実行時の記録 | 17³ | 33³ | 65³ |
|---|---:|---:|---:|
| wall秒 | 1158.7171156 | 1157.5888819 | 1397.2933975 |
| CPU秒 | 1299.828125 | 1300.21875 | 1554.0 |
| record gzip bytes | 79,165,353 | 79,613,292 | 80,211,579 |
| sparse fiber NPZ bytes | 152,490,290 | 153,374,815 | 153,774,786 |

このwall時間には別検証processとの同時実行を含む。preflightの実費であり、専用環境でのgrid scaling、
TT対sparse、online評価速度のbenchmarkではない。bytesは圧縮ファイルの保存量で、実行時メモリではない。
17³/33³のsealは上に保持し、65³と全体のsealを以下に加える。

```text
main JSON normalized SHA256:
65a0046d2b54c1192a84fbce481da0e804364cf62566d2e9c18204e372158ec1
main scientific result digest:
ca64e1235a4dc94a4736bc051f9bbd780835a77d3ed0718bf886f93064c1ef7f
65³ grid JSON normalized SHA256:
537aa84783c33d8d280ecc690f22fb0d32addf718f6ffe72ff3e876591dcfda5
65³ grid scientific result digest:
6e3ca03e59f74a9a0bc8b5633beef5ffe16cb87f2b20377d94ea025c19398980
65³ record gzip byte SHA256:
a6335cc6b13e122b9f74f8d52ccfaace69fe65ffb14f916982489f6357d2128a
65³ NPZ byte SHA256:
31133a6e46149371ec35c5f5579c2f3ccb50ec50a194be05716238e33f93da2e
```

### 結論と次の問い

入力・solve精度・残差評価を分離して得た修正候補は、全三格子・全三次組でも登録した基準を通過した。
exactという語は**丸め済み外部行列A/D/F/Xの残差**に限る。LBM symbolの厳密性、SSM存在、
連続球やgrid-uniform半径、三次W/Rの評価の正しさ、有限振幅改善、TT優位性は未認証である。
次のQ012gで封印fiberを実座標の三次W/Rとして評価し、独立対照、欠陥次数`3 → 4`、有限振幅比較を行う。
自然Fourier sparse baselineを保持し、元の棄却結果や未検証の課題を成功へ読み替えない。
