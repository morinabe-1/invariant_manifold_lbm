...
## H1正式結果 2026-09-08 — 全件完了、独立F4一致は不合格

source `71bedc93c11ccfb1a39bf2447e49e19cec1e699c`で全三格子の**2,094 block組・5,478対称列**を計算した。
親Q012h1とS0の全数再監査、全lower-order入力照合を通過し、全7項・5変異・独立F4を保存した。
格子ごとの保存後全entry再計算と、主/独立方式の全列比較が完了し、mainの実際のexit 0を確認した。
別CLIの全入力・全entry再監査も完了し、実際のexit 0と同じH1不合格判定を確認した。
この監査は旧保存物を上書きせず、三格子×二方式の全F4と全60物理照合列をfresh入力から再構築した。

**H1の元の相対差上限1e-8は、514列が不合格**だった。小さい値を0とみなしたり、floorを変更したりしない。
全列を別記述の補償和normで再計算し、全ての列別誤差と元の判定を再現した。

| 格子 | 不合格列/1,826 | 不合格のうちfloor未満 | floor以上の不合格 | 最大F4相対差 | 物理照合20列の最大相対差 |
|---|---:|---:|---:|---:|---:|
| 17³ | 182 | 174 | 8 | 7.132708315656039e-5 | 3.7440674110800584e-15 |
| 33³ | 174 | 174 | 0 | 3.4151436387255513e-6 | 9.372574837385335e-15 |
| 65³ | 158 | 158 | 0 | 1.9470194064750898e-7 | 1.6706767133667565e-14 |

各格子でF4 normが1e-14未満の列は174列。65³ではそのうち16列は元の相対差gateを通過している。
17³の最大例はgroup `(27,27,27,29)`・column 0（共役にも同値）。参照F4 normは約3.7411e-16、絶対差は約7.1327e-19だった。
一方、floor以上の8不合格は `(3,21,21,72)`と`(3,54,54,72)`の各4列で、参照normは最大約1.0523e-10。
これらではcollision側の差がF4の差とほぼ等しく、composition側の差は約1e-29以下だった。
これは**差の位置の観測**であり、moment形成の丸めだけが原因と証明した結果ではない。

値を見て選び直していない物理照合20列/格子は全60列が、全off-wave FFT係数の漏れ込みで通過した。
5変異も各格子で実際の非零検出例を得た。ただし、これらの通過は残りの列の独立一致を保証せず、514不合格を打ち消さない。

正式6 processのPIDは17³の27204/28868、33³の33304/18444、65³の29588/35644で、全て実際のexit 0を取得した。
最大working setは**1,229,852,672 bytes**、最大private commitは**2,526,855,168 bytes**（約2.35 GiB）で、各3 GiB上限以内。
最長grid/routeは65³ workerの**576.3324秒**。全入力・物理計算・保存・再計算を含むH1の測定であり、H2 solveを含むH3ではない。
正式run内の66対照テストは3.07秒で通過。補償和auditの7件と実artifactの8件、計15件も31.40秒で通過した。
最後に新規・保存物・先行回帰の計**212テスト**を再実行し、53.67秒で通過した。Ruff・format・compileも検査した。

- [H1 manifest](../research/artifacts/q012h2_d3q27_quartic_forcing.json)のファイルSHA256:
  `b2ec4ed701a677c6ddbca1016b45c3de3d4861ee9c20d98592bfb262e3d58df6`。
- [全5,478列の補償和再集計](../research/artifacts/q012h2_d3q27_quartic_forcing_diagnostics.json)のファイルSHA256:
  `6f6c72eef1f8d93f6083ae0e88ce52a205daacad880e163df46bb913600004a3`。audit sourceは`3f3550f`で固定した。

H1 manifest、三格子×2 routeのJSON/ZIP、再集計の計14ファイルは**41,470,266 bytes**。
判定は`h1_passed=false`。H2およびsolveを含むH3が未実行なので、Q012h2全体は`not_evaluated`を保持する。
検証評価は **Needs revision（F4一致の不成立）**。全数で確認した不合格として共有できるが、四次solveの入力受理ではない。
次の[Q012h2a算術精度診断](D3Q27_QUARTIC_FORCING_PRECISION.md)を`4ffbbb0`で事前登録した。
同じ全698組/格子についてGMP 128/192 bitの二方式とmoment-only armを比較し、旧514列を全て残す。
Q012gの悪化の修復、全四次chart、SSM存在、自然疎表現に対するTTの優位性は引き続き未検証である。
