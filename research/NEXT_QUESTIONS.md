...
## Q011ac: degree-9 uniform refined-envelope indexed-modulus audit — 事前登録

### 問い

Q011uでdegree 9に残った5 modulus-overlap aggregateについて、uniform transformed-residual envelopeを
external group 164まで拡張し、全可換coordinate monomialとFourier-sector-compatible targetをexact
indexed modulusだけで分離できるか。

Q011uのdegree-9 inventoryは

- aggregate: `220 = 215 old-modulus-separated + 5 overlap`
- expanded product control: `2002`
- overlap／external group:
  - `[0,0,0,9] / 177`
  - `[0,0,1,8] / 177`
  - `[0,0,2,7] / 177`
...
である。Q011acは5 overlapを個別indexへ展開し、Q011uの215 separationと合わせてdegree 9全体を判定する。

### uniform refined envelopeと新external group

半径はQ011z／Q011aa／Q011abと同じ
...
group 0／1／2／3の24 identifierと、external group 177／167／164の`8 / 8 / 20` identifierである。
3 external groupは互いにdisjointなので、unique externalは`36`、直接必要なidentifierは`60`である。

証明書列の単調性を保つため、Q011abの36 uniform recordもexactに保存する。うち32 recordは今回の集合と
重なり、今回直接使わないexternal group 178の4 recordを追加保持するので、uniform envelope全体は`64`
identifierとする。group 164のsector histogramは`0:2 / 1:4 / 4:5 / 13:5 / 16:4`である。

### sealed input

Q011abで直接照合した6 artifact、runner、33 digest、outcome、claim boundaryとQ011l／Q011o sourceを
同じ固定値で再照合し、Q011abも直接封印する。

- Q011ab artifact／runner newline-normalized SHA-256:
  `18a6e146d0d24af9e9bab22f45668bec6c5bc25efcdb05faee2a7228e1870a98` /
  `dc4511ea76419b065c5de7b84d02e040b98f50c31e863b6232afdfaedf0f677b`
- Q011ab input／inventory／sector／product／result digest:
  `cbb93d239d4f23ec2ca3fb4dd2bb1bdc89af5756c5f2918ccb303c3442f233d9` /
  `dd22f821362306d7c7bf9513a8f990253134f989a997ec5fb2677b6a1b9ad967` /
  `3d523a17677d993113d9d17a515485db00759e490514bdbe287c6927c6525c9f` /
  `c12cee86ac5e4a9aa56a9e86996eb40756c3d327a9838aba5443b67a42177663` /
...
\[
11440,\ 25740,\ 34320,\ 4320,\ 11440,
\qquad\text{total }87260
\]

へ固定する。output wave histogramは次とする。

- `[0,0,0,9]`:
  `0:1624 / 1:1484 / 2:1204 / 3:894 / 4:604 / 5:374 / 6:204 / 7:98 / 8:46 / 9:46 / 10:98 / 11:204 / 12:374 / 13:604 / 14:894 / 15:1204 / 16:1484`
- `[0,0,1,8]`:
  `0:3794 / 1:3499 / 2:2804 / 3:2014 / 4:1304 / 5:754 / 6:380 / 7:159 / 8:59 / 9:59 / 10:159 / 11:380 / 12:754 / 13:1304 / 14:2014 / 15:2804 / 16:3499`
- `[0,0,2,7]`:
  `0:5200 / 1:4800 / 2:3840 / 3:2696 / 4:1672 / 5:914 / 6:424 / 7:162 / 8:52 / 9:52 / 10:162 / 11:424 / 12:914 / 13:1672 / 14:2696 / 15:3840 / 16:4800`
- `[0,7,0,2]`:
  `0:320 / 1:394 / 2:304 / 3:354 / 4:256 / 5:274 / 6:176 / 7:154 / 8:88 / 9:88 / 10:154 / 11:176 / 12:274 / 13:256 / 14:354 / 15:304 / 16:394`
- `[9,0,0,0]`:
  `1:1960 / 3:1680 / 5:1200 / 7:660 / 8:220 / 9:220 / 10:660 / 12:1200 / 14:1680 / 16:1960`

external group 177のsectorは`0:4 / 3:2 / 14:2`、167は`0:4 / 4:2 / 13:2`、164は
`0:2 / 1:4 / 4:5 / 13:5 / 16:4`である。exact Fourier law

\[
b_{\mathrm{out}}=(b_1+\cdots+b_9)\bmod17
\]

によりcompatible comparisonを`10072 / 23232 / 31584 / 2304 / 15680`、合計`82872`へ固定する。
...
\[
R=\prod_{i=1}^{9}(u_i+\rho)-\prod_{i=1}^{9}u_i,
\qquad
I_p=\left[\max\left(0,\prod_i\ell_i-R\right),\prod_i u_i+R\right]
\]
...
事前登録用のexact-rational pilotでは全82872 comparisonがindividual modulusで分離し、unresolvedは0だった。
relation countはproduct below target `57120`、target below product `25752`である。aggregate別minimum gapは

- `[0,0,0,9]`: `5.620482708927926e-5`
- `[0,0,1,8]`: `6.297629130186793e-6`
- `[0,0,2,7]`: `6.974283071507663e-5`
- `[0,7,0,2]`: `7.648737435877863e-4`
- `[9,0,0,0]`: `4.589058211328851e-4`
...
1. Q011k／Q011u／Q011x／Q011y／Q011z／Q011aa／Q011ab artifact、runner、38 digest、outcome、
   claim boundaryとQ011l／Q011o sourceを再現する。
2. degree-9の220 aggregate、2002 expanded control、215 separation、5 overlap tuple／external groupを再現する。
3. 全\(\theta_b\le\rho\le r_{\mathrm{old},b}\)、60 directly relevant interval、64-record monotone
   envelope、Q011ab 36 recordのexact preservationを再現する。
4. group membership、87260 monomial、wave／target histogram、82872 compatible comparisonを再現する。
5. 全product interval、target interval、strict gap、framed exact-record digestを再現する。
6. Q011uの215 separationと5 refined full auditが220 aggregateを重複なく被覆することを再現する。
7. finite strict JSON、input／inventory／sector／product／result digest、runner provenanceを再現する。

一つでも落ちれば`inconclusive`とし、degree-9 nonresonanceを主張しない。

### hypothesis gateと停止規則

1. uniform \(\rho\)-discが全Q011y enclosureを含み、全Q011k旧discに含まれる。
2. 220 aggregate、87260 monomial、82872 comparisonがdegree 9を完全被覆する。
3. 全82872 comparisonがindividual modulus separationで、unresolvedが0である。
4. global minimum exact modulus gapが`>=5e-6`である。
5. Q011uの215 old separationと5 refined full auditからdegree-9 external nonresonanceが従う。
...
として`accepted`とする。certified degreesを2--9、tail-certifiedを91以降、missing rangeをdegrees 10--90へ更新する。

1件でもinterval overlapが残れば
`at least one degree-9 refined indexed-modulus product remains inseparable from an external target`
として`rejected`とし、最初のoverlapだけをexact complex phaseへ送る。

`accepted`ならQ011adでdegree 10へ進む。`rejected`ならQ011adで最初のremaining productをphase-sensitive
complex discへ送る。`inconclusive`なら最初のvalidity failureだけを修復する。

### 主張境界

本gateは固定17² repaired exact map、fixed conservation leaf、degree 9、Q011uの5 overlap aggregate、
Q011y transformed-residual enclosure、uniform \(\rho\)-disc、x-Fourier sector、indexed modulus product
formulaに限る。degrees 10--90、all-order nonresonance、higher graph smoothness、SSM existence／uniqueness、
normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

### Q011ac 実行結果

7 artifact、runner、38 direct digest、outcome、claim boundaryとQ011l／Q011o sourceを照合し、
validity `7 / 7`を通過した。degree-9 inventoryは
`220 aggregate / 2002 expanded control / 215 old-separated / 5 overlap`で事前登録どおりだった。

60 directly relevant identifierにQ011abから4 carry-overを加えた64-record monotone uniform envelopeを
再構成し、Q011abの既存36 recordがexactに保存されることを確認した。`87260` monomialを列挙し、
Fourier sectorで`82872` comparisonへ絞った。全比較がindividual modulusで分離し、unresolvedは`0`
だった。relation countはproduct below target `57120`、target below product `25752`である。
...
この結果はdegree 9だけのcertificateであり、degrees 10--90、all-order nonresonance、higher graph
smoothness、SSM existence／uniqueness、normal attraction、basinは未認証である。停止規則どおり、
...
8 artifact、runner、43 direct digest、outcome、claim boundaryとQ011l／Q011o sourceを照合し、
validity `7 / 7`を通過した。degree-10 inventoryは
`286 aggregate / 3003 expanded control / 281 old-separated / 5 overlap`で事前登録どおりだった。

56 directly relevant identifierに新external group 165とQ011ac carry-overを統合した68-record monotone
uniform envelopeを再構成し、Q011acの既存64 recordがexactに保存されることを確認した。`339960`
monomialを列挙し、Fourier sectorで`466872` comparisonへ絞った。全比較がindividual modulusで分離し、
unresolvedは`0`だった。relation countはproduct below target `44232`、target below product `422640`である。
...
で、global minimumは登録下限`5e-6`を通過した。minimum witnessは
`(block=16;center=151)^6 × (block=1;center=151)^2 × block=0;center=149 × block=1;center=152`
対target `block=14;center=146`だった。
...
この結果はdegree 10だけのcertificateであり、degrees 11--90、all-order nonresonance、higher graph
smoothness、SSM existence／uniqueness、normal attraction、basinは未認証である。停止規則どおり、
...
事前登録どおり9 artifact、runner、48 direct digest、Q011l／Q011o sourceを封印し、validity `7 / 7`を
通過した。degree-11 inventoryは

`364 aggregate / 4368 expanded control / 350 old-separated / 14 overlap`

だった。52 directly relevant identifierを再構成し、Q011adの68 recordをexactに保存する84-record
monotone uniform envelopeを得た。unique center-modulus evaluationは50件だった。

14 overlapの全2299104 monomialをsingle passで列挙し、compatible／incompatibleを
`383062 / 1916042`へ分割した。Fourier-compatible comparisonは820492件だった。末尾3 aggregate
`[9,2,0,0] / [10,1,0,0] / [11,0,0,0]`はexternal group 161のtarget sectorと交わらず、product
intervalを作る前にexact Fourier structureだけで除外された。残る383062 compatible monomialだけに
product intervalを作り、全820492 comparisonを評価した。完全なmonomial／pair／product／comparison
listは保持せず、boundary、histogram、extrema、witnessと4 framed digest stateだけを保持した。

全820492 comparisonがindividual modulusで分離し、unresolvedは0だった。relation countはproduct below
target `630112`、target below product `190380`である。global minimumはaggregate `[0,7,2,2]`の

`(block=16;center=151)^6 × block=1;center=151 × (block=1;center=152)^2 × (block=0;center=147)^2`

対target `block=14;center=146`、relation `product_below_target`で、gap
...
- uniform-record digest:
  `b099c8294d2ff14bbf36886f085e39cddffaaf2220b9c0e47206404cee023391`
- monomial／pair／product／comparison framed digest:
...
本certificateはdegree 11に限られ、degrees 12--90、all-order nonresonance、higher graph smoothness、
...
事前登録どおり10 artifact、runner、53 direct digest、Q011l／Q011o sourceを封印し、validity `8 / 8`を
通過した。degree-12 inventoryは

`455 aggregate / 6188 expanded control / 426 old-separated / 29 overlap`

だった。60 directly relevant identifierを再構成し、Q011aeの84 recordをexactに保存する92-record
monotone uniform envelopeを得た。unique center-modulus evaluationは56件だった。

29 overlapの全36596091 monomialを、selected groupごとの`4 / 2 / 3 / 6` exact modulus classと17-sector
Fourier multiplicityへ分割し、213618 modulus signatureへ圧縮した。compatible original monomial／product
signatureは`6904665 / 184154`だった。weak-composition fiber cardinalityをexactに保持し、Q011aeの
degree-11 full-stream oracleへ同じ圧縮を適用して全14 aggregateのmonomial count、wave histogram、
compatible count、comparison countを完全再現した。

1116256 distinct comparisonは13980960 weighted comparisonを重複・欠落なく表し、unresolvedは両方とも
0だった。weighted relation countはproduct below target `7676904`、target below product `6304056`、
distinct relation countは`648656 / 467600`だった。global minimumはaggregate `[0,6,2,4]`、source

`(block=16;center=151)^5 × block=1;center=151 × block=0;center=149 × block=1;center=152 × (block=0;center=147)^4`

対target `block=14;center=146`、wave multiplicity `2`、relation `product_below_target`で、gap
...
- class／histogram／signature／pair digest:
  `269187f8489521c7e37ae8a91669b9dc020ac10d4ef1d42272bb636fa7bc9b8c` /
...
- product／comparison framed digest:
...
本certificateはdegree 12に限られ、degrees 13--90、all-order nonresonance、higher graph smoothness、
...
Q011afまでの11 artifactと58 direct digestを封印し、validity `9 / 9`を通過した。degree-13 inventory
`560 / 8568 / 516 / 44`を再現し、68 directly relevant identifierと、Q011afの92 recordをexactに
保存してgroup 156の8 recordだけを加えた100-record uniform envelopeを構成した。

44 overlapの全218102520 monomialを1116561 modulus signatureへexactに分割した。1004653 compatible
signatureは38119852 original monomialを表す。pair-factorized巡回畳み込みで162 coefficient matrixを
exact `int64`評価し、44 outward-dyadic bound matrixと340 classification matrixにより6290384 distinct
comparison（77400104 weighted comparison）を評価した。weighted relationは`47068304 / 30331800 / 0`、
distinct relationは`3874124 / 2416260 / 0`で、全比較がstrictに分離した。Q011afのdegree-12 certificateも
同じ外向き実装でoracleとして完全再現した。

outward global lower boundは`5.793046996660499e-6`だった。近傍138件のexact Fraction refinementは
global minimum `5.793047003290255e-6`と2 tieを再現した。canonical witnessはaggregate `[0,5,2,6]`、
class count `[[0,0,0,0],[0,5],[0,0,2],[0,0,0,6,0,0]]`、source
`(block=16;center=151)^5 × (block=1;center=152)^2 × (block=0;center=147)^6`、target
`block=14;center=146`、wave multiplicity `3`、relation `product_below_target`である。
...
- uniform／class／outward-base digest:
...
本certificateはdegree 13に限られ、degrees 14--90、all-order nonresonance、higher graph smoothness、
SSM existence／uniqueness、normal attraction、basinは未認証である。停止規則どおり、次はQ011ahで
degree 14を監査する。

## Q011ah: degree-14 batched outward-dyadic Fourier／modulus audit — 事前登録

### 問い

Q011uでdegree 14に残った63 modulus-overlap aggregateについて、Q011agのpair-factorized exact
Fourier multiplicityとoutward-dyadic product enclosureを独立runnerで再構成し、degree-14 external
nonresonanceを認証できるか。

Q011uのdegree-14 inventoryは`680 aggregate / 11628 expanded control / 617 old-separated / 63 overlap`
である。overlap tuple／external groupを次へ固定する。

- `[0,2,0,12] / 167`, `[0,2,1,11] / 167`, `[0,2,2,10] / 167`
- `[0,4,0,10] / 165`, `[0,4,1,9] / 165`, `[0,4,2,8] / 165`, `[0,4,3,7] / 165`
- `[2,5,2,5] / 162`, `[2,5,3,4] / 162`, `[2,5,4,3] / 162`, `[2,5,5,2] / 162`,
  `[2,5,6,1] / 162`, `[2,5,7,0] / 162`
- `[3,4,0,7] / 162`, `[3,4,1,6] / 162`, `[3,4,2,5] / 162`, `[3,4,3,4] / 162`,
  `[3,4,4,3] / 162`, `[3,4,5,2] / 162`, `[3,4,6,1] / 162`, `[3,4,7,0] / 162`
- `[3,7,0,4] / 159`, `[3,7,1,3] / 159`, `[3,7,2,2] / 159`, `[3,7,3,1] / 159`,
  `[3,7,4,0] / 159`
- `[4,3,0,7] / 162`, `[4,3,1,6] / 162`, `[4,3,2,5] / 162`, `[4,3,3,4] / 162`,
  `[4,3,4,3] / 162`, `[4,3,5,2] / 162`, `[4,3,6,1] / 162`, `[4,3,7,0] / 162`
- `[4,6,0,4] / 159`, `[4,6,1,3] / 159`, `[4,8,2,0] / 157`
- `[5,2,0,7] / 162`, `[5,2,1,6] / 162`, `[5,2,2,5] / 162`, `[5,2,3,4] / 162`,
  `[5,2,4,3] / 162`, `[5,2,5,2] / 162`, `[5,2,6,1] / 162`, `[5,2,7,0] / 162`
- `[5,7,0,2] / 156`, `[5,7,1,1] / 156`, `[5,7,2,0] / 156`
- `[6,1,0,7] / 162`, `[6,1,1,6] / 162`, `[6,1,2,5] / 162`
- `[6,6,0,2] / 156`, `[6,6,1,1] / 156`, `[6,6,2,0] / 156`
- `[7,5,0,2] / 156`, `[7,5,1,1] / 156`, `[7,5,2,0] / 156`
- `[8,0,6,0] / 161`
- `[8,4,0,2] / 156`, `[8,4,1,1] / 156`, `[8,4,2,0] / 156`
- `[9,1,2,2] / 158`, `[9,1,3,1] / 158`

exact inventory canonical digestは
`3ef339e30f7a2a531d4dc83e83a4d5f33d505a23e41d3cc29c7b41afa206e1b6`とする。

### fixed multiplicity counts

Q011agと同じ`4 / 2 / 3 / 6` modulus class、group 0／1およびgroup 2／3のexact cyclic convolution、
target-sector `int64` matrix productを使う。group-signature recordは19440件、coefficient matrixは232件、
最大wave coefficientは548、最大live aggregateは254016 signatureとする。

63 aggregateの元monomial数は

`503880 / 1272960 / 1944800 / 680680 / 1601600 / 2252250 / 2402400 / 15966720 / 13305600 / 8467200 / 4064256 / 1354752 / 241920 / 14414400 / 28828800 / 33264000 / 27720000 / 17640000 / 8467200 / 2822400 / 504000 / 4752000 / 6912000 / 5184000 / 2304000 / 504000 / 22651200 / 45302400 / 52272000 / 43560000 / 27720000 / 13305600 / 4435200 / 792000 / 9147600 / 13305600 / 544500 / 27181440 / 54362880 / 62726400 / 52272000 / 33264000 / 15966720 / 5322240 / 950400 / 3421440 / 3041280 / 950400 / 23557248 / 47114496 / 54362880 / 5189184 / 4612608 / 1441440 / 6918912 / 6150144 / 1921920 / 540540 / 8108100 / 7207200 / 2252250 / 16473600 / 7321600`

でtotal `893043240`、modulus-signature数は

`18564 / 39312 / 54054 / 15015 / 30030 / 38610 / 39600 / 90720 / 75600 / 50400 / 26460 / 10080 / 2160 / 79200 / 138600 / 151200 / 126000 / 84000 / 44100 / 16800 / 3600 / 20160 / 26880 / 20160 / 9600 / 2400 / 110880 / 194040 / 211680 / 176400 / 117600 / 61740 / 23520 / 5040 / 30870 / 41160 / 1890 / 133056 / 232848 / 254016 / 211680 / 141120 / 74088 / 28224 / 6048 / 9408 / 8064 / 2688 / 133056 / 232848 / 254016 / 12348 / 10584 / 3528 / 15120 / 12960 / 4320 / 4620 / 17325 / 14850 / 4950 / 55440 / 26400`

でtotal `4091730`とする。Fourier-compatible signature数は

`9636 / 20028 / 27486 / 7335 / 14890 / 19075 / 19640 / 80640 / 67200 / 44400 / 22860 / 8400 / 1680 / 67200 / 121800 / 134400 / 112000 / 74000 / 38100 / 14000 / 2800 / 20160 / 26880 / 20160 / 9600 / 2400 / 94080 / 170520 / 188160 / 156800 / 103600 / 53340 / 19600 / 3920 / 30870 / 41160 / 1890 / 112896 / 204624 / 225792 / 188160 / 124320 / 64008 / 23520 / 4704 / 9408 / 8064 / 2688 / 112896 / 204624 / 225792 / 12348 / 10584 / 3528 / 15120 / 12960 / 4320 / 2970 / 17325 / 14850 / 4950 / 55440 / 26400`

でtotal `3543001`、元monomialへ戻したcompatible multiplicityは

`111460 / 286160 / 443468 / 102190 / 244780 / 347546 / 373184 / 2730176 / 2272888 / 1456320 / 708920 / 241280 / 44008 / 2422080 / 4754880 / 5427840 / 4517680 / 2898400 / 1414800 / 483200 / 88640 / 882480 / 1290080 / 965200 / 429440 / 92800 / 3729440 / 7309440 / 8337280 / 6938500 / 4454480 / 2177380 / 744920 / 137040 / 1718860 / 2510300 / 125070 / 4496192 / 8816112 / 10057888 / 8370464 / 5372736 / 2625376 / 897904 / 165080 / 865408 / 775040 / 238944 / 4000768 / 7860592 / 8976544 / 1324488 / 1187056 / 365480 / 1776480 / 1592768 / 490048 / 157520 / 2088750 / 1872740 / 576190 / 2874800 / 1254240`

でtotal `152292218`とする。

target multiplicityを戻したweighted comparison数は

`324728 / 838600 / 1305920 / 204380 / 489560 / 695092 / 746368 / 5460352 / 4545776 / 2912640 / 1417840 / 482560 / 88016 / 4844160 / 9509760 / 10855680 / 9035360 / 5796800 / 2829600 / 966400 / 177280 / 2525920 / 3660160 / 2758240 / 1217600 / 269920 / 7458880 / 14618880 / 16674560 / 13877000 / 8908960 / 4354760 / 1489840 / 274080 / 5006760 / 7251400 / 250140 / 8992384 / 17632224 / 20115776 / 16740928 / 10745472 / 5250752 / 1795808 / 330160 / 1730816 / 1550080 / 477888 / 8001536 / 15721184 / 17953088 / 2648976 / 2374112 / 730960 / 3552960 / 3185536 / 980096 / 315040 / 4177500 / 3745480 / 1152380 / 4257840 / 1852960`

でtotal `310135908`、実際に判定するdistinct signature／target comparison数は

`71052 / 150672 / 208848 / 29340 / 59560 / 76300 / 78560 / 506880 / 422880 / 280320 / 145920 / 54720 / 11040 / 430400 / 773600 / 844800 / 704800 / 467200 / 243200 / 91200 / 18400 / 103040 / 138240 / 104960 / 48640 / 12160 / 602560 / 1083040 / 1182720 / 986720 / 654080 / 340480 / 127680 / 25760 / 157780 / 211680 / 10080 / 723072 / 1299648 / 1419264 / 1184064 / 784896 / 408576 / 153216 / 30912 / 57344 / 50176 / 16128 / 723072 / 1299648 / 1419264 / 75264 / 65856 / 21168 / 92160 / 80640 / 25920 / 22440 / 105600 / 92400 / 29700 / 170720 / 80960`
...
big-endian digestというQ011agの包含契約を変更せず使う。Q011agのdegree-13 certificateをoracleとし、
`218102520 / 1116561 / 1004653 / 38119852 / 77400104 / 6290384`のcount、weighted relation
`47068304 / 30331800`、distinct relation `3874124 / 2416260`、unresolved `0 / 0`、minimum lower
`5.793046996660499e-6`とwitnessを再現する。

直接必要な集合はselected 24 identifierとexternal group 167／165／162／159／157／156／161／158の
56 unique identifier、合計80 identifierである。Q011agの100 uniform recordをexactに保存し、group 157／
158から12 recordだけを加える112-record monotone envelopeとする。unique center-modulus evaluationは
67件、uniform-record digestは`4d1402e2a71c2de26fa35d9fac739a0258b35b0a2873a95148fe5be4454b6ac3`
とする。

Q011agまでの入力を再照合し、Q011agを直接封印する。

- Q011ag artifact／runner newline-normalized SHA-256:
  `76c162133c228aecf988fb121dafc86c1dfae5c44cab465772534d2c863393fb` /
  `cf27aba440b291ebfba5f020a1cf77537335520cfdc9490290de836069cfd11f`
- Q011ag input／inventory／compression／product／result digest:
  `8a6156a2667469bd0c0666a344e79a04cbc761a3ab0bb134e0853baf6c432aa9` /
  `a901dedf2014c6d6738160890b09bc55118724324b6624494971c1688550f0d7` /
  `f117716264aac394b4e2de41bbbf5a0f5868a55bd3c12cc2f00afe4c87181e95` /
  `2b4b4d8315017fd162dcbf5aebeb4fd7c5c33ec34280564a36a9d8149f9a28f8` /
...
direct digest countは`63`とする。

design-only pilotでは、weighted relation countはproduct below target `204194352`、target below product
`105941556`、distinct relation countは`14247584 / 7643836`、unresolvedは双方0だった。全比較のminimum
certified lower boundは`5.650630797937594e-6`（`0x1.7b351910fffffp-18`）だった。

最小近傍278 comparisonのexact Fraction refinementでは、exact global minimumは
`5.650630805194988e-6`（`0x1.7b3519192bcd2p-18`）で、target `block=14;center=146`と対称な
`block=3;center=146`の2件がtieだった。canonical witnessはaggregate `[0,4,2,8]`、class count
`[[0,0,0,0],[0,4],[0,1,1],[0,0,0,8,0,0]]`、source

`(block=16;center=151)^4 × block=0;center=149 × block=1;center=152 × (block=0;center=147)^8`

対target `block=14;center=146`、wave multiplicity `2`、relation `product_below_target`とする。
...
- compact pilot／candidate-record／exact-minimum digest:
  `e473b5893927b2bf87665caa358f2a567caf608b8da715e9fd348a34a90c5bd0` /
...
1. Q011kからQ011agまでの12 artifact、runner、63 direct digest、outcome、claim boundaryとQ011l／Q011o sourceを再現する。
2. degree-14の680 aggregate、11628 control、617 separation、63 overlap tuple／external groupを再現する。
3. 80 directly relevant interval、112-record monotone envelope、Q011ag 100 recordのexact preservationを再現する。
4. 4／2／3／6 modulus class、weak-composition bijection、Q011ag outward-dyadic oracleを再現する。
5. 全893043240 monomial、4091730 signature、wave histogram、compatible multiplicity、weighted／distinct comparisonを再現する。
6. 19440 factor record、232 coefficient matrix、63 product-bound matrix、480 classification matrixと登録digestを再現する。
7. outward conversion／演算の包含、finite、順序、非負性、`int64`非overflowを再現する。
8. 全comparisonのstrict separation、minimum certified lower boundと278 exact refinement／2 tieを再現する。
9. 617 old separationと63 batched full auditが全680 aggregateを被覆し、strict JSON、section digest、runner provenanceを再現する。

### hypothesis gateと停止規則

1. uniform \(\rho\)-discが全Q011y enclosureを含み、全Q011k旧discに含まれる。
2. multiplicity compressionが元monomialを重複・欠落なく分割し、Q011ag certificateを外向きoracleで再現する。
3. outward-dyadic intervalが全exact Fraction product／target intervalを含み、整数係数にoverflowがない。
4. 全310135908 weighted comparisonを表す21891420 distinct comparisonがoutward intervalでstrictに分離する。
5. global certified lower boundが`>=5e-6`で、278候補のexact refinementが登録minimum／tieを再現する。
6. 617 old separationと63 batched full auditからdegree-14 nonresonanceが従う。
...
として`accepted`とする。certified degreesを2--14、tail-certifiedを91以降、missing rangeをdegrees 15--90へ更新する。

Q011ag oracle、integer exactness、外向き包含のいずれかが一致しなければ`inconclusive`とする。1件でも
outward interval overlapが残れば、exact refinementを許さず
`at least one degree-14 outward-dyadic indexed-modulus product remains inseparable from an external target`
として`rejected`とする。`accepted`ならQ011aiでdegree 15へ進む。

### 主張境界

本gateは固定17² repaired exact map、fixed conservation leaf、degree 14、Q011uの63 overlap aggregate、
Q011y transformed-residual enclosure、uniform \(\rho\)-disc、x-Fourier multiplicity polynomial、exact
modulus-class fiber、outward-rounded dyadic product enclosureに限る。degrees 15--90、all-order
nonresonance、higher graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／
force／wall、D3Q27を認証しない。
...
## Q011al: block-zero structured-row eigendisc clearance — 事前登録

### 問いと判定境界

Q011akで残った125440 comparisonは、source monomialがblock-0 identifierを2個以上含む場合に
限られた。この局在化を用い、block-common \(\theta_0\)の代わりにblock 0のeigenbasisと
entrywise exact family uncertaintyからrow-specific Gershgorin radiusを構成すると、Q011aj aggregate
index 99の全comparisonを分離できるか。

design-only pilotでは全141120 distinct comparisonがstrictに分離した。ただし、この成功は登録
obstructionだけを解消するもので、残り153 overlap aggregateを監査するまでdegree-16
external nonresonanceに昇格しない。登録結果が再現した場合のclassificationは

`the registered degree-16 obstruction is cleared by rigorously contained block-zero structured-row eigendiscs`

とし、scientific outcomeを`accepted`とする。Q011akのblock-common certificate棄却はそのまま保存する。

### sealed input

Q011akまでの16 artifact、runner、83 direct digest、outcome、claim boundaryとQ011l／Q011o sourceを
再照合する。Q011akは直接封印する。

- Q011ak artifact／runner newline-normalized SHA-256:
  `21d99ca113c5c71b573134d075dc0f01bdc03f0eeea4157dd1a62b1689e7ae59` /
  `140a4631da777450fbbe5eb3462579c9d2f5b64f3970eeda26511502ea1302f4`
- Q011ak input／radius／interval／obstruction／result digest:
  `710bcb6b724112bcf8f7c6cc166009accb176d325c5f6b2b911621d6a4b0ff42` /
  `2c6fc37c40262ba5b76121ca1e3655e1ecd760b88ad27063f932bda532ff62ab` /
  `25174294bfe13f1de5124793594bb14702617337d41f69e8ff9b43afd3c12386` /
  `984f9cd6c4d2db107385e500f64f89e242f2764a826d44f78bb56cbd283e3ec4` /
...
### structured-row radiusの証明式

block-0 exact familyを\(A\)、Q011kのbinary64 proposalを\(A_0\)、同じ決定的eigenbasisを
\(V\)、inverse candidateを\(W\)、diagonal centerを\(D\)とする。

\[
E=WV-I,\qquad R_0=A_0V-VD,\qquad
F=V^{-1}AV-D=(I+E)^{-1}W\{R_0+(A-A_0)V\}.
\]

Q011kの\(\varepsilon\ge\lVert E\rVert_\infty<1\)と
\(p\ge\lVert R_0\rVert_\infty\)を保存する。entrywise family radius
\(\Delta_{pq}\ge |(A-A_0)_{pq}|\)、directed-MPFRで上から評価した
\(|V_{qj}|\)と\(|W_{ip}|\)を用い、

\[
u_q=\sum_j|V_{qj}|,　
s_i=\sum_p|W_{ip}|\sum_q\Delta_{pq}u_q,　
b_i=\left(\sum_p|W_{ip}|\right)p+s_i,
\]

\[
\boxed{r_i=b_i+\frac{\varepsilon}{1-\varepsilon}\max_\ell b_\ell}
\]

とする。これは\(\sum_j|F_{ij}|\le r_i\)を上から抑えるため、actual block-0 spectrumは
\(\bigcup_iD(D_{ii},r_i)\)に含まれる。point residualのcancellationを用いず、Q011kの全体norm
\(p\)を各inverse row sumへ掛ける。

primary 256-bit／replay 384-bitのRoundUp protocolを使い、全binary64 inputをexactに変換する。
45000個の\(V/W\) absolute upperと22500個のentrywise family radiusを全件ハッシュする。

- primary／replay eigenbasis-absolute stream digest:
...
- entrywise-family stream digest:
  `8271e94081187efb1b864e43cd2425255217e3ac77238804b839bb44dd26b491`
- primary／replay 150-row record digest:
...
replayの150 radiusはprimaryにcoordinatewise strictly containedすることを登録する。primaryのNeumann
correction hexは`0x1.0a55a6c03e205p-71`、maximum row radiusはcenter index 34の
`0x1.331a87c57c090p-31`で、Q011y common \(\theta_0=\mathtt{0x1.5cbff506e79d2p-26}\)に含まれる。

selected center 144--149のprimary radius hexは順に

`0x1.ce2c730243e12p-35 / 0x1.d3b4549712506p-35 / 0x1.14c4118741b29p-34 /`
`0x1.087daaa7f30b9p-34 / 0x1.12db20142d612p-36 / 0x1.384c1f0aea1b4p-35`

とし、6 selected-row digestは
`247b63924428434d2f73ec7d8a74955126845e29678082e95f9f7a77b3e2bfed`とする。

### Gershgorin componentとselected clusterの個数

exact center distanceとprimary radiusから、2円板の非交差がexact squared-distanceで証明できない場合に
edgeを置く。150円板は73 componentとなり、selectedとexternalを混ぜるcomponentは0とする。
selected componentは

- component 70: `[144,146]`
- component 71: `[145,147]`
- component 72: `[148,149]`

の3個である。component-record digestは
`afd18241199f27fd43eaae643ed4c64b3d76e9ffad880a502fc9cf84354a4d5f`とする。selected／external間の
maximum-coordinate lower gapのminimumはcenter `149 / 143`の`0x1.900710b9c775fp-6` > 0、
gap-record digestは`ac6dd0cedd3230e10a9e0b054eeb354f3d7c239ca2dae974927a0dbd5c651bcc`とする。

Gershgorinのsecond theoremにより、他の円板componentから分離した\(k\)個の円板の和はexactly
\(k\)個の固有値を含む。従って上記3 componentは全selected block-0固有値を過不足なく含む。

### hybrid envelopeと登録obstruction

Q011akの204 identifierのうち、selected `block=0;center=144--149`の6件だけをprimary row radiusへ
置き換える。残り198件はQ011y blockwise radiusを保存する。全hybrid intervalは対応する
Q011ak blockwise intervalに含まれ、204-record digestは
`a7d56141743bc62f3684d54975ad7d74f6274b3a3da7078d6ce1ce7a31f8092e`とする。modulus classは
`4 / 2 / 3 / 6`のまま、membership digestも
`269187f8489521c7e37ae8a91669b9dc020ac10d4ef1d42272bb636fa7bc9b8c`のままとする。

aggregate index 99の35280 signature、1732864 compatible monomial、3465728 weighted／141120 distinct
comparisonを同じexact Fourier multiplicityで評価する。登録relationはweightedもdistinctも

`product_below_target = all / target_below_product = 0 / overlap = 0`

とする。minimum outward gapは`0x1.2de9e9dffffffp-26` > 0である。canonical witnessは
target `block=11;center=4`、left／right index `385 / 0`、wave multiplicity `35`、block-0 multiplicity
`0`、class count

`[[5,0,0,0],[0,6],[0,0,4],[0,0,0,0,0,1]]`

とする。sourceは

`(block=16;center=142)^5 × (block=16;center=151)^6 × (block=1;center=152)^4 × block=1;center=149`

で、exact interval gap hexは`0x1.2de9f1c1d911ep-26`とする。witness／aggregate-record digestは
...
1. Q011akまでの16 artifact、runner、83 direct digest、outcome、boundaryとsourceが再現する。
2. Q011kと同じblock-0 exact family、proposal、eigenbasis／inverse、selected indicesが再現する。
3. 256／384-bit RoundUp、45000 basis absolute、22500 family entry、全stream digest、禁止MPFR flag非発生が
   再現する。
4. 150 row formula、primary／replay digest、coordinatewise strict containment、Q011y \(\theta_0\)への包含が
   再現する。
5. 73 Gershgorin component、0 mixed component、3 selected component、positive selected／external gapが再現する。
6. 6 selected-rowのみを置き換えた204 hybrid interval、Q011ak containment、modulus classが再現する。
7. index 99のsignature／monomial／comparison数、all-separated relation、positive witnessとdigestが再現する。
8. strict JSON、section digest、runner provenanceが再現する。

### hypothesis gateと停止規則

1. structured-row formulaがactual block-0 transformed matrixの全row sumを上から抑える。
2. selected Gershgorin componentはexternal componentから分離し、exactly 6 selected eigenvalueを含む。
3. 6 refined source intervalはQ011ak intervalに含まれ、残り198 intervalは変更されない。
4. 141120 distinct／3465728 weighted comparisonが重複・欠落なく分類される。
5. 全comparisonが`product_below_target`となり、overlapが0である。
6. minimum outward gapおよびcanonical exact gapがstrictly positiveである。

全6項目が通る場合、登録obstruction clearanceを`accepted`とする。ただしdegree-16 certificateへは
昇格せず、certified degreesは2--15および91以降、missing rangeは16--90のままとする。事前登録値の
いずれかが失敗した場合は`inconclusive`とする。登録どおりacceptedなら、Q011amでこのhybrid
envelopeをdegree 16の154 overlap aggregate全体へ適用する。

### 主張境界

本gateは固定17² repaired exact map、fixed conservation leaf、block 0のQ011k exact root family、selected
center 144--149、degree 16、Q011aj aggregate index 99、Q011yの非零block radius、exact x-Fourier
multiplicity、outward-rounded dyadic product enclosureに限る。block-0 external componentのQ011u modulus-groupへの
再同定、他の153 overlap aggregate、degree-16 external nonresonance、degrees 17--90、all-order
...
## Q011an: degree-16 conjugacy-closed active-block clearance audit — 事前登録
...
精密化するとoverlapは24960から6240へ減ったが、最初のremaining targetが`block=4;center=114`へ
移った。block 4と13は共役なrepresentative／transported pairなので、片側だけを精密化する20-identifier
設計は採用しない。この観測は設計閉鎖にだけ用い、正式な事前登録集合を、実行前に次の共役閉な24
identifierへ固定する。
...
とし、scientific outcomeを`accepted`、actual resonance outcomeを
`ruled_out_within_registered_degree_sixteen_scope`とする。ここでruled outするのは登録したdegree-16
external relationだけであり、全次数のresonanceではない。

### sealed input

Q011amまでの18 artifact、runner、93 direct digest、outcome、claim boundaryを再照合する。Q011amは直接
封印する。

- Q011am artifact／runner newline-normalized SHA-256:
  `c4808edd49dec80cb613834d829b6f411c516838f7183c13f6a1e01371cf8f51` /
  `c703511110165f66ebcc24ddb93b309c94a2e7b8e7e7372d4fa492e54ee25e53`
- Q011am input／factorization／enumeration／obstruction／result digest:
  `5378793ad9b9f0f681fbcf145e10a32ec217939ab4daa5f5014b1ba0d7b465ba` /
  `894ad9beb5d5bbd6bf72d3f82b8c303d669d7dc344ce044c4651af20e499d5f1` /
  `24a2c1bd3efd8a9d82c25323c4c379b727c022dc2af084cf1e949bd1bade614c` /
  `ce39bfbc7ef0de8320782de9b8108408cac5359524d9b3ab8339d3be9518dc6d` /
...
p_i=\|W_{i,:}\|_1\,\varepsilon_{\rm point}
 +\sum_j |W_{ij}|_{\rm up}\sum_k\Delta_{jk}\|V_{k,:}\|_1,
\qquad
r_i=p_i+\frac{\delta}{1-\delta}\max_\ell p_\ell
\]

とする。全absolute、和、積、平方根はMPFR `RoundUp`で評価する。primary 256 bitとindependent replay
384 bitを実行し、全153 replay radiusが対応primary radiusへstrictly containedとなることを要求する。
block 16／13へは共役transportし、representativeと同一radiusを用いる。

各representative familyは23409 entry、全row 27 nonzeroを持つ。登録digestと最大radiusは次とする。

- block 1 family digest:
  `d0f1c026613f59efec0ffa1d70f6d05182457272f9045b89fce661fc2e20145f`
- block 1 primary／replay basis digest:
  `b96f3098ab2eb610fa9b9c21b2cd844c6f4ead1b5bab0691033728bb59fcc9e7` /
  `6ae4b6b32c14e68dc11f77fe4440a8e719df62997a993431e6ae21ce2d9412c3`
- block 1 primary／replay 153-row digest:
  `4722f6383bf3e925a9336131ac0c638697b3420d2c6a905e49d25e1c6b644e6c` /
  `697ed5d2415bfabe4c6145c977fa1ab25ad141adde0efa87b5679d8d00b0d8a3`
- block 1 maximum row radius／Neumann correction:
  center 95、`0x1.66e37547d73acp-35 / 0x1.0892c5566fb86p-77`
- block 4 family digest:
  `5ca9d1bd018fcd9d6b13216f5c31462dda74949ac66dd086cfc65c1710fdf387`
- block 4 primary／replay basis digest:
  `e98283eaef5a4812789364a3e588b111abe1f5876a89e4dbe8f6bf22119bde27` /
  `09104ea6b052efa341eb83c245650c21ea22eda5cffe2f90c77c0ca386490895`
- block 4 primary／replay 153-row digest:
  `ea3006ca6421f4ffd814ddf4c82c63e4a9ddcdf53be943a6b7776e7c8d7b7fee` /
  `4e12fd5321ee15afc35bbd7d6918553d3978b3cbe238e4d128f6be803ad60f6a`
- block 4 maximum row radius／Neumann correction:
  center 138、`0x1.0fac306f306aap-30 / 0x1.8d25920c52235p-69`

### Gershgorin componentと共役transport

exact center squared-distanceとprimary radiusからdisc-intersection graphを作る。active componentがinactive
discを一つも含まず、Gershgorin separation theoremにより各active unionがdisc数と同数の固有値を含む
ことを要求する。

- block 1／16: 96 component、active membership
  `[142] / [143] / [144] / [145] / [148] / [149] / [150,151]`
- block 1／16 component digest:
  `d884728455a0caafe43cf935fc2c7aa36c199fb45bd0b19fc9394d4fc8715bc9`
- block 1／16 minimum active／inactive coordinate gap:
  center `151 / 152`、`0x1.fd8363e29a48cp-8`、digest
  `015897fb41eff306d2c95ba8ade84d3801ead868c0ce85cd16a5617d24cd352f`
- block 4／13: 98 component、active membership `[114,116] / [115,117]`
- block 4／13 component digest:
  `864649df456f79af3d99cdbbdf0a903dd673ad5f636fb3d510780246581e22b7`
- block 4／13 minimum active／inactive coordinate gap:
  center `115 / 123`、`0x1.3faaabade11c4p-4`、digest
  `6f75e77076817dc40857d7ab1694b674e2bddb041112b83ffb509c0999104943`
...
### aggregate 55 clearanceとdegree-16 bridge

aggregate 55のselected-type count `[3,12,0,1]`、24 target、1560 signature、204240 compatible
monomial、37440 distinct／602720 weighted comparisonを同じexact Fourier multiplicityで再評価する。
group pool sizeは`20 / 13 / 1 / 6`、left／right pool sizeは`260 / 6`とする。

登録relation countは

- distinct: `product_below_target 24960 / target_below_product 12480 / overlap 0`
- weighted: `product_below_target 388480 / target_below_product 214240 / overlap 0`
...
1. Q011amまでの18 artifact、runner、93 direct digest、outcome、boundaryが再現する。
2. block 1／4 exact family、proposal、canonical eigenbasis／inverse、Q011k proof scalarが再現する。
...
全7項目が通る場合、登録classificationを`accepted`とする。certified degreesは2--16および91以降、
missing rangeは17--90へ縮小する。登録値のいずれかが失敗した場合は`inconclusive`とし、degree 16を
certified setへ追加しない。後付けthresholdで結果を変更しない。

登録どおりacceptedなら、次はQ011aoでdegree 17を監査する。aggregate数と費用をdesign-onlyに見積もり、
Q011amの階層exact-multiplicity表現をそのまま再利用できる場合だけ正式なdegree-17 full sweepを事前登録
する。degree 16の追加精密化やphase-sensitive diskへは戻らない。
...
## Q011aq: degree-18 component-safe hierarchical full sweep — 事前登録

### 問い

Q011apで固定した252 modulus-overlap aggregateを、188-identifier component-safe envelope、独立な
source-disc反復選択、全target disc、exact x-Fourier multiplicity、outward-rounded dyadic product boundで
全件直接走査したとき、全relationをstrictに分離できるか。さらにQ011uで既に分離した1078 aggregateを
containmentで保存し、degree 18の全1330 external aggregateを分離できるか。

Q011apはrelationを一件も評価していない。Q011aqでは以下を固定してから初めて正式full sweepを実行する。
未観測のrelation count、matrix digest、minimum witnessは事前のacceptance thresholdにせず、最初の正式
artifactでoutcomeとして封印する。

### sealed input

Q011apまでの21 artifact、runner、108 direct digest、resource decision、scientific boundaryを再照合し、
Q011apを直接封印する。

- Q011ap artifact／runner newline-normalized SHA-256:
  `e0e50ce2ffbfd94389d7e446948fe4a04b7a9aac47213ba8a5ec0717aa78317b` /
  `1a3049db74228754189b8abaf7514ac28780260e9e5d18629589d415628ab313`
- Q011ap input／inventory／envelope／resource／result digest:
  `e39b183a97916bb1c108a23ea6cf9ae3328f7423c8d83199154f37e4e1dc306c` /
  `ffc0d158333068d535a71f2dbb1cfaec3d495c7a2ee169dda9aff5cf07727141` /
  `9cdc661ff2354b1dfb7655c5819ec7ecdb655dd8369886af0609cc7df3b68379` /
  `87679bf4427084316b0686cf362c03e25fdb0b7f57be01f9fd34d310381c386b` /
...
Q011apの`study_gate=passed`、resource decision
`go_for_degree_eighteen_preregistration`、scientific／actual-resonance outcome
`not_evaluated / not_evaluated`を要求する。

### fixed inventory and envelope

Q011ap artifactから次を再構成し、stored recordとexactに一致させる。

- degree aggregate／old separated／direct overlap: `1330 / 1078 / 252`
- overlap count first／last: `[0,0,0,18] / [18,0,0,0]`
- count／external-index／inventory digest:
  `ab1d05431038fd8e1331106f83beb3e26477f489b2e537dde3a29c28a4e99d1f` /
  `86cd7247a3db94d6be1e9fe49a9868415f3aae37b4e1e84af3a23691d51e9277` /
  `9f052dc63d04fab009e2bef1dca55c171294a808dc572d5e2348e95c3ed8b0f8`
- selected／target／reused／new／final identifier: `24 / 164 / 160 / 28 / 188`
- final disc record／selected class membership digest:
  `177bf6efc8f75a8a3fe4de1a08caf642aeb108fabd9ba325a7b1d1fdf31bea1b` /
  `269187f8489521c7e37ae8a91669b9dc020ac10d4ef1d42272bb636fa7bc9b8c`
- selected class count: `4 / 2 / 3 / 6`

component内部のper-eigenvalue labelは仮定しない。各selected factorは認証済みdiscを反復付きで独立に
選び、各aggregateに登録された全target discを比較する。source／targetのdisc union coverageを超える
ラベル対応を導入しない。

### fixed hierarchical arithmetic

Q011an／Q011aoと同じ実装契約を用いる。

1. 4 selected groupのclass weak compositionをexactに列挙する。
...
- class-power／group-signature／pair-key: `268 / 139922 / 125`
- cached pair-signature entry／convolution: `1450127 / 2277951`
- original monomial／modulus signature: `50931347136 / 112289821`
- peak live signature／two product-array bytes: `2102100 / 33633600`
- distinct／weighted comparison upper bound: `996565068 / 485076664408`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを
変更しない。

### validity gate

1. Q011apまでの21 artifact、runner、108 direct digest、outcome、boundaryが再現する。
2. Q011apのinventory、envelope、class membership、resource contractがexactに再現する。
3. 252 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
...
7. section digestとrunner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 18をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 188 discがselected 24 sourceと164 targetをlabel-safeに被覆する。
2. 252 direct overlap-inventory aggregateの全compatible comparisonに`overlap=0`である。
3. 1078 old-modulus aggregateと252 direct aggregateの和がdegree-18全1330 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-18 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the component-safe hierarchical sweep certifies degree-18 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを
`ruled_out_within_registered_degree_eighteen_scope`とする。certified degreesは2--18および91以降、
missing rangeは19--90へ縮む。

validな計算で一つでもoverlapが残ればclassificationを
`the component-safe hierarchical degree-18 sufficient certificate is rejected`、scientific outcomeを
`rejected`、actual resonance outcomeを`not_established`とする。最初のoverlap witnessだけを次の精密化
候補にし、後付けthresholdや未登録disc relabellingで結果を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree 18、Q011uの1330 external
aggregate、Q011apの188-identifier component-safe envelope、独立source-disc反復選択、全target disc、
exact x-Fourier multiplicity、hierarchical outward-dyadic product protocolに限る。degrees 19--90、
all-order nonresonance、Q011t graphとのhigher-order一致、higher graph smoothness、SSM existence／
uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。
...
## Q011au: degree-20 block-support-coalesced resource feasibility — 事前登録

### 問い

Q011atでdegree 19を認証した同じ6 source hull、exact Fourier multiplicity、全target個別比較、hierarchical
outward protocolをdegree 20へ拡張するとき、relationを評価する前のinventoryと組合せ資源が、Q011apから
変更しないabsolute limitを満たすか。Q011auはdesign-only監査であり、degree-20 product--target relation、
minimum gap、actual resonanceを一件も評価しない。

### sealed input

Q011atまでの25 artifact、runner、126 direct digest、accepted outcome、degree-18 regression、resource
semantics、claim boundaryを再照合し、Q011atを直接封印する。

- Q011at artifact／runner newline-normalized SHA-256:
  `21a93ebdbccb8a1ebebbe75622296088b18fd384c80922479029652ae6f474fb` /
  `e9ab6411173392658d19592687677b6bc77d645ed6850b5675abbf89e1fc2212`
- Q011at input／preparation／sweep／result digest:
  `ded7430633a2c2fbbd95c4e9342ad1f3fcaeb4afdc8bf14dda7dc3d09f9e177b` /
  `3525d4893b78d8ba0c6d5cf979b3e15f47ade979c3835c051af5489f339de47a` /
  `4b93500bfcfdb90e52f0f807c2d2ceb1b44d61f11cbbc4208e84b111dc3b4352` /
  `3d148b680a877b06c30301c7c2f88f0bae696bb6128363daa8e783e394998ace`

Q011atの`study_gate=passed`、scientific outcome `accepted`、actual resonance outcome
`ruled_out_within_registered_degree_nineteen_scope`、certified degrees 2--19と91以降を要求する。

### degree-20 exact inventory

Q011uの4 selected log-modulus typeと186 external componentをexactに再構成し、degree 20の全
\(\binom{23}{3}=1771\) count tupleを辞書順で再評価する。old modulus intervalだけで1450 aggregateを
分離し、残る321 aggregateと対応target componentだけをQ011auへ渡す。

- expanded control／old separated／overlap: `53130 / 1450 / 321`
- overlap count first／last: `[0,1,14,5] / [20,0,0,0]`
- unique external component／multi-target／maximum component count: `19 / 8 / 2`
- target identifier／block counts:
  `220 / [8,8,8,14,26,28,12,6,4,4,6,12,28,26,14,8,8]`
- count／external-index／overlap／target／inventory digest:
  `78a913e344a1bd9fd9d3d220d4027143c6a2037a0a3adee99d2799a451a1388b` /
  `d78653ed8127948911f3ac38b078afc4ee5fdb49f681790cf78f35ace58a17ef` /
  `11cede7057448f755605ed057bdf5ca0569afff3b12561b7d99c08e9a55d53e6` /
  `107ff7d808b34b5ef0cccbde0e714c9173ff9827d0ffaf9d60374ca41563552f` /
  `6df657dae3c06bb9de48b5d5deaddf5188baa823ac7a9b95985b1a894d7441a0`

### envelope and source coalescing

selected source 24 identifierはQ011asの6 hullに同じmembershipで置く。全220 targetを個別に保持し、Q011arの
184 final disc recordは全件bitwiseに再利用する。degree 20で初めて必要な60 targetだけをQ011ak blockwise
formulaで追加し、final inventoryを244 recordとする。target merge、共役folding、label collapseは用いない。

- prior reused／added／removed／final identifier: `184 / 60 / 0 / 244`
- prior-record／added-identifier／added-record／final-record digest:
  ...
  `38e69a885bd761f0264a4b0f5d1b1ad5b36e85bb5558ff8e14e4316f6cd5080e` /
...
- class-power／group-key／group-signature: `111 / 74 / 400`
- pair-key／cached pair-signature entry／convolution: `172 / 4448 / 5211`
- modulus signature／peak live signature／two-array bytes: `10287 / 110 / 1760`
- original／maximum aggregate monomial: `184398553391 / 4122518400`
- safe `int64` convolution crude upper: `70082812800`
- distinct／weighted comparison upper: `109992 / 2182943190492`
...
- projected wall／2倍safety seconds:
  `29431755603/577447840000 / 29431755603/288723920000`
- 1.5倍tracemalloc／process-memory safety bytes: `81476 / 224852`

### validity gate

1. Q011atまでの25 artifact、runner、126 direct digest、outcome、boundaryが再現する。
2. degree-20 inventoryの全count、順序、target membership、5 digestが再現する。
3. Q011arの184 recordを全てbitwiseに保存し、60追加recordとfinal 244 recordのdigestが再現する。
...
`go_for_degree_twenty_coalesced_preregistration`とする。一つでも超えれば
`stop_before_degree_twenty_coalesced_full_sweep`とする。観測後のlimit変更、target folding、degree-20 relationの
先行確認で判定を変えない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree-20 modulus-overlap inventory、Q011asの
6 source hull、全220 target disc、組合せ資源とcalibrated projectionだけを扱う。degree-20 external
nonresonance、actual resonance、minimum gap、all-order result、higher smoothness、SSM existence／uniqueness、
normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

GoならQ011avで入力、全321 aggregate、全220 target、outward arithmetic、success／rejection条件を固定して
degree-20 full sweepを事前登録する。Stopなら最初のfailed resource dimensionだけを改善し、inconclusiveなら
最初のvalidity failureだけを修正する。

### Q011au 封印結果

全6 validity gateと、Q011apから変更しなかった全7 resource gateが通過した。degree 20の1771 aggregateは
1450 old nonoverlapと321 overlapへ分かれ、Q011arの184 discを全てbitwiseに保存して60 targetだけを追加した
244-disc inventoryを得た。Q011asの6 source hullと全220 target個別比較semanticsも保存した。
...
resource decisionは`go_for_degree_twenty_coalesced_preregistration`、scientific／actual resonance outcomeは
`not_evaluated / not_evaluated`である。certified degreesは2--19および91以降、missing rangeは20--90の
ままである。次はQ011avで、Q011auの固定inventory、全321 aggregate、全220 target、exact Fourier
multiplicity、outward arithmetic、success／rejection条件を封印してからdegree-20 relationを評価する。

## Q011av: degree-20 block-support-coalesced full sweep — 事前登録

### 問い

Q011auで固定した321 modulus-overlap aggregateを、Q011asの6 block-support source hull、全220 target disc、
exact 17-point Fourier multiplicity、outward-rounded binary64 hierarchical product intervalで全件走査したとき、
全relationをstrictに分離できるか。さらにQ011uで既に分離した1450 aggregateを保存し、degree 20の全1771
external aggregateを分離できるか。

Q011auはdegree-20 relationを一件も評価していない。Q011avでは以下を固定してから初めて正式full sweepを
実行する。未観測のrelation count、matrix digest、minimum witnessはacceptance thresholdにせず、最初の正式
artifactでoutcomeとして封印する。

### sealed input

Q011auまでの26 artifact、runner、131 direct digest、resource Go、Q011at accepted certificate、scientific
boundaryを再照合し、Q011auを直接封印する。

- Q011au artifact／runner newline-normalized SHA-256:
  `029229b9e02584ac2000e50aab8a7ac36a19e16099effb97223c04f79d44327a` /
  `0d393a9a04ac034f4e714b6dee2b71d817cdb3605feeaebdad65bfcbe7098032`
- Q011au input／inventory／envelope／resource／result digest:
  `1da477970508fec8465b8982c2465a837ae686cce262f5ee3aedbd431bdc75e6` /
  `3ff79ea56b49a1d16886582c7d661edbfed49474fee6300c51133b9af7b2f47b` /
  `64ead411e78c0993380f7ab3d05772adc4b8658d2e905a4fce3cc744dc6c87c7` /
  `eaf79c72235430d1996f4318b2815929f57267ae8f3867da1bc2f06a9a0c473e` /
...
Q011auの`study_gate=passed`、resource decision
`go_for_degree_twenty_coalesced_preregistration`、scientific／actual resonance outcome
`not_evaluated / not_evaluated`、relation evaluation count 0を要求する。

### fixed inventory, hull, and targets

Q011au artifactから次をexactに再構成する。

- degree aggregate／old separated／direct overlap: `1771 / 1450 / 321`
- overlap count first／last: `[0,1,14,5] / [20,0,0,0]`
- selected／target／final identifier: `24 / 220 / 244`
- old／merged source class count: `[4,2,3,6] / [1,1,2,2]`
- inventory／final-disc／hull／merged-membership digest:
  `6df657dae3c06bb9de48b5d5deaddf5188baa823ac7a9b95985b1a894d7441a0` /
  `149fce2623b188288f872594edba6f34465044ac9a36a7adbe4981b978230abc` /
  `f54f1a1f70d953b4c1255e9f9627d7ca43229a3d1e26c11b716d23fd56ea1eb6` /
  `975ebf1077279cbfcc6fd46e5a34fcd67260ff953a718eb51985794314fcd8b8`

各selected identifierは登録merged hullにcontainedし、wave blockとcenter indexを保持する。全220 targetは
個別比較し、target merge、共役folding、per-eigenvalue label collapseを導入しない。

### fixed hierarchical arithmetic

Q011atと同じcomponent-safe outward protocolをdegree 20へ適用する。
...
- class-power／group-signature／pair-key: `111 / 400 / 172`
- cached pair-signature entry／convolution: `4448 / 5211`
- original monomial／modulus signature: `184398553391 / 10287`
- peak live signature／two product-array bytes: `110 / 1760`
- distinct／weighted comparison upper: `109992 / 2182943190492`
- safe convolution crude upper: `70082812800`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを変更しない。

### validity gate

1. Q011auまでの26 artifact、runner、131 direct digest、outcome、boundaryが再現する。
2. Q011au inventory／target envelope／source hull／resource contractがexactに再現する。
3. 321 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
4. original monomial、signature、convolution、peak、comparison upper countが登録値と一致する。
5. aggregate／bound／coefficient／classification summaryがstrict finite JSONとdigestへ封印される。
6. first overlapがある場合は辞書順最初のwitness、separated relationがある場合はglobal minimum witnessを
   exact rational reconstructionで封印する。
7. Q011at degree-19 accepted certificate、Q011au section digest、runner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 20をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 6 merged hullがselected 24 sourceを包含し、220 target discがbitwiseに保存される。
2. 321 direct aggregateの全compatible comparisonに`overlap=0`である。
3. 1450 old-modulus aggregateと321 direct aggregateの和がdegree-20全1771 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-20 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the block-support-coalesced component-safe sweep certifies degree-20 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを`ruled_out_within_registered_degree_twenty_scope`とする。certified
degreesは2--20および91以降、missing rangeは21--90へ縮む。

validな計算で一つでもoverlapが残ればclassificationを
`the block-support-coalesced degree-20 sufficient certificate is rejected`、scientific outcomeを`rejected`、
actual resonance outcomeを`not_established`とする。最初のoverlap witnessだけを次の精密化候補にし、
後付けthreshold、class split、target foldingで結果を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree 20、Q011uの1771 external aggregate、
Q011asの6 source hull、Q011auの220 target disc、exact x-Fourier multiplicity、hierarchical outward-dyadic
product protocolに限る。degrees 21--90、all-order nonresonance、Q011t graphとのhigher-order一致、higher
graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。
...
## Q011ax: degree-21 block-support-coalesced full sweep — 事前登録

### 問い

Q011awで固定した6 source hull、364 overlap aggregate、全228 targetを、exact 17-point Fourier multiplicityと
outward-rounded hierarchical product intervalで全件走査したとき、全compatible product--target relationを
strictに分離できるか。さらにQ011uで既に分離した1660 aggregateを保存し、degree 21の全2024 external
aggregateを分離できるか。

Q011awはdegree-21 relationを一件も評価していない。Q011axでは以下を固定してから初めて正式full sweepを
実行する。未観測のrelation count、matrix digest、minimum witnessはacceptance thresholdへ使わず、最初の
正式artifactでoutcomeとして封印する。

### sealed input

Q011awまでの28 artifact、runner、140 direct digest、resource Go、Q011av accepted certificate、scientific
boundaryを再照合し、Q011awを直接封印する。

- Q011aw artifact／runner newline-normalized SHA-256:
  `0228b387f0308e98051085bdf68e16c7baef4326467c102e152e7ae3f189425e` /
  `ece845c00b7a49bd9bd12e7c30b763150d18b64fa68539b03c43c3748ca827cc`
- Q011aw input／inventory／envelope／resource／result digest:
  `9e76ca9a07ca6eb442a9fdb397f72ace03084f7e3f2c0b30fa312660c49455c2` /
  `2d1cdd3aaff4b3b224b212b8baf719293ce4595552a2da40da54d7e3592af0ce` /
  `e4e7435a47e12a262a7ca6f2fc56125d4d18cbf3f0c0b348f3ff6cbee3c2ad49` /
  `16ae0966509a3f9d4a608b025dfac11de996f9613e1a2f8f198a41e495d5f488` /
...
Q011awの`study_gate=passed`、resource decision
`go_for_degree_twenty_one_coalesced_preregistration`、relation evaluation count 0、Q011av degree-20 accepted
certificate、certified degrees 2--20と91以降を要求する。

### fixed inventory, hull, and targets

Q011aw artifactから次をexactに再構成する。

- degree aggregate／old separated／direct overlap: `2024 / 1660 / 364`
- overlap count first／last: `[0,0,13,8] / [19,0,2,0]`
- selected／target／final identifier: `24 / 228 / 252`
- old／merged source class count: `[4,2,3,6] / [1,1,2,2]`
- inventory／final-disc／hull／merged-membership digest:
  `1999df10bd6342c60c2dee30f1597c0e993225431792464ee866d5c94c1ccbe3` /
  `1dac3d5c4a74e297af45d07c63b4317ef9e54f02ada9fbbcf1375c84c84ce8a2` /
...
各selected identifierは登録merged hullにcontainedし、wave blockとcenter indexを保持する。全228 targetは
個別比較し、target merge、共役folding、per-eigenvalue label collapseを禁止する。

### fixed hierarchical arithmetic

Q011avと同じcomponent-safe outward protocolをdegree 21へ適用する。
...
- class-power／group-signature／pair-key: `121 / 482 / 192`
- cached pair-signature entry／convolution: `5523 / 6448`
- original monomial／modulus signature: `294674427372 / 12458`
- peak live signature／two product-array bytes: `126 / 2016`
- distinct／weighted comparison upper: `146928 / 3951865509552`
- safe convolution crude upper: `133491072000`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを変更しない。

### validity gate

1. Q011awまでの28 artifact、runner、140 direct digest、outcome、boundaryが再現する。
2. Q011aw inventory、target envelope、source hull、resource contractがexactに再現する。
3. 364 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
4. original monomial、signature、convolution、peak、comparison upper countが登録値と一致する。
5. aggregate／bound／coefficient／classification summaryがstrict finite JSONとdigestへ封印される。
6. first overlapがある場合は辞書順最初のwitness、separated relationがある場合はglobal minimum witnessを
   exact rational reconstructionで封印する。
7. Q011av degree-20 accepted certificate、Q011aw section digest、runner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 21をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 6 merged hullがselected 24 sourceを包含し、228 target discが個別に保存される。
2. 364 direct aggregateの全compatible comparisonに`overlap=0`である。
3. 1660 old-modulus aggregateと364 direct aggregateの和がdegree-21全2024 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-21 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the block-support-coalesced component-safe sweep certifies degree-21 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを`ruled_out_within_registered_degree_twenty_one_scope`とする。
certified degreesは2--21および91以降、missing rangeは22--90へ縮める。

validな計算で一つでもoverlapが残ればclassificationを
`the block-support-coalesced degree-21 sufficient certificate is rejected`、scientific outcomeを`rejected`、
...
本gateは固定17² repaired exact map、fixed conservation leaf、degree 21、Q011uの2024 external aggregate、
Q011asの6 source hull、Q011awの228 target disc、exact x-Fourier multiplicity、hierarchical outward-dyadic
product protocolに限る。degrees 22--90、all-order nonresonance、Q011t graphとのhigher-order一致、higher
graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

acceptedなら次はQ011ayでdegree 22をdesign-onlyに資源監査する。rejectedなら最初のoverlapだけを監査し、
inconclusiveなら最初のvalidity failureだけを修正する。

### Q011ax 封印結果

全7 validity gateと全5 hypothesis gateが通過した。Q011awまでの28 artifact、140 direct digest、degree-21
inventory、252-disc envelope、6 source hull、全228 target、resource identityは全て再現した。364 direct
aggregateを正式走査し、1660 old aggregateと合わせてdegree 21の全2024 aggregateを分離した。

- original monomial／modulus signature／peak: `294674427372 / 12458 / 126`
...
classificationは
`the block-support-coalesced component-safe sweep certifies degree-21 external nonresonance`、scientific outcomeは
`accepted`、actual resonance outcomeは`ruled_out_within_registered_degree_twenty_one_scope`である。
certified degreesは2--21および91以降、missing rangeは22--90へ縮んだ。主張は固定17² repaired exact map、
fixed conservation leaf、登録degree-21 external relationに限定する。次はQ011ayとしてdegree 22を
design-onlyに資源監査し、relationはまだ評価しない。

## Q011ay: degree-22 block-support-coalesced resource feasibility — 事前登録

### 問い

Q011axでdegree 21を認証した同じ6 source hull、exact Fourier multiplicity、全target個別比較、hierarchical
outward protocolをdegree 22へ拡張するとき、relationを評価する前のinventoryと組合せ資源が、Q011apから
変更しないabsolute limitを満たすか。Q011ayはdesign-only監査であり、degree-22 product--target relation、
minimum gap、actual resonanceを一件も評価しない。

### sealed input

Q011axまでの29 artifact、runner、144 direct digest、accepted outcome、Q011av／Q011ax certificate、resource
semantics、claim boundaryを再照合し、Q011axを直接封印する。

- Q011ax artifact／runner newline-normalized SHA-256:
  `9491de98d4e598290d18cab3397ca33c77d9a657b41c9ef2e92d720202a656f6` /
  `f66825a7da2875ba913378b51424cbffa2b8ffbf925f03ed0f15885621bebb1b`
- Q011ax input／preparation／sweep／result digest:
  `ae807c1263b3eca590f364a692d3e2b3f7b7fd4439e495c43394b032b06ff7dd` /
  `dbc04b7b5a74cc09aed1dbcc7dd23777e6abe7ce549f7e28b19f023dc5c1844f` /
  `2f01475c317e70c9153b7d0a9dfe0d3d2e594e0baf784b4f1c4b39d732213b78` /
  `f6e10009cb266cc0a99cdbdcafd6c38f8864bc21c2adbfd58585865115c88da2`

Q011axの`study_gate=passed`、scientific outcome `accepted`、actual resonance outcome
`ruled_out_within_registered_degree_twenty_one_scope`、certified degrees 2--21と91以降を要求する。

### degree-22 exact inventory

Q011uの4 selected log-modulus typeと186 external componentをexactに再構成し、degree 22の全
\(\binom{25}{3}=2300\) count tupleを辞書順で再評価する。old modulus intervalだけで1901 aggregateを
分離し、残る399 aggregateと対応target componentだけをQ011ayへ渡す。

- expanded control／old separated／overlap: `80730 / 1901 / 399`
- overlap count first／last: `[0,2,17,3] / [18,0,4,0]`
- unique external component／multi-target／maximum component count: `23 / 18 / 2`
- target identifier／block counts:
  `268 / [12,14,12,16,24,32,20,6,4,4,6,20,32,24,16,12,14]`
- count／external-index／overlap／target／inventory digest:
  `0124324b2fca6152c578c38e61ae9d34f6b9c63a3431675b045007d29a0b0d0a` /
  `4d3a3e9e7f9b1a05bcea2b1408d399416e48f97a60c889d9eb04be24fdfb9c8b` /
  `a7a775e06dbcb766af856d8245a92b2b774cf84b1d6eb8b100513f6521689bda` /
  `761ac7894a0be04c45c7ab33c1a9af3b644010afd4637391fd7a5ff6540d30f7` /
  `3b29a19b57224bffff08be5c08008af1c22d6ce4b127746eecc4afd25ae4fb7c`

### monotone envelope and source coalescing

selected source 24 identifierはQ011asの6 hullに同じmembershipで置く。全268 current targetを個別に保持する。
Q011awの252 final disc recordのうち236件はcurrent active setでもbitwiseに再利用され、16件はdegree 22では
非アクティブになる。ただし証明資産を単調に保つため16件も削除せず、56 new targetだけをQ011ak blockwise
formulaで追加し、monotone final inventoryを308 recordとする。target merge、共役folding、label collapseは
用いない。

- prior／active reused／inactive retained／added／removed／final identifier:
  `252 / 236 / 16 / 56 / 0 / 308`
- current active identifier／record digest: `292 / 8a131500088ac05813be44f5ef7e0fa8aa19a295f5fa5249b60b15b8cb289d8e`
- prior-record／inactive-identifier／added-identifier／added-record／final-record digest:
  `1dac3d5c4a74e297af45d07c63b4317ef9e54f02ada9fbbcf1375c84c84ce8a2` /
  `165bcbb773a353449b42eef214ce2ce5847b1b3b8fa71b5319484eb085bc8d24` /
  `b8888796e3913b2a6e611b43fa7566d06b19f6f7b22826e11cf95f4d78078315` /
  `6dbbf717cc569dcd6740faf15c1a1d10d268233053693288a4cbf01ca2a8df3f` /
...
- class-power／group-key／group-signature: `118 / 78 / 459`
- pair-key／cached pair-signature entry／convolution: `202 / 6558 / 7438`
- modulus signature／peak live signature／two-array bytes: `14686 / 121 / 1936`
- original／maximum aggregate monomial: `508134615924 / 13349107200`
- safe `int64` convolution crude upper: `226934822400`
- distinct／weighted comparison upper: `194280 / 7968716751096`
- class-power／group-key／pair-key／resource-record digest:
  `fbf4eb25e4778dd074814724aeba434dbd915dd2d222e387f4a7821d1ecab057` /
  `3fda2f2b47a792b233fdd66566dbcb2fed131f952d2e683c93534f6341b92d13` /
  `e5bfe06f04969e0d68dd9e140bf93c18c3a8b629a8a5a269343b2eadd64d0424` /
...
1. Q011axまでの29 artifact、runner、144 direct digest、outcome、boundaryが再現する。
2. degree-22 inventoryの全count、順序、target membership、5 digestが再現する。
3. Q011awの252 recordを全てbitwiseに保存し、16 inactive retention、56追加record、final 308 recordのdigestが
   再現する。
4. Q011asの6 source hullとmembershipがbitwiseに再現する。
5. 全resource count、projection、integer safety、非構築フラグ、relation count 0が再現する。
6. 全sectionがstrict finite JSON、section digest、runner provenanceへ封印される。
...
validity通過後に全7 limitを満たせばresource decisionを
`go_for_degree_twenty_two_coalesced_preregistration`とする。一つでも超えれば
`stop_before_degree_twenty_two_coalesced_full_sweep`とする。観測後のlimit変更、target folding、degree-22
relationの先行確認で判定を変えない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree-22 modulus-overlap inventory、Q011asの
6 source hull、全268 current target disc、308-record monotone envelope、組合せ資源とcalibrated projectionだけを
扱う。degree-22 external nonresonance、actual resonance、minimum gap、all-order result、higher smoothness、
SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

GoならQ011azで入力、全399 aggregate、全268 target、outward arithmetic、success／rejection条件を固定して
degree-22 full sweepを事前登録する。Stopなら最初のfailed resource dimensionだけを改善し、inconclusiveなら
最初のvalidity failureだけを修正する。

### Q011ay 封印結果

全6 validity gateと全7 resource gateが通過した。Q011axまでの29 artifact、144 direct digest、degree-22
inventory、308-record monotone envelope、6 source hull、resource identityは全て再現した。product--target
relationは一件も評価しておらず、relation evaluation countは0である。

- resource decision: `go_for_degree_twenty_two_coalesced_preregistration`
...
certified degreesは2--21および91以降、missing rangeは22--90のままである。本結果は固定17² repaired
exact map、fixed conservation leaf、degree-22 full sweepの設計資源に限定され、degree-22 external
nonresonance、actual resonance、minimum gap、all-order result、SSM existence／uniquenessを認証しない。
次はQ011azとして全399 overlap aggregateの正式full sweepを事前登録する。

## Q011az: degree-22 block-support-coalesced full sweep — 事前登録

### 問い

Q011ayで固定した6 source hull、399 overlap aggregate、全268 current targetを、exact 17-point Fourier
multiplicityとoutward-rounded hierarchical product intervalで全件走査したとき、全compatible
product--target relationをstrictに分離できるか。さらにQ011uで既に分離した1901 aggregateを保存し、
degree 22の全2300 external aggregateを分離できるか。

Q011ayはdegree-22 relationを一件も評価していない。Q011azでは以下を固定してから初めて正式full sweepを
実行する。未観測のrelation count、matrix digest、minimum witnessはacceptance thresholdへ使わず、最初の
正式artifactでoutcomeとして封印する。

### sealed input

Q011ayまでの30 artifact、runner、149 direct digest、resource Go、Q011ax accepted certificate、scientific
boundaryを再照合し、Q011ayを直接封印する。

- Q011ay artifact／runner newline-normalized SHA-256:
  `f841626d0661cc243ecbf845ba637df184557e22bfd5ace53c68def7ffca6ffe` /
  `aaa922266ec2bd2f1ddecf3945488063074791f546bae90c0fc0398c642958a5`
- Q011ay input／inventory／envelope／resource／result digest:
  `5206c5447df97adcb8be822572df9f26870c36f5ed761271d44bca1f2f9c577d` /
  `add1979028f33f8c29dacf1b52a00aa8c8335c58700bfa47b4660eaba2bc4dcc` /
  `4a0a7ed164ad535c0d064209de73fb577752ef31986b2f187239cda974379791` /
  `39a1307d0ad006ca315ef0896066870003fc0d990417abeb00f6d329d09477ad` /
...
Q011ayの`study_gate=passed`、resource decision
`go_for_degree_twenty_two_coalesced_preregistration`、relation evaluation count 0、Q011ax degree-21 accepted
certificate、certified degrees 2--21と91以降を要求する。

### fixed inventory, monotone envelope, hull, and active targets

Q011ay artifactから次をexactに再構成する。

- degree aggregate／old separated／direct overlap: `2300 / 1901 / 399`
- overlap count first／last: `[0,2,17,3] / [18,0,4,0]`
- selected／current target／active／monotone final identifier: `24 / 268 / 292 / 308`
- prior／inactive retained／added／removed identifier: `252 / 16 / 56 / 0`
- old／merged source class count: `[4,2,3,6] / [1,1,2,2]`
...
各selected identifierは登録merged hullにcontainedし、wave blockとcenter indexを保持する。比較対象は全268
current targetだけとし、16 inactive retained discをdegree-22 relationへ混入させない。target merge、共役
folding、per-eigenvalue label collapseを禁止する。

### fixed hierarchical arithmetic

Q011axと同じcomponent-safe outward protocolをdegree 22へ適用する。
...
4. product modulusをbinary64のdownward／upward rounded interval productで囲う。
5. output blockごとのexact integer Fourier multiplicityが正のentryだけをcompatibleとする。
6. compatible entryを全268 current target discに対して`product_below_target`、`target_below_product`、
   `overlap`へ排他的に分類する。
7. full monomial listとfull classification matrixは保持せず、aggregate summary、matrix hash、最初の
   overlap、global minimum separated witnessだけを保持する。
...
- class-power／group-signature／pair-key: `118 / 459 / 202`
- cached pair-signature entry／convolution: `6558 / 7438`
- original monomial／modulus signature: `508134615924 / 14686`
- peak live signature／two product-array bytes: `121 / 1936`
- distinct／weighted comparison upper: `194280 / 7968716751096`
- safe convolution crude upper: `226934822400`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを変更しない。

### validity gate

1. Q011ayまでの30 artifact、runner、149 direct digest、outcome、boundaryが再現する。
2. Q011ay inventory、monotone envelope、active target set、source hull、resource contractがexactに再現する。
3. 399 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
4. original monomial、signature、convolution、peak、comparison upper countが登録値と一致する。
5. aggregate／bound／coefficient／classification summaryがstrict finite JSONとdigestへ封印される。
6. first overlapがある場合は辞書順最初のwitness、separated relationがある場合はglobal minimum witnessを
   exact rational reconstructionで封印する。
7. Q011ax degree-21 accepted certificate、Q011ay section digest、runner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 22をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 6 merged hullがselected 24 sourceを包含し、268 current target discだけが個別比較される。
2. 399 direct aggregateの全compatible comparisonに`overlap=0`である。
3. 1901 old-modulus aggregateと399 direct aggregateの和がdegree-22全2300 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-22 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the block-support-coalesced component-safe sweep certifies degree-22 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを`ruled_out_within_registered_degree_twenty_two_scope`とする。
certified degreesは2--22および91以降、missing rangeは23--90へ縮める。

validな計算で一つでもoverlapが残ればclassificationを
`the block-support-coalesced degree-22 sufficient certificate is rejected`、scientific outcomeを`rejected`、
actual resonance outcomeを`not_established`とする。最初のoverlap witnessだけを次の精密化候補にし、後付けの
threshold、class split、target foldingで結果を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree 22、Q011uの2300 external aggregate、
Q011asの6 source hull、Q011ayの268 current target disc、exact x-Fourier multiplicity、hierarchical
outward-dyadic product protocolに限る。degrees 23--90、all-order nonresonance、Q011t graphとのhigher-order
一致、higher graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、
D3Q27を認証しない。

acceptedなら次はQ011baでdegree 23をdesign-onlyに資源監査する。rejectedなら最初のoverlapだけを監査し、
inconclusiveなら最初のvalidity failureだけを修正する。

### Q011az 封印結果

全7 validity gateと全5 hypothesis gateが通過した。Q011ayまでの30 artifactと149 direct digest、degree-22
inventory、308-record monotone envelope、6 source hull、全268 current target、固定resource identityを再現し、
16 inactive monotone proof recordを比較対象から除外した。399/399 direct aggregateがstrictに分離した。
...
classificationは
`the block-support-coalesced component-safe sweep certifies degree-22 external nonresonance`、scientific outcomeは
`accepted`、actual resonance outcomeは`ruled_out_within_registered_degree_twenty_two_scope`となった。
certified degreesは2--22および91以降、missing rangeは23--90へ縮んだ。主張は固定17² repaired exact map、
fixed conservation leaf、登録degree-22 external relationに限定する。次はQ011baとしてdegree 23をdesign-onlyに
資源監査する。

## Q011ba: degree-23 block-support-coalesced resource feasibility — 事前登録

### 問い

Q011azでdegree 22を認証した同じ6 source hull、exact Fourier multiplicity、全target個別比較、hierarchical
outward product protocolをdegree 23へ拡張したとき、relationを一件も評価せず、Q011apから変更しないabsolute
resource limit内で正式full sweepを事前登録できるか。Q011baはdesign-only監査であり、degree-23
product--target relation、actual resonance、minimum gapを評価しない。

### sealed input

Q011azまでの31 artifact、runner、153 direct digest、accepted outcome、Q011ax／Q011az certificate、resource
semantics、claim boundaryを再照合し、Q011azを直接封印する。

- Q011az artifact／runner newline-normalized SHA-256:
  `ca8dd35c73afc60f8aa66a7c9a57c17595c13123a3e613b6ab4125d55ee85c19` /
  `d956b43019ff8067440db23c22bd1be689b588ded2281a09e0430b441a9a4b53`
- Q011az input／preparation／sweep／result digest:
  `80958bc98451a43248ee3ac246360ae6cfc42ff336a38c3291192a953f950c48` /
  `0b8625ae85fed8309ef566eb2f56d79df5f81c527e6a5017f6bc7af509e6e1fb` /
  `3d69c53a16a88efd68c79399cc453912273d03277fe8e9ce05b113c21d5bf9d0` /
  `43a425e1d41535a3347d1ed88c40ddf281d35713b45ecf32ec6d64fafa03437f`

Q011azの`study_gate=passed`、scientific outcome `accepted`、actual resonance outcome
`ruled_out_within_registered_degree_twenty_two_scope`、certified degrees 2--22と91以降を要求する。

### degree-23 exact inventory

Q011uのexact rational-log intervalを変更せず再構成し、4 selected merged groupのweak compositionを辞書順に
全列挙する。degree 23の全2600 aggregateをold modulusだけで分離できる2161件とoverlap 439件へexactに
分け、残るaggregateと対応target componentだけをQ011baへ渡す。

- degree aggregate／expanded control／old separated／overlap:
  `2600 / 98280 / 2161 / 439`
- overlap count first／last: `[0,1,16,6] / [23,0,0,0]`
- selected group size: `[8,4,4,8]`
- unique external component／target／multi-component aggregate／maximum component:
  `26 / 300 / 14 / 2`
- target block count:
  `[20,12,12,18,26,38,20,4,10,10,4,20,38,26,18,12,12]`
- count／external-group／overlap-record／target-record／inventory digest:
  `831c1a4148af3de8f54b970bf69332d479881fb73841cab2ac69ce21a88cb011` /
  `0d8b457f374f84f88000983085236cfcdf1a6714cef633ebcf4ab338b9efaf25` /
  `e54fb4a6ba3212ec816dd204a0b83dc94a72d2fb284754691ef6e725feaea983` /
  `968c2a14ceb7ec2d24240e68308b6f0d84599ca492d3f114b3a8ffbb875d3ab4` /
  `b19f8d2abbd7d8e6e56e48991dee9c4154a4521455ef1a9939a545e893a87a79`

target folding、共役folding、per-eigenvalue label collapse、relation evaluationを禁止する。

### monotone envelope and source hull

Q011ayの308-record monotone proof inventoryをpriorとする。degree-23 current active inventoryはselected 24と
target 300の計324件である。priorとの共通276 recordをbitwiseに再利用し、32 inactive prior recordを削除せず
保持し、48 new target recordだけを追加した356-record monotone inventoryを作る。

- prior／active reused／inactive retained／added／active／final:
  `308 / 276 / 32 / 48 / 324 / 356`
- prior／active-reused／inactive-identifier digest:
  `b987d5025f745a441d274e32f0823f05ca5be2522a6ec0762aae412e2277aacc` /
  `89741ab7484386648defa718b846def1753b5285021a459c9c042b973246561a` /
  `7cbf8c5fab33c81ea357b6836bd5d77ce2c5a4aac89492488e26de4ba6a63040`
- added-identifier／added-record／active-record／final-record digest:
  `1e2e35f8d3d23295e04b9ce13279777d1c206e276ad2fcc2349bd97acc57e80d` /
  `771be5b30f8d56ad42d3914b3f75177e3f382b12183727ceb12f17c0f81b5cd2` /
...
inactive 32 recordはmonotone proof assetに限り、degree-23 full sweepのtarget比較には使わない。

### design-only resource arithmetic

Q011asの6 block-support hullを用い、class power、group pool、pair pool、cyclic convolutionの組合せ資源だけを
exact integerで数える。product-bound matrix、Fourier coefficient matrix、classification matrixを構築せず、
product--target relation evaluation countを0に固定する。

- class-power／group-key／group-signature: `124 / 83 / 489`
- pair-key／cached pair-signature entry／convolution: `208 / 7149 / 8085`
- original monomial／maximum aggregate monomial: `1010254243160 / 18688750080`
- modulus signature／peak live signature／two-array bytes: `16792 / 144 / 2304`
- safe `int64` convolution crude upper: `317708751360`
- distinct／weighted comparison upper: `252396 / 18941796258208`
- class-power／group-key／pair-key／resource-record digest:
  `5868262796bba44f50f33f4153cf34a4d9e931a6d8d2f1da417adc7882a831f3` /
  `a53c5f4ef4308ff7fe563becd6177a47b80c4dfa488b6d5314fca90f3d3c4a99` /
  `99a2ee6c6f84c366550734d95bf108a604c55bcea168cfe0cdf9746774baa351` /
...
projectionはQ011ao design-only pilotから同じ式で外挿するdiagnosticであり、科学的acceptance thresholdではない。

### validity gate

1. Q011azまでの31 artifact、runner、153 direct digest、outcome、boundaryが再現する。
2. degree-23 inventoryの全count、順序、target membership、5 digestが再現する。
3. Q011ayの308 prior recordを一件も削除せず、276 common recordをbitwiseに再利用し、32 inactiveと48 addedを
   明示した356-record monotone inventoryが再現する。
4. Q011asの6 source hullとmembershipがbitwiseに再現する。
5. class power、group／pair pool、convolution、signature、memory、comparison upperの全countとdigestが再現する。
6. relation objectを構築せず、全section、runner provenance、strict finite JSONがdigestへ封印される。

一つでも失敗すれば`inconclusive_resource_audit`とし、resource Go／Stopを判定しない。

### resource gate and stopping rule

validity通過後、Q011apから変更しない次のabsolute limitを順に評価する。

1. modulus signature `<= 125000000`
2. peak live signature `<= 2500000`
3. two product-array bytes `<= 41943040`
4. exact convolution `<= 2500000`
5. distinct comparison upper `<= 1100000000`
6. 2倍safety wall time `<= 1200 s`
7. 1.5倍process-memory safety `<= 6442450944 bytes`

全て通れば`go_for_degree_twenty_three_coalesced_preregistration`、一つでも超えれば
`stop_before_degree_twenty_three_coalesced_full_sweep`とする。観測後のlimit変更、target folding、degree-23
relationの試し打ちで判定を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree-23 modulus-overlap inventory、Q011asの
6 source hull、Q011ayから単調拡張した356-record proof inventory、exact combinatorial resource countだけを扱う。
degree-23 external nonresonance、actual resonance、minimum gap、all-order result、higher smoothness、SSM
existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

GoならQ011bbで入力、全439 aggregate、全300 current target、outward arithmetic、success／rejection条件を
固定して正式full sweepを事前登録する。Stopなら最初のfailed resource dimensionだけを改善し、inconclusiveなら
最初のvalidity failureだけを修正する。

### Q011ba 封印結果

全6 validity gateと全7 resource gateが通過した。Q011azまでの31 artifact、153 direct digest、degree-23
inventory、356-record monotone inventory、6 source hull、全固定resource countを再現した。relation evaluation
countは0である。
...
resource decisionは`go_for_degree_twenty_three_coalesced_preregistration`となった。certified degreesは2--22
および91以降、missing rangeは23--90のままである。本結果は固定17² repaired exact map、fixed conservation
leaf、degree-23 full sweepの設計資源に限定され、degree-23 external nonresonance、actual resonance、minimum
...
## Q011bc: degree-24 block-support-coalesced resource feasibility — 事前登録

### 問い

Q011bbでdegree 23を認証した同じ6 source hull、exact Fourier multiplicity、全target個別比較、hierarchical
outward product protocolをdegree 24へ拡張したとき、relationを一件も評価せず、Q011apから変更しないabsolute
resource limit内で正式full sweepを事前登録できるか。Q011bcはdesign-only監査であり、degree-24
product--target relation、actual resonance、minimum gapを評価しない。

### sealed input

Q011bbまでの33 artifact、runner、162 direct digest、accepted outcome、Q011az／Q011bb certificate、resource
semantics、claim boundaryを再照合し、Q011bbを直接封印する。

- Q011bb artifact／runner newline-normalized SHA-256:
  `bd89978432eaf2f5203b9f8cf6a06e7ca1b07cee55eb63cd967e589ed390c8c9` /
  `f1602a3ef6199fd08d821c07c81364ca75040335210bf076151cbf5a18afc63d`
- Q011bb input／preparation／sweep／result digest:
  `8d88eaecd1a711eb0732453df55c688fd148888faf06c95352b004c6f85d0f82` /
  `ce253864c388d80fff70620a5acf965c5003b5f26e5ce26f6fa8ee8065a2ad77` /
  `34ee1fd45e880b4676212122515ef87ab4a8dce7906695e702a3fe4fc738c8eb` /
  `eb85fac8d6fffa1cf1262f8519666bc79eeeda160f03a7e4de0c6f0f004657fe`

Q011bbの`study_gate=passed`、scientific outcome `accepted`、actual resonance outcome
`ruled_out_within_registered_degree_twenty_three_scope`、certified degrees 2--23と91以降を要求する。

### degree-24 exact inventory

Q011uのexact rational-log intervalを変更せず再構成し、4 selected merged groupのweak compositionを辞書順に
全列挙する。degree 24の全2925 aggregateをold modulusだけで分離できる2401件とoverlap 524件へexactに
分け、残るaggregateと対応target componentだけをQ011bcへ渡す。

- degree aggregate／expanded control／old separated／overlap:
  `2925 / 118755 / 2401 / 524`
- overlap count first／last: `[0,0,16,8] / [24,0,0,0]`
- selected group size: `[8,4,4,8]`
- unique external component／target／multi-component aggregate／maximum component:
  `31 / 372 / 9 / 2`
- target block count:
  `[18,20,12,20,26,39,20,16,24,24,16,20,39,26,20,12,20]`
- count／external-group／overlap-record／target-record／inventory digest:
  `b5e641f2a7b7764b25da0846ad2f36fc28791ab3522f7d47419b944eb71c0763` /
  `24ca3811e34d21e80f59c75ef12225c03f699c1e3b90aa802c172e51282baf97` /
  `707bf12d1d09e7084fac790797ab64f657716feabd3a902adcb32e89f74c8a81` /
  `434a4dbb1ebb2d6844a4603418e0388962ffc9e453a9b457326f6a10652488d0` /
  `8b173d8399cd2258cfc7cbb5f5939ef5682841c646819e1669cf3d7337822477`

target folding、共役folding、per-eigenvalue label collapse、relation evaluationを禁止する。

### monotone envelope and source hull

Q011baの356-record monotone proof inventoryをpriorとする。degree-24 current active inventoryはselected 24と
target 372の計396件である。priorとの共通308 recordをbitwiseに再利用し、48 inactive prior recordを削除せず
保持し、88 new target recordだけを追加した444-record monotone inventoryを作る。

- prior／active reused／inactive retained／added／active／final:
  `356 / 308 / 48 / 88 / 396 / 444`
- prior／active-reused／inactive-identifier digest:
  `ce14eaf3da422fdd4f3e46d41777eba079d1d6d929203941602621252feec2f9` /
...
- added-identifier／added-record／active-record／final-record digest:
  ...
  `2a775cd927e78fca4f8a14c122a0c4deabfb4c8af0c88dcd3d363c5ecdb8820c` /
...
inactive 48 recordはmonotone proof assetに限り、degree-24 full sweepのtarget比較には使わない。
...
- class-power／group-key／group-signature: `135 / 90 / 580`
- pair-key／cached pair-signature entry／convolution: `230 / 6939 / 8054`
- original monomial／maximum aggregate monomial: `2246535043109 / 38476838400`
- modulus signature／peak live signature／two-array bytes: `19942 / 153 / 2448`
- safe `int64` convolution crude upper: `654106252800`
- distinct／weighted comparison upper: `331524 / 46505597729848`
- class-power／group-key／pair-key／resource-record digest:
...
1. Q011bbまでの33 artifact、runner、162 direct digest、outcome、boundaryが再現する。
2. degree-24 inventoryの全count、順序、target membership、5 digestが再現する。
3. Q011baの356 prior recordを一件も削除せず、308 common recordをbitwiseに再利用し、48 inactiveと88 addedを
   明示した444-record monotone inventoryが再現する。
4. Q011asの6 source hullとmembershipがbitwiseに再現する。
5. class power、group／pair pool、convolution、signature、memory、comparison upperの全countとdigestが再現する。
6. relation objectを構築せず、全section、runner provenance、strict finite JSONがdigestへ封印される。
...
全て通れば`go_for_degree_twenty_four_coalesced_preregistration`、一つでも超えれば
`stop_before_degree_twenty_four_coalesced_full_sweep`とする。観測後のlimit変更、target folding、degree-24
relationの試し打ちで判定を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree-24 modulus-overlap inventory、Q011asの
6 source hull、Q011baから単調拡張した444-record proof inventory、exact combinatorial resource countだけを扱う。
degree-24 external nonresonance、actual resonance、minimum gap、all-order result、higher smoothness、SSM
existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

GoならQ011bdで入力、全524 aggregate、全372 current target、outward arithmetic、success／rejection条件を
固定して正式full sweepを事前登録する。Stopなら最初のfailed resource dimensionだけを改善し、inconclusiveなら
最初のvalidity failureだけを修正する。
...
## Q011bd: degree-24 block-support-coalesced full sweep — 事前登録

### 問い

Q011bcで固定した6 source hull、524 overlap aggregate、全372 current targetを、exact 17-point Fourier
multiplicityとoutward-rounded hierarchical product intervalで全件走査したとき、全compatible
product--target relationをstrictに分離できるか。さらにQ011uで既に分離した2401 aggregateを保存し、
degree 24の全2925 external aggregateを分離できるか。

Q011bcはdegree-24 relationを一件も評価していない。Q011bdでは以下を固定してから初めて正式full sweepを
実行する。未観測のrelation count、matrix digest、minimum witnessはacceptance thresholdへ使わず、最初の
正式artifactでoutcomeとして封印する。

### sealed input

Q011bcまでの34 artifact、runner、167 direct digest、resource Go、Q011bb accepted certificate、scientific
boundaryを再照合し、Q011bcを直接封印する。

- Q011bc artifact／runner newline-normalized SHA-256:
  `a6e0ab8762fecc9ed006a1692b634cc5ad6937a3fcb909554ab8e352c0a74a2e` /
  `f34211ca52a08845a7453737b1c29c63ee0b5ba9d540b7c47e04c6308ba650a6`
- Q011bc input／inventory／envelope／resource／result digest:
  `0109bd6dcac3685c4a414d4018779b1f013472fbb44b444eb33edab4a73bb358` /
  `41ee0d1a6e37236f920436fce19dc4a32db1f112afe7ed3ad33d2c83a596171f` /
  `8391403853a090ed93999aedc337dd6f6904db548679cf03f88b07c956e2c3a1` /
  `3d7c25527cd76b4b3d69ad50a800b5cd6dcd3dcdfa68df5b2e0a91ce45d4ea92` /
...
Q011bcの`study_gate=passed`、resource decision
`go_for_degree_twenty_four_coalesced_preregistration`、relation evaluation count 0、Q011bb degree-23 accepted
certificate、certified degrees 2--23と91以降を要求する。

### fixed inventory, monotone envelope, hull, and active targets

Q011bc artifactから次をexactに再構成する。

- degree aggregate／old separated／direct overlap: `2925 / 2401 / 524`
- overlap count first／last: `[0,0,16,8] / [24,0,0,0]`
- selected／current target／active／monotone final identifier: `24 / 372 / 396 / 444`
- prior／inactive retained／added／removed identifier: `356 / 48 / 88 / 0`
- old／merged source class count: `[4,2,3,6] / [1,1,2,2]`
- inventory／active-disc／monotone-final／hull／merged-membership digest:
  `8b173d8399cd2258cfc7cbb5f5939ef5682841c646819e1669cf3d7337822477` /
  `537e3eb54d25dd8be80b49450226e5978c18d2cb490e31345c0e67228222d5d9` /
...
各selected identifierは登録merged hullにcontainedし、wave blockとcenter indexを保持する。比較対象は全372
current targetだけとし、48 inactive retained discをdegree-24 relationへ混入させない。target merge、共役
folding、per-eigenvalue label collapseを禁止する。

### fixed hierarchical arithmetic

Q011bbと同じcomponent-safe outward protocolをdegree 24へ適用する。
...
4. product modulusをbinary64のdownward／upward rounded interval productで囲う。
5. output blockごとのexact integer Fourier multiplicityが正のentryだけをcompatibleとする。
6. compatible entryを全372 current target discに対して`product_below_target`、`target_below_product`、
   `overlap`へ排他的に分類する。
7. full monomial listとfull classification matrixは保持せず、aggregate summary、matrix hash、最初の
   overlap、global minimum separated witnessだけを保持する。
...
- class-power／group-signature／pair-key: `135 / 580 / 230`
- cached pair-signature entry／convolution: `6939 / 8054`
- original monomial／modulus signature: `2246535043109 / 19942`
- peak live signature／two product-array bytes: `153 / 2448`
- distinct／weighted comparison upper: `331524 / 46505597729848`
- safe convolution crude upper: `654106252800`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを変更しない。

### validity gate

1. Q011bcまでの34 artifact、runner、167 direct digest、outcome、boundaryが再現する。
2. Q011bc inventory、monotone envelope、active target set、source hull、resource contractがexactに再現する。
3. 524 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
4. original monomial、signature、convolution、peak、comparison upper countが登録値と一致する。
5. aggregate／bound／coefficient／classification summaryがstrict finite JSONとdigestへ封印される。
6. first overlapがある場合は辞書順最初のwitness、separated relationがある場合はglobal minimum witnessを
   exact rational reconstructionで封印する。
7. Q011bb degree-23 accepted certificate、Q011bc section digest、runner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 24をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 6 merged hullがselected 24 sourceを包含し、372 current target discだけが個別比較される。
2. 524 direct aggregateの全compatible comparisonに`overlap=0`である。
3. 2401 old-modulus aggregateと524 direct aggregateの和がdegree-24全2925 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-24 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the block-support-coalesced component-safe sweep certifies degree-24 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを`ruled_out_within_registered_degree_twenty_four_scope`とする。
certified degreesは2--24および91以降、missing rangeは25--90へ縮める。

validな計算で一つでもoverlapが残ればclassificationを
`the block-support-coalesced degree-24 sufficient certificate is rejected`、scientific outcomeを`rejected`、
actual resonance outcomeを`not_established`とする。最初のoverlap witnessだけを次の精密化候補にし、後付けの
threshold、class split、target foldingで結果を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree 24、Q011uの2925 external aggregate、
Q011asの6 source hull、Q011bcの372 current target disc、exact x-Fourier multiplicity、hierarchical
outward-dyadic product protocolに限る。degrees 25--90、all-order nonresonance、Q011t graphとのhigher-order
一致、higher graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、
D3Q27を認証しない。
...
## Q011be: degree-25 block-support-coalesced resource feasibility — 事前登録

### 問い

Q011bdでdegree 24を認証した同じ6 source hull、exact Fourier multiplicity、全target個別比較、hierarchical
outward product protocolをdegree 25へ拡張したとき、relationを一件も評価せず、Q011apから変更しないabsolute
resource limit内で正式full sweepを事前登録できるか。Q011beはdesign-only監査であり、degree-25
product--target relation、actual resonance、minimum gapを評価しない。

### sealed input

Q011bdまでの35 artifact、runner、171 direct digest、accepted outcome、Q011bb／Q011bd certificate、resource
semantics、claim boundaryを再照合し、Q011bdを直接封印する。

- Q011bd artifact／runner newline-normalized SHA-256:
  `ea93ccb0c6e1827656b1f2de440b09253ad764342a117dcda2f369f97afbb348` /
  `6730702dfb449a20374a849b4120a3e25bd6d4841736879dcfb095590edf63f5`
- Q011bd input／preparation／sweep／result digest:
  `7f2453b7a26d40c7539210fe4b62b396b15a0e8cc31277d8066270624d4a8ecd` /
  `1183aebb6c8425735d266582ae793cb259fa1378914e2a98c661373888765df0` /
  `9256b47555a2f86a5a3040a5f1885bfdc64e34c7d5e3156eae7956da57eb6e99` /
  `d93d7d2c96d5498c5b188062564c64b7f9d3be6b947e5029e1006d0f1116ec1b`

Q011bdの`study_gate=passed`、scientific outcome `accepted`、actual resonance outcome
`ruled_out_within_registered_degree_twenty_four_scope`、certified degrees 2--24と91以降を要求する。

### degree-25 exact inventory

Q011uのexact rational-log intervalを変更せず再構成し、4 selected merged groupのweak compositionを辞書順に
全列挙する。degree 25の全3276 aggregateをold modulusだけで分離できる2634件とoverlap 642件へexactに
分け、残るaggregateと対応target componentだけをQ011beへ渡す。

- degree aggregate／expanded control／old separated／overlap:
  `3276 / 142506 / 2634 / 642`
- overlap count first／last: `[0,6,4,15] / [23,1,1,0]`
- selected group size: `[8,4,4,8]`
- unique external component／target／multi-component aggregate／maximum component:
  `31 / 388 / 5 / 2`
- target block count:
  `[14,20,12,20,28,39,22,16,30,30,16,22,39,28,20,12,20]`
- count／external-group／overlap-record／target-record／inventory digest:
  `d5972a449e6d41daa7f7b40a4969a81053cc20f6b9c2fa45bc9f79c455ff1da0` /
  `f514e94b3c997d88e1de023a5e4ce380ee023b0adae716f0ac29e943a28fd773` /
  `6c09f64357e7c8fe97538a8c86ef603d5613124610794f3ad520e3bba5e4d1a7` /
  `a9cce960bff79a1c150725a3d48d3d88655e6edfbaba54eca2c0bfd4c382a49e` /
  `671081caf81ecdf21ecfafa72f25c2d10244fd5930847821a6d539e50f7eef7f`

target folding、共役folding、per-eigenvalue label collapse、relation evaluationを禁止する。

### monotone envelope and source hull

Q011bcの444-record monotone proof inventoryをpriorとする。degree-25 current active inventoryはselected 24と
target 388の計412件である。priorとの共通372 recordをbitwiseに再利用し、72 inactive prior recordを削除せず
保持し、40 new target recordだけを追加した484-record monotone inventoryを作る。

- prior／active reused／inactive retained／added／active／final:
  `444 / 372 / 72 / 40 / 412 / 484`
- prior／active-reused／inactive-identifier digest:
  `efa60dc94609adf881016585bf7df04e8cb12ea23f35d03a908508a4f5018c8f` /
...
- added-identifier／added-record／active-record／final-record digest:
  ...
  `24aee58b60322394209ba465007aa478531b6eef8c5bcfa0d7708df4b4255653` /
...
inactive 72 recordはmonotone proof assetに限り、degree-25 full sweepのtarget比較には使わない。
...
- class-power／group-key／group-signature: `141 / 95 / 605`
- pair-key／cached pair-signature entry／convolution: `257 / 6752 / 7913`
- original monomial／maximum aggregate monomial: `5101735096404 / 87603516000`
- modulus signature／peak live signature／two-array bytes: `24845 / 132 / 2112`
- safe `int64` convolution crude upper: `1489259772000`
- distinct／weighted comparison upper: `438196 / 110329304013568`
- class-power／group-key／pair-key／resource-record digest:
...
1. Q011bdまでの35 artifact、runner、171 direct digest、outcome、boundaryが再現する。
2. degree-25 inventoryの全count、順序、target membership、5 digestが再現する。
3. Q011bcの444 prior recordを一件も削除せず、372 common recordをbitwiseに再利用し、72 inactiveと40 addedを
   明示した484-record monotone inventoryが再現する。
4. Q011asの6 source hullとmembershipがbitwiseに再現する。
5. class power、group／pair pool、convolution、signature、memory、comparison upperの全countとdigestが再現する。
6. relation objectを構築せず、全section、runner provenance、strict finite JSONがdigestへ封印される。
...
全て通れば`go_for_degree_twenty_five_coalesced_preregistration`、一つでも超えれば
`stop_before_degree_twenty_five_coalesced_full_sweep`とする。観測後のlimit変更、target folding、degree-25
relationの試し打ちで判定を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree-25 modulus-overlap inventory、Q011asの
6 source hull、Q011bcから単調拡張した484-record proof inventory、exact combinatorial resource countだけを扱う。
degree-25 external nonresonance、actual resonance、minimum gap、all-order result、higher smoothness、SSM
existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

GoならQ011bfで入力、全642 aggregate、全388 current target、outward arithmetic、success／rejection条件を
固定して正式full sweepを事前登録する。Stopなら最初のfailed resource dimensionだけを改善し、inconclusiveなら
最初のvalidity failureだけを修正する。

### Q011be 封印結果

全6 validity gateと全7 resource gateが通過した。Q011bdまでの35 artifactと171 direct digest、degree-25
inventory、484-record monotone envelope、6 source hull、全388 current target、固定resource identityを再現した。
...
resource decisionは`go_for_degree_twenty_five_coalesced_preregistration`、scientific outcomeとactual resonance
outcomeはいずれも`not_evaluated`となった。従ってcertified degreesは2--24および91以降、missing rangeは
25--90のままである。主張は固定17² repaired exact map、fixed conservation leaf、degree-25 design-only
resource feasibilityに限定する。次はQ011bfとしてdegree-25 full sweepを事前登録する。

## Q011bf: degree-25 block-support-coalesced full sweep — 事前登録

### 問い

Q011beで固定した6 source hull、642 overlap aggregate、全388 current targetを、exact 17-point Fourier
multiplicityとoutward-rounded hierarchical product intervalで全件走査したとき、全compatible
product--target relationをstrictに分離できるか。さらにQ011uで既に分離した2634 aggregateを保存し、
degree 25の全3276 external aggregateを分離できるか。

Q011beはdegree-25 relationを一件も評価していない。Q011bfでは以下を固定してから初めて正式full sweepを
実行する。未観測のrelation count、matrix digest、minimum witnessはacceptance thresholdへ使わず、最初の
正式artifactでoutcomeとして封印する。

### sealed input

Q011beまでの36 artifact、runner、176 direct digest、resource Go、Q011bd accepted certificate、scientific
boundaryを再照合し、Q011beを直接封印する。

- Q011be artifact／runner newline-normalized SHA-256:
  `3a0910b5868c9f3c9a92f58833db4b7a856a9b789d8716b6aee09794efeb6825` /
  `f6b35e6f6f845db31a7adcbb57e21a4407daf6d4eaa004584d124c78a32ce7a8`
- Q011be input／inventory／envelope／resource／result digest:
  `078ad563285bbad781c69be860085c1634ecd1cf6dbb145fe20781b59e593aa9` /
  `86fe9208f4a39dfe34ad5a1282c1d95005bfffd20758782f993214c14a334cc2` /
  `485bcbaa0959347ff205d28a00ef616ab2e252a5d150bce88f8f0fc0669d5b60` /
  `1876303981d327c34c00b0eb4259d1e3ced76ac65923303d83c5d8498cbc8108` /
...
Q011beの`study_gate=passed`、resource decision
`go_for_degree_twenty_five_coalesced_preregistration`、relation evaluation count 0、Q011bd degree-24 accepted
certificate、certified degrees 2--24と91以降を要求する。

### fixed inventory, monotone envelope, hull, and active targets

Q011be artifactから次をexactに再構成する。

- degree aggregate／old separated／direct overlap: `3276 / 2634 / 642`
- overlap count first／last: `[0,6,4,15] / [23,1,1,0]`
- selected／current target／active／monotone final identifier: `24 / 388 / 412 / 484`
- prior／inactive retained／added／removed identifier: `444 / 72 / 40 / 0`
- old／merged source class count: `[4,2,3,6] / [1,1,2,2]`
- inventory／active-disc／monotone-final／hull／merged-membership digest:
  `671081caf81ecdf21ecfafa72f25c2d10244fd5930847821a6d539e50f7eef7f` /
  `83a0c3306322f91866bfe3c51cc82f61b5b7329210ed3d184c251bf3b35ffe5b` /
...
各selected identifierは登録merged hullにcontainedし、wave blockとcenter indexを保持する。比較対象は全388
current targetだけとし、72 inactive retained discをdegree-25 relationへ混入させない。target merge、共役
folding、per-eigenvalue label collapseを禁止する。

### fixed hierarchical arithmetic

Q011bdと同じcomponent-safe outward protocolをdegree 25へ適用する。
...
4. product modulusをbinary64のdownward／upward rounded interval productで囲う。
5. output blockごとのexact integer Fourier multiplicityが正のentryだけをcompatibleとする。
6. compatible entryを全388 current target discに対して`product_below_target`、`target_below_product`、
   `overlap`へ排他的に分類する。
7. full monomial listとfull classification matrixは保持せず、aggregate summary、matrix hash、最初の
   overlap、global minimum separated witnessだけを保持する。
...
- class-power／group-signature／pair-key: `141 / 605 / 257`
- cached pair-signature entry／convolution: `6752 / 7913`
- original monomial／modulus signature: `5101735096404 / 24845`
- peak live signature／two product-array bytes: `132 / 2112`
- distinct／weighted comparison upper: `438196 / 110329304013568`
- safe convolution crude upper: `1489259772000`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを変更しない。

### validity gate

1. Q011beまでの36 artifact、runner、176 direct digest、outcome、boundaryが再現する。
2. Q011be inventory、monotone envelope、active target set、source hull、resource contractがexactに再現する。
3. 642 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
4. original monomial、signature、convolution、peak、comparison upper countが登録値と一致する。
5. aggregate／bound／coefficient／classification summaryがstrict finite JSONとdigestへ封印される。
6. first overlapがある場合は辞書順最初のwitness、separated relationがある場合はglobal minimum witnessを
   exact rational reconstructionで封印する。
7. Q011bd degree-24 accepted certificate、Q011be section digest、runner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 25をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 6 merged hullがselected 24 sourceを包含し、388 current target discだけが個別比較される。
2. 642 direct aggregateの全compatible comparisonに`overlap=0`である。
3. 2634 old-modulus aggregateと642 direct aggregateの和がdegree-25全3276 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-25 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the block-support-coalesced component-safe sweep certifies degree-25 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを`ruled_out_within_registered_degree_twenty_five_scope`とする。
certified degreesは2--25および91以降、missing rangeは26--90へ縮める。

validな計算で一つでもoverlapが残ればclassificationを
`the block-support-coalesced degree-25 sufficient certificate is rejected`、scientific outcomeを`rejected`、
actual resonance outcomeを`not_established`とする。最初のoverlap witnessだけを次の精密化候補にし、後付けの
threshold、class split、target foldingで結果を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree 25、Q011uの3276 external aggregate、
Q011asの6 source hull、Q011beの388 current target disc、exact x-Fourier multiplicity、hierarchical
outward-dyadic product protocolに限る。degrees 26--90、all-order nonresonance、Q011t graphとのhigher-order
一致、higher graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、
D3Q27を認証しない。

acceptedなら次はQ011bgでdegree 26をdesign-onlyに資源監査する。rejectedなら最初のoverlapだけを監査し、
inconclusiveなら最初のvalidity failureだけを修正する。

### Q011bf 封印結果

全7 validity gateと全5 hypothesis gateが通過した。Q011beまでの36 artifactと176 direct digest、degree-25
inventory、484-record monotone envelope、6 source hull、全388 current target、固定resource identityを再現し、
72 inactive monotone proof recordを比較対象から除外した。642/642 direct aggregateがstrictに分離した。
...
classificationは
`the block-support-coalesced component-safe sweep certifies degree-25 external nonresonance`、scientific outcomeは
`accepted`、actual resonance outcomeは`ruled_out_within_registered_degree_twenty_five_scope`となった。
certified degreesは2--25および91以降、missing rangeは26--90へ縮んだ。主張は固定17² repaired exact map、
fixed conservation leaf、登録degree-25 external relationに限定する。次はQ011bgとしてdegree 26をdesign-onlyに
資源監査する。

## Q011bg: degree-26 block-support-coalesced resource feasibility — 事前登録

### 問い

Q011bfでdegree 25を認証した同じ6 source hull、exact Fourier multiplicity、全target個別比較、hierarchical
outward product protocolをdegree 26へ拡張したとき、relationを一件も評価せず、Q011apから変更しないabsolute
resource limit内で正式full sweepを事前登録できるか。Q011bgはdesign-only監査であり、degree-26
product--target relation、actual resonance、minimum gapを評価しない。

### sealed input

Q011bfまでの37 artifact、runner、180 direct digest、accepted outcome、Q011bd／Q011bf certificate、resource
semantics、claim boundaryを再照合し、Q011bfを直接封印する。

- Q011bf artifact／runner newline-normalized SHA-256:
  `8c4d5fe2c7749ee4b91178cfcb4e32cf6409a27dc7df64733b3275d2c76a64fd` /
  `ff8bfc4018d597acc212f46f0a113e7087d0a4bc68e26d891ad930137e2332e9`
- Q011bf input／preparation／sweep／result digest:
  `2f48d35cdefbb3ab59a5c30d5587ac3d9302b32b66d7b7d2c0ddc98983d94d55` /
  `ef8765c04dfac225bc59291086b8d63de6f7d983341ca4e3789c73446f20b764` /
  `fa75929d3361737937d9a60c666c4348223165d6c0dcbea018d255fce1595f4a` /
  `8323471e2e56020d1a888a636a84e36d42f106f869938386b87492e2103ecb66`

Q011bfの`study_gate=passed`、scientific outcome `accepted`、actual resonance outcome
`ruled_out_within_registered_degree_twenty_five_scope`、certified degrees 2--25と91以降を要求する。

### degree-26 exact inventory

Q011uのexact rational-log intervalを変更せず再構成し、4 selected merged groupのweak compositionを辞書順に
全列挙する。degree 26の全3654 aggregateをold modulusだけで分離できる2875件とoverlap 779件へexactに
分け、残るaggregateと対応target componentだけをQ011bgへ渡す。

- degree aggregate／expanded control／old separated／overlap:
  `3654 / 169911 / 2875 / 779`
- overlap count first／last: `[0,0,26,0] / [26,0,0,0]`
- selected group size: `[8,4,4,8]`
- unique external component／target／multi-component aggregate／maximum component:
  `35 / 460 / 0 / 1`
- target block count:
  `[20,24,12,22,30,39,33,22,38,38,22,33,39,30,22,12,24]`
- count／external-group／overlap-record／target-record／inventory digest:
  `d0c6845965da169acbff69249dcf27b9cf87191bcc7b7b10dc4fdd3e3610f8b9` /
  `d675a3929b4dc58eea0692634943c1b4f4ebc0724b325432a68fa6024217cc0b` /
  `9bb81ca3502933143c8890fec0ad3a597b7ecf58a569a1c19cb8979dbbb622c4` /
  `1297a8180c9a5e0bd3effc7e38afbcdf461f66e50470e9b5eeb98119b116cdf2` /
  `deb3612c4a8b09a4419df60ccbb97bf10ea0b5d5b469f2528956aad397d41a38`

target folding、共役folding、per-eigenvalue label collapse、relation evaluationを禁止する。

### monotone envelope and source hull

Q011beの484-record monotone proof inventoryをpriorとする。degree-26 current active inventoryはselected 24と
target 460の計484件である。priorとの共通400 recordをbitwiseに再利用し、84 inactive prior recordを削除せず
保持し、84 new target recordだけを追加した568-record monotone inventoryを作る。

- prior／active reused／inactive retained／added／active／final:
  `484 / 400 / 84 / 84 / 484 / 568`
- prior／active-reused／inactive-identifier digest:
  `119b0c40858f42c8973eb71c98b7e100c353b9926d3a3cd3e16824c100997cd5` /
...
- added-identifier／added-record／active-record／final-record digest:
  ...
  `5c1d8dbecec84a132762e1216b7284096e58ac0122281f2a5c327a210902c37d` /
...
inactive 84 recordはmonotone proof assetに限り、degree-26 full sweepのtarget比較には使わない。
...
- class-power／group-key／group-signature: `150 / 99 / 632`
- pair-key／cached pair-signature entry／convolution: `290 / 8262 / 9474`
- original monomial／maximum aggregate monomial: `11279576045274 / 160320160000`
- modulus signature／peak live signature／two-array bytes: `32455 / 156 / 2496`
- safe `int64` convolution crude upper: `2725442720000`
- distinct／weighted comparison upper: `576636 / 241078715222912`
- class-power／group-key／pair-key／resource-record digest:
...
1. Q011bfまでの37 artifact、runner、180 direct digest、outcome、boundaryが再現する。
2. degree-26 inventoryの全count、順序、target membership、5 digestが再現する。
3. Q011beの484 prior recordを一件も削除せず、400 common recordをbitwiseに再利用し、84 inactiveと84 addedを
   明示した568-record monotone inventoryが再現する。
4. Q011asの6 source hullとmembershipがbitwiseに再現する。
5. class power、group／pair pool、convolution、signature、memory、comparison upperの全countとdigestが再現する。
6. relation objectを構築せず、全section、runner provenance、strict finite JSONがdigestへ封印される。
...
全て通れば`go_for_degree_twenty_six_coalesced_preregistration`、一つでも超えれば
`stop_before_degree_twenty_six_coalesced_full_sweep`とする。観測後のlimit変更、target folding、degree-26
relationの試し打ちで判定を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree-26 modulus-overlap inventory、Q011asの
6 source hull、Q011beから単調拡張した568-record proof inventory、exact combinatorial resource countだけを扱う。
degree-26 external nonresonance、actual resonance、minimum gap、all-order result、higher smoothness、SSM
existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

GoならQ011bhで入力、全779 aggregate、全460 current target、outward arithmetic、success／rejection条件を
固定して正式full sweepを事前登録する。Stopなら最初のfailed resource dimensionだけを改善し、inconclusiveなら
最初のvalidity failureだけを修正する。

### Q011bg 封印結果

全6 validity gateと全7 resource gateが通過した。Q011bfまでの37 artifactと180 direct digest、degree-26
inventory、568-record monotone envelope、6 source hull、全460 current target、固定resource identityを再現した。
relation object、product-bound matrix、Fourier coefficient matrix、classification matrixは構築していない。
...
resource decisionは`go_for_degree_twenty_six_coalesced_preregistration`、scientific outcomeとactual resonance
outcomeはいずれも`not_evaluated`となった。従ってcertified degreesは2--25および91以降、missing rangeは
26--90のままである。主張は固定17² repaired exact map、fixed conservation leaf、degree-26 design-only
resource feasibilityに限定する。次はQ011bhとしてdegree-26 full sweepを事前登録する。

## Q011bh: degree-26 block-support-coalesced full sweep — 事前登録

### 問い

Q011bgで固定した6 source hull、779 overlap aggregate、全460 current targetを、exact 17-point Fourier
multiplicityとoutward-rounded hierarchical product intervalで全件走査したとき、全compatible
product--target relationをstrictに分離できるか。さらにQ011uで既に分離した2875 aggregateを保存し、
degree 26の全3654 external aggregateを分離できるか。

Q011bgはdegree-26 relationを一件も評価していない。Q011bhでは以下を固定してから初めて正式full sweepを
実行する。未観測のrelation count、matrix digest、minimum witnessはacceptance thresholdへ使わず、最初の
正式artifactでoutcomeとして封印する。

### sealed input

Q011bgまでの38 artifact、runner、185 direct digest、resource Go、Q011bf accepted certificate、scientific
boundaryを再照合し、Q011bgを直接封印する。

- Q011bg artifact／runner newline-normalized SHA-256:
  `c0501905dcd50807492e99a4ecac25520209aad10a15e0f253c80508613f2b68` /
  `d67038df358cf636011d85d58fe1f5fd7541008443904f7019aca6e74622d857`
- Q011bg input／inventory／envelope／resource／result digest:
  `00f8d11b7ff9ab7acc90ca5c8611d9eb3ed2b72ccdd98eec88564b1e7fb882be` /
  `9946663f12a1edec3469799102bde0424606d3e0c981677bedbd71a9fb7b9a60` /
  `5a6fdb92df21d64cc1e29b4500ffff85e396a26771ba859f57e2280842a20c28` /
  `7949f6adb8226fe5d19f189978912b562ddff07fb4b091d915c12d68a5f20d78` /
...
Q011bgの`study_gate=passed`、resource decision
`go_for_degree_twenty_six_coalesced_preregistration`、relation evaluation count 0、Q011bf degree-25 accepted
certificate、certified degrees 2--25と91以降を要求する。

### fixed inventory, monotone envelope, hull, and active targets

Q011bg artifactから次をexactに再構成する。

- degree aggregate／old separated／direct overlap: `3654 / 2875 / 779`
- overlap count first／last: `[0,0,26,0] / [26,0,0,0]`
- selected／current target／active／monotone final identifier: `24 / 460 / 484 / 568`
- prior／inactive retained／added／removed identifier: `484 / 84 / 84 / 0`
- old／merged source class count: `[4,2,3,6] / [1,1,2,2]`
- inventory／active-disc／monotone-final／hull／merged-membership digest:
  `deb3612c4a8b09a4419df60ccbb97bf10ea0b5d5b469f2528956aad397d41a38` /
  `9018bde11bed25ed407451b745e439c1a941ee68f3753da86a4743144f671a05` /
...
各selected identifierは登録merged hullにcontainedし、wave blockとcenter indexを保持する。比較対象は全460
current targetだけとし、84 inactive retained discをdegree-26 relationへ混入させない。target merge、共役
folding、per-eigenvalue label collapseを禁止する。

### fixed hierarchical arithmetic

Q011bfと同じcomponent-safe outward protocolをdegree 26へ適用する。
...
4. product modulusをbinary64のdownward／upward rounded interval productで囲う。
5. output blockごとのexact integer Fourier multiplicityが正のentryだけをcompatibleとする。
6. compatible entryを全460 current target discに対して`product_below_target`、`target_below_product`、
   `overlap`へ排他的に分類する。
7. full monomial listとfull classification matrixは保持せず、aggregate summary、matrix hash、最初の
   overlap、global minimum separated witnessだけを保持する。
...
- class-power／group-signature／pair-key: `150 / 632 / 290`
- cached pair-signature entry／convolution: `8262 / 9474`
- original monomial／modulus signature: `11279576045274 / 32455`
- peak live signature／two product-array bytes: `156 / 2496`
- distinct／weighted comparison upper: `576636 / 241078715222912`
- safe convolution crude upper: `2725442720000`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを変更しない。

### validity gate

1. Q011bgまでの38 artifact、runner、185 direct digest、outcome、boundaryが再現する。
2. Q011bg inventory、monotone envelope、active target set、source hull、resource contractがexactに再現する。
3. 779 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
4. original monomial、signature、convolution、peak、comparison upper countが登録値と一致する。
5. aggregate／bound／coefficient／classification summaryがstrict finite JSONとdigestへ封印される。
6. first overlapがある場合は辞書順最初のwitness、separated relationがある場合はglobal minimum witnessを
   exact rational reconstructionで封印する。
7. Q011bf degree-25 accepted certificate、Q011bg section digest、runner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 26をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 6 merged hullがselected 24 sourceを包含し、460 current target discだけが個別比較される。
2. 779 direct aggregateの全compatible comparisonに`overlap=0`である。
3. 2875 old-modulus aggregateと779 direct aggregateの和がdegree-26全3654 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-26 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the block-support-coalesced component-safe sweep certifies degree-26 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを`ruled_out_within_registered_degree_twenty_six_scope`とする。
certified degreesは2--26および91以降、missing rangeは27--90へ縮める。

validな計算で一つでもoverlapが残ればclassificationを
`the block-support-coalesced degree-26 sufficient certificate is rejected`、scientific outcomeを`rejected`、
actual resonance outcomeを`not_established`とする。最初のoverlap witnessだけを次の精密化候補にし、後付けの
threshold、class split、target foldingで結果を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree 26、Q011uの3654 external aggregate、
Q011asの6 source hull、Q011bgの460 current target disc、exact x-Fourier multiplicity、hierarchical
outward-dyadic product protocolに限る。degrees 27--90、all-order nonresonance、Q011t graphとのhigher-order
一致、higher graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、
D3Q27を認証しない。
...
## Q011bi: degree-27 block-support-coalesced resource feasibility — 事前登録

### 問い

Q011bhでdegree 26を認証した同じ6 source hull、exact Fourier multiplicity、全target個別比較、hierarchical
outward product protocolをdegree 27へ拡張したとき、relationを一件も評価せず、Q011apから変更しないabsolute
resource limit内で正式full sweepを事前登録できるか。Q011biはdesign-only監査であり、degree-27
product--target relation、actual resonance、minimum gapを評価しない。

### sealed input

Q011bhまでの39 artifact、runner、189 direct digest、accepted outcome、Q011bf／Q011bh certificate、resource
semantics、claim boundaryを再照合し、Q011bhを直接封印する。

- Q011bh artifact／runner newline-normalized SHA-256:
  `34294fc77fd5ade05ab61812088fdee6d8cec63a5e22c0eac555c3647b073349` /
  `b934c50138890dd329d2d0dc75c78f98df9540d3e9b5eb3a463347f2c1a9b192`
- Q011bh input／preparation／sweep／result digest:
  `895a724a63a66c04929fbaaf95149b93444f8199f78a8494e2ae835bc800877a` /
  `8d61ae48890284651ca81e458478f1716a64f28f64149daf92aa0936ddf08b9f` /
  `d8b4ca8a9dfdc34b8fd4cb4574f0b0c54fdb158c3cd8b2a6ecf6fe06dfb3c333` /
  `86269e13f502c70439a1381edf2f532c50491a5c313ce974307db623a4ae9db4`

Q011bhの`study_gate=passed`、scientific outcome `accepted`、actual resonance outcome
`ruled_out_within_registered_degree_twenty_six_scope`、certified degrees 2--26と91以降を要求する。

### degree-27 exact inventory

Q011uのexact rational-log intervalを変更せず再構成し、4 selected merged groupのweak compositionを辞書順に
全列挙する。degree 27の全4060 aggregateをold modulusだけで分離できる3101件とoverlap 959件へexactに
分け、残るaggregateと対応target componentだけをQ011biへ渡す。

- degree aggregate／expanded control／old separated／overlap:
  `4060 / 201376 / 3101 / 959`
- overlap count first／last: `[0,4,3,20] / [26,0,1,0]`
- selected group size: `[8,4,4,8]`
- unique external component／target／multi-component aggregate／maximum component:
  `38 / 488 / 0 / 1`
- target block count:
  `[20,26,12,22,34,39,31,32,38,38,32,31,39,34,22,12,26]`
- count／external-group／overlap-record／target-record／inventory digest:
  `7fb69ae484ac6c0fe5cefad46a3d7a42175ed51250dd763216b46841a86dd695` /
  `79f4abce73265951b8cbae1c5cd93d249480629aa3185722e20d560446e5bf26` /
  `db57390c234b941eddcb29cfade6cef3b94ba8682835aa4f8a5481744c337c48` /
  `df98a7c8ffeca51792708b467141322d54fe8c10051078e47ccfea142ab976cc` /
  `33b99cc5426e4dbcb7acf767daec77e0792b96776d7f6ef4d3e974cf317ab4ef`

target folding、共役folding、per-eigenvalue label collapse、relation evaluationを禁止する。

### monotone envelope and source hull

Q011bgの568-record monotone proof inventoryをpriorとする。degree-27 current active inventoryはselected 24と
target 488の計512件である。priorとの共通476 recordをbitwiseに再利用し、92 inactive prior recordを削除せず
保持し、36 new target recordだけを追加した604-record monotone proof inventoryを作る。

- prior／active reused／inactive retained／added／active／final:
  `568 / 476 / 92 / 36 / 512 / 604`
- prior／active-reused／inactive-identifier digest:
  `7077f7c72acd166c15b12962f4c273b3837898e74689b582329cfd31ece98753` /
  `bbecc9f3f95d9d8aa0e0fb18d2c567bd7a2ddde7bd215be64d5d1c4db7806a90` /
  `0fa1279c56f06e9de2ff8c20128d611d0e53e01981628fa88f05fe1c201f1b0d`
- added-identifier／added-record／active-record／final-record digest:
  `c937b27a824b59155ced06dac5e1e4e766d36de6cbb59c6e0293ded96ad03e2e` /
  `38f1b3c821f73c4db97d497a033b7a84725d2c845443bde9e01326d17d7b8fe0` /
...
inactive 92 recordはmonotone proof assetに限り、degree-27 full sweepのtarget比較には使わない。
...
- original monomial／maximum aggregate monomial: `25561324661692 / 272544272000`
- modulus signature／peak live signature／two-array bytes: `43285 / 180 / 2880`
- safe `int64` convolution crude upper: `4633252624000`
- distinct／weighted comparison upper: `763404 / 502998748050560`
- class-power／group-key／pair-key／resource-record digest:
  `975dd65873ad2fbfcabc1b038ee3ecc5be7cace52eb7475cb4ef7d257f87af36` /
  `2f29c923bf95adfca2e64ad1332a472256155496b86269115b4fcc97e8e2a999` /
  `7d5b89286d86c0c38379af39b6e5257c7625ce811b7cd74a910f0914407563c1` /
...
1. Q011bhまでの39 artifact、runner、189 direct digest、outcome、boundaryが再現する。
2. degree-27 inventoryの全count、順序、target membership、5 digestが再現する。
3. Q011bgの568 prior recordを一件も削除せず、476 common recordをbitwiseに再利用し、92 inactiveと36 addedを
   明示した604-record monotone inventoryが再現する。
4. Q011asの6 source hullとmembershipがbitwiseに再現する。
5. class power、group／pair pool、convolution、signature、memory、comparison upperの全countとdigestが再現する。
6. relation objectを構築せず、全section、runner provenance、strict finite JSONがdigestへ封印される。
...
全て通れば`go_for_degree_twenty_seven_coalesced_preregistration`、一つでも超えれば
`stop_before_degree_twenty_seven_coalesced_full_sweep`とする。観測後のlimit変更、target folding、degree-27
relationの試し打ちで判定を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree-27 modulus-overlap inventory、Q011asの
6 source hull、Q011bgから単調拡張した604-record proof inventory、exact combinatorial resource countだけを扱う。
degree-27 external nonresonance、actual resonance、minimum gap、all-order result、higher smoothness、SSM
existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

GoならQ011bjで入力、全959 aggregate、全488 current target、outward arithmetic、success／rejection条件を
固定して正式full sweepを事前登録する。Stopなら最初のfailed resource dimensionだけを改善し、inconclusiveなら
最初のvalidity failureだけを修正する。
...
## Q011bj: degree-27 block-support-coalesced full sweep — 事前登録

### 問い

Q011biで固定した6 source hull、959 overlap aggregate、全488 current targetを、exact 17-point Fourier
multiplicityとoutward-rounded hierarchical product intervalで全件走査したとき、全compatible
product--target relationをstrictに分離できるか。さらにQ011uで既に分離した3101 aggregateを保存し、
degree 27の全4060 external aggregateを分離できるか。

Q011biはdegree-27 relationを一件も評価していない。Q011bjでは以下を固定してから初めて正式full sweepを
実行する。未観測のrelation count、matrix digest、minimum witnessはacceptance thresholdへ使わず、最初の
正式artifactでoutcomeとして封印する。

### sealed input

Q011biまでの40 artifact、runner、194 direct digest、resource Go、Q011bh accepted certificate、scientific
boundaryを再照合し、Q011biを直接封印する。

- Q011bi artifact／runner newline-normalized SHA-256:
  `61e4f1fd790674e8365013c3e43391c83d5ddfe39c4c48869bc70e90ed088e63` /
  `b6b84bf50d0f02793a74caffdc48a04a52d8af67b3aa3b712eba9eb7440fd328`
- Q011bi input／inventory／envelope／resource／result digest:
  `057af0c2ee831420b8b97790d7fbcf21429656da6935201a1b8293f0082a6345` /
  `b044c0833d2471a9a630a4deabb9dcc8fdebcedf5998483d88188aeb48914134` /
  `403e31aeb201e9f0dd40a07949b1f563c07ab858e9a40080cff8b8b5938264e0` /
  `679ba25402815b46ded29807a054273bde6c95a09515d70c0e957f015cba4bad` /
...
Q011biの`study_gate=passed`、resource decision
`go_for_degree_twenty_seven_coalesced_preregistration`、relation evaluation count 0、Q011bh degree-26 accepted
certificate、certified degrees 2--26と91以降を要求する。

### fixed inventory, monotone envelope, hull, and active targets

Q011bi artifactから次をexactに再構成する。

- degree aggregate／old separated／direct overlap: `4060 / 3101 / 959`
- overlap count first／last: `[0,4,3,20] / [26,0,1,0]`
- selected／current target／active／monotone final identifier: `24 / 488 / 512 / 604`
- prior／inactive retained／added／removed identifier: `568 / 92 / 36 / 0`
- old／merged source class count: `[4,2,3,6] / [1,1,2,2]`
- inventory／active-disc／monotone-final／hull／merged-membership digest:
  `33b99cc5426e4dbcb7acf767daec77e0792b96776d7f6ef4d3e974cf317ab4ef` /
  `1a6fd9676b3ccdca51587ef4587937f8f0af8a778bdb4ce364a89ef0fde2db08` /
...
各selected identifierは登録merged hullにcontainedし、wave blockとcenter indexを保持する。比較対象は全488
current targetだけとし、92 inactive retained discをdegree-27 relationへ混入させない。target merge、共役
folding、per-eigenvalue label collapseを禁止する。

### fixed hierarchical arithmetic

Q011bhと同じcomponent-safe outward protocolをdegree 27へ適用する。
...
4. product modulusをbinary64のdownward／upward rounded interval productで囲う。
5. output blockごとのexact integer Fourier multiplicityが正のentryだけをcompatibleとする。
6. compatible entryを全488 current target discに対して`product_below_target`、`target_below_product`、
   `overlap`へ排他的に分類する。
7. full monomial listとfull classification matrixは保持せず、aggregate summary、matrix hash、最初の
   overlap、global minimum separated witnessだけを保持する。
...
- class-power／group-signature／pair-key: `154 / 705 / 341`
- cached pair-signature entry／convolution: `10293 / 11649`
- original monomial／modulus signature: `25561324661692 / 43285`
- peak live signature／two product-array bytes: `180 / 2880`
- distinct／weighted comparison upper: `763404 / 502998748050560`
- safe convolution crude upper: `4633252624000`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを変更しない。

### validity gate

1. Q011biまでの40 artifact、runner、194 direct digest、outcome、boundaryが再現する。
2. Q011bi inventory、monotone envelope、active target set、source hull、resource contractがexactに再現する。
3. 959 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
4. original monomial、signature、convolution、peak、comparison upper countが登録値と一致する。
5. aggregate／bound／coefficient／classification summaryがstrict finite JSONとdigestへ封印される。
6. first overlapがある場合は辞書順最初のwitness、separated relationがある場合はglobal minimum witnessを
   exact rational reconstructionで封印する。
7. Q011bh degree-26 accepted certificate、Q011bi section digest、runner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 27をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 6 merged hullがselected 24 sourceを包含し、488 current target discだけが個別比較される。
2. 959 direct aggregateの全compatible comparisonに`overlap=0`である。
3. 3101 old-modulus aggregateと959 direct aggregateの和がdegree-27全4060 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-27 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the block-support-coalesced component-safe sweep certifies degree-27 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを`ruled_out_within_registered_degree_twenty_seven_scope`とする。
certified degreesは2--27および91以降、missing rangeは28--90へ縮める。

validな計算で一つでもoverlapが残ればclassificationを
`the block-support-coalesced degree-27 sufficient certificate is rejected`、scientific outcomeを`rejected`、
actual resonance outcomeを`not_established`とする。最初のoverlap witnessだけを次の精密化候補にし、後付けの
threshold、class split、target foldingで結果を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree 27、Q011uの4060 external aggregate、
Q011asの6 source hull、Q011biの488 current target disc、exact x-Fourier multiplicity、hierarchical
outward-dyadic product protocolに限る。degrees 28--90、all-order nonresonance、Q011t graphとのhigher-order
一致、higher graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、
D3Q27を認証しない。
...
## Q011bk: degree-28 block-support-coalesced resource feasibility — 事前登録

### 問い

Q011bjでdegree 27を認証した同じ6 source hull、exact Fourier multiplicity、全target個別比較、hierarchical
outward product protocolをdegree 28へ拡張したとき、relationを一件も評価せず、Q011apから変更しないabsolute
resource limit内で正式full sweepを事前登録できるか。Q011bkはdesign-only監査であり、degree-28
product--target relation、actual resonance、minimum gapを評価しない。

### sealed input

Q011bjまでの41 artifact、runner、198 direct digest、accepted outcome、Q011bh／Q011bj certificate、resource
semantics、claim boundaryを再照合し、Q011bjを直接封印する。

- Q011bj artifact／runner newline-normalized SHA-256:
...
- Q011bj input／preparation／sweep／result digest:
...
Q011bjの`study_gate=passed`、scientific outcome `accepted`、actual resonance outcome
`ruled_out_within_registered_degree_twenty_seven_scope`、certified degrees 2--27と91以降を要求する。

### degree-28 exact inventory

Q011uのexact rational-log intervalを変更せず再構成し、4 selected merged groupのweak compositionを辞書順に
全列挙する。degree 28の全4495 aggregateをold modulusだけで分離できる3325件とoverlap 1170件へexactに
分け、残るaggregateと対応target componentだけをQ011bkへ渡す。

- degree aggregate／expanded control／old separated／overlap:
  `4495 / 237336 / 3325 / 1170`
- overlap count first／last: `[0,3,2,23] / [25,1,2,0]`
- selected group size: `[8,4,4,8]`
- unique external component／target／multi-component aggregate／maximum component:
  `49 / 612 / 2 / 3`
- target block count:
  `[22,28,20,22,35,49,47,40,54,54,40,47,49,35,22,20,28]`
- count／external-group／overlap-record／target-record／inventory digest:
  `57ec3fb44805445f6551d07a559c3db5d86f87b178bbbe36a2ff367eb07c8687` /
  `d4a000e8a40f4072942ce7a800f49c35c0c818e2e2817c78f5d94757f6acdaeb` /
  `ceeb0b4133a0b6724a0335ff6cac6a3092ae8871fd362edbbf20537ec25ff8b3` /
  `213c785d2df051dcde94460e45b28d1fd68fbaac6d53372764230935ca4e56c8` /
  `8f252e9fef87b452eb34d4de5710dbe8c93e3882c9d48fd3a15a78c05d00e23a`

2 multi-component aggregateでは、接触する全external componentのidentifier和集合をtarget groupとする。
target folding、共役folding、per-eigenvalue label collapse、relation evaluationを禁止する。

### monotone envelope and source hull

Q011biの604-record monotone proof inventoryをpriorとする。degree-28 current active inventoryはselected 24と
target 612の計636件である。priorとの共通512 recordをbitwiseに再利用し、92 inactive prior recordを削除せず
保持し、124 new target recordだけを追加した728-record monotone proof inventoryを作る。
...
- added-identifier／added-record／active-record／final-record digest:
  ...
  `bc2aac7fa7405e3628b600298c2fad3116fa38b3bc1132798f41a64e19c11f38` /
...
inactive 92 recordはmonotone proof assetに限り、degree-28 full sweepのtarget比較には使わない。
...
- original monomial／maximum aggregate monomial: `53517100201931 / 445981536000`
- modulus signature／peak live signature／two-array bytes: `57529 / 182 / 2912`
- safe `int64` convolution crude upper: `7581686112000`
...
1. Q011bjまでの41 artifact、runner、198 direct digest、outcome、boundaryが再現する。
2. degree-28 inventoryの全count、順序、multi-component membership、5 digestが再現する。
3. Q011biの604 prior recordを一件も削除せず、512 common recordをbitwiseに再利用し、92 inactiveと124 addedを
   明示した728-record monotone inventoryが再現する。
4. Q011asの6 source hullとmembershipがbitwiseに再現する。
5. class power、group／pair pool、convolution、signature、memory、comparison upperの全countとdigestが再現する。
6. relation objectを構築せず、全section、runner provenance、strict finite JSONがdigestへ封印される。
...
全て通れば`go_for_degree_twenty_eight_coalesced_preregistration`、一つでも超えれば
`stop_before_degree_twenty_eight_coalesced_full_sweep`とする。観測後のlimit変更、target folding、degree-28
relationの試し打ちで判定を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree-28 modulus-overlap inventory、Q011asの
6 source hull、Q011biから単調拡張した728-record proof inventory、exact combinatorial resource countだけを扱う。
degree-28 external nonresonance、actual resonance、minimum gap、all-order result、higher smoothness、SSM
existence／uniqueness、normal attraction、basin、他grid／force／wall、D3Q27を認証しない。

GoならQ011blで入力、全1170 aggregate、全612 current target、multi-component membership、outward
...
## Q011bl: degree-28 block-support-coalesced full sweep — 事前登録

### 問い

Q011bkで固定した6 source hull、1170 overlap aggregate、全612 current targetを、exact 17-point Fourier
multiplicityとoutward-rounded hierarchical product intervalで全件走査したとき、全compatible
product--target relationをstrictに分離できるか。さらにQ011uで既に分離した3325 aggregateを保存し、
degree 28の全4495 external aggregateを分離できるか。

Q011bkはdegree-28 relationを一件も評価していない。Q011blでは以下を固定してから初めて正式full sweepを
実行する。未観測のrelation count、matrix digest、minimum witnessはacceptance thresholdへ使わず、最初の
正式artifactでoutcomeとして封印する。

### sealed input

Q011bkまでの42 artifact、runner、203 direct digest、resource Go、Q011bj accepted certificate、scientific
boundaryを再照合し、Q011bkを直接封印する。

- Q011bk artifact／runner newline-normalized SHA-256:
  `bcd6f8608368637075be06c517ca31cf207d522175d10f726e91c534d582749c` /
  `571969fc442b000e319d7e8f12be75a1d60bbd6cccb0cbff3b6c822b8cfbf0a6`
- Q011bk input／inventory／envelope／resource／result digest:
  `93b5e34ef4203e7f80dd3dee04863ad6c194b4f5e92ff401b0de3a058855f75c` /
  `beef4f6d0a325dd6c191e51787349f8f2534100a9997ac048ebf2d14ca1d789b` /
  `cbc61f563f2e44219d998e6ab6f5c51096143997dc02e8782b304f2b54557053` /
  `57c947604d892000ab4a0fe1e66efad1c129d09caf9ccb8054602f745da8dbca` /
...
Q011bkの`study_gate=passed`、resource decision
`go_for_degree_twenty_eight_coalesced_preregistration`、relation evaluation count 0、Q011bj degree-27 accepted
certificate、certified degrees 2--27と91以降を要求する。

### fixed inventory, monotone envelope, hull, and active targets

Q011bk artifactから次をexactに再構成する。

- degree aggregate／old separated／direct overlap: `4495 / 3325 / 1170`
- overlap count first／last: `[0,3,2,23] / [25,1,2,0]`
- selected／current target／active／monotone final identifier: `24 / 612 / 636 / 728`
- prior／inactive retained／added／removed identifier: `604 / 92 / 124 / 0`
- external component／multi-component aggregate／maximum component: `49 / 2 / 3`
- old／merged source class count: `[4,2,3,6] / [1,1,2,2]`
- inventory／active-disc／monotone-final／hull／merged-membership digest:
  `8f252e9fef87b452eb34d4de5710dbe8c93e3882c9d48fd3a15a78c05d00e23a` /
  `3ff49a56c7f7d04700e3fabbfc0016cb7bafe58ead2bf2679d2983b55e18485f` /
...
各selected identifierは登録merged hullにcontainedし、wave blockとcenter indexを保持する。2
multi-component aggregateではQ011bkが固定した全接触componentのidentifier和集合だけをtarget groupとして
比較する。92 inactive retained discをdegree-28 relationへ混入させない。target merge、共役folding、
per-eigenvalue label collapseを禁止する。

### fixed hierarchical arithmetic

Q011bjと同じcomponent-safe outward protocolをdegree 28へ適用する。
...
4. product modulusをbinary64のdownward／upward rounded interval productで囲う。
5. output blockごとのexact integer Fourier multiplicityが正のentryだけをcompatibleとする。
6. compatible entryを登録target groupの各discに対して`product_below_target`、`target_below_product`、
   `overlap`へ排他的に分類する。
7. full monomial listとfull classification matrixは保持せず、aggregate summary、matrix hash、最初の
   overlap、global minimum separated witnessだけを保持する。
...
- class-power／group-signature／pair-key: `161 / 810 / 398`
- cached pair-signature entry／convolution: `12938 / 14505`
- original monomial／modulus signature: `53517100201931 / 57529`
- peak live signature／two product-array bytes: `182 / 2912`
- distinct／weighted comparison upper: `995152 / 971674189747228`
- safe convolution crude upper: `7581686112000`

wall time、tracemalloc、process memoryの実測値はdiagnosticとして記録してよいが、accepted／rejectedを変更しない。

### validity gate

1. Q011bkまでの42 artifact、runner、203 direct digest、outcome、boundaryが再現する。
2. Q011bk inventory、multi-component membership、monotone envelope、active target set、source hull、resource
   contractがexactに再現する。
3. 1170 aggregateを欠落・重複なく辞書順で処理し、全integer／outward interval invariantが通る。
4. original monomial、signature、convolution、peak、comparison upper countが登録値と一致する。
5. aggregate／bound／coefficient／classification summaryがstrict finite JSONとdigestへ封印される。
6. first overlapがある場合は辞書順最初のwitness、separated relationがある場合はglobal minimum witnessを
   exact rational reconstructionで封印する。
7. Q011bj degree-27 accepted certificate、Q011bk section digest、runner provenanceが再現する。

一つでも失敗すれば`inconclusive`とし、degree 28をcertified setへ追加しない。

### hypothesis gate and stopping rule

validity通過後、次を順に判定する。

1. 6 merged hullがselected 24 sourceを包含し、登録された612 current target discだけが個別比較される。
2. 1170 direct aggregateの全compatible comparisonに`overlap=0`である。
3. 3325 old-modulus aggregateと1170 direct aggregateの和がdegree-28全4495 aggregateを覆う。
4. global minimumのoutward gapとexact rational gapがともにstrictly positiveである。
5. claimを登録degree-28 external nonresonanceだけに限定する。

全5項目が通ればclassificationを
`the block-support-coalesced component-safe sweep certifies degree-28 external nonresonance`、scientific outcomeを
`accepted`、actual resonance outcomeを`ruled_out_within_registered_degree_twenty_eight_scope`とする。
certified degreesは2--28および91以降、missing rangeは29--90へ縮める。

validな計算で一つでもoverlapが残ればclassificationを
`the block-support-coalesced degree-28 sufficient certificate is rejected`、scientific outcomeを`rejected`、
actual resonance outcomeを`not_established`とする。最初のoverlap witnessだけを次の精密化候補にし、後付けの
threshold、class split、target foldingで結果を変更しない。

### 主張境界と次の変更

本gateは固定17² repaired exact map、fixed conservation leaf、degree 28、Q011uの4495 external aggregate、
Q011asの6 source hull、Q011bkの612 current target disc、exact x-Fourier multiplicity、hierarchical
outward-dyadic product protocolに限る。degrees 29--90、all-order nonresonance、Q011t graphとのhigher-order
一致、higher graph smoothness、SSM existence／uniqueness、normal attraction、basin、他grid／force／wall、
D3Q27を認証しない。

acceptedなら次はQ011bmでdegree 29をdesign-onlyに資源監査する。rejectedなら最初のoverlapだけを監査し、
inconclusiveなら最初のvalidity failureだけを修正する。

### Q011bl 封印結果（accepted）

全7 validity gateと全5 hypothesis gateが通過した。Q011bkまでの42 artifact、runner、203 direct digestを
再照合し、1170 overlap aggregateを欠落・重複なく一度ずつ処理した。2 multi-component aggregateは接触する
全external componentのidentifier和集合をtarget groupとして保持し、全612 current targetだけを比較した。

- fully separated／unresolved aggregate: `1170 / 0`
- distinct relation count: `418956 / 510388 / 0`
- weighted relation count: `26932451117752 / 30784153208098 / 0`
- minimum witness: aggregate `956`、selected counts `[12, 15, 1, 0]`、
...
scientific outcomeは`accepted`、actual resonance outcomeは
`ruled_out_within_registered_degree_twenty_eight_scope`である。3325 old-modulus aggregateとの和でdegree 28の
全4495 aggregateを分離したため、certified degreesは2--28および91以降、missing rangeは29--90へ縮んだ。
92 inactive monotone proof recordは保持したが比較には用いていない。主張は固定17² repaired exact map、fixed
conservation leaf、登録degree-28 external relationだけに限り、all-order resultやSSM uniquenessへは外挿しない。
次はQ011bmとしてdegree 29をdesign-onlyに資源監査する。
