"""Q011jn component-safe complex phase discs for Q011cb flatten ordinal ninety-six."""
...
q011ih = q011jl.q011ih
q011b = q011jm.q011b
q011z = q011jm.q011z

_ExactDisc = q011jl._ExactDisc
...
SOURCE_VARIANTS = q011jl.SOURCE_VARIANTS
SOURCE_POWER_MAXIMUM_COUNTS = (1, 1, 12, 12, 9, 9, 9, 9, 5, 5, 0, 0, 7, 7)
RADIUS_SIGNATURE_REPRESENTATIVE_VARIANTS = q011jl.RADIUS_SIGNATURE_REPRESENTATIVE_VARIANTS
EXPECTED_SOURCE_RADIUS_HEX = q011jl.EXPECTED_SOURCE_RADIUS_HEX
EXPECTED_TARGET_RADIUS_HEX = q011jl.EXPECTED_TARGET_RADIUS_HEX
EXPECTED_SOURCE_RECORD_DIGEST = q011jl.EXPECTED_SOURCE_RECORD_DIGEST
EXPECTED_TARGET_RECORD_DIGEST = q011jl.EXPECTED_TARGET_RECORD_DIGEST
...
Q011JM_ARTIFACT_SHA256 = "f28bb4afc96c330b7588b9aaf8c76b3cc8b9f2e1518b10d2dfe6faa4614ad13d"
Q011JM_RUNNER_SHA256 = "0f92eb7127b734fc929554c67fdf82fa0349b3db0204a6cd5ed3552e7c3df228"
Q011JM_DIGEST_NAMES = (
    "input_digest_sha256",
...
EXPECTED_Q011JM_ALLOCATION_RECORD_DIGEST = (
    "45007a1939813601a87f7e9791faa2fb10277911a4f50ed14b0303bd9bbe8476"
)

RESOLVED_CLASSIFICATION = (
    "the component-safe complex phase discs resolve the ninety-seventh "
    "Q011cb persistent refined witness"
)
PERSISTENT_CLASSIFICATION = (
    "the ninety-seventh Q011cb persistent refined witness persists under "
    "component-safe complex phase discs"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011jn component-safe phase-disc audit is inconclusive"
...
def _q011jn_framed_record_digest() -> Any:
    return q011ih.q011cx._BASE_FRAMED_RECORD_DIGEST("q011jn-component-safe-phase-comparisons-v1")


def _q011jn_exact_fraction_sequence_sha256(
    _domain: bytes,
    values: tuple[Fraction, ...],
) -> str:
    return q011ih.q011cx._BASE_EXACT_FRACTION_SEQUENCE_SHA256(
        b"q011jn-complex-margin-expression-v1",
        values,
    )
...
def _protocol_globals_are_restored() -> bool:
    return bool(
        ...
        and q011jm._protocol_globals_are_restored()
    )


def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011jm._sealed_input_audit()
    ...
    digests = tuple(cycle[name] for name in Q011JM_DIGEST_NAMES)
    checks = {
        "q011jm_two_hundred_fifty_one_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["artifact_count"] == 251
            and prior["direct_digest_count"] == 1_142
            and len(artifacts) == 251
            and all(prior["checks"].values())
        ),
        "q011jm_artifact_and_runner_sha256_match": bool(
            _file_sha256(artifact_path) == Q011JM_ARTIFACT_SHA256
            and _file_sha256(runner_path) == Q011JM_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011JM_RUNNER_SHA256
        ),
        "q011jm_section_digests_match": digests == Q011JM_DIGESTS,
        "q011jm_valid_interval_inert_outcome_and_boundary_reproduce": bool(
            artifact["study_gate"] == "passed"
            and artifact["refinement_outcome"] == "partition_inert_persistent"
            and artifact["scientific_outcome"] == "not_evaluated"
            and artifact["actual_resonance_outcome"] == "not_established"
            and cycle["failed_validity_order"] == []
            and theorem["individual_partition_is_interval_inert_for_ninety_seventh_q011cb_witness"]
        ...
        "q011jm_parent_partition_and_relations_reproduce": bool(
            fixed["ninety_seventh_parent_witness_selection_audit"]["selected_flat_ordinal"]
            == PARENT_FLAT_ORDINAL
            and fixed["compatible_allocation_count"] == EXPECTED_INDIVIDUAL_WAVE_ALLOCATION_COUNT
            and fixed["compatible_allocation_digest_sha256"] == q011jm.EXPECTED_COMPATIBLE_DIGEST
        ...
        "q011jm_artifact_is_finite_strict_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "one_thousand_one_hundred_forty_six_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 1_146
        ),
    }
    artifacts["q011jm"] = artifact
    return (
        {
            "prior_q011jm_sealed_input_audit": prior,
            "q011jm": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011JM_DIGEST_NAMES),
...
def _complex_from_record(record: dict[str, Any]) -> tuple[Fraction, Fraction]:
    return q011jl._complex_from_record(record)
...
def _fixed_phase_input_audit(
    ...
    fixed, _lookup, wave_compatible, _parent = q011jm._fixed_individual_input_audit(artifacts)
    stored = artifacts["q011jm"]["cycle"]
    partition = stored["individual_allocation_interval_audit"]
    q011jl_phase = artifacts["q011jl"]["cycle"]["fixed_component_safe_phase_input_audit"]
    source_records = q011jl_phase["source_phase_disc_records"]
    target_record = q011jl_phase["target_phase_disc_record"]
    source_discs = tuple(
        _ExactDisc(
    ...
    memberships = q011jl_phase["active_component_memberships_by_block"]
    active_indices = (*range(10), 12, 13)
    checks = {
        "q011jm_fixed_parent_replays_and_stored_interval_partition_is_inert": bool(
            fixed == stored["fixed_individual_partition_input_audit"]
            and fixed["passed"]
            and partition["passed"]
            and partition["all_product_target_intersection_and_center_records_equal_parent"]
        ),
        "sealed_q011jl_phase_records_reconstruct_exactly": bool(
            q011jl_phase["passed"]
            and all(q011jl_phase["checks"].values())
        ...
        "registered_active_pairs_zero_power_pair_and_conjugacy_reproduce": bool(
            ...
            and SOURCE_POWER_MAXIMUM_COUNTS[10:12] == (0, 0)
            and all(
    ...
    audit = {
        ...
        "inactive_zero_power_identifiers": [source_discs[index].identifier for index in (10, 11)],
        ...
        "q011jm_compatible_wave_allocation_count": len(wave_compatible),
        "q011jm_compatible_wave_allocation_digest_sha256": fixed[
...
def _component_wave_counts(individual_counts: tuple[int, ...]) -> tuple[int, ...]:
    if len(individual_counts) != 12:
        raise ValueError("Q011jn requires twelve Q011jm individual counts")
    return (
        *individual_counts[:4],
...
def _phase_wave_counts(phase_counts: list[int]) -> tuple[int, ...]:
    if len(phase_counts) != 14 or phase_counts[10:12] != [0, 0]:
        raise ValueError("Q011jn requires fourteen canonical counts with an inactive 148 pair")
    return (
        *phase_counts[:4],
...
def _allocation_inventory(
    ...
    audit = {
        ...
        "inactive_zero_power_identifiers": [disc.identifier for disc in source_discs[10:12]],
        ...
        "allocation_protocol_adapter": {
            ...
            "ordinal_ninety_six_active_totals": [1, 12, 9, 5, 7],
            "canonical_zero_power_indices": [10, 11],
...
def _phase_product_audit(
    ...
    replacements = {
        ...
        "PARENT_FLAT_ORDINAL": PARENT_FLAT_ORDINAL,
        "_q011ih_framed_record_digest": _q011jn_framed_record_digest,
        "_q011ih_exact_fraction_sequence_sha256": _q011jn_exact_fraction_sequence_sha256,
    }
    try:
        ...
        for name, value in _Q011IH_PROTOCOL_BASELINE.items():
            setattr(q011ih, name, value)
    audit["comparison_stream_domain"] = "q011jn-component-safe-phase-comparisons-v1"
    return audit


def _registered_parameters() -> dict[str, Any]:
    return {
        "protocol": "q011jn-ninety-seventh-component-safe-phase-v1",
        "degree": DEGREE,
        "parent_flat_ordinal": PARENT_FLAT_ORDINAL,
        "output_block": OUTPUT_BLOCK,
        "target_identifier": TARGET_IDENTIFIER,
        ...
        "comparison_stream_domain": "q011jn-component-safe-phase-comparisons-v1",
        "complex_margin_digest_domain": "q011jn-complex-margin-expression-v1",
        "q011jn_allocation_enumerator": {
            "source_runner_sha256": _file_sha256(Path(__file__).resolve()),
...
def run_degree_thirty_four_ninety_seventh_component_safe_phase_audit() -> dict[str, Any]:
    ...
    validity_gates = {
        "q011jm_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "252 artifacts and 1146 direct digests reproduce",
            "value": sealed["checks"],
        },
        "q011jm_parent_partition_and_interval_inert_result_replay": {
            "passed": phase_input["checks"][
                "q011jm_fixed_parent_replays_and_stored_interval_partition_is_inert"
            ],
            "threshold": "ordinal 96, 1667 individual wave allocations and inert result replay",
            "value": phase_input["q011jm_compatible_wave_allocation_digest_sha256"],
        },
        "exact_centers_radii_components_target_and_zero_power_pair_reproduce": {
            "passed": bool(
                phase_input["checks"]["sealed_q011jl_phase_records_reconstruct_exactly"]
        ...
        "strict_stream_section_digests_runner_and_protocol_provenance_reproduce": {
            "passed": bool(
                ...
                and runner["filename"]
                == "q011jn_degree34_ninety_seventh_component_safe_phase_discs.py"
    ...
    diagnostic_gates = {
        "only_the_registered_ninety_seventh_q011cb_witness_is_refined": {
            "passed": bool(
            ...
            "threshold": "ordinal 96 only, one target and no other overlap",
    ...
    cycle = {
        "question": (
            ...
            "strictly separate every complex product in Q011cb flatten ordinal 96?"
        ),
        **input_sections,
    ...
    prior_theorem = artifacts["q011jm"]["cycle"]["theorem_consequence"]
    cycle["theorem_consequence"] = {
        "component_safe_complex_phase_discs_resolve_ninety_seventh_q011cb_witness": resolved,
        "ninety_seventh_q011cb_witness_persists_under_component_safe_phase_discs": persistent,
        "q011an_component_internal_eigenvalue_labels_are_assumed": False,
        "q011jm_ordinal_ninety_six_interval_inert_diagnostic_is_preserved": True,
    ...
    cycle["claim_boundary"] = (
        ...
        "ordinal 96, its 1667-to-701 active component-wave quotient and 15278 "
        ...
        "does not reevaluate ordinals 0 through 95 or classify the later 44703 "
    ...
    cycle["next_change"] = (
        "Preregister Q011jo to audit the next Q011cb refined overlap in registered order."
        if resolved
        else (
            "Preregister Q011jo to refine only the first unresolved component-safe phase witness."
        )
        if persistent
        else "Repair only the first Q011jn validity failure."
    )
    if not (
        ...
        raise RuntimeError("Q011jn cycle failed strict serialization or digest")
    return cycle


def run_q011jn_study() -> dict[str, Any]:
    started = perf_counter()
    cycle = run_degree_thirty_four_ninety_seventh_component_safe_phase_audit()
    ...
    return {
        ...
        "mathematical_scope": {
            "diagnostic": "ninety-seventh Q011cb witness component-safe complex phase discs",
            "grid": [SIZE, SIZE],
