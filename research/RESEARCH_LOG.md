...
観測timeoutで再起動せず、全17³/33³/65³・63方向・192 caseを最後まで計算した。
最終親の全数保存後監査、実行前後の入力／source一致、10 validityとH1/H2/H3が全て通過した。
05:39:40 JST時点でexit 0とprocessの終了を確認した。このworker handleは以後pollしない。
126 profile、元1,152 field・384欠陥、4,599全vector比較を全件照合した。holdout60 caseのexact記録も元Q012g1と一致した。
元一般方向の3不合格、holdoutの29半減不達・14悪化を全件再現した。旧H4/H5と元Q012gの棄却は不変である。
65³・index21・`.032`の最大悪化比は元5.834017983080117、九次予測5.834017984274533だった。
同例の先頭項同士の比は2.44118567に過ぎず、四次までの予測のvector相対誤差は約91.70%、六次までで6.58493e-5だった。
また65³・index0・`.008`では先頭項の比が0.5未満でも全欠陥比は0.5を超える。先頭項だけを判定に使わない実例となる。
逆にindex6・`.008`は欠陥半減を再現するが旧H5 compositeはfalseであり、保存量の総合判定と欠陥比を混同しない。
最終親・全三gridの外部SHAを封印した追加12検査は、fresh入力chain・全63child・全4,992 Gram集計・反例改変検知を含み、65.54秒で通過した。
既存58回帰も29.35秒で通過した。計70件を確認し、Ruff・整形・compileallも通過した。
解析検証手順に従い、この結果は`passed / worker_only`とする。全960 caseの診断成功や三次chartの改善・存在認証には一般化しない。
本記録のcommit時点でmainは未開始。同じsource・係数・振幅・方向・閾値を保持して、次にmainと全primary record対応を検証する。

## 2026-09-08: Q012g3 の65³一般方向を閉じ、残る二つの反例を分類

反例34の結果と9検査は`2f4031d`に保存した。その後、同じworkerの登録一般11方向・44 caseを全件監査した。
22 profile・元264 field・88欠陥・803全vector比較が揃い、両方式H1・Gram・独立照合・全11方向H3が通過した。
全元／予測fitを閉じたOLS式で、全1,144切断Gram norm²を補償和でも確認し、全childを外部SHAに封印した。
元H4の不合格集合34/51/52を厳密に保持した。方向51は元三次傾き4.102354、九次予測4.102421で上限超過を再現した。
方向51も四次までの欠陥傾きはほぼ4、五次まで4.083367、六次まで4.102421となった。
方向52は傾きが元の区間内であり、最小振幅比の元0.10292984・予測0.10292377が基準0.1を超える別の反例である。
先頭のC4/C3 normからの比も`.001`で0.10310159なので、五次以上だけが比の超過を生んだとは言わない。
この先頭項の比を全欠陥比の代わりに使わず、振幅半径の認証にも一般化しない。
全方向の監査と反例削除・元判定／比書換え・末尾case／C9欠落の拒否を含む追加10テストが4.08秒で通過した。
既存48件と合わせた58テストも警告error化で通過した（30.71秒）。Ruff・整形・compileallも通過した。
解析検証手順に従い、傾き不達・係数比不達・元判定の再現を分離した。元Q012gの棄却と全Q012g3未判定は不変である。
同じPID 27060／handle 51090はholdoutを継続中。最終workerの全監査・終了前にmainを開始しない。

## 2026-09-08: Q012g3 の65³・登録反例34で傾き超過を再現

PID 27060／handle 51090の同じworkerを継続している。一般方向0–7の32 caseではH1/H3・Gram・独立照合が通過した。
続いて登録反例34の全4 case・元24 field・8欠陥・73全vector比較を保存後監査した。
元三次傾き`4.103841695537577`に対し九次予測は`4.103894349187252`で、元H4のfalseを保持してH3が通った。
最小振幅の欠陥比も`0.0801557831453807`から`0.08014626737054589`へ再現した。
閉じたOLS式で元／予測fitを別途計算し、全104切断Gram norm²も補償和で照合した。
同じW3の四次までの欠陥予測の傾きはほぼ4、五次までで4.07769、六次までで4.10389だった。
有限窓での高次寄与を元の傾き上限超過と結び付けられるが、新しい高次chart、漸近次数の変更、G3単独の原因とは主張しない。
旧H4の成功への書換え、予測判定の書換え、末尾case/C9欠落・座標改変を含む追加9テストが通過した（1.88秒）。
既存二格子の22回帰・合成恒等式17件と合わせた48テストも警告error化で通過した（26.69秒）。
検証手順に沿い、元の不合格の再現と、その不合格の解消を区別した。元Q012gの棄却・全体未判定を保持する。
全source digestは封印時と同じで、正式177対照も不変。worker全192 caseの保存後監査・終了前にmainを開始しない。
方向34の封印と詳細値は[次数別診断](../docs/D3Q27_CUBIC_DEFECT_DIAGNOSIS.md)に保存した。

## 2026-09-08: Q012g3 の合成項を代数的に整理

前goal turnはPID 27060／handle 51090の生存と進行を確認したverified waitである。
開始後の観測では同じworkerの65³が2方向・8 caseを保存した。最終gridは未完了で、主計算は開始しない。
待機中に、三次Wと三次Rの合成J0–J9の全表を新しい補足資料へ整理した。
Taylor係数を含む対称多重線形写像B/Tと、置換数2/3/6を区別した。
全20種類の項を40組の次数選択の列挙と照合し、二次/三次の各組合せを座標多項式の積でも別途検証した。
有理数による全係数・正/負/0の直接値の厳密一致、表の全行、誤係数対照を含む17テストが0.16秒で通過した。
W3とR2だけでも六次項T(r2,r2,r2)が残り得る。これは高次欠陥をG3一項の誤りと断定しないための構造上の注意である。
実反例のCnにおける各項の支配性の測定ではなく、実格子の受理を意味しない。
分析検証手順に沿って恒等式・数値再現・原因の帰属を分離し、稼働中のscientific sourceと正式177対照は変更していない。
補足17件と既存17³/33³の保存後監査22件を合わせた39テストも通過した（24.15秒）。Ruff・整形・compileallも通過した。
その後65³は3方向・12 caseの保存まで進んだ。全192 caseとmain960 caseの判定はまだ行わない。

## 2026-09-08: Q012g3 の33³を全監査し、既存の悪化を再現

前goal turnは実行器`7412b46`と17³の結果`504bdef`をcommitしたためprogressである。
同じPID 27060／handle 51090の生存・進行を確認して継続し、33³の全21方向・64 caseが完了した。
42 profile・元384 field・128欠陥・1,533全vector比較、全21方向childのSHA/内容/headerを全数監査した。
H1/H2/H3・Gram・独立照合の不達は0。最大再構成差`1.5336496636226352e-14`は元floor以内だった。
登録holdoutの`.032`全10 caseの半減不達、index 2/4/7/16の4悪化を次数別予測でもそのまま再現した。
index 16の元欠陥比`1.316172134164264`に対し、予測は`1.3161721341163626`。旧H5 compositeはfalseのまま保持する。
この反例の三次欠陥は四次までの予測でvector相対誤差約49.84%、六次まででは`9.03438e-5`だった。
九次予測の誤差は約`8.6714e-10`。低次数の丸めや有理式の残りだけで悪化を説明せず、高次項も分けて残す。
これらは登録済み反例一件の記述であり、純粋なleading C4だけによる全方向の説明や新しい高次chartではない。
33³の追加11テストは独立OLS式・全1,664 Gram集計・全holdout合否・反例削除/旧判定改変/末尾座標改変を含む。
17³の11回帰と合わせて22件が警告error化で通過した（24.93秒）。Ruff・整形も通過した。
検証手順に沿い、共有可能な二格子の途中結果と全体の未確定判定を分離した。
同じworkerが65³へ進んでいる。sourceを編集せず、全192 caseの終了・保存後監査前にmain960 caseを開始しない。
...
二次／三次の物理経路、非零内部力学の九次までの合成、局所有理式の商と全残り分子を計算し、
collision→stream→filterを元順序のまま接続した。C0–C9や低次数の丸めを0へ修理しない。
primaryと独立方式の全係数場36/37比較、全Gramと符号付き交差項、残り込みの再構成を小格子で検査した。
非立方2×3×4局所場と2実座標・7³人工chartを用い、非単位density・非零momentum定数、複素共役経路、
反復/重複monomial、余分な1/6、G3欠落、C0削除、末尾C9成分改変、不正入力・overflowを対象にした。
新規45と先行Q012g2の77、計122テストは警告error化で全件通過した（17.90秒）。Ruff・整形も通過した。
検証手順に沿って人工場の結果と実LBMの科学的判定を区別した。実格子17³/33³/65³の診断は未実行である。
次はrunnerの元record/field再現、holdout exact照合、保存後監査と逐次worker/mainを実装する。

## 2026-09-08: Q012g3 実LBMの次数別欠陥診断を事前登録

前goal turnは保存量診断`97c109c`、人工オラクルの登録`0bb6ce1`と受理`e6ead08`をcommitしたためprogressである。
current worktreeはcleanで、全主/workerは終了済み。物理計算の再起動や並行実行はない。
元Q012gの全一般方向・holdout、三格子960 caseを次問へ登録した。
独立側は一般`0..7,34,51,52`、holdout`0..7,16,21`の192 caseとし、既知のH4反例と各格子最悪の欠陥比3/16/21も含めた。
元特殊288 caseの未解像fitは保持するが、元からdiagnostic枠であるため本問の仮説範囲には含めない。

元の小振幅の三次欠陥は最小で17³ `1.33994e-12`、33³ `5.51534e-12`、65³ `1.35331e-11`。
各格子の元floor `5.50261e-13 / 1.48822e-12 / 4.11401e-12`と区別し、小さい値の相対精度を自動的に主張しない。
小さな一様配列の既存局所moment評価はx運動量`-1.3877787807814457e-17`を返した。
exactな基準運動量0とは別の丸め済み局所演算であり、rho0/j0を1/0へ上書きしない設計とした。
C0–C9、元と同じ順序のcollision→stream→filter、有理式の残り、次数間の符号付きGramを保持する。
独立側はmonomial次数の全組合せ列挙、逆数の多項係数和、Hessian bilinear式を使う。
H1/H2/H3を再構成・holdout予測・fit再現へ分け、元Q012gの閾値と棄却は変更しない。
この時点では新規helper/runnerは未実装、実LBM診断は未実行。登録をcommitしてから人工対照付き実装へ進む。

## 2026-09-08: Q012g2 全96人工caseの次数別オラクルを受理

登録commit `0bb6ce1`の後に新規helper/runnerと専用テストを実装し、71テストを警告error化で通過させた（13.23秒）。
実行前の親入力とsourceを照合してから、01:12:43 JST、PID 37164で全96 caseを保存した。exit 0で終了済み。
6 validity gateとH1、全数読戻し・再生成監査が通過し、`passed / accepted`と確定した。
6方向×二次／三次の12組・1,824係数は別記述のFraction式と比較誤差0だった。
記述的なexact比較でも全1,824値が厳密一致したが、新しい受理条件には追加していない。
全2,976 case値と3成分の残りidentityの最大絶対誤差は`1.6601845766184287e-19`で、元の`256*eps*scale`以内だった。
有理式の残りは最大`1.7635351165435856e-12`であり、P9単独をexactな有限振幅欠陥とは扱わない。
非単位定数density・複素経路・重複monomial・空support・誤factor・G3欠落・低次数残り・不正入力の対照を通過した。

追加の保存後検査では、検査側が係数表の`numerator`を単一分数と誤認して3テストが失敗した。
decoderとその回帰テストだけを修正し、新規71＋保存後6、計77テストが通過した（15.87秒）。
正式artifact、helper/runnerのSHAは修正前後で同一。科学的な再計算、係数の修正、判定上限の変更はない。
保存後には全direct mapの独立Fraction式、三次欠陥の多項係数による逆数和、末尾成分改変の拒否も確認した。
Ruff・整形確認・compileallは通過した。新規source・artifact・evidence digestを設計書に固定した。
最後にQ012g1 mainの保存後監査10件と新規77件を併せた87テストを再実行し、全件通過した（85.21秒）。

この受理は診断器の人工代数に限定する。元Q012gのH4・欠陥比不達・総合棄却は保持する。
次は同じ実LBMへのstream/filter接続、丸め済み物理場との整合、全元一般方向・holdoutの次数別欠陥と
次数間干渉を対象にした問いを登録する。新しい高次chart・SSM存在・連続球・TT優位性は未認証である。

## 2026-09-08: Q012g2 次数別欠陥診断の代数オラクルを事前登録

Q012g1の受理をcommit `97c109c`で保存した後、次の問いを登録した。
三次chartの有限振幅悪化を直接説明する前に、非自明なG2/G3を持つ人工写像で、
九次までの合成・有理式係数・残りを独立なFraction式と照合する。
6方向・4振幅・両符号・二次／三次の96 case、係数と全成分の比較、人工対照・改変検知を固定した。
有理式のprimaryは係数漸化式、referenceは有限幾何級数とし、共通のNumPy合成器を使わない。
低次数の丸め残りを0にせず、P9単独とexactな残りを分離する。
ここで求めるのは既存W/Rの合成次数であり、W9のsolveや新しい不変多様体ではない。
後続実LBM監査へQ012g1の検証済み集計・丸め済みf*基準・元閾値を採用することも明記した。
本書をcommitしてから実装する。実LBMでの元H4・欠陥比不達の原因はまだ未診断である。

## 2026-09-08: Q012g1 全192 caseを完了し、保存量診断を受理

前turnは独立workerの全数監査`4095b35`と主計算の開始・二格子監査`df6597e`をcommitしたためprogressである。
primary PID 5024／handle 14558を重複起動せず継続し、00:39:06 JSTに全三格子の最終JSONを保存してexit 0で終了した。
同handleは終了済みであり再pollしない。helper/runnerは実行前封印と同一で、条件・係数・物理場の変更はない。
全192元record・1,152 fieldの一致、31,104 population和・4,608成分の符号付きexact identity、12人工違反を全数監査した。
独立GMP workerの全48 case・288 field・全分解項・12対照も整数方式と厳密一致した。8 validity gateとH1が成立し、`passed / accepted`である。
保存後に別記述のFraction式で全数値・元反例・判定を再集計し、末尾case/成分・集計・child sealの改変も拒否した。
実装71、worker監査10、main監査10、親監査53、計144テストが通過した（204.00秒）。Ruff・整形確認・compileallも通過した。

元の不達は65³の両degree・W/W(R)葉の質量256成分、最大`5.46839381544819e-13`。
exactな場の誤差は全4,608成分で上限以内（最大`8.804966277311955e-17`）。両側の総和だけを交換した誤差も全件通過し、最大`2.1195324866078255e-16`だった。
65³の基準側集計誤差はsite平均`+5.322701185384562e-13`で、葉の差へ一定の負号付きで現れた。
同じ葉のA側集計項は約±1.5e-14、exact場誤差は約1.2e-19以下であり、基準側の集計が元超過の主要項である。
記述用の基準側のみ交換も全3,072葉成分が上限以内（最大`1.483672740625478e-14`）だが、新しい受理条件にしない。
丸め済み基準と解析的基準の質量site平均差`-2^-54`は別に保持し、exact場誤差を厳密な0とも解釈しない。

これは全192 holdoutの保存量診断であり、元Q012g全1,248 caseの再判定ではない。元H4の3方向と欠陥比の不達、総合棄却は保持する。
検証スキルの確認でも、worker部分集合と全数、場の誤差と集計誤差、基準差と物理保存則を分離した主張のみを採用した。
保存後監査に未実行事項はない。SSM存在・連続球・長時間trajectory・TT優位性はこの問の範囲外である。
次は検証済み集計の後続採用条件を明記し、既存W2/W3の次数別欠陥診断を事前登録する。

## 2026-09-08: Q012g1 primaryを開始、17³/33³の全128 caseを監査

独立GMP workerを全数監査してcommit `4095b35`で保存した後、同じ封印runnerによる主192 caseを開始した。
primary PID 5024／handle 14558、開始00:13:59 JST。終了済みworkerとは同時実行していない。
17³は00:15:48、33³は00:18:46に全64 caseずつを保存した。両格子の全768 field・3,072成分の読戻し監査を通過した。
別記述のFraction identity、全population/moment式、失敗集計でも全件を照合した。
さらに、対応する独立32 case・全192 field・8対照は、全数値とhashがGMP workerと厳密一致した。
主17³/33³のexact場誤差最大は`8.804966277311955e-17 / 8.544841785008639e-17`。
総和だけの交換後の最大は`1.851200288566921e-16 / 2.0246424615809405e-16`で、元上限以内だった。
この範囲は元の保存量gateも合格しており、65³の全方向へ一般化しない。

65³は同じ主計算で進行中。全192 case向けartifact監査10テストを準備し、最後の方向・成分の改変や欠測も拒否する設計とした。
共通の独立式を再利用するためworkerテストを整理し、既存10テストは再通過した（41.34秒）。Ruffと整形確認も通過した。
新規10テストはcollection確認までであり、最終親JSONの保存・SHA確定後に全件実行する。実LBM helper/runnerは封印したままである。
未完了primaryの最終判定は出さず、現在の二つのchildと変更中の監査テストはmain閉鎖時にまとめて検証・commitする。
次は同じhandle 14558の終了を確認し、全48 GMP case・288 field・全分解項・12対照との一致とH1を全登録範囲で判定する。

## 2026-09-08: Q012g1 独立48 caseを完了し、元超過64成分を集計項へ切り分け

前turnは実装・検証の`ba59086`と進捗記録`4685dd5`をcommitしたためprogressである。
今turnはPID 12416／handle 41088のlive状態を再確認し、同じGMP workerを継続した。timeoutによる再起動はない。
00:08:34 JSTに全三格子・48 caseを閉じ、exit 0。handle 41088は終了済みであり再pollしない。
全元recordと288 fieldが一致し、7,776 population和・1,152成分のexact identity・全12人工違反を保存後に再監査した。
診断器とは別のFraction式・27速度のmoment式・失敗リスト集計でも全件照合した。実装71件と新規artifact10件、計81テストが通過した。
Ruff・整形確認・compileallも通過した。helper/runnerのsourceは実行前封印と同じである。

部分集合の元超過は65³の両degree・W/W(R)葉の質量64成分のみ。最大`5.442959425608896e-13`だった。
exactな場の誤差は三格子の全1,152成分が上限以内（最大`8.787347164458049e-17`）。
両側の総和だけを交換した誤差も全件通過し、最大`2.1195324866078255e-16`だった。
65³の基準平衡の集計誤差はsite平均`+5.322701185384562e-13`で、葉の誤差へ負号付きで現れた。
A側の集計項は約±1.2e-14、exactな葉の場誤差は約1.2e-19以下であり、この部分集合では基準側の集計が主要項だった。
基準だけを交換する記述用対照も全768葉成分が通り、最大`1.2081335173664605e-14`。受理条件には追加しない。
workerと三childのSHA、数表、符号付き分解を設計書へ封印した。

これは全192 caseの受理ではない。次は同じ封印runnerによるprimary整数方式で全登録範囲を再生成し、
全48 case・288 field・全分解項・12対照をGMP workerと厳密一致させる。Q012g1のH1・総合判定は保留する。
元Q012gの棄却と次数条件・欠陥比の不達は保持し、この保存量の診断へ混ぜない。

## 2026-09-07: Q012g1 独立GMP workerを開始、17³/33³を保存後監査

実装と実行前封印をcommit `ba59086`で固定してから、登録48 caseの独立workerを開始した。
PID 12416／handle 41088、開始23:46:07 JST。17³・33³は各16 case・96 field・384成分の全件再現と保存後監査を通過した。
診断器とは別に書いたFraction identity・独立な27速度のmoment式でも、この32 case・192 field・768成分を全件照合した。
元record/fieldを変更せず、両格子のexact場誤差の最大は`8.787347164458049e-17 / 8.460784664912575e-17`だった。
両側の総和だけを交換した誤差の最大は`1.851200288566921e-16 / 2.0246424615809405e-16`。
この32 caseは元の保存量gateも合格しているので、65³の不達原因を説明した結果へ一般化しない。
丸め済み平衡の解析的基準とのsite平均差は質量`-1/18014398509481984`、三運動量0であり、総和の丸めとは別に保存した。

65³は同じworkerで進行中。23:55時点では最初の4 caseを完了し、後続fieldを逐次集計していた。
helper/runnerの実行前SHAはこの時点でも一致した。主192 caseは未開始で、Q012g1の総合判定は保留する。
次は同じhandleの終了を確認し、全三格子のworker出力を全数監査してSHAを固定する。
その後に初めてprimaryを開始し、全48 case・288 field・全分解項・12対照を整数方式と照合する。
新規artifact監査テストは最終出力待ちであり、現在の中間JSONとともにまだcommit対象に含めない。
新規71実装テスト、親の53テスト、Ruff・整形・compileallの通過は実装commitに記録済みである。

## 2026-09-07: Q012g1 exact保存量診断器を実装・人工検証

親追加入力封印`f984246`の後、登録済みの新規helper/runnerと専用テストを実装した。
元のmap、二次/三次chart、係数、source chainを変更せず、旧case全recordと場の指紋の一致を要求する。
primaryは指数別1,024項以下の整数chunk、workerは全float値からGMP有理数への逐項和である。
27 populationのexact総和、4 moment、元の符号付き誤差、総和/減算/平均の各項、二つの反実仮想を保存する。
丸め済み平衡と解析的保存量も分けた。失敗行は途中の証拠と箇所を残し、欠測を0や合格に置換しない。
新規71テストが通過した。最大有限値・全有限指数bin・chunk境界・相殺・subnormal・1 ulp・不正入力と、
全件読戻しの改変検知、4種の人工保存量違反、worker終了前のprimary拒否を含む。
人工試験で見つけたNumPy boolの保存形式を修正して再実行した。実LBM診断はまだ開始していない。
親封印の11項目も全て通過し、旧反例256成分を保持した。Ruff・整形確認・compileallは通過した。
親Q012gの保存後監査53テストも再実行して全件通過した（96.99秒）。
実行前source sealを設計書へ記録した。実装をcommit後、48 caseの独立GMP workerを完了してから192 caseへ進む。
保存量の原因・H1の成否は未判定であり、Q012gの棄却と次数・欠陥比の不達を変更しない。

## 2026-09-07: Q012g1の親入力を追加封印し、実装開始条件を確認

Q012g全数結果をcommit `e665c5c`で保存後、条件付き設計`90883ee`で要求した親結果の追加入力封印を行った。
main/65³ childのnormalized SHA、scientific digest、親の確定判定を設計書へ追加した。
親は12 validity・generic/holdout解像性・全件監査・144 case再現を通過し、65³全64 holdoutの質量葉不達が残る。
したがって登録した開始条件は成立した。元の192 case、1,152 field、4,608成分、48 caseのGMP worker、対照と閾値は変更しない。
H4の3方向とH5欠陥比の不達を保存量診断の成功へ読み替えない。
入力ファイルとdigest、親commit内のbyte seal、文書の対象件数を照合した。
この追記のcommit後、実装・人工対照検証へ進む。新規helper/runnerと実LBM診断はまだ未実施である。

## 2026-09-07: Q012g全三格子を完了・監査し、H4/H5を棄却

前turnはQ012g1の条件付き設計をcommit `90883ee`で固定したためprogressである。
今turnは同じ主計算PID 29360／handle 89608をlive確認し、65³の残りの物理caseと11巡の費用測定を待った。
23:06:45 JSTに最終JSONを保存し、exit 0で終了した。timeoutによる再起動、source・条件の変更は行っていない。
全三格子・1,248 caseを終え、12 validity gateとgeneric/holdout解像性は通過した。Q012gは`passed / rejected`とする。

H1（実評価・固定葉構造）、H2（48立方対称性）、H3（独立物理三次式）は全三格子で通過した。
H4の一般方向gateは17³/33³/65³で64/64、64/64、61/64。
65³のindex 34/51は三次傾き`4.103841695537577 / 4.102353999806372`が上限4.1を超えた。
index 52は最小振幅の欠陥比`0.10292984357429616`が上限0.1を超えた。近いからと許容値を変えない。
全72特殊direction-grid fitは未解像でnullを保持した。最初の8方向のworker結果を全一般方向へ一般化しなかった。

H5のholdout総合通過は64/64、32/64、0/64。33³の`.032`全32方向が半減不達、5方向は二次より欠陥が大きい。
65³は`.008`で17/32、`.032`で32/32が半減不達で、`.032`全32方向は二次より欠陥が大きい。
最大比は65³ holdout index 21、振幅`.032`の`5.834017983080117`だった。
全192 holdoutはpositiveだが、65³の全64 caseは二次/三次のW葉・W(R)葉の質量成分で上限を超えた。
運動量とPhi(W)−Wの保存差はholdoutで上限以内。保存量誤差のholdout内最大は`5.46839381544819e-13`。
部位の同定であり、丸めだけが原因という結論ではない。H4・H5欠陥比と保存量gateを分離する。

全1,248 case、全fit、全診断witness、1,320 timing sample・45 warmup、全保存量成分を監査した。
別processの144 caseは全数値・field hashで一致した。全三次係数から独立再構成した準備配列もworker/main全6組と一致した。
親結果に全childが厳密に含まれること、全入力・人工対照、12 validity・5仮説・総合判定を確認した。
53テストが62.72秒で通過した。Ruff/format/compileallと差分検査も通過した。
これは全保存数値の監査と登録144 caseの再現であり、未保存の全物理場のfresh再計算ではない。

main normalized SHA `6a201f212b321d8d7b3d4d143306633608c9df4c50c763e05dcd044695002944`、
scientific digest `aea8c955e3b258766bc516f6ebcc3130ff6a477ac0901bad9dcc656236676b8e`。
65³ child normalized SHA `1b5b6248ad2b60b4bf67baa70bfd59615e63a6c59a3328b64f8f3dde31d5a01f`。
次はQ012g1の追加入力封印後、同じ192 holdout caseの保存量不達を診断する。元Q012gは書き換えない。
H4の3方向とH5の欠陥比不達は次の次数別診断へ残し、SSM存在・連続球・長時間・TT優位性を主張しない。

## 2026-09-07: Q012gの主計算を追跡し、保存量診断を開始条件付きで設計

前turnは元fiberと全準備配列の同一性を7テストで確認し、commit `10390be`を作成したためprogressである。
今turnは同じ主計算PID 29360／handle 89608のlive状態と継続出力を確認した。
65³の次数評価256 caseを終え、有限振幅へ進んだ時点で、まだ格子JSON・全mainは閉じていない。
途中のfinite/resolved出力を次数gateや全体の合格へ読み替えず、sourceと条件を固定したまま待機する。

待機中の設計として、Q012g1で保存量の数値不達だけをexactな集計へ切り分ける手順を条件付き事前登録した。
全192 holdout caseの1,152 fieldを元hashまで再現し、4,608成分の差を場そのもの・総和・減算・平均の丸めへ分解する。
primaryの指数別整数chunk和と、48 caseの別process/GMP逐項和を独立に照合する設計とした。
元のfloat `5e-13`をexactな二進有理数へ写した同じ閾値を使い、元のmap・係数・振幅・方向を変えない。
仮説が通ってもQ012gの判定を更新せず、欠陥比不達は別の次数別診断へ残す。

本問のhelper/runnerはまだ実装せず、実LBM診断も実行していない。
開始には親の全三格子終了、validity・解像性・全件監査・144 case再現の通過、親commitとmain/65³の追加封印を必要とする。
親のvalidityが不十分なら先にその障害を扱う。計算が稼働中であること自体をblockerや失敗とはしない。
手順の詳細は[Q012g1の条件付き設計](../docs/D3Q27_CUBIC_CONSERVATION_DIAGNOSIS.md)。
