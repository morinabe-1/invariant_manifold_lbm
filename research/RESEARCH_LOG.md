...
旧S0・H1の全再監査と514不合格の再現を通過し、17³ primaryの全698組・1,826列を保存・全件再計算した。
workerは7組・25列後、tuple `(0,0,77,77)`の保存恒等式検証で停止。main PID 30092の実exit 1を取得した。
**原因**: 軌道因子sqrt(2)を差に掛ける演算と、各寄与へ掛けてから差を取る演算が有限精度では同一でなかった。
実入力の128/192 bitで22/21成分、最大相対差3.17e-39/1.18e-58を再現した。RAM条件や誤差上限の不達ではない。
**改善**: 独立方式のblock正規化後のcollision/compositionからF4を定義する。raw多項式・全入力・主方式・閾値は不変。
修正前の人工3不合格を再現し、修正後の関連14検査を通した。旧source・部分出力・初回資源停止recordを保持する。
停止artifactの全entryと実停止tupleの再構築を加え、計167回帰テストが警告error化で通過した（67.32秒）。
**次**: 回帰検証とcommit後、attempt03として全三格子・全列を再実行する。旧primaryだけを新sourceの結果へ混ぜない。
`validate-data`に従い、全件未完了のinconclusiveとし、高精度候補やmoment-only説明をまだ受理しない。
詳細・保存物のhashは[正規化順序の修正](../docs/D3Q27_QUARTIC_FORCING_PRECISION.md#ram回復後の再実行と正規化順序の修正-2026-09-08)へ。
...
**実装**: GMP七項式と元Taylor単項式のnilpotent代入を分離した。元binary64値を正確に取り込み、
整数factor・moment和・Fourier係数を指定精度で形成する。moment-only armではmoment vectorだけを一度丸める。
全104座標・698組/格子を保持し、両精度の全有限二進値・丸め後配列・七項を保存し全件再構築する実行器へ接続した。
**検証**: 新規68件＋先行H1の81件、計149テストが43.56秒で通過した。
人工126混合組×二精度、厳密ゼロ/微小非零moment、入力外内部座標、階乗・共役・末尾改変・偽process・資源失敗を検査した。
**境界**: `validate-data`に従い部品検証と実LBM結果を分離する。新精度の正式結果は未評価、旧514不合格を保持する。
**次**: 新sourceをcommit後、親H1の全entry監査、三格子・二process方式、保存後全件再構築と別CLI監査を実行する。
高精度一致とmoment-only説明の成否は別判定。H2/H3、全四次chart、元振幅の改善、SSM、疎表現対TT費用は未完了のまま。
詳細・再現手順は[Q012h2a実装検証](../docs/D3Q27_QUARTIC_FORCING_PRECISION.md#実装検証-2026-09-08--正式計算前)へ。

## 2026-09-08: Q012h2 H1を全件計算 — 独立F4一致の514不合格を保持

source `71bedc9`で三格子×698組・全5,478対称列のF4を実装のまま計算し、全entryを保存後に再計算した。
全入力/親/S0の再監査、正式6 processとmainの実exit 0を確認した。別CLIの全入力・全entry再構築も実exit 0で完了した。
**結果**: H1相対差1e-8に182/174/158列が不合格。source `3f3550f`の補償和auditで全列の誤差・判定を再現した。
506不合格は参照normが1e-14未満だったが、17³の8列はfloor以上。floorを上げたり微小値を0へ射影したりしない。
最大相対差は7.1327e-5 / 3.4151e-6 / 1.9470e-7。固定した物理20列/格子の全60列と全5変異/格子は通過した。
**分析**: 17³のfloor以上8列ではcollision側の差が支配的で、composition側は約1e-29以下。算術の原因はまだ未確定。
**資源**: H1-onlyの最大working set 1,229,852,672 bytes、private commit 2,526,855,168 bytes、最長route 576.3324秒。
14保存物41,470,266 bytesを保持した。H2 solveを含むH3の通過とは呼ばない。
実装197件、正式66件、追加の補償和/実artifact15件が通過した。最終の回帰計212件も53.67秒で通過した。
Ruff・format・compileも検査した。科学的な514不合格は別に保持する。
**次**: 全698組・全104座標を維持する[Q012h2a精度診断](../docs/D3Q27_QUARTIC_FORCING_PRECISION.md)を`4ffbbb0`で事前登録した。
GMP 128/192 bitの全算術二方式とmoment-only armで原因を切り分ける。旧H1 source/結果、Q012gの悪化、存在・TTの未解決事項は変更しない。
`validate-data`の評価はNeeds revision（F4一致）。詳細・hashは[H1正式結果](../docs/D3Q27_QUARTIC_PILOT.md#h1正式結果-2026-09-08--全件完了独立f4一致は不合格)へ。

## 2026-09-08: Q012h2 H1の実forcingと独立合成を実装

**問い**: 実LBMの全lower-order fiberを保ったまま、四次の7項を元のTaylor単項式の合成と独立に照合できるか。
**実装**: compactな全pair/triple索引、実B/C/Dと内部合成、4 labelled nilpotent変数の参照方式、物理FFTと差分filterを追加した。
全104座標の内部力学を保持し、入力の4座標以外の生成成分も使う。S0の固定698組と厳密な入力・基底照合へ接続した。
**部品検証**: 全126人工混合組で7項が一致、全560対称積がQ012h1と一致。小格子の全FFT漏れ、3軸filter、5変異も検査した。
新規66件と先行131件、計197テストが19.90秒で通過し、Ruff・format・compileも通過した。
**境界**: `validate-data`に従い、この時点では実LBMの正式F4やH2/H3の成功を主張しない。
**次**: 7 sourceをcommitして全親/S0監査後に三格子のH1を実行する。全結果と変異を保存し、別processと保存後全entry再構築で検証する。
詳細は[H1実装検証](../docs/D3Q27_QUARTIC_PILOT.md#h1実装検証-2026-09-08--実入力の正式計算前)へ。

## 2026-09-08: Q012h2 S0の全数距離走査と検証対象の選択が完了

source `6a88122`で正式実行し、全17³/33³/65³・4,991,220 block tupleの距離を二方式で全件照合した。
6走査processと3保存後監査processの実exit 0、mainの実exit 0を確認した。別CLIの全件再監査も実exit 0で通過した。
最大絶対差は各格子とも6.661338147750939e-16で、上限1e-13以内。
最小距離は17³/33³/65³で3.380353055559837e-5 / 2.868783042231371e-7 / 2.7471969932246765e-7だった。

登録した主方式のscore/ordinalから、三格子共通698組・1,826対称column/格子を固定した。
全78 block・104座標、652 seed・共役closure・最大432寸法を保持し、column数はPowerShellでも独立に再集計した。
全30非空binのうち2件では、独立方式自身のscoreの最小代表が主方式と異なった。
near tieのtupleと値を[結果](../docs/D3Q27_QUARTIC_PILOT.md#s0正式結果-2026-09-08--選択のみq012h2全体は未判定)に残し、選択を後から差し替えない。

正式runに保存された9 processの最大working setは230,297,600 bytes、private commitは1,547,694,080 bytes。
13保存物は合計34,933,404 bytes。S0の資源であり、実H2/H3入力・F4・全数solveの費用へ一般化しない。
正式run内86テストも通過した（15.68秒）。事前258テスト、先行親の全監査、sourceの封印を保持した。
`validate-data`に従い、S0だけを`passed=true`とし、Q012h2全体は`not_evaluated`を維持する。
次に選択artifactをcommitで固定して、698組/格子の実四次forcingと独立多項式合成を構築する。
元Q012gの棄却・全三格子の四次chart・SSM存在・TT優位性の未完了事項は変わらない。

## 2026-09-08: Q012h2の正式起動前に数値runtimeの上書きを修正

`0d42b7a`をpush後、正式S0出力を作る前の親全監査がQ012h0の`fresh frame readback differs`でexit 1となった。
新runnerがBLAS関連3環境変数を1へ変え、先行の厳密保存後監査と異なる起動条件を作っていた。
通常環境の先行Q012h1全監査は通過済み。pytestではNumPyが先に読み込まれるため、この起動順の差を見逃していた。
新runnerの上書きだけを削除し、先行source・record・閾値を変更しない。別process importの2回帰テストを追加する。
正式S0全数走査・実LBM F4はまだ未実行。修正を新commitにし、継承runtimeで親の全監査を再確認する。
修正後の別process全監査はexit 0で全親条件が通過した。3環境変数は未設定のまま保持する。
新規86件＋先行172件、計258テストが通過（19.15秒）し、Ruff・format・compileも通過した。
...
全16次元inventory×35四つ組について、Kronecker/対称軌道方式と有理数monomial代入の全entryが一致した。
全5重複pattern、全slot permutation、Taylor係数を検査し、外部次元23/27を含む登録件数1,120組・4,482列も確認した。
これはoperatorの構成検査であり、全1,120組の数値solveを実行したという意味ではない。

実・複素表示×外部次元23/27の全4場合では、集合分割の7項群からのforcingを独立Fraction合成と全330列で完全照合した。
二〜四次の既知H/G全係数、実表示への全係数の往復、複素共役対、左右基底、保存量4座標の厳密な0を検査した。
各forcing項群の削除、composition符号反転、G2/G3省略、不要な複素共役を負の対照として検出した。
主方式の演算関数を使用不可にして独立方式を実行する部品テストも通過したが、別processの実行証跡の代用にはしない。

既存のSVDとSylvester＋固定3回refinementは変更せず、全5重複pattern×外部次元23/27と1次元block例の11正例を検査した。
最大432次元を含む既知解と、singular compatible/incompatible・nonsingular ill-conditionedの3負の対照が通過した。
丸め済み行列の外部/full残差を整数演算とGMP有理演算で照合し、外部1e-10とfull 1e-9の閾値を分離した。
5e-10相当の残差が外部条件だけで不合格になる回帰テストを加え、未refineのSylvester結果は診断に留めた。

新規118テストが警告error化で通過した（88.68秒）。Ruff、format、compile、ローカル文書リンク225件の検査も通過した。
Q012h0の別CLI再監査はexit 0、全7 validity・H1/H2/H3が通過し、先行source/親/保存結果の封印を維持した。
`validate-data`の方針に従い、この結果は部品検証として共有し、Q012h1の正式受理とは区別する。
全1,120 caseの正式solve・実在する独立workerのPID/exit/source・排他的保存と全entry再監査は未実行である。
Q012h1は未判定。実LBMの全104座標・三格子、四次改善、SSM存在、TT優位性の未検証事項を保持する。

## 2026-09-08: Q012h1の四次人工オラクルを事前登録

Q012h0全数censusを`1348b8a`で受理した後、[次の代数オラクル](../docs/D3Q27_QUARTIC_ORACLE.md)を固定した。
全5重複patternを含む16人工次元inventory×35四つ組×外部次元23/27、計1,120 operator caseを使う。
四次forcingは、集合分割によるraw微分式と、既知graph/内部力学を持つ多項式の独立Fraction合成で全混合columnを照合する。
複素共役対・非正規block・G2/G3/G4・両表示の往復・特異compatible/incompatibleを明示した。

単純な三角写像だけではB(V,H3)・B(H2,H2)・C(V,V,H2)が消え、四次forcingの重要な項を検証できない。
そこで多様体上で消える横断非線形項をrecipeへ含め、各項群が非零であることをcoverage条件にした。
この設計上の観察を、実LBMの新しい数値結果と混同しない。recipe・case・閾値を実装前にcommitする。
仕様の全1,120組・4,482列・最大寸法432と、集合分割の重複数4/3/6を別の整数列挙で確認した。
ローカル文書リンクと差分チェックが通過した。これは仕様の整合性チェックであり、候補式やsolverの実験的受理ではない。
Q012h1は未実装・未実行で未判定。実LBMの全104座標・三格子と実測資源pilotは後続に保持する。
...
元のtciと保全元リポジトリ・未コミット草稿は変更しない。区切りごとの検証済み変更をpushする。

問いは、四次の対称入力column数と波数histogramを、小さな人工例で異なる方法から再現できるか。
既存Q012h0事前登録を保持し、block列挙、block生成関数、個別座標生成関数を実装した。
個別座標monomialの直接列挙で、次数1〜4と四次の全5重複pattern、1/2/3次元blockを全件照合した。

草稿のsector集計は、検証関数が消費したiteratorを再走査していたため分類が変わる。
正規化済みinventoryを使うよう修正し、iterator/list一致を回帰テストにした。
inventory復元の二重走査も修正した。bool/floatによる整数欄の代用、重複・欠落・波数改変を検査し、
count総数の一致だけでは十分でないことも対照で確認した。機械整数からPython整数へ正規化し、overflowを防ぐ。

新規127テストは0.37秒で通過。D3Q27二次・基礎の既存43件も含む170件が104.01秒で通過した。
Ruff、整形、compileall、先行Q012g3のsource/artifact seal照合も通過した。
READMEは54行の概要を保持し、細部は[Q012h0設計書](../docs/D3Q27_QUARTIC_RESOURCE_CENSUS.md)へ追記した。

これは正式Q012h0の受理ではない。全78 block・104座標の本計算、独立process照合、三格子のfresh frame、
排他的保存と保存後監査は次の実装単位で接続する。四次solve、実RSS/時間、残差改善、SSM存在、TT優位性は未検証。

## 2026-09-08: Q012g3 全960 caseの次数別診断を受理

前turnはmainの65³・反例34の監査を`098e33f`へ保存したprogressである。
再開時にはPID 35240が不在、handle 31939も消失し、65³の全方向・grid・最終親が保存されていた。
11:17 JST時点のこの観測は終了確認であって、元processのexit codeや最終consoleを再取得したものではない。
元計算は再起動せず、別processで全保存後監査を再実行し、全960 case・576 profileが通過、exit 0を確認した。
元5,760 field・1,920欠陥、全288 child・三grid・最終親、holdout全192 case・1,152 exact fieldを全件照合した。
開始前後・今回の再読時で入力とsourceは一致し、保存された正式177対照もexit 0・177 passedだった。
独立workerの全192 case・4,599 vector比較の全監査を通し、mainの全対応192 caseのprimary科学的recordが厳密一致した。
main全960 caseを独立の第二式でも再計算した、とは主張しない。

全10 validityとH1/H2/H3が成立し、Q012g3を `passed / accepted` と判定する。
元Q012gの3次数不合格、81半減不達・37悪化は全件再現され、元の `passed / rejected` を保持した。
65³のH4反例51は予測傾き4.10242111001829の超過、反例52は予測比0.10292376971677472の超過という別の原因を維持した。
最悪holdout65³・方向21・`.032`の元比5.834017983080117は予測5.834017984274533となった。
同じW3欠陥の四次までのvector相対誤差約91.70%に対し、六次までで6.58493e-5、九次までで約3.28809e-10だった。
高次寄与と有理式の残りを含む分解の成立であり、新しい高次chartの構築や悪化の修復ではない。
再構成差の最大は65³・一般方向46・degree3・`.002`のPhi再構成4.244566574097749e-14で、元floor4.1140092794350784e-12以内だった。

最終親と三gridの外部SHAを固定した追加18保存後検査が169.49秒で通過した。
fresh全入力chain・全worker監査、全192 fitの独立OLS、全12,480切断Gram norm²の補償和、旧判定と末尾成分の改変拒否を含む。
既存main二格子・反例34・worker65³一般方向の47検査も73.81秒で通過した。計65件を警告error化で確認し、Ruff・整形・compileallも通過した。
解析検証手順に従い、全960 caseの診断受理と元1,248 caseのモデル評価を区別する。元特殊288 case・null fitは対象外のまま保存した。
次は、この診断に基づく高次補正の必要性・四次構築の可否と費用を事前登録して評価する。
SSM存在・連続球・grid-uniform実用半径・長時間trajectory・3DのTT費用優位は未認証であり、研究goalは未完了である。

## 2026-09-08: Q012g3 mainでも65³の反例34を再現

同じPID 35240／handle 31939を継続し、登録一般方向34の全4振幅が保存されてから別途監査した。
2 profile、元24 field・8欠陥、H1/H3・Gramが通過した。mainはprimaryだけで、grid内の二方式vector比較数は0である。
一方、外部SHAで封印した独立workerの同じ方向の全primary科学的recordと厳密一致した。
元三次傾き`4.103841695537577`に対し予測は`4.103894349187252`、最小振幅比は元`0.0801557831453807`に対し予測`0.08014626737054589`だった。
閉じたOLS式で元／予測fitを照合し、全52切断Gram norm²も補償和で再検査した。
同じW3欠陥の四次までの予測傾きは約4、五次まで`4.077688458854748`、六次まで`4.1038942623554435`となった。
有限振幅窓での傾き超過を再現した結果であり、旧H4はfalseのまま保持する。新しい高次chartや漸近次数の変更は示していない。
main方向34の外部SHAを固定した追加10検査と、既存worker反例34・65³一般方向の19検査、計29件が警告error化で通過した（8.00秒）。
旧判定の成功への書換え、C9／末尾case欠落、末尾座標改変の拒否も含む。Ruff・整形・compileallも通過した。
解析検証手順に従い、反例の再現と修復を区別する。今回の4 caseは65³の部分監査であり、全320 caseの完了ではない。
sourceと正式177対照は封印したまま。同じmainは次の方向へ進んでおり、残る反例51/52・全960 case・全worker対応・終了時照合・最終親の監査を続ける。

## 2026-09-08: Q012g3 mainの33³全320 caseを監査

同一processの進行を確認して待機し、33³の全96方向・320 caseが保存された後に別途全数監査した。
192 profile、元1,920 field・640欠陥、holdout64 caseのexact記録、全96 childのSHA/header/body、H1/H2/H3・Gramが通過した。
独立workerに対応する全21方向・64 caseのprimary科学的recordも厳密一致した。これで二格子のmain640 caseとworker対応128 caseを監査済みとなる。
一般64方向の旧H4成功と、有限振幅`.032`の32半減不達・方向2/4/7/16/23の5悪化を全て保持して再現した。
独立対象外の方向23・`.032`では、旧欠陥比`1.1349501394251245`に対し予測`1.1349501393864596`。旧H5はfalseのままである。
この同じW3欠陥の切断予測は四次まででvector相対誤差`0.412617433263631`、六次までで`9.259700967342776e-5`、九次までで`1.1068671250188703e-9`だった。
新しいW4/W6/W9や、全方向の単一項への原因帰属を示した結果ではない。
全64一般fitを閉じたOLS式、全4,160切断Gram norm²を補償和で再検査した。
傾き差最大`0.00017371130414334246`、最小振幅比の相対差最大`0.00039331366732360686`は登録上限以内だった。
全再構成差最大はholdout方向9・degree2・`.008`のPhi再構成`1.5381118046148136e-14`で、元floor`1.4882165908113665e-12`以内だった。
外部grid SHAと全childを封印した追加14件は33.74秒で通過した。方向23の削除・旧H5／比書換え・C9欠落・tail／末尾座標改変も拒否した。
main N17の13件とworker N33の11件、関連24件も49.24秒で通過した。計38件を警告error化で確認し、Ruff・整形・compileallも通過した。
解析検証手順に従い、二格子の途中結果と全体判定、元の悪化の再現と解消を区別する。元Q012gの棄却を保持する。
同じPID 35240／handle 31939は65³へ進んだ。封印source・正式177対照は不変で、全960 case・全worker対応・終了時入力/source・最終親監査は未完了である。

## 2026-09-08: Q012g3 mainの17³全320 caseを監査

独立192 caseの完了とmain開始を保存した前段に続き、同じPID 35240／handle 31939の17³全96方向を監査した。
一般64方向×4振幅とholdout32方向×2振幅の全320 case、192 profile、元1,920 field・640欠陥が揃った。
全96 childのnormalized SHA/header/body、全登録schedule・旧record・holdout exact記録を照合し、H1/H2/H3・Gramが通過した。
独立workerの登録21方向・64 caseに対応する全primary科学的recordも厳密一致した。他格子の対応や全960 caseの独立計算を意味しない。
全64一般方向の元／予測fitを閉じたOLS式で、全4,160切断Gram norm²を補償和で再確認した。
元N17のH4は64/64、holdout H5は64/64を維持した。全holdoutの欠陥比は予測でも0.5以下で、vector相対誤差も登録上限以内だった。
元との傾き差最大は`0.00024180368661497198`、最小振幅比の相対差最大は`0.0005470464888657247`だった。
最大再構成差は一般方向10・degree3・`.008`のPhi再構成で`5.7870239958157445e-15`、元floorは`5.502608491768111e-13`。
外部grid SHAと全childを封印した追加13件は38.65秒で通過した。末尾case・C9欠落、tail／比／exact field／末尾座標の改変拒否も含む。
既存worker N17の11件と合成恒等式17件も13.33秒で通過し、計41件を警告error化で確認した。Ruff・整形・compileallも通過した。
解析検証手順に従い、一格子の保存後監査と全研究gateの受理を分離する。元Q012gの棄却は保持し、新しい高次chartは構築していない。
source digestと正式177対照は実行前封印のまま不変。同じmainが33³へ進んでおり、33³/65³・全replay・入力/sourceの終了時照合・最終親の監査は未完了である。
