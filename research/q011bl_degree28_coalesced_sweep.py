"""Q011bl block-support-coalesced degree-twenty-eight nonresonance audit."""
...
q011bi = q011bk.q011bi
q011bj = q011bk.q011bj
...
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 728
...
EXPECTED_PEAK_LIVE_SIGNATURE_COUNT = 182
...
ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-28 "
    "external nonresonance"
)
REJECTED_CLASSIFICATION = (
    "the block-support-coalesced degree-28 sufficient certificate is rejected"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011bl degree-28 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = (
    "ruled_out_within_registered_degree_twenty_eight_scope"
)

Q011BK_ARTIFACT_SHA256 = "bcd6f8608368637075be06c517ca31cf207d522175d10f726e91c534d582749c"
Q011BK_RUNNER_SHA256 = "571969fc442b000e319d7e8f12be75a1d60bbd6cccb0cbff3b6c822b8cfbf0a6"
Q011BK_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bk._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bk_degree28_resource_estimate.json"
    runner_path = Path(q011bk.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BK_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_twenty_eight_coalesced_resource_audit"]
    checks = {
        "q011bk_forty_one_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 198
            and len(artifacts) == 41
            and all(prior["checks"].values())
        ),
        "q011bk_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011BK_ARTIFACT_SHA256,
        "q011bk_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BK_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BK_RUNNER_SHA256
        ),
        "q011bk_section_digests_match": digests == Q011BK_DIGESTS,
        "q011bk_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bk.GO_DECISION
            and artifact["scientific_outcome"] == q011bk.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bk.ACTUAL_RESONANCE_OUTCOME
            ...
            and not theorem["degree_twenty_eight_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011bk_preserves_q011bj_accepted_certificate": bool(
            theorem["q011bj_degree_twenty_seven_certificate_is_preserved"]
            and artifacts["q011bj"]["scientific_outcome"] == "accepted"
            and artifacts["q011bj"]["cycle"]["theorem_consequence"][
                "degree_twenty_seven_external_nonresonance_is_certified"
            ]
        ...
        "q011bk_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bk_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bk_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "two_hundred_three_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 203
        ),
    }
    artifacts["q011bk"] = artifact
    return (
        {
            "prior_q011bk_sealed_input_audit": prior,
            "q011bk": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BK_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_twenty_eight_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011bk._degree_twenty_eight_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011bk._degree_twenty_eight_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bk._degree_twenty_eight_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bk"]["cycle"]
    stored_inventory = stored_cycle["degree_twenty_eight_inventory_audit"]
    stored_envelope = stored_cycle["degree_twenty_eight_envelope_audit"]
    stored_resource = stored_cycle["degree_twenty_eight_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bk_degree_twenty_eight_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bk_degree_twenty_eight_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bk_degree_twenty_eight_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011bi_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            == q011bk.EXPECTED_HULL_RECORD_DIGEST
            ...
            == q011bk.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"]
            == q011bk.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope[
            "inactive_retained_q011bi_identifier_count"
        ],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_twenty_nine_or_higher_claimed": False,
...
def run_degree_twenty_eight_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_twenty_eight_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bk_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "42 artifacts and 203 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "1170 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-28 coalesced resource identities",
            "value": {
        ...
        "q011bj_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bj"]["scientific_outcome"] == "accepted"
                and artifacts["q011bj"]["cycle"]["theorem_consequence"][
                    "degree_twenty_seven_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011bl_degree28_coalesced_sweep.py"
            ),
            "threshold": "degree-27 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 612 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_twenty_eight": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "3325 preserved plus 1170 direct equals 4495",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_twenty_eight_scope": {
            "passed": True,
            "threshold": "no degrees 29--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-28 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-twenty-eight external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 29)) if accepted else list(range(2, 28))
    missing_degrees = list(range(29, 91)) if accepted else list(range(28, 91))
    cycle["theorem_consequence"] = {
        "degree_twenty_eight_external_nonresonance_is_certified": accepted,
        "an_actual_degree_twenty_eight_external_resonance_is_ruled_out": accepted,
        "registered_degree_twenty_eight_sufficient_certificate_is_rejected": rejected,
    ...
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-twenty-eight external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 612 current Q011bk "
        ...
        "not compared. It makes no claim for degrees 29--90, all-order nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011bm degree-twenty-nine resource estimate before "
        "preregistering any degree-twenty-nine full sweep."
        if accepted
        else "Audit only the first registered degree-twenty-eight overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011bl validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011bl cycle failed strict serialization or digest")
    return cycle


def run_q011bl_study() -> dict[str, Any]:
    cycle = run_degree_twenty_eight_coalesced_audit()
    sweep = cycle["degree_twenty_eight_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 612 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-28 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_twenty_nine_or_higher_claim": False,
