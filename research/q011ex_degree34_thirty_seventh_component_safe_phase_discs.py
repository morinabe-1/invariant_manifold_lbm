"""Q011ex component-safe complex phase discs for Q011cb flatten ordinal thirty-six."""
...
EXPECTED_BRIDGE_RECORD_DIGEST = "e6dd92eb78e5c8804b10a16ca9a4283938783fb3935cdf280522f10c3881f9ec"
EXPECTED_PAIRED_RECORD_DIGEST = "7510318313be3bf43fc4e6aa78ae41c99ac643a77ee9cb20cac33ed8b889c2ec"

Q011EW_ARTIFACT_SHA256 = "54ff04e262fddc43e46390830d107eb74926d23f169a4bd016628660311b5113"
Q011EW_RUNNER_SHA256 = "bf5ee5ffac63677feea886e4067246dce38d804f3ca34a30935d99ba10078351"
Q011EW_DIGEST_NAMES = (
    "input_digest_sha256",
...
EXPECTED_Q011EW_ALLOCATION_RECORD_DIGEST = (
    "8f6724eace4e86f823a1def5c2f5440f27154e38a4677120f6189ecb36f23ed7"
)

RESOLVED_CLASSIFICATION = "the component-safe complex phase discs resolve the thirty-seventh Q011cb persistent refined witness"
PERSISTENT_CLASSIFICATION = (
    "the thirty-seventh Q011cb persistent refined witness persists under component-safe "
    "complex phase discs"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011ex component-safe phase-disc audit is inconclusive"
...
def _q011ex_framed_record_digest(domain: str) -> Any:
    if domain != "q011ch-component-safe-phase-comparisons-v1":
        raise ValueError(f"unexpected inherited comparison domain: {domain}")
    return q011cx._BASE_FRAMED_RECORD_DIGEST("q011ex-component-safe-phase-comparisons-v1")


def _q011ex_exact_fraction_sequence_sha256(
    domain: bytes,
    values: tuple[Fraction, ...],
) -> str:
    if domain == b"q011ch-complex-margin-expression-v1":
        domain = b"q011ex-complex-margin-expression-v1"
    return q011cx._BASE_EXACT_FRACTION_SEQUENCE_SHA256(domain, values)


def _q011ex_power_tables(
...
def _adapted_q011cx_globals_are_restored() -> bool:
    return bool(
        ...
        and (
            q011ew.q011da._Q011CY_PROTOCOL_OVERRIDE is q011ew._Q011DA_PROTOCOL_BASELINE
            or q011ew.q011da._Q011CY_PROTOCOL_OVERRIDE is q011ew._Q011DA_PROTOCOL_OVERRIDE
        )
        and (
            q011ew.q011da.q011cy._Q011CW_PROTOCOL_OVERRIDE
            is q011ew.q011da._Q011CY_PROTOCOL_BASELINE
            or q011ew.q011da.q011cy._Q011CW_PROTOCOL_OVERRIDE is q011ew._Q011DA_PROTOCOL_OVERRIDE
        )
        and q011ew.q011da.q011cy.q011cw._Q011CK_PROTOCOL_OVERRIDE
        is q011ew.q011da.q011cy._Q011CW_PROTOCOL_BASELINE
        and q011ew.q011da.q011cy.q011cw._protocol_globals_are_restored()
    )


def _adapted_q011ew_globals_are_restored() -> bool:
...
def _adapted_q011cx_protocol_overrides() -> dict[str, Any]:
    ...
    overrides["_power_tables"] = _q011ex_power_tables
    return overrides
...
def _q011cx_protocol_overrides() -> dict[str, Any]:
    return {
        ...
        "_q011cx_framed_record_digest": _q011ex_framed_record_digest,
        "_q011cx_exact_fraction_sequence_sha256": (_q011ex_exact_fraction_sequence_sha256),
...
def _q011cx_protocol_context() -> Iterator[None]:
    ...
    original_q011ew_restore = q011ew._protocol_globals_are_restored
    original_q011da_restore = q011ew.q011da._protocol_globals_are_restored
    try:
        for name, value in overrides.items():
            setattr(q011cx, name, value)
        q011ew._protocol_globals_are_restored = _adapted_q011ew_globals_are_restored
        q011ew.q011da._protocol_globals_are_restored = _adapted_q011da_globals_are_restored
        yield
    finally:
        q011ew.q011da._protocol_globals_are_restored = original_q011da_restore
        q011ew._protocol_globals_are_restored = original_q011ew_restore
...
def _protocol_globals_are_restored() -> bool:
    return bool(
        ...
        and q011ew._protocol_globals_are_restored()
        and q011cj._protocol_globals_are_restored()
    )


def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011ew._sealed_input_audit()
    ...
    digests = tuple(cycle[name] for name in Q011EW_DIGEST_NAMES)
    checks = {
        "q011ew_one_hundred_thirty_one_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["artifact_count"] == 131
            and prior["direct_digest_count"] == 602
            and len(artifacts) == 131
            and all(prior["checks"].values())
        ),
        "q011ew_artifact_and_runner_sha256_match": bool(
            _file_sha256(artifact_path) == Q011EW_ARTIFACT_SHA256
            and _file_sha256(runner_path) == Q011EW_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011EW_RUNNER_SHA256
        ),
        "q011ew_section_digests_match": digests == Q011EW_DIGESTS,
        "q011ew_valid_interval_inert_outcome_and_boundary_reproduce": bool(
            artifact["study_gate"] == "passed"
            and artifact["refinement_outcome"] == "partition_inert_persistent"
            and artifact["scientific_outcome"] == "not_evaluated"
            and artifact["actual_resonance_outcome"] == "not_established"
            and cycle["failed_validity_order"] == []
            and theorem["individual_partition_is_interval_inert_for_thirty_seventh_q011cb_witness"]
        ),
        "q011ew_recent_boundaries_reproduce": bool(
            theorem["q011ev_ordinal_thirty_five_phase_resolution_is_preserved"]
        ...
        "q011ew_parent_partition_and_relations_reproduce": bool(
            fixed["thirty_seventh_parent_witness_selection_audit"]["selected_flat_ordinal"]
            == PARENT_FLAT_ORDINAL
            and fixed["compatible_allocation_count"] == EXPECTED_INDIVIDUAL_WAVE_ALLOCATION_COUNT
            and fixed["compatible_allocation_digest_sha256"] == q011ew.EXPECTED_COMPATIBLE_DIGEST
        ...
        "q011ew_artifact_is_finite_strict_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "six_hundred_six_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 606
        ),
    }
    artifacts["q011ew"] = artifact
    return (
        {
            "prior_q011ew_sealed_input_audit": prior,
            "q011ew": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011EW_DIGEST_NAMES),
...
def _fixed_phase_input_audit(
    ...
    adapted["q011cw"] = artifacts["q011ew"]
    with _q011cx_protocol_context():
        audit, sources, target, wave_compatible = q011cx._fixed_phase_input_audit(adapted)
    audit["checks"]["q011ew_fixed_parent_and_interval_partition_replay_bitwise"] = audit[
        "checks"
    ].pop("q011cw_fixed_parent_and_interval_partition_replay_bitwise")
    audit["q011ew_compatible_wave_allocation_count"] = audit.pop(
        "q011cw_compatible_wave_allocation_count"
    )
    audit["q011ew_compatible_wave_allocation_digest_sha256"] = audit.pop(
...
def _component_wave_counts(individual_counts: tuple[int, ...]) -> tuple[int, ...]:
    if len(individual_counts) != 12:
        raise ValueError("Q011ex requires twelve Q011ew individual counts")
    return (
        individual_counts[0],
        individual_counts[1],
...
def _allocation_inventory(
    ...
    audit = {
        ...
        "allocation_protocol_adapter": {
            ...
            "ordinal_thirty_six_totals": [13, 9, 5, 4, 3],
...
def _phase_product_audit(
    ...
    comparison["comparison_stream_domain"] = "q011ex-component-safe-phase-comparisons-v1"
...
def _registered_parameters() -> dict[str, Any]:
    ...
    registered["q011ex_allocation_enumerator"] = {
        "source_runner_sha256": _file_sha256(Path(__file__).resolve()),
...
def run_degree_thirty_four_thirty_seventh_component_safe_phase_audit() -> dict[str, Any]:
    ...
    validity_gates = {
        "q011ew_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "132 artifacts and 606 direct digests reproduce",
            "value": sealed["checks"],
        },
        "q011ew_parent_partition_and_interval_inert_result_replay": {
            "passed": phase_input["checks"][
                "q011ew_fixed_parent_and_interval_partition_replay_bitwise"
            ],
            "threshold": "ordinal 36, 2837 individual wave allocations and inert result replay",
            "value": phase_input["q011ew_compatible_wave_allocation_digest_sha256"],
        },
        ...
        "strict_stream_section_digests_runner_and_protocol_provenance_reproduce": {
            "passed": bool(
                ...
                and runner["filename"]
                == "q011ex_degree34_thirty_seventh_component_safe_phase_discs.py"
    ...
    diagnostic_gates = {
        "only_the_registered_thirty_seventh_q011cb_witness_is_refined": {
            "passed": bool(
            ...
            "threshold": "ordinal 36 only, one target and no other overlap",
    ...
    cycle = {
        "question": (
            ...
            "strictly separate every complex product in Q011cb flatten ordinal 36?"
        ),
        **input_sections,
    ...
    cycle["theorem_consequence"] = {
        "component_safe_complex_phase_discs_resolve_thirty_seventh_q011cb_witness": resolved,
        "thirty_seventh_q011cb_witness_persists_under_component_safe_phase_discs": persistent,
        "q011an_component_internal_eigenvalue_labels_are_assumed": False,
        "q011ew_interval_inert_diagnostic_is_preserved": True,
    ...
    cycle["claim_boundary"] = (
        ...
        "ordinal 36, its 2837-to-945 component-wave quotient and 20786 registered "
        "component-safe phase allocations and target block=7;center=44. It does not "
        "reevaluate ordinals 0 through 35 or classify the later 44763 Q011cb refined "
    ...
    cycle["next_change"] = (
        "Preregister Q011ey to audit the next Q011cb refined overlap in registered order."
        if resolved
        else (
            "Preregister Q011ey to refine only the first unresolved component-safe phase witness."
        )
        if persistent
        else "Repair only the first Q011ex validity failure."
    )
    if not (
        ...
        raise RuntimeError("Q011ex cycle failed strict serialization or digest")
    return cycle


def run_q011ex_study() -> dict[str, Any]:
    started = perf_counter()
    cycle = run_degree_thirty_four_thirty_seventh_component_safe_phase_audit()
    ...
    return {
        ...
        "mathematical_scope": {
            "diagnostic": "thirty-seventh Q011cb witness component-safe complex phase discs",
            "grid": [SIZE, SIZE],
