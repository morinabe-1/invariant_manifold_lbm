"""Q011bp block-support-coalesced degree-thirty nonresonance audit."""
...
q011bn = q011bo.q011bn
q011bl = q011bn.q011bl
...
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 912
...
ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-30 "
    "external nonresonance"
)
REJECTED_CLASSIFICATION = (
    "the block-support-coalesced degree-30 sufficient certificate is rejected"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011bp degree-30 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = (
    "ruled_out_within_registered_degree_thirty_scope"
)

Q011BO_ARTIFACT_SHA256 = "31cfae714f2a367ee3c1e4da92a644428618bf81d959dbdfa77bf542b3527dab"
Q011BO_RUNNER_SHA256 = "6af80ea8a7957ceff8cb6fe3705296b3685d36e70bab007053bf0c42d43e6ee0"
Q011BO_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bo._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bo_degree30_resource_estimate.json"
    runner_path = Path(q011bo.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BO_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_thirty_coalesced_resource_audit"]
    checks = {
        "q011bo_forty_five_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 216
            and len(artifacts) == 45
            and all(prior["checks"].values())
        ),
        "q011bo_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011BO_ARTIFACT_SHA256,
        "q011bo_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BO_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BO_RUNNER_SHA256
        ),
        "q011bo_section_digests_match": digests == Q011BO_DIGESTS,
        "q011bo_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bo.GO_DECISION
            and artifact["scientific_outcome"] == q011bo.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bo.ACTUAL_RESONANCE_OUTCOME
            and cycle["failed_validity_order"] == []
            and cycle["failed_resource_limit_order"] == []
            and theorem[
                "degree_thirty_coalesced_full_sweep_preregistration_is_resource_supported"
            ]
            and not theorem["degree_thirty_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011bo_preserves_q011bn_accepted_certificate": bool(
            theorem["q011bn_degree_twenty_nine_certificate_is_preserved"]
            and artifacts["q011bn"]["scientific_outcome"] == "accepted"
            and artifacts["q011bn"]["cycle"]["theorem_consequence"][
                "degree_twenty_nine_external_nonresonance_is_certified"
            ]
        ...
        "q011bo_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bo_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bo_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "two_hundred_twenty_one_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 221
        ),
    }
    artifacts["q011bo"] = artifact
    return (
        {
            "prior_q011bo_sealed_input_audit": prior,
            "q011bo": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BO_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_thirty_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011bo._degree_thirty_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011bo._degree_thirty_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bo._degree_thirty_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bo"]["cycle"]
    stored_inventory = stored_cycle["degree_thirty_inventory_audit"]
    stored_envelope = stored_cycle["degree_thirty_envelope_audit"]
    stored_resource = stored_cycle["degree_thirty_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bo_degree_thirty_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bo_degree_thirty_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bo_degree_thirty_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011bm_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            == q011bo.EXPECTED_HULL_RECORD_DIGEST
            ...
            == q011bo.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"]
            == q011bo.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope[
            "inactive_retained_q011bm_identifier_count"
        ],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_thirty_one_or_higher_claimed": False,
...
def run_degree_thirty_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_thirty_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bo_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "46 artifacts and 221 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "1675 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-30 coalesced resource identities",
            "value": {
        ...
        "q011bn_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bn"]["scientific_outcome"] == "accepted"
                and artifacts["q011bn"]["cycle"]["theorem_consequence"][
                    "degree_twenty_nine_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011bp_degree30_coalesced_sweep.py"
            ),
            "threshold": "degree-29 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 772 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_thirty": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "3781 preserved plus 1675 direct equals 5456",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_thirty_scope": {
            "passed": True,
            "threshold": "no degrees 31--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-30 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-thirty external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 31)) if accepted else list(range(2, 30))
    missing_degrees = list(range(31, 91)) if accepted else list(range(30, 91))
    cycle["theorem_consequence"] = {
        "degree_thirty_external_nonresonance_is_certified": accepted,
        "an_actual_degree_thirty_external_resonance_is_ruled_out": accepted,
        "registered_degree_thirty_sufficient_certificate_is_rejected": rejected,
    ...
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-thirty external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 772 current Q011bo "
        ...
        "retained but not compared. It makes no claim for degrees 31--90, all-order "
        "nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011bq degree-thirty-one resource estimate before "
        "preregistering any degree-thirty-one full sweep."
        if accepted
        else "Audit only the first registered degree-thirty overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011bp validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011bp cycle failed strict serialization or digest")
    return cycle


def run_q011bp_study() -> dict[str, Any]:
    cycle = run_degree_thirty_coalesced_audit()
    sweep = cycle["degree_thirty_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 772 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-30 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_thirty_one_or_higher_claim": False,
