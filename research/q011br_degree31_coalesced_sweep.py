"""Q011br block-support-coalesced degree-thirty-one nonresonance audit."""
...
q011bi = q011bq.q011bi
q011bp = q011bq.q011bp
...
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 1_032
...
ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-31 external nonresonance"
)
REJECTED_CLASSIFICATION = "the block-support-coalesced degree-31 sufficient certificate is rejected"
INCONCLUSIVE_CLASSIFICATION = "the Q011br degree-31 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = "ruled_out_within_registered_degree_thirty_one_scope"

Q011BQ_ARTIFACT_SHA256 = "de1366f80aa8ac41f8d11684e2930722fa6de0a4f31501e25975f7c26df35d20"
Q011BQ_RUNNER_SHA256 = "1f59385b474114a511d889a20a326fdeb0f028b74ee7584f682a565f1b69ef6c"
Q011BQ_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bq._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bq_degree31_resource_estimate.json"
    runner_path = Path(q011bq.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BQ_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_thirty_one_coalesced_resource_audit"]
    checks = {
        "q011bq_forty_seven_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 225
            and len(artifacts) == 47
            and all(prior["checks"].values())
        ),
        "q011bq_artifact_sha256_matches": _file_sha256(artifact_path) == Q011BQ_ARTIFACT_SHA256,
        "q011bq_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BQ_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BQ_RUNNER_SHA256
        ),
        "q011bq_section_digests_match": digests == Q011BQ_DIGESTS,
        "q011bq_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bq.GO_DECISION
            and artifact["scientific_outcome"] == q011bq.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bq.ACTUAL_RESONANCE_OUTCOME
            and cycle["failed_validity_order"] == []
            and cycle["failed_resource_limit_order"] == []
            and theorem[
                "degree_thirty_one_coalesced_full_sweep_preregistration_is_resource_supported"
            ]
            and not theorem["degree_thirty_one_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011bq_preserves_q011bp_accepted_certificate": bool(
            theorem["q011bp_degree_thirty_certificate_is_preserved"]
            and artifacts["q011bp"]["scientific_outcome"] == "accepted"
            and artifacts["q011bp"]["cycle"]["theorem_consequence"][
                "degree_thirty_external_nonresonance_is_certified"
            ]
        ...
        "q011bq_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bq_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bq_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "two_hundred_thirty_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 230
        ),
    }
    artifacts["q011bq"] = artifact
    return (
        {
            "prior_q011bq_sealed_input_audit": prior,
            "q011bq": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BQ_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_thirty_one_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011bq._degree_thirty_one_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011bq._degree_thirty_one_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bq._degree_thirty_one_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bq"]["cycle"]
    stored_inventory = stored_cycle["degree_thirty_one_inventory_audit"]
    stored_envelope = stored_cycle["degree_thirty_one_envelope_audit"]
    stored_resource = stored_cycle["degree_thirty_one_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bq_degree_thirty_one_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bq_degree_thirty_one_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bq_degree_thirty_one_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011bo_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            and envelope["source_hull_record_digest_sha256"] == q011bq.EXPECTED_HULL_RECORD_DIGEST
            and envelope["source_merged_membership_digest_sha256"]
            == q011bq.EXPECTED_MERGED_MEMBERSHIP_DIGEST
            and envelope["active_disc_record_digest_sha256"] == q011bq.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"] == q011bq.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope["inactive_retained_q011bo_identifier_count"],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_thirty_two_or_higher_claimed": False,
...
def run_degree_thirty_one_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_thirty_one_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bq_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "48 artifacts and 230 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "1989 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-31 coalesced resource identities",
            "value": {
        ...
        "q011bp_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bp"]["scientific_outcome"] == "accepted"
                and artifacts["q011bp"]["cycle"]["theorem_consequence"][
                    "degree_thirty_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011br_degree31_coalesced_sweep.py"
            ),
            "threshold": "degree-30 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 892 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_thirty_one": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "3995 preserved plus 1989 direct equals 5984",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_thirty_one_scope": {
            "passed": True,
            "threshold": "no degrees 32--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-31 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-thirty-one external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 32)) if accepted else list(range(2, 31))
    missing_degrees = list(range(32, 91)) if accepted else list(range(31, 91))
    cycle["theorem_consequence"] = {
        "degree_thirty_one_external_nonresonance_is_certified": accepted,
        "an_actual_degree_thirty_one_external_resonance_is_ruled_out": accepted,
        "registered_degree_thirty_one_sufficient_certificate_is_rejected": rejected,
    ...
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-thirty-one external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 892 current Q011bq "
        "target discs, exact Fourier multiplicities and outward-rounded hierarchical "
        "product intervals. One hundred sixteen inactive monotone proof records are "
        "retained but not compared. It makes no claim for degrees 32--90, all-order "
        "nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011bs degree-thirty-two resource estimate before "
        "preregistering any degree-thirty-two full sweep."
        if accepted
        else "Audit only the first registered degree-thirty-one overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011br validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011br cycle failed strict serialization or digest")
    return cycle


def run_q011br_study() -> dict[str, Any]:
    cycle = run_degree_thirty_one_coalesced_audit()
    sweep = cycle["degree_thirty_one_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 892 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-31 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_thirty_two_or_higher_claim": False,
