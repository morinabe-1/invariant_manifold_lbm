"""Q011bv block-support-coalesced degree-thirty-three nonresonance audit."""
...
q011bi = q011bu.q011bi
q011bt = q011bu.q011bt
q011br = q011bt.q011br
...
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 1_292
...
EXPECTED_PEAK_LIVE_SIGNATURE_COUNT = 272
...
ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-33 external nonresonance"
)
REJECTED_CLASSIFICATION = "the block-support-coalesced degree-33 sufficient certificate is rejected"
INCONCLUSIVE_CLASSIFICATION = "the Q011bv degree-33 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = "ruled_out_within_registered_degree_thirty_three_scope"

Q011BU_ARTIFACT_SHA256 = "e3228fa06b67266c0f8d37ea78487d781d43f5aa1a99d38965027a7006b8dd86"
Q011BU_RUNNER_SHA256 = "8dcdcc8cf0604c479389bfcb45b5b19f043cc40c365b4d92f526a4078bd141c8"
Q011BU_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bu._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bu_degree33_resource_estimate.json"
    runner_path = Path(q011bu.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BU_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_thirty_three_coalesced_resource_audit"]
    checks = {
        "q011bu_fifty_one_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 243
            and len(artifacts) == 51
            and all(prior["checks"].values())
        ),
        "q011bu_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011BU_ARTIFACT_SHA256,
        "q011bu_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BU_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BU_RUNNER_SHA256
        ),
        "q011bu_section_digests_match": digests == Q011BU_DIGESTS,
        "q011bu_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bu.GO_DECISION
            and artifact["scientific_outcome"] == q011bu.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bu.ACTUAL_RESONANCE_OUTCOME
            ...
            and not theorem["degree_thirty_three_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011bu_preserves_q011bt_accepted_certificate": bool(
            theorem["q011bt_degree_thirty_two_certificate_is_preserved"]
            and artifacts["q011bt"]["scientific_outcome"] == "accepted"
            and artifacts["q011bt"]["cycle"]["theorem_consequence"][
                "degree_thirty_two_external_nonresonance_is_certified"
            ]
        ...
        "q011bu_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bu_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bu_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "two_hundred_forty_eight_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 248
        ),
    }
    artifacts["q011bu"] = artifact
    return (
        {
            "prior_q011bu_sealed_input_audit": prior,
            "q011bu": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BU_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_thirty_three_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011bu._degree_thirty_three_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011bu._degree_thirty_three_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bu._degree_thirty_three_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bu"]["cycle"]
    stored_inventory = stored_cycle["degree_thirty_three_inventory_audit"]
    stored_envelope = stored_cycle["degree_thirty_three_envelope_audit"]
    stored_resource = stored_cycle["degree_thirty_three_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bu_degree_thirty_three_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bu_degree_thirty_three_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bu_degree_thirty_three_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011bs_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            and envelope["source_hull_record_digest_sha256"] == q011bu.EXPECTED_HULL_RECORD_DIGEST
            and envelope["source_merged_membership_digest_sha256"]
            == q011bu.EXPECTED_MERGED_MEMBERSHIP_DIGEST
            and envelope["active_disc_record_digest_sha256"] == q011bu.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"] == q011bu.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope["inactive_retained_q011bs_identifier_count"],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_thirty_four_or_higher_claimed": False,
...
def run_degree_thirty_three_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_thirty_three_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bu_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "52 artifacts and 248 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "2686 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-33 coalesced resource identities",
            "value": {
        ...
        "q011bt_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bt"]["scientific_outcome"] == "accepted"
                and artifacts["q011bt"]["cycle"]["theorem_consequence"][
                    "degree_thirty_two_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011bv_degree33_coalesced_sweep.py"
            ),
            "threshold": "degree-32 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 1128 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_thirty_three": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "4454 preserved plus 2686 direct equals 7140",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_thirty_three_scope": {
            "passed": True,
            "threshold": "no degrees 34--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-33 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-thirty-three external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 34)) if accepted else list(range(2, 33))
    missing_degrees = list(range(34, 91)) if accepted else list(range(33, 91))
    cycle["theorem_consequence"] = {
        "degree_thirty_three_external_nonresonance_is_certified": accepted,
        "an_actual_degree_thirty_three_external_resonance_is_ruled_out": accepted,
        "registered_degree_thirty_three_sufficient_certificate_is_rejected": rejected,
    ...
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-thirty-three external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 1128 current Q011bu "
        ...
        "retained but not compared. It makes no claim for degrees 34--90, all-order "
        "nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011bw degree-thirty-four resource estimate before "
        "preregistering any degree-thirty-four full sweep."
        if accepted
        else "Audit only the first registered degree-thirty-three overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011bv validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011bv cycle failed strict serialization or digest")
    return cycle


def run_q011bv_study() -> dict[str, Any]:
    cycle = run_degree_thirty_three_coalesced_audit()
    sweep = cycle["degree_thirty_three_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 1128 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-33 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_thirty_four_or_higher_claim": False,
