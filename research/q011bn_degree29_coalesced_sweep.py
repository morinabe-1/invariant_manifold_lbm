"""Q011bn block-support-coalesced degree-twenty-nine nonresonance audit."""
...
q011bi = q011bm.q011bi
q011bl = q011bm.q011bl
...
EXPECTED_DEGREE_AGGREGATE_COUNT = 4_960
EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT = 3_563
EXPECTED_OVERLAP_AGGREGATE_COUNT = 1_397
EXPECTED_SELECTED_IDENTIFIER_COUNT = 24
EXPECTED_TARGET_IDENTIFIER_COUNT = 684
EXPECTED_ACTIVE_IDENTIFIER_COUNT = 708
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 808
EXPECTED_INACTIVE_RETAINED_IDENTIFIER_COUNT = 100
EXPECTED_MULTI_TARGET_AGGREGATE_COUNT = 3
EXPECTED_MAXIMUM_EXTERNAL_COMPONENT_COUNT = 3
EXPECTED_MERGED_CLASS_COUNTS = (1, 1, 2, 2)
EXPECTED_CLASS_POWER_COUNT = 173
EXPECTED_GROUP_SIGNATURE_COUNT = 928
EXPECTED_PAIR_POOL_KEY_COUNT = 465
EXPECTED_CONVOLUTION_CALL_COUNT = 18_653
EXPECTED_ORIGINAL_MONOMIAL_COUNT = 104_792_593_656_144
EXPECTED_MODULUS_SIGNATURE_COUNT = 75_504
EXPECTED_PEAK_LIVE_SIGNATURE_COUNT = 210
EXPECTED_TWO_PRODUCT_ARRAY_BYTES = 3_360
EXPECTED_DISTINCT_COMPARISON_UPPER_BOUND = 1_254_872
EXPECTED_WEIGHTED_COMPARISON_UPPER_BOUND = 1_742_501_892_291_632
EXPECTED_SAFE_INT64_CRUDE_BOUND = 12_888_866_390_400

ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-29 "
    "external nonresonance"
)
REJECTED_CLASSIFICATION = (
    "the block-support-coalesced degree-29 sufficient certificate is rejected"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011bn degree-29 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = (
    "ruled_out_within_registered_degree_twenty_nine_scope"
)

Q011BM_ARTIFACT_SHA256 = "591802ac980655093cb17ab5eb9c1b078443199d0e13d1c488ed0aaeb5fb5f33"
Q011BM_RUNNER_SHA256 = "ace58b6bfda71576d080161c766491efe53a9c3a355a69bbfae34a32b5c71081"
Q011BM_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bm._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bm_degree29_resource_estimate.json"
    runner_path = Path(q011bm.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BM_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_twenty_nine_coalesced_resource_audit"]
    checks = {
        "q011bm_forty_three_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 207
            and len(artifacts) == 43
            and all(prior["checks"].values())
        ),
        "q011bm_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011BM_ARTIFACT_SHA256,
        "q011bm_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BM_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BM_RUNNER_SHA256
        ),
        "q011bm_section_digests_match": digests == Q011BM_DIGESTS,
        "q011bm_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bm.GO_DECISION
            and artifact["scientific_outcome"] == q011bm.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bm.ACTUAL_RESONANCE_OUTCOME
            and cycle["failed_validity_order"] == []
            and cycle["failed_resource_limit_order"] == []
            and theorem[
                "degree_twenty_nine_coalesced_full_sweep_preregistration_is_resource_supported"
            ]
            and not theorem["degree_twenty_nine_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011bm_preserves_q011bl_accepted_certificate": bool(
            theorem["q011bl_degree_twenty_eight_certificate_is_preserved"]
            and artifacts["q011bl"]["scientific_outcome"] == "accepted"
            and artifacts["q011bl"]["cycle"]["theorem_consequence"][
                "degree_twenty_eight_external_nonresonance_is_certified"
            ]
            and theorem["certified_external_nonresonance_degrees"] == list(range(2, 29))
            and theorem["missing_external_nonresonance_degrees"] == list(range(29, 91))
        ),
        "q011bm_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bm_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bm_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "two_hundred_twelve_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 212
        ),
    }
    artifacts["q011bm"] = artifact
    return (
        {
            "prior_q011bm_sealed_input_audit": prior,
            "q011bm": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BM_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_twenty_nine_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011bm._degree_twenty_nine_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011bm._degree_twenty_nine_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bm._degree_twenty_nine_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bm"]["cycle"]
    stored_inventory = stored_cycle["degree_twenty_nine_inventory_audit"]
    stored_envelope = stored_cycle["degree_twenty_nine_envelope_audit"]
    stored_resource = stored_cycle["degree_twenty_nine_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bm_degree_twenty_nine_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bm_degree_twenty_nine_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bm_degree_twenty_nine_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011bk_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            == q011bm.EXPECTED_HULL_RECORD_DIGEST
            ...
            == q011bm.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"]
            == q011bm.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "maximum_external_component_count": inventory[
            "maximum_external_component_count"
        ],
        ...
        "inactive_retained_identifier_count": envelope[
            "inactive_retained_q011bk_identifier_count"
        ],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_thirty_or_higher_claimed": False,
...
def run_degree_twenty_nine_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_twenty_nine_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bm_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "44 artifacts and 212 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "1397 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-29 coalesced resource identities",
            "value": {
        ...
        "q011bl_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bl"]["scientific_outcome"] == "accepted"
                and artifacts["q011bl"]["cycle"]["theorem_consequence"][
                    "degree_twenty_eight_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011bn_degree29_coalesced_sweep.py"
            ),
            "threshold": "degree-28 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            "passed": bool(
                ...
                == EXPECTED_INACTIVE_RETAINED_IDENTIFIER_COUNT
                and fixed["multi_target_aggregate_count"]
                == EXPECTED_MULTI_TARGET_AGGREGATE_COUNT
                and fixed["maximum_external_component_count"]
                == EXPECTED_MAXIMUM_EXTERNAL_COMPONENT_COUNT
            ),
            "threshold": "six source hulls, 24 sources and 684 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_twenty_nine": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "3563 preserved plus 1397 direct equals 4960",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_twenty_nine_scope": {
            "passed": True,
            "threshold": "no degrees 30--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-29 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-twenty-nine external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 30)) if accepted else list(range(2, 29))
    missing_degrees = list(range(30, 91)) if accepted else list(range(29, 91))
    cycle["theorem_consequence"] = {
        "degree_twenty_nine_external_nonresonance_is_certified": accepted,
        "an_actual_degree_twenty_nine_external_resonance_is_ruled_out": accepted,
        "registered_degree_twenty_nine_sufficient_certificate_is_rejected": rejected,
        ...
        "q011bl_degree_twenty_eight_certificate_is_preserved": True,
    }
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-twenty-nine external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 684 current Q011bm "
        "target discs, exact Fourier multiplicities and outward-rounded hierarchical "
        "product intervals. One hundred inactive monotone proof records are retained but "
        "not compared. It makes no claim for degrees 30--90, all-order nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011bo degree-thirty resource estimate before "
        "preregistering any degree-thirty full sweep."
        if accepted
        else "Audit only the first registered degree-twenty-nine overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011bn validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011bn cycle failed strict serialization or digest")
    return cycle


def run_q011bn_study() -> dict[str, Any]:
    cycle = run_degree_twenty_nine_coalesced_audit()
    sweep = cycle["degree_twenty_nine_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 684 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-29 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_thirty_or_higher_claim": False,
