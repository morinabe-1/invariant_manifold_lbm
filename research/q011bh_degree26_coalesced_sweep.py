"""Q011bh block-support-coalesced degree-twenty-six nonresonance audit."""
...
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 568
...
EXPECTED_PEAK_LIVE_SIGNATURE_COUNT = 156
...
ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-26 "
    "external nonresonance"
)
REJECTED_CLASSIFICATION = (
    "the block-support-coalesced degree-26 sufficient certificate is rejected"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011bh degree-26 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = (
    "ruled_out_within_registered_degree_twenty_six_scope"
)

Q011BG_ARTIFACT_SHA256 = "c0501905dcd50807492e99a4ecac25520209aad10a15e0f253c80508613f2b68"
Q011BG_RUNNER_SHA256 = "d67038df358cf636011d85d58fe1f5fd7541008443904f7019aca6e74622d857"
Q011BG_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bg._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bg_degree26_resource_estimate.json"
    runner_path = Path(q011bg.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BG_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_twenty_six_coalesced_resource_audit"]
    checks = {
        "q011bg_thirty_seven_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 180
            and len(artifacts) == 37
            and all(prior["checks"].values())
        ),
        "q011bg_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011BG_ARTIFACT_SHA256,
        "q011bg_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BG_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BG_RUNNER_SHA256
        ),
        "q011bg_section_digests_match": digests == Q011BG_DIGESTS,
        "q011bg_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bg.GO_DECISION
            and artifact["scientific_outcome"] == q011bg.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bg.ACTUAL_RESONANCE_OUTCOME
            and cycle["failed_validity_order"] == []
            and cycle["failed_resource_limit_order"] == []
            and theorem[
                "degree_twenty_six_coalesced_full_sweep_preregistration_is_resource_supported"
            ]
            and not theorem["degree_twenty_six_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011bg_preserves_q011bf_accepted_certificate": bool(
            theorem["q011bf_degree_twenty_five_certificate_is_preserved"]
            and artifacts["q011bf"]["scientific_outcome"] == "accepted"
            and artifacts["q011bf"]["cycle"]["theorem_consequence"][
                "degree_twenty_five_external_nonresonance_is_certified"
            ]
        ...
        "q011bg_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bg_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bg_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "one_hundred_eighty_five_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 185
        ),
    }
    artifacts["q011bg"] = artifact
    return (
        {
            "prior_q011bg_sealed_input_audit": prior,
            "q011bg": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BG_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_twenty_six_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011bg._degree_twenty_six_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011bg._degree_twenty_six_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bg._degree_twenty_six_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bg"]["cycle"]
    stored_inventory = stored_cycle["degree_twenty_six_inventory_audit"]
    stored_envelope = stored_cycle["degree_twenty_six_envelope_audit"]
    stored_resource = stored_cycle["degree_twenty_six_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bg_degree_twenty_six_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bg_degree_twenty_six_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bg_degree_twenty_six_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011be_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            == q011bg.EXPECTED_HULL_RECORD_DIGEST
            ...
            == q011bg.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"]
            == q011bg.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope[
            "inactive_retained_q011be_identifier_count"
        ],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_twenty_seven_or_higher_claimed": False,
...
def run_degree_twenty_six_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_twenty_six_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bg_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "38 artifacts and 185 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "779 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-26 coalesced resource identities",
            "value": {
        ...
        "q011bf_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bf"]["scientific_outcome"] == "accepted"
                and artifacts["q011bf"]["cycle"]["theorem_consequence"][
                    "degree_twenty_five_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011bh_degree26_coalesced_sweep.py"
            ),
            "threshold": "degree-25 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 460 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_twenty_six": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "2875 preserved plus 779 direct equals 3654",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_twenty_six_scope": {
            "passed": True,
            "threshold": "no degrees 27--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-26 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-twenty-six external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 27)) if accepted else list(range(2, 26))
    missing_degrees = list(range(27, 91)) if accepted else list(range(26, 91))
    cycle["theorem_consequence"] = {
        "degree_twenty_six_external_nonresonance_is_certified": accepted,
        "an_actual_degree_twenty_six_external_resonance_is_ruled_out": accepted,
        "registered_degree_twenty_six_sufficient_certificate_is_rejected": rejected,
    ...
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-twenty-six external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 460 current Q011bg "
        "target discs, exact Fourier multiplicities and outward-rounded hierarchical "
        "product intervals. Eighty-four inactive monotone proof records are retained but "
        "not compared. It makes no claim for degrees 27--90, all-order nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011bi degree-twenty-seven resource estimate before "
        "preregistering any degree-twenty-seven full sweep."
        if accepted
        else "Audit only the first registered degree-twenty-six overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011bh validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011bh cycle failed strict serialization or digest")
    return cycle


def run_q011bh_study() -> dict[str, Any]:
    cycle = run_degree_twenty_six_coalesced_audit()
    sweep = cycle["degree_twenty_six_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 460 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-26 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_twenty_seven_or_higher_claim": False,
