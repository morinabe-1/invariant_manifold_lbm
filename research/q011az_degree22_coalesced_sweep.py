"""Q011az block-support-coalesced degree-twenty-two nonresonance audit."""
...
q011ax = q011ay.q011ax
q011an = q011ax.q011an
q011b = q011ay.q011b
q011z = q011ay.q011z
...
EXPECTED_DEGREE_AGGREGATE_COUNT = 2_300
EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT = 1_901
EXPECTED_OVERLAP_AGGREGATE_COUNT = 399
EXPECTED_SELECTED_IDENTIFIER_COUNT = 24
EXPECTED_TARGET_IDENTIFIER_COUNT = 268
EXPECTED_ACTIVE_IDENTIFIER_COUNT = 292
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 308
EXPECTED_INACTIVE_RETAINED_IDENTIFIER_COUNT = 16
EXPECTED_MERGED_CLASS_COUNTS = (1, 1, 2, 2)
EXPECTED_CLASS_POWER_COUNT = 118
EXPECTED_GROUP_SIGNATURE_COUNT = 459
EXPECTED_PAIR_POOL_KEY_COUNT = 202
EXPECTED_CONVOLUTION_CALL_COUNT = 7_438
EXPECTED_ORIGINAL_MONOMIAL_COUNT = 508_134_615_924
EXPECTED_MODULUS_SIGNATURE_COUNT = 14_686
EXPECTED_PEAK_LIVE_SIGNATURE_COUNT = 121
EXPECTED_TWO_PRODUCT_ARRAY_BYTES = 1_936
EXPECTED_DISTINCT_COMPARISON_UPPER_BOUND = 194_280
EXPECTED_WEIGHTED_COMPARISON_UPPER_BOUND = 7_968_716_751_096
EXPECTED_SAFE_INT64_CRUDE_BOUND = 226_934_822_400

ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-22 "
    "external nonresonance"
)
REJECTED_CLASSIFICATION = (
    "the block-support-coalesced degree-22 sufficient certificate is rejected"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011az degree-22 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = (
    "ruled_out_within_registered_degree_twenty_two_scope"
)

Q011AY_ARTIFACT_SHA256 = "f841626d0661cc243ecbf845ba637df184557e22bfd5ace53c68def7ffca6ffe"
Q011AY_RUNNER_SHA256 = "aaa922266ec2bd2f1ddecf3945488063074791f546bae90c0fc0398c642958a5"
Q011AY_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011ay._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011ay_degree22_resource_estimate.json"
    runner_path = Path(q011ay.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011AY_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_twenty_two_coalesced_resource_audit"]
    checks = {
        "q011ay_twenty_nine_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 144
            and len(artifacts) == 29
            and all(prior["checks"].values())
        ),
        "q011ay_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011AY_ARTIFACT_SHA256,
        "q011ay_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011AY_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011AY_RUNNER_SHA256
        ),
        "q011ay_section_digests_match": digests == Q011AY_DIGESTS,
        "q011ay_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011ay.GO_DECISION
            and artifact["scientific_outcome"] == q011ay.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011ay.ACTUAL_RESONANCE_OUTCOME
            and cycle["failed_validity_order"] == []
            and cycle["failed_resource_limit_order"] == []
            and theorem[
                "degree_twenty_two_coalesced_full_sweep_preregistration_is_resource_supported"
            ]
            and not theorem["degree_twenty_two_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011ay_preserves_q011ax_accepted_certificate": bool(
            theorem["q011ax_degree_twenty_one_certificate_is_preserved"]
            and artifacts["q011ax"]["scientific_outcome"] == "accepted"
            and artifacts["q011ax"]["cycle"]["theorem_consequence"][
                "degree_twenty_one_external_nonresonance_is_certified"
            ]
            and theorem["certified_external_nonresonance_degrees"] == list(range(2, 22))
            and theorem["missing_external_nonresonance_degrees"] == list(range(22, 91))
        ),
        "q011ay_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011ay_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011ay_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "one_hundred_forty_nine_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 149
        ),
    }
    artifacts["q011ay"] = artifact
    return (
        {
            "prior_q011ay_sealed_input_audit": prior,
            "q011ay": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011AY_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_twenty_two_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011ay._degree_twenty_two_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011ay._degree_twenty_two_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011ay._degree_twenty_two_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011ay"]["cycle"]
    stored_inventory = stored_cycle["degree_twenty_two_inventory_audit"]
    stored_envelope = stored_cycle["degree_twenty_two_envelope_audit"]
    stored_resource = stored_cycle["degree_twenty_two_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011ay_degree_twenty_two_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011ay_degree_twenty_two_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011ay_degree_twenty_two_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011aw_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            == q011ay.EXPECTED_HULL_RECORD_DIGEST
            ...
            == q011ay.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"]
            == q011ay.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope[
            "inactive_retained_q011aw_identifier_count"
        ],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_twenty_three_or_higher_claimed": False,
...
def run_degree_twenty_two_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_twenty_two_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011ay_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "30 artifacts and 149 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "399 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-22 coalesced resource identities",
            "value": {
        ...
        "q011ax_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011ax"]["scientific_outcome"] == "accepted"
                and artifacts["q011ax"]["cycle"]["theorem_consequence"][
                    "degree_twenty_one_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011az_degree22_coalesced_sweep.py"
            ),
            "threshold": "degree-21 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 268 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_twenty_two": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "1901 preserved plus 399 direct equals 2300",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_twenty_two_scope": {
            "passed": True,
            "threshold": "no degree 23--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-22 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-twenty-two external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 23)) if accepted else list(range(2, 22))
    missing_degrees = list(range(23, 91)) if accepted else list(range(22, 91))
    cycle["theorem_consequence"] = {
        "degree_twenty_two_external_nonresonance_is_certified": accepted,
        "an_actual_degree_twenty_two_external_resonance_is_ruled_out": accepted,
        "registered_degree_twenty_two_sufficient_certificate_is_rejected": rejected,
        ...
        "q011ax_degree_twenty_one_certificate_is_preserved": True,
    }
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-twenty-two external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 268 current Q011ay "
        "target discs, exact Fourier multiplicities and outward-rounded hierarchical "
        "product intervals. Sixteen inactive monotone proof records are retained but not "
        "compared. It makes no claim for degrees 23--90, all-order nonresonance, higher "
        "graph smoothness, SSM existence or uniqueness, normal attraction, a basin, "
        "other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011ba degree-twenty-three resource estimate before "
        "preregistering any degree-twenty-three full sweep."
        if accepted
        else "Audit only the first registered degree-twenty-two overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011az validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011az cycle failed strict serialization or digest")
    return cycle


def run_q011az_study() -> dict[str, Any]:
    cycle = run_degree_twenty_two_coalesced_audit()
    sweep = cycle["degree_twenty_two_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 268 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-22 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_twenty_three_or_higher_claim": False,
