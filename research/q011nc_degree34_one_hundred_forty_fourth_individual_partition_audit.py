"""Q011nc individual-disc partition audit for Q011cb flatten ordinal 143."""
...
EXPECTED_CLASS_TOTALS = (1, 12, 7, 2, 5, 7)
...
EXPECTED_OCCUPIED_RECORD_DIGEST = "ff70e648eb2e88d299ccd191da5341c9c82779e63c67d4ad53e444fb387e85a6"
...
EXPECTED_PARENT_INTERSECTION_WIDTH_HEX = "0x1.3ba116d224b81p-31"
EXPECTED_PARENT_CENTER_RELATION = "target_below_product"
EXPECTED_PARENT_CENTER_GAP_HEX = "0x1.33d0627131834p-28"
EXPECTED_PARENT_WITNESS_DIGEST = "d553e6fd6185290fa192bbf42cb791d87b042c169ae56535401b67e84d66f5e4"
EXPECTED_PARENT_PRODUCT_DIGEST = "8cac361587f274489eda6f25cbe50e48c2a6d1a6f7b8376e89951754d258f59d"
...
Q011NB_ARTIFACT_SHA256 = "bb6e8adcc92740beb99a9ce42adcc7b0b5fed009aa0841ec5860987fe84ecb06"
Q011NB_RUNNER_SHA256 = "617e311a391283303dcf9edc41249a0f2db9e6064a4f07bf07b45205179835b8"
Q011NB_DIGEST_NAMES = (
    "input_digest_sha256",
...
Q011NB_DIGESTS = (
    "737fe6360a7162d2835ee775124d3e93a382995f852ad8fb8a6e32563430081c",
    ...
    "e2f13738250d09743e494b78df7adecfbf1e5fb7d5addae1f5ac3d4179e223bb",
)
EXPECTED_Q011NB_STREAM_DIGEST = "9629a42bdb674440b63b30eebf63cad8292d29a719e4d708fcf65e4d9dfe5d92"
EXPECTED_ORDINAL_ONE_HUNDRED_FORTY_TWO_RESOLUTION_DIGEST = (
    "d16bd69692ef992ecc675d954307992f26d80c0cacc57e0fa3a33053e4a3fd2c"
)
ALLOCATION_STREAM_DOMAIN = "q011nc-individual-partition-allocation-records-v1"

INERT_CLASSIFICATION = (
    "the individual-disc partition is interval-inert for the one-hundred-forty-fourth "
    "Q011cb witness"
)
RESOLVED_CLASSIFICATION = (
    "the one-hundred-forty-fourth Q011cb witness is resolved by individual partition"
)
EFFECTIVE_PERSISTENT_CLASSIFICATION = (
    "the individual-disc partition changes intervals but the one-hundred-forty-fourth "
    "Q011cb witness persists"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011nc individual-disc partition audit is inconclusive"
...
def _protocol_globals_are_restored() -> bool:
    return bool(
        ...
        and q011nb._protocol_globals_are_restored()
    )
...
def _sealed_input_audit_cached() -> tuple[
    ...
    prior, artifacts = q011nb._sealed_input_audit()
    ...
    digests = tuple(cycle[name] for name in Q011NB_DIGEST_NAMES)
    checks = {
        "q011nb_three_hundred_forty_four_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["artifact_count"] == 344
            and prior["direct_digest_count"] == 1_560
            and len(artifacts) == 344
            and all(prior["checks"].values())
        ),
        "q011nb_artifact_and_runner_sha256_match": bool(
            _file_sha256(artifact_path) == Q011NB_ARTIFACT_SHA256
            and _file_sha256(runner_path) == Q011NB_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011NB_RUNNER_SHA256
        ),
        "q011nb_section_digests_match": digests == Q011NB_DIGESTS,
        "q011nb_valid_phase_resolution_and_boundary_reproduce": bool(
        ...
        "q011nb_registered_ordinal_one_hundred_forty_two_resolution_reproduces": bool(
        ...
        "q011nb_artifact_is_finite_strict_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "one_thousand_five_hundred_sixty_five_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 1_565
        ),
    }
    artifacts["q011nb"] = artifact
    return (
        {
            "prior_q011nb_sealed_input_audit": prior,
            "q011nb": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011NB_DIGEST_NAMES),
...
def _fixed_individual_input_audit(
    ...
    q011nb_cycle = artifacts["q011nb"]["cycle"]
    q011nb_comparison = q011nb_cycle["complex_phase_product_disc_audit"]
    selection["checks"][
        "ordinals_zero_through_one_hundred_forty_two_are_sealed_phase_resolutions"
    ] = bool(
        previous_check
        and q011nb_cycle["theorem_consequence"][
            "component_safe_complex_phase_discs_resolve_one_hundred_forty_third_q011cb_witness"
        ]
        and q011nb_comparison["global_minimum_margin_witness"][
            "witness_digest_sha256"
        ]
        == EXPECTED_ORDINAL_ONE_HUNDRED_FORTY_TWO_RESOLUTION_DIGEST
    )
    selection["checks"][
        "registered_ordinal_one_hundred_forty_three_parent_witness_reproduces"
    ] = registered_check
    ...
    selection["previous_phase_resolved_ordinals"] = list(range(143))
    selection["ordinal_one_hundred_forty_two_resolution_digest_sha256"] = (
        EXPECTED_ORDINAL_ONE_HUNDRED_FORTY_TWO_RESOLUTION_DIGEST
    )
    selection["one_hundred_forty_fourth_parent_witness"] = selection.pop(
        "third_parent_witness"
    )
    selection["one_hundred_forty_fourth_parent_witness_digest_sha256"] = (
        selection.pop("third_parent_witness_digest_sha256")
    )
    selection["passed"] = all(selection["checks"].values())
    fixed["one_hundred_forty_fourth_parent_witness_selection_audit"] = selection
    fixed["checks"].pop("registered_third_parent_selection_reproduces")
    fixed["checks"][
        "registered_one_hundred_forty_fourth_parent_selection_reproduces"
    ] = selection["passed"]
...
def _interval_record(lower: Fraction, upper: Fraction) -> dict[str, Any]:
    if not lower <= upper:
        raise RuntimeError("Q011nc constructed an unordered interval")
    return {
...
def run_degree_thirty_four_one_hundred_forty_fourth_individual_partition_audit() -> dict[str, Any]:
    ...
    validity_gates = {
        "q011nb_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "345 artifacts and 1565 direct digests reproduce",
            "value": sealed["checks"],
        },
        "registered_one_hundred_forty_fourth_parent_witness_reproduces": {
            "passed": selection["passed"],
            "threshold": "ordinal 143, class counts, 1667 multiplicity and exact witness",
            "value": selection[
                "one_hundred_forty_fourth_parent_witness_digest_sha256"
            ],
        },
        ...
        "all_class_total_degree_and_output_constraints_close": {
            ...
            "threshold": "1667 allocations, fixed class totals, degree 34 and output block 7",
        ...
        "strict_section_digests_runner_and_protocol_provenance_reproduce": {
            "passed": bool(
                ...
                == 64
                and runner["filename"]
                == "q011nc_degree34_one_hundred_forty_fourth_individual_partition_audit.py"
    ...
    diagnostic_gates = {
        "only_the_registered_one_hundred_forty_fourth_q011cb_signature_is_refined": {
            "passed": bool(
            ...
            "threshold": "ordinal 143 only, one target and no other aggregate",
    ...
    cycle = {
        "question": (
            "Does splitting Q011cb flatten ordinal 143 into twelve singleton "
    ...
    prior_theorem = artifacts["q011nb"]["cycle"]["theorem_consequence"]
    cycle["theorem_consequence"] = {
        "individual_partition_is_interval_inert_for_one_hundred_forty_fourth_q011cb_witness": inert,
        "one_hundred_forty_fourth_q011cb_witness_is_resolved_by_individual_partition": resolved,
        "individual_partition_changes_intervals_but_one_hundred_forty_fourth_q011cb_witness_persists": (
            effective_persistent
        ),
        "q011nb_ordinal_one_hundred_forty_two_phase_resolution_is_preserved": True,
        **{
            name: value
    ...
    cycle["claim_boundary"] = (
        "This diagnostic concerns only Q011cb flatten ordinal 143, its 1667 "
        ...
        "It uses ordinals 0 through 142 only as sealed selection boundaries and does "
        "not reevaluate them. It does not evaluate complex phase, the later 44656 "
    ...
    cycle["next_change"] = (
        "Preregister Q011nd to expand only the registered 1667 wave allocations into component-safe complex phase discs."
        if inert
        else "Preregister Q011nd to audit flatten ordinal 144 in registered order."
        if resolved
        else (
            "Preregister Q011nd to refine only the first remaining individual-allocation overlap by component-safe complex phase."
        )
        if effective_persistent
        else "Repair only the first Q011nc validity failure."
    )
    if not (
        ...
        raise RuntimeError("Q011nc cycle failed strict serialization or digest")
    return cycle


def run_q011nc_study() -> dict[str, Any]:
    cycle = run_degree_thirty_four_one_hundred_forty_fourth_individual_partition_audit()
    ...
    return {
        ...
        "mathematical_scope": {
            "diagnostic": (
                "one-hundred-forty-fourth Q011cb witness individual-disc partition"
            ),
            "grid": [SIZE, SIZE],
