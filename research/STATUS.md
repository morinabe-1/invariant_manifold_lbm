...
閾値・恒等式検査・全対象・元入力を維持し、回帰確認とcommit後にattempt03で三格子を最初から再実行する。
[停止原因と修正](../docs/D3Q27_QUARTIC_FORCING_PRECISION.md#ram回復後の再実行と正規化順序の修正-2026-09-08)を参照。
全独立一致とmoment-only仮説は未評価。旧H1の514不合格とH2/H3未評価は保持する。
修正後は停止artifactの全entryと実停止tupleを含む167回帰テストが通過した（67.32秒）。正式再実行の合格とは別である。
