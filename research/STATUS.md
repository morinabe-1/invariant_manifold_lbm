...
runnerは未実装、実格子の診断は未実行。次は元record/fieldとの全件同一性・保存後監査・worker→mainの実行処理を実装する。
...
| 次の主課題 | Q012g3 helperと小格子45件＋先行77件を検証。全960 case・独立192 case、元反例・各格子最悪例を保持。runner実装後にworker→mainを逐次実行する |
