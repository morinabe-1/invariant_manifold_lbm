...
3運動量・Phi(W)−Wの保存差はholdoutで上限以内。超過原因はまだ断定せず、欠陥比とは分けて診断する。
...
次はQ012g1の追加入力封印後に保存量数値gateを診断する。H4の3方向とH5の欠陥比不達は別の次数別診断に残す。
...
| 次の主課題 | Q012g1: 追加入力封印後、同じ全192 holdout caseの保存量をexactに分解。元判定・閾値・mapは保持。次数/欠陥比は別問 |
