...
runnerは未実装、実格子の診断は未実行。次は封印・fresh再生成・保存/再読・逐次worker/mainの実行処理を実装する。
...
| 次の主課題 | Q012g3 helper・保存記録監査を実装。新規100件＋先行87件通過。全960 case・独立192 caseの実格子診断は未実行。runner実装後にworker→mainを逐次実行する |
