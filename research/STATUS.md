...
実格子はまだ開始していない。検証後にsourceを封印・commitし、独立worker192 caseから逐次実行する。
...
| 次の主課題 | Q012g3 runnerを実装、212テスト通過。全960 case・独立192 caseの実格子診断は未開始。source封印後にworker→mainを逐次実行する |
