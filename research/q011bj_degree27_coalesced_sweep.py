"""Q011bj block-support-coalesced degree-twenty-seven nonresonance audit."""
...
q011bg = q011bi.q011bg
q011bh = q011bi.q011bh
...
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 604
...
EXPECTED_PEAK_LIVE_SIGNATURE_COUNT = 180
...
ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-27 "
    "external nonresonance"
)
REJECTED_CLASSIFICATION = (
    "the block-support-coalesced degree-27 sufficient certificate is rejected"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011bj degree-27 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = (
    "ruled_out_within_registered_degree_twenty_seven_scope"
)

Q011BI_ARTIFACT_SHA256 = "61e4f1fd790674e8365013c3e43391c83d5ddfe39c4c48869bc70e90ed088e63"
Q011BI_RUNNER_SHA256 = "b6b84bf50d0f02793a74caffdc48a04a52d8af67b3aa3b712eba9eb7440fd328"
Q011BI_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bi._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bi_degree27_resource_estimate.json"
    runner_path = Path(q011bi.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BI_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_twenty_seven_coalesced_resource_audit"]
    checks = {
        "q011bi_thirty_nine_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 189
            and len(artifacts) == 39
            and all(prior["checks"].values())
        ),
        "q011bi_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011BI_ARTIFACT_SHA256,
        "q011bi_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BI_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BI_RUNNER_SHA256
        ),
        "q011bi_section_digests_match": digests == Q011BI_DIGESTS,
        "q011bi_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bi.GO_DECISION
            and artifact["scientific_outcome"] == q011bi.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bi.ACTUAL_RESONANCE_OUTCOME
            ...
            and not theorem["degree_twenty_seven_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011bi_preserves_q011bh_accepted_certificate": bool(
            theorem["q011bh_degree_twenty_six_certificate_is_preserved"]
            and artifacts["q011bh"]["scientific_outcome"] == "accepted"
            and artifacts["q011bh"]["cycle"]["theorem_consequence"][
                "degree_twenty_six_external_nonresonance_is_certified"
            ]
        ...
        "q011bi_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bi_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bi_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "one_hundred_ninety_four_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 194
        ),
    }
    artifacts["q011bi"] = artifact
    return (
        {
            "prior_q011bi_sealed_input_audit": prior,
            "q011bi": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BI_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_twenty_seven_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011bi._degree_twenty_seven_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011bi._degree_twenty_seven_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bi._degree_twenty_seven_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bi"]["cycle"]
    stored_inventory = stored_cycle["degree_twenty_seven_inventory_audit"]
    stored_envelope = stored_cycle["degree_twenty_seven_envelope_audit"]
    stored_resource = stored_cycle["degree_twenty_seven_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bi_degree_twenty_seven_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bi_degree_twenty_seven_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bi_degree_twenty_seven_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011bg_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            == q011bi.EXPECTED_HULL_RECORD_DIGEST
            ...
            == q011bi.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"]
            == q011bi.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope[
            "inactive_retained_q011bg_identifier_count"
        ],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_twenty_eight_or_higher_claimed": False,
...
def run_degree_twenty_seven_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_twenty_seven_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bi_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "40 artifacts and 194 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "959 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-27 coalesced resource identities",
            "value": {
        ...
        "q011bh_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bh"]["scientific_outcome"] == "accepted"
                and artifacts["q011bh"]["cycle"]["theorem_consequence"][
                    "degree_twenty_six_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011bj_degree27_coalesced_sweep.py"
            ),
            "threshold": "degree-26 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 488 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_twenty_seven": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "3101 preserved plus 959 direct equals 4060",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_twenty_seven_scope": {
            "passed": True,
            "threshold": "no degrees 28--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-27 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-twenty-seven external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 28)) if accepted else list(range(2, 27))
    missing_degrees = list(range(28, 91)) if accepted else list(range(27, 91))
    cycle["theorem_consequence"] = {
        "degree_twenty_seven_external_nonresonance_is_certified": accepted,
        "an_actual_degree_twenty_seven_external_resonance_is_ruled_out": accepted,
        "registered_degree_twenty_seven_sufficient_certificate_is_rejected": rejected,
    ...
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-twenty-seven external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 488 current Q011bi "
        "target discs, exact Fourier multiplicities and outward-rounded hierarchical "
        "product intervals. Ninety-two inactive monotone proof records are retained but "
        "not compared. It makes no claim for degrees 28--90, all-order nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011bk degree-twenty-eight resource estimate before "
        "preregistering any degree-twenty-eight full sweep."
        if accepted
        else "Audit only the first registered degree-twenty-seven overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011bj validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011bj cycle failed strict serialization or digest")
    return cycle


def run_q011bj_study() -> dict[str, Any]:
    cycle = run_degree_twenty_seven_coalesced_audit()
    sweep = cycle["degree_twenty_seven_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 488 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-27 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_twenty_eight_or_higher_claim": False,
