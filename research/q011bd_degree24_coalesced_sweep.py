"""Q011bd block-support-coalesced degree-twenty-four nonresonance audit."""
...
q011bb = q011bc.q011bb
q011an = q011bb.q011an
q011b = q011bc.q011b
q011z = q011bc.q011z
...
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 444
...
EXPECTED_PEAK_LIVE_SIGNATURE_COUNT = 153
...
ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-24 "
    "external nonresonance"
)
REJECTED_CLASSIFICATION = (
    "the block-support-coalesced degree-24 sufficient certificate is rejected"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011bd degree-24 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = (
    "ruled_out_within_registered_degree_twenty_four_scope"
)

Q011BC_ARTIFACT_SHA256 = "a6e0ab8762fecc9ed006a1692b634cc5ad6937a3fcb909554ab8e352c0a74a2e"
Q011BC_RUNNER_SHA256 = "f34211ca52a08845a7453737b1c29c63ee0b5ba9d540b7c47e04c6308ba650a6"
Q011BC_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bc._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bc_degree24_resource_estimate.json"
    runner_path = Path(q011bc.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BC_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_twenty_four_coalesced_resource_audit"]
    checks = {
        "q011bc_thirty_three_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 162
            and len(artifacts) == 33
            and all(prior["checks"].values())
        ),
        "q011bc_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011BC_ARTIFACT_SHA256,
        "q011bc_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BC_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BC_RUNNER_SHA256
        ),
        "q011bc_section_digests_match": digests == Q011BC_DIGESTS,
        "q011bc_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bc.GO_DECISION
            and artifact["scientific_outcome"] == q011bc.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bc.ACTUAL_RESONANCE_OUTCOME
            and cycle["failed_validity_order"] == []
            and cycle["failed_resource_limit_order"] == []
            and theorem[
                "degree_twenty_four_coalesced_full_sweep_preregistration_is_resource_supported"
            ]
            and not theorem["degree_twenty_four_external_nonresonance_is_certified"]
            and resource["product_target_relation_evaluation_count"] == 0
        ),
        "q011bc_preserves_q011bb_accepted_certificate": bool(
            theorem["q011bb_degree_twenty_three_certificate_is_preserved"]
            and artifacts["q011bb"]["scientific_outcome"] == "accepted"
            and artifacts["q011bb"]["cycle"]["theorem_consequence"][
                "degree_twenty_three_external_nonresonance_is_certified"
            ]
        ...
        "q011bc_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bc_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bc_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "one_hundred_sixty_seven_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 167
        ),
    }
    artifacts["q011bc"] = artifact
    return (
        {
            "prior_q011bc_sealed_input_audit": prior,
            "q011bc": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BC_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_twenty_four_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    (
        ...
        external_indices,
        target_groups,
    ) = q011bc._degree_twenty_four_inventory_audit(artifacts)
    envelope, merged_classes, hull_lookup = q011bc._degree_twenty_four_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bc._degree_twenty_four_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bc"]["cycle"]
    stored_inventory = stored_cycle["degree_twenty_four_inventory_audit"]
    stored_envelope = stored_cycle["degree_twenty_four_envelope_audit"]
    stored_resource = stored_cycle["degree_twenty_four_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bc_degree_twenty_four_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bc_degree_twenty_four_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bc_degree_twenty_four_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011ba_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            == q011bc.EXPECTED_HULL_RECORD_DIGEST
            ...
            == q011bc.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"]
            == q011bc.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope[
            "inactive_retained_q011ba_identifier_count"
        ],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_twenty_five_or_higher_claimed": False,
...
def run_degree_twenty_four_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_twenty_four_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bc_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "34 artifacts and 167 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "524 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-24 coalesced resource identities",
            "value": {
        ...
        "q011bb_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bb"]["scientific_outcome"] == "accepted"
                and artifacts["q011bb"]["cycle"]["theorem_consequence"][
                    "degree_twenty_three_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011bd_degree24_coalesced_sweep.py"
            ),
            "threshold": "degree-23 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 372 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_twenty_four": {
            "passed": bool(
                EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT
                + sweep["fully_separated_overlap_aggregate_count"]
                == EXPECTED_DEGREE_AGGREGATE_COUNT
            ),
            "threshold": "2401 preserved plus 524 direct equals 2925",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_twenty_four_scope": {
            "passed": True,
            "threshold": "no degrees 25--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-24 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-twenty-four external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 25)) if accepted else list(range(2, 24))
    missing_degrees = list(range(25, 91)) if accepted else list(range(24, 91))
    cycle["theorem_consequence"] = {
        "degree_twenty_four_external_nonresonance_is_certified": accepted,
        "an_actual_degree_twenty_four_external_resonance_is_ruled_out": accepted,
        "registered_degree_twenty_four_sufficient_certificate_is_rejected": rejected,
    ...
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-twenty-four external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 372 current Q011bc "
        "target discs, exact Fourier multiplicities and outward-rounded hierarchical "
        "product intervals. Forty-eight inactive monotone proof records are retained but "
        "not compared. It makes no claim for degrees 25--90, all-order nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011be degree-twenty-five resource estimate before "
        "preregistering any degree-twenty-five full sweep."
        if accepted
        else "Audit only the first registered degree-twenty-four overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011bd validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011bd cycle failed strict serialization or digest")
    return cycle


def run_q011bd_study() -> dict[str, Any]:
    cycle = run_degree_twenty_four_coalesced_audit()
    sweep = cycle["degree_twenty_four_block_support_coalesced_sweep"]
    return {
        "schema_version": 1,
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 372 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-24 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_twenty_five_or_higher_claim": False,
