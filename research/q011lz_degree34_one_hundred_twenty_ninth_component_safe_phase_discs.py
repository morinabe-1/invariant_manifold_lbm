"""Q011lz component-safe complex phase discs for Q011cb flatten ordinal one hundred twenty-eight."""
...
import research.q011ly_degree34_one_hundred_twenty_ninth_individual_partition_audit as q011ly
...
Q011LY_ARTIFACT_SHA256 = "b538ec0ca51e4cdaa911cb4ecaabf1fa111fa2b221d868155cf68eacc75ef68b"
Q011LY_RUNNER_SHA256 = "cdbedb604528c71ed578bafc1ca2aee423a5bf392d7a889af530d8a429d00904"
Q011LY_DIGEST_NAMES = (
    "input_digest_sha256",
...
EXPECTED_Q011LY_ALLOCATION_RECORD_DIGEST = (
    "65971f80a63db6d9fad8e8525d15742b99b4f7cfa72257fb3267c2eff193828c"
)

RESOLVED_CLASSIFICATION = (
    "the component-safe complex phase discs resolve the one-hundred-twenty-ninth "
    "Q011cb persistent refined witness"
)
PERSISTENT_CLASSIFICATION = (
    "the one-hundred-twenty-ninth Q011cb persistent refined witness persists under "
    "component-safe complex phase discs"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011lz component-safe phase-disc audit is inconclusive"
...
def _q011lz_framed_record_digest() -> Any:
    return q011ih.q011cx._BASE_FRAMED_RECORD_DIGEST("q011lz-component-safe-phase-comparisons-v1")


def _q011lz_exact_fraction_sequence_sha256(
    _domain: bytes,
    values: tuple[Fraction, ...],
) -> str:
    return q011ih.q011cx._BASE_EXACT_FRACTION_SEQUENCE_SHA256(
        b"q011lz-complex-margin-expression-v1",
        values,
    )
...
def _protocol_globals_are_restored() -> bool:
    return bool(
        ...
        and q011ly._protocol_globals_are_restored()
    )


def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011ly._sealed_input_audit()
    artifact_path = _artifact_directory() / (
        "q011ly_degree34_one_hundred_twenty_ninth_individual_partition_audit.json"
    )
    runner_path = Path(q011ly.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    fixed = cycle["fixed_individual_partition_input_audit"]
    partition = cycle["individual_allocation_interval_audit"]
    theorem = cycle["theorem_consequence"]
    digests = tuple(cycle[name] for name in Q011LY_DIGEST_NAMES)
    checks = {
        "q011ly_three_hundred_fifteen_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["artifact_count"] == 315
            and prior["direct_digest_count"] == 1_430
            and len(artifacts) == 315
            and all(prior["checks"].values())
        ),
        "q011ly_artifact_and_runner_sha256_match": bool(
            _file_sha256(artifact_path) == Q011LY_ARTIFACT_SHA256
            and _file_sha256(runner_path) == Q011LY_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011LY_RUNNER_SHA256
        ),
        "q011ly_section_digests_match": digests == Q011LY_DIGESTS,
        "q011ly_valid_interval_inert_outcome_and_boundary_reproduce": bool(
            ...
            and theorem[
                "individual_partition_is_interval_inert_for_one_hundred_twenty_ninth_q011cb_witness"
            ]
            and theorem["q011lx_ordinal_one_hundred_twenty_seven_phase_resolution_is_preserved"]
            and theorem[
                "q011lw_ordinal_one_hundred_twenty_seven_interval_inert_diagnostic_is_preserved"
            ]
            and all(
                value
        ...
        "q011ly_parent_partition_and_relations_reproduce": bool(
            fixed["one_hundred_twenty_ninth_parent_witness_selection_audit"][
                "selected_flat_ordinal"
            ]
            == PARENT_FLAT_ORDINAL
            and fixed["compatible_allocation_count"] == EXPECTED_INDIVIDUAL_WAVE_ALLOCATION_COUNT
            and fixed["compatible_allocation_digest_sha256"] == q011ly.EXPECTED_COMPATIBLE_DIGEST
        ...
        "q011ly_artifact_is_finite_strict_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "one_thousand_four_hundred_thirty_four_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 1_434
        ),
    }
    artifacts["q011ly"] = artifact
    return (
        {
            "prior_q011ly_sealed_input_audit": prior,
            "q011ly": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011LY_DIGEST_NAMES),
...
def _fixed_phase_input_audit(
    ...
    fixed, _lookup, wave_compatible, _parent = q011ly._fixed_individual_input_audit(artifacts)
    stored = artifacts["q011ly"]["cycle"]
    ...
    checks = {
        "q011ly_fixed_parent_replays_and_stored_interval_partition_is_inert": bool(
        ...
        "registered_active_pairs_zero_power_pair_and_conjugacy_reproduce": bool(
            ...
            and all(SOURCE_POWER_MAXIMUM_COUNTS[index] > 0 for index in (*range(10), 12, 13))
    ...
    audit = {
        ...
        "q011ly_compatible_wave_allocation_count": len(wave_compatible),
        "q011ly_compatible_wave_allocation_digest_sha256": fixed[
...
def _component_wave_counts(individual_counts: tuple[int, ...]) -> tuple[int, ...]:
    if len(individual_counts) != 12:
        raise ValueError("Q011lz requires twelve Q011ly individual counts")
    return (
        *individual_counts[:4],
...
def _phase_wave_counts(phase_counts: list[int]) -> tuple[int, ...]:
    if len(phase_counts) != 14 or phase_counts[10:12] != [0, 0]:
        raise ValueError(
            "Q011lz requires fourteen canonical counts with an inactive center-148 pair"
        )
    return (
        *phase_counts[:4],
...
def _allocation_inventory(
    ...
    audit = {
        ...
        "active_source_variant_order": [
            source_discs[index].identifier for index in (*range(10), 12, 13)
        ],
        "inactive_zero_power_identifiers": [source_discs[index].identifier for index in (10, 11)],
        ...
        "allocation_protocol_adapter": {
            ...
            "ordinal_one_hundred_twenty_eight_component_totals": [1, 12, 9, 5, 0, 7],
...
def _phase_product_audit(
    ...
    replacements = {
        ...
        "PARENT_FLAT_ORDINAL": PARENT_FLAT_ORDINAL,
        "_q011ih_framed_record_digest": _q011lz_framed_record_digest,
        "_q011ih_exact_fraction_sequence_sha256": _q011lz_exact_fraction_sequence_sha256,
    }
    try:
        ...
        for name, value in _Q011IH_PROTOCOL_BASELINE.items():
            setattr(q011ih, name, value)
    audit["comparison_stream_domain"] = "q011lz-component-safe-phase-comparisons-v1"
    return audit


def _registered_parameters() -> dict[str, Any]:
    return {
        "protocol": "q011lz-one-hundred-twenty-ninth-component-safe-phase-v1",
        "degree": DEGREE,
        "parent_flat_ordinal": PARENT_FLAT_ORDINAL,
        "output_block": OUTPUT_BLOCK,
        "target_identifier": TARGET_IDENTIFIER,
        ...
        "comparison_stream_domain": "q011lz-component-safe-phase-comparisons-v1",
        "complex_margin_digest_domain": "q011lz-complex-margin-expression-v1",
        "q011lz_allocation_enumerator": {
            "source_runner_sha256": _file_sha256(Path(__file__).resolve()),
...
def run_degree_thirty_four_one_hundred_twenty_ninth_component_safe_phase_audit() -> dict[str, Any]:
    ...
    validity_gates = {
        "q011ly_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "316 artifacts and 1434 direct digests reproduce",
            "value": sealed["checks"],
        },
        "q011ly_parent_partition_and_interval_inert_result_replay": {
            "passed": phase_input["checks"][
                "q011ly_fixed_parent_replays_and_stored_interval_partition_is_inert"
            ],
            "threshold": "ordinal 128, 1940 individual wave allocations and inert result replay",
            "value": phase_input["q011ly_compatible_wave_allocation_digest_sha256"],
        },
        ...
        "strict_stream_section_digests_runner_and_protocol_provenance_reproduce": {
            "passed": bool(
                ...
                and runner["filename"]
                == "q011lz_degree34_one_hundred_twenty_ninth_component_safe_phase_discs.py"
    ...
    diagnostic_gates = {
        "only_the_registered_one_hundred_twenty_ninth_q011cb_witness_is_refined": {
            "passed": bool(
            ...
            "threshold": "ordinal 128 only, one target and no other overlap",
    ...
    cycle = {
        "question": (
            ...
            "strictly separate every complex product in Q011cb flatten ordinal 128?"
        ),
        **input_sections,
    ...
    prior_theorem = artifacts["q011ly"]["cycle"]["theorem_consequence"]
    cycle["theorem_consequence"] = {
        "component_safe_complex_phase_discs_resolve_one_hundred_twenty_ninth_q011cb_witness": resolved,
        "one_hundred_twenty_ninth_q011cb_witness_persists_under_component_safe_phase_discs": persistent,
        "q011an_component_internal_eigenvalue_labels_are_assumed": False,
        "q011ly_ordinal_one_hundred_twenty_eight_interval_inert_diagnostic_is_preserved": True,
    ...
    cycle["claim_boundary"] = (
        ...
        "ordinal 128, its 1940-to-701 active component-wave quotient and 15278 "
        ...
        "does not reevaluate ordinals 0 through 127 or classify the later 44671 "
    ...
    cycle["next_change"] = (
        "Preregister Q011ma to audit the next Q011cb refined overlap in registered order."
        if resolved
        else (
            "Preregister Q011ma to refine only the first unresolved component-safe phase witness."
        )
        if persistent
        else "Repair only the first Q011lz validity failure."
    )
    if not (
        ...
        raise RuntimeError("Q011lz cycle failed strict serialization or digest")
    return cycle


def run_q011lz_study() -> dict[str, Any]:
    started = perf_counter()
    cycle = run_degree_thirty_four_one_hundred_twenty_ninth_component_safe_phase_audit()
    ...
    return {
        ...
        "mathematical_scope": {
            "diagnostic": "one-hundred-twenty-ninth Q011cb witness component-safe complex phase discs",
            "grid": [SIZE, SIZE],
