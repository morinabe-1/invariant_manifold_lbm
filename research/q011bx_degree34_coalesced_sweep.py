"""Q011bx block-support-coalesced degree-thirty-four nonresonance audit."""
...
q011bv = q011bw.q011bv
q011an = q011bv.q011an
q011b = q011bw.q011b
q011z = q011bw.q011z
...
EXPECTED_DEGREE_AGGREGATE_COUNT = 7_770
EXPECTED_OLD_SEPARATED_AGGREGATE_COUNT = 4_632
EXPECTED_OVERLAP_AGGREGATE_COUNT = 3_138
EXPECTED_SELECTED_IDENTIFIER_COUNT = 24
EXPECTED_TARGET_IDENTIFIER_COUNT = 1_300
EXPECTED_ACTIVE_IDENTIFIER_COUNT = 1_324
EXPECTED_MONOTONE_IDENTIFIER_COUNT = 1_464
EXPECTED_INACTIVE_RETAINED_IDENTIFIER_COUNT = 140
EXPECTED_MULTI_TARGET_AGGREGATE_COUNT = 48
EXPECTED_MAXIMUM_EXTERNAL_COMPONENT_COUNT = 3
EXPECTED_MERGED_CLASS_COUNTS = (1, 1, 2, 2)
EXPECTED_CLASS_POWER_COUNT = 200
EXPECTED_GROUP_SIGNATURE_COUNT = 1_191
EXPECTED_PAIR_POOL_KEY_COUNT = 885
EXPECTED_CONVOLUTION_CALL_COUNT = 52_596
EXPECTED_ORIGINAL_MONOMIAL_COUNT = 2_119_833_971_218_270
EXPECTED_MODULUS_SIGNATURE_COUNT = 221_676
EXPECTED_PEAK_LIVE_SIGNATURE_COUNT = 306
EXPECTED_TWO_PRODUCT_ARRAY_BYTES = 4_896
EXPECTED_DISTINCT_COMPARISON_UPPER_BOUND = 3_448_960
EXPECTED_WEIGHTED_COMPARISON_UPPER_BOUND = 30_455_371_778_893_216
EXPECTED_SAFE_INT64_CRUDE_BOUND = 123_300_484_070_400

ACCEPTED_CLASSIFICATION = (
    "the block-support-coalesced component-safe sweep certifies degree-34 external nonresonance"
)
REJECTED_CLASSIFICATION = "the block-support-coalesced degree-34 sufficient certificate is rejected"
INCONCLUSIVE_CLASSIFICATION = "the Q011bx degree-34 coalesced audit is inconclusive"
ACCEPTED_ACTUAL_RESONANCE_OUTCOME = "ruled_out_within_registered_degree_thirty_four_scope"

Q011BW_ARTIFACT_SHA256 = "d4f3f836e4dd2f89a22895768a0fba3b384e9d6dbcba0ac200cd3bc77f42baf4"
Q011BW_RUNNER_SHA256 = "d47f9646896fe3ccee6652f9243f14de106321d084a8050492c057cbc8b986c7"
Q011BW_DIGEST_NAMES = (
    "input_digest_sha256",
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bw._sealed_input_audit()
    artifact_path = _artifact_directory() / "q011bw_degree34_resource_estimate.json"
    runner_path = Path(q011bw.__file__).resolve()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    cycle = artifact["cycle"]
    digests = tuple(cycle[name] for name in Q011BW_DIGEST_NAMES)
    theorem = cycle["theorem_consequence"]
    resource = cycle["degree_thirty_four_coalesced_resource_audit"]
    checks = {
        "q011bw_fifty_three_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["direct_digest_count"] == 252
            and len(artifacts) == 53
            and all(prior["checks"].values())
        ),
        "q011bw_artifact_sha256_matches": _file_sha256(artifact_path)
        == Q011BW_ARTIFACT_SHA256,
        "q011bw_runner_sha256_matches": bool(
            _file_sha256(runner_path) == Q011BW_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BW_RUNNER_SHA256
        ),
        "q011bw_section_digests_match": digests == Q011BW_DIGESTS,
        "q011bw_registered_resource_go_reproduces": bool(
            artifact["study_gate"] == "passed"
            and artifact["resource_decision"] == q011bw.GO_DECISION
            and artifact["scientific_outcome"] == q011bw.SCIENTIFIC_OUTCOME
            and artifact["actual_resonance_outcome"] == q011bw.ACTUAL_RESONANCE_OUTCOME
        ...
        "q011bw_preserves_q011bv_accepted_certificate": bool(
            theorem["q011bv_degree_thirty_three_certificate_is_preserved"]
            and artifacts["q011bv"]["scientific_outcome"] == "accepted"
            and artifacts["q011bv"]["cycle"]["theorem_consequence"][
                "degree_thirty_three_external_nonresonance_is_certified"
            ]
        ...
        "q011bw_claim_boundary_is_present": bool(cycle["claim_boundary"]),
        "q011bw_package_source_metadata_matches": artifact["source"] == source_metadata(),
        "q011bw_artifact_is_strict_finite_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "two_hundred_fifty_seven_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 257
        ),
    }
    artifacts["q011bw"] = artifact
    return (
        {
            "prior_q011bw_sealed_input_audit": prior,
            "q011bw": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BW_DIGEST_NAMES),
                "digests": list(digests),
                "resource_decision": cycle["resource_decision"],
                "degree_thirty_four_relation_evaluation_count": resource[
...
def _fixed_input_audit(
    ...
    envelope, merged_classes, hull_lookup = q011bw._degree_thirty_four_envelope_audit(
        artifacts, selected_groups, external_merged, target_groups
    )
    resource = q011bw._degree_thirty_four_resource_audit(
        merged_classes, selected_groups, overlap_counts, target_groups
    )
    stored_cycle = artifacts["q011bw"]["cycle"]
    stored_inventory = stored_cycle["degree_thirty_four_inventory_audit"]
    stored_envelope = stored_cycle["degree_thirty_four_envelope_audit"]
    stored_resource = stored_cycle["degree_thirty_four_coalesced_resource_audit"]
    selected_identifiers = set().union(*map(set, selected_groups))
    target_identifiers = set().union(*map(set, target_groups))
    checks = {
        "q011bw_degree_thirty_four_inventory_recomputes_bitwise": bool(
            inventory["passed"] and inventory == stored_inventory
        ),
        "q011bw_degree_thirty_four_envelope_recomputes_bitwise": bool(
            envelope["passed"] and envelope == stored_envelope
        ),
        "q011bw_degree_thirty_four_resource_contract_recomputes_bitwise": bool(
        ...
        "registered_inventory_and_identifier_counts_reproduce": bool(
            ...
            and envelope["inactive_retained_q011bu_identifier_count"]
        ...
        "registered_hull_membership_and_active_targets_reproduce": bool(
            ...
            and envelope["source_hull_record_digest_sha256"] == q011bw.EXPECTED_HULL_RECORD_DIGEST
            and envelope["source_merged_membership_digest_sha256"]
            == q011bw.EXPECTED_MERGED_MEMBERSHIP_DIGEST
            and envelope["active_disc_record_digest_sha256"] == q011bw.EXPECTED_ACTIVE_RECORD_DIGEST
            and envelope["final_disc_record_digest_sha256"] == q011bw.EXPECTED_FINAL_RECORD_DIGEST
        ),
    ...
    audit = {
        ...
        "inactive_retained_identifier_count": envelope["inactive_retained_q011bu_identifier_count"],
...
def _registered_parameters() -> dict[str, Any]:
    return {
        ...
        "degree_thirty_five_or_higher_claimed": False,
...
def run_degree_thirty_four_coalesced_audit() -> dict[str, Any]:
    ...
    sweep_sections = {"degree_thirty_four_block_support_coalesced_sweep": sweep}
    ...
    validity_gates = {
        "q011bw_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "54 artifacts and 257 direct digests reproduce",
            "value": sealed["checks"],
        },
        ...
        "all_registered_aggregates_and_arithmetic_invariants_pass": {
            ...
            "threshold": "3138 aggregates, exact integer convolutions and outward arrays",
        ...
        "registered_resource_counts_reproduce": {
            ...
            "threshold": "all preregistered degree-34 coalesced resource identities",
            "value": {
        ...
        "q011bv_certificate_sections_and_runner_provenance_reproduce": {
            "passed": bool(
                artifacts["q011bv"]["scientific_outcome"] == "accepted"
                and artifacts["q011bv"]["cycle"]["theorem_consequence"][
                    "degree_thirty_three_external_nonresonance_is_certified"
                ]
                and len(input_digest) == len(preparation_digest) == len(sweep_digest) == 64
                and runner["filename"] == "q011bx_degree34_coalesced_sweep.py"
            ),
            "threshold": "degree-33 certificate, three section digests and runner metadata",
            "value": {
                "input": input_digest,
    ...
    hypothesis_gates = {
        "six_hulls_cover_sources_and_all_current_targets_are_preserved": {
            ...
            "threshold": "six source hulls, 24 sources and 1300 current targets only",
            "value": fixed["checks"],
        },
        ...
        "old_and_direct_aggregates_cover_degree_thirty_four": {
            ...
            "threshold": "4632 preserved plus 3138 direct equals 7770",
            "value": {
        ...
        "claim_is_limited_to_registered_degree_thirty_four_scope": {
            "passed": True,
            "threshold": "no degrees 35--90, all-order, SSM or basin extrapolation",
            "value": "fixed 17x17 leaf and registered degree-34 external relation only",
        },
    }
    ...
    cycle = {
        "question": (
            ...
            "every degree-thirty-four external product-target relation?"
        ),
        **input_sections,
    ...
    certified_degrees = list(range(2, 35)) if accepted else list(range(2, 34))
    missing_degrees = list(range(35, 91)) if accepted else list(range(34, 91))
    cycle["theorem_consequence"] = {
        "degree_thirty_four_external_nonresonance_is_certified": accepted,
        "an_actual_degree_thirty_four_external_resonance_is_ruled_out": accepted,
        "registered_degree_thirty_four_sufficient_certificate_is_rejected": rejected,
        ...
        "q011bv_degree_thirty_three_certificate_is_preserved": True,
    }
    cycle["claim_boundary"] = (
        "This certificate concerns only the registered degree-thirty-four external "
        "product-target relations for the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, using the six Q011as source hulls, all 1300 current Q011bw "
        ...
        "retained but not compared. It makes no claim for degrees 35--90, all-order "
        "nonresonance, "
        "higher graph smoothness, SSM existence or uniqueness, normal attraction, a "
        "basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Run a design-only Q011by degree-thirty-five resource estimate before "
        "preregistering any degree-thirty-five full sweep."
        if accepted
        else "Audit only the first registered degree-thirty-four overlap witness before "
        "changing any hull or target representation."
        if rejected
        else "Repair only the first Q011bx validity failure before changing the sweep."
    )
    if not (
        ...
        raise RuntimeError("Q011bx cycle failed strict serialization or digest")
    return cycle


def run_q011bx_study() -> dict[str, Any]:
    cycle = run_degree_thirty_four_coalesced_audit()
    ...
    return {
        ...
        "arithmetic_runtime": {
            ...
            "target_comparisons": "all 1300 current target discs without folding",
            "inactive_monotone_records_compared": False,
            "full_monomial_list_retained": False,
            "full_classification_matrices_retained": False,
            "convolution_call_count": sweep["convolution_call_count"],
        ...
        "mathematical_scope": {
            "diagnostic": "degree-34 block-support-coalesced external nonresonance",
            "grid": [SIZE, SIZE],
            "fixed_conservation_leaf": True,
            "degree": DEGREE,
            "degree_thirty_five_or_higher_claim": False,
