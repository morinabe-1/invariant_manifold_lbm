"""Q011nj component-safe complex phase discs for Q011cb flatten ordinal one hundred forty-six."""
...
EXPECTED_BRIDGE_RECORD_DIGEST = "52cc5e9ee3e160a917e7e06a43f7abd4c3ddd2047f5e9762ec8f48516cd9ee1b"
EXPECTED_PAIRED_RECORD_DIGEST = "5c078fc4a3d0d181b5c9cdc904f363b43aa42344f7a429b21a4d4ff6d633a86e"

Q011NI_ARTIFACT_SHA256 = "267c637818a036d83f68d6fa85c92c99ff03618c16cd1f9c0917f76610d899ec"
Q011NI_RUNNER_SHA256 = "07ffeebee9e32c868bf1d726abe717fa4530903e4afef12245041a4b06be95aa"
Q011NI_DIGEST_NAMES = (
    "input_digest_sha256",
...
Q011NI_DIGESTS = (
    "19e93fb4aefd4eadcdb2396d4678dcc516c0599aa49a331c558de6342d4836cf",
    ...
    "46a853f1009fb06ae4ce7f72c8371f4a96ee0005c23a1da0ab48c5edbd1dc5e9",
)
EXPECTED_Q011NI_ALLOCATION_RECORD_DIGEST = (
    "804129b6c135813785a7ac4b1bca9f94a8ee10d6e74f797e665e86163dc550d4"
)
EXPECTED_Q011NI_ALLOCATION_STREAM_DIGEST = (
    "b140c69635a66c4d357b4c1a5f65b3db28d42af36b5a7e10c4bf9e1f6f932445"
)

RESOLVED_CLASSIFICATION = (
    "the component-safe complex phase discs resolve the one-hundred-forty-seventh "
    "Q011cb persistent refined witness"
)
PERSISTENT_CLASSIFICATION = (
    "the one-hundred-forty-seventh Q011cb persistent refined witness persists under "
    "component-safe complex phase discs"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011nj component-safe phase-disc audit is inconclusive"
...
def _q011nj_framed_record_digest() -> Any:
    return q011ih.q011cx._BASE_FRAMED_RECORD_DIGEST("q011nj-component-safe-phase-comparisons-v1")


def _q011nj_exact_fraction_sequence_sha256(
    _domain: bytes,
    values: tuple[Fraction, ...],
) -> str:
    return q011ih.q011cx._BASE_EXACT_FRACTION_SEQUENCE_SHA256(
        b"q011nj-complex-margin-expression-v1",
        values,
    )
...
def _protocol_globals_are_restored() -> bool:
    return bool(
        ...
        and q011ni._protocol_globals_are_restored()
    )


def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011ni._sealed_input_audit()
    ...
    digests = tuple(cycle[name] for name in Q011NI_DIGEST_NAMES)
    checks = {
        "q011ni_three_hundred_fifty_one_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["artifact_count"] == 351
            and prior["direct_digest_count"] == 1_592
            and len(artifacts) == 351
            and all(prior["checks"].values())
        ),
        "q011ni_artifact_and_runner_sha256_match": bool(
            _file_sha256(artifact_path) == Q011NI_ARTIFACT_SHA256
            and _file_sha256(runner_path) == Q011NI_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011NI_RUNNER_SHA256
        ),
        "q011ni_section_digests_match": digests == Q011NI_DIGESTS,
        "q011ni_valid_interval_inert_outcome_and_boundary_reproduce": bool(
            ...
            and theorem[
                "q011nh_ordinal_one_hundred_forty_five_phase_resolution_is_preserved"
            ]
            and theorem[
        ...
        "q011ni_parent_partition_and_relations_reproduce": bool(
            fixed["one_hundred_forty_seventh_parent_witness_selection_audit"]["selected_flat_ordinal"]
            == PARENT_FLAT_ORDINAL
            and fixed["compatible_allocation_count"] == EXPECTED_INDIVIDUAL_WAVE_ALLOCATION_COUNT
            and fixed["compatible_allocation_digest_sha256"] == q011ni.EXPECTED_COMPATIBLE_DIGEST
        ...
        "q011ni_artifact_is_finite_strict_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "one_thousand_five_hundred_ninety_six_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 1_596
        ),
    }
    artifacts["q011ni"] = artifact
    return (
        {
            "prior_q011ni_sealed_input_audit": prior,
            "q011ni": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011NI_DIGEST_NAMES),
...
def _fixed_phase_input_audit(
    ...
    checks = {
        "q011ni_fixed_parent_replays_and_stored_interval_partition_is_inert": bool(
            fixed["passed"]
        ...
        "q011ni_wave_inventory_reconstructs": bool(
            len(wave_compatible) == EXPECTED_INDIVIDUAL_WAVE_ALLOCATION_COUNT
            and fixed["compatible_allocation_digest_sha256"] == q011ni.EXPECTED_COMPATIBLE_DIGEST
            and wave_digest == q011ni.EXPECTED_COMPATIBLE_DIGEST
            ...
            == q011ni.EXPECTED_LAST_COMPATIBLE_COUNTS
        ),
    ...
    audit = {
        ...
        "q011ni_compatible_wave_allocation_count": len(wave_compatible),
        "q011ni_compatible_wave_allocation_digest_sha256": wave_digest,
        "wave_inventory_source": "sealed Q011ni allocation-classification records",
        "label_semantics": (
...
def _component_wave_counts(individual_counts: tuple[int, ...]) -> tuple[int, ...]:
    if len(individual_counts) != 14:
        raise ValueError("Q011nj requires fourteen Q011ni individual counts")
    return (
        *individual_counts[:4],
...
def _phase_wave_counts(phase_counts: list[int]) -> tuple[int, ...]:
    if len(phase_counts) != 14:
        raise ValueError("Q011nj requires fourteen active canonical counts")
    return (
        *phase_counts[:4],
...
def _allocation_inventory(
    ...
    audit = {
        ...
        "allocation_protocol_adapter": {
            ...
            "ordinal_one_hundred_forty_six_active_totals": [1, 12, 9, 5, 2, 5],
...
def _phase_product_audit(
    ...
    replacements = {
        ...
        "PARENT_FLAT_ORDINAL": PARENT_FLAT_ORDINAL,
        "_q011ih_framed_record_digest": _q011nj_framed_record_digest,
        "_q011ih_exact_fraction_sequence_sha256": _q011nj_exact_fraction_sequence_sha256,
    }
    try:
        ...
        for name, value in _Q011IH_PROTOCOL_BASELINE.items():
            setattr(q011ih, name, value)
    audit["comparison_stream_domain"] = "q011nj-component-safe-phase-comparisons-v1"
    return audit


def _registered_parameters() -> dict[str, Any]:
    return {
        "protocol": "q011nj-one-hundred-forty-seventh-component-safe-phase-v1",
        "degree": DEGREE,
        "parent_flat_ordinal": PARENT_FLAT_ORDINAL,
        "output_block": OUTPUT_BLOCK,
        "target_identifier": TARGET_IDENTIFIER,
        ...
        "comparison_stream_domain": "q011nj-component-safe-phase-comparisons-v1",
        "complex_margin_digest_domain": "q011nj-complex-margin-expression-v1",
        "q011nj_allocation_enumerator": {
            "source_runner_sha256": _file_sha256(Path(__file__).resolve()),
...
def run_degree_thirty_four_one_hundred_forty_seventh_component_safe_phase_audit() -> dict[str, Any]:
    ...
    validity_gates = {
        "q011ni_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "352 artifacts and 1596 direct digests reproduce",
            "value": sealed["checks"],
        },
        "q011ni_parent_partition_and_interval_inert_result_replay": {
            "passed": phase_input["checks"][
                "q011ni_fixed_parent_replays_and_stored_interval_partition_is_inert"
            ],
            "threshold": "ordinal 146, 2799 individual wave allocations and inert result replay",
            "value": phase_input["q011ni_compatible_wave_allocation_digest_sha256"],
        },
        ...
        "component_safe_allocation_and_active_component_quotient_reproduce": {
            ...
            "threshold": (
                "2799 individual waves quotient to 1560 active component waves; "
        ...
        "strict_stream_section_digests_runner_and_protocol_provenance_reproduce": {
            "passed": bool(
                ...
                and runner["filename"]
                == "q011nj_degree34_one_hundred_forty_seventh_component_safe_phase_discs.py"
    ...
    diagnostic_gates = {
        "only_the_registered_one_hundred_forty_seventh_q011cb_witness_is_refined": {
            "passed": bool(
            ...
            "threshold": "ordinal 146 only, one target and no other overlap",
    ...
    cycle = {
        "question": (
            ...
            "strictly separate every complex product in Q011cb flatten ordinal 146?"
        ),
        **input_sections,
    ...
    prior_theorem = artifacts["q011ni"]["cycle"]["theorem_consequence"]
    cycle["theorem_consequence"] = {
        "component_safe_complex_phase_discs_resolve_one_hundred_forty_seventh_q011cb_witness": resolved,
        "one_hundred_forty_seventh_q011cb_witness_persists_under_component_safe_phase_discs": persistent,
        "q011an_component_internal_eigenvalue_labels_are_assumed": False,
        "q011ni_ordinal_one_hundred_forty_six_interval_inert_diagnostic_is_preserved": True,
    ...
    cycle["claim_boundary"] = (
        ...
        "ordinal 146, its 2799-to-1560 active component-wave quotient and 34182 "
        ...
        "ordinals 0 through 145 or classify the later 44653 "
    ...
    cycle["next_change"] = (
        "Preregister Q011nk to audit flatten ordinal 147 in registered order."
        if resolved
        else (
            "Preregister Q011nk to refine only the first unresolved component-safe phase witness."
        )
        if persistent
        else "Repair only the first Q011nj validity failure."
    )
    if not (
        ...
        raise RuntimeError("Q011nj cycle failed strict serialization or digest")
    return cycle


def run_q011nj_study() -> dict[str, Any]:
    started = perf_counter()
    cycle = run_degree_thirty_four_one_hundred_forty_seventh_component_safe_phase_audit()
    ...
    return {
        ...
        "mathematical_scope": {
            "diagnostic": "one-hundred-forty-seventh Q011cb witness component-safe complex phase discs",
            "grid": [SIZE, SIZE],
