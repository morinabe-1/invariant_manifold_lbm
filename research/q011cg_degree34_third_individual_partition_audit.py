"""Q011cg individual-disc partition audit for Q011cb flatten ordinal two."""
...
EXPECTED_LAST_COMPATIBLE_COUNTS = (13, 0, 9, 0, 0, 5, 0, 2, 0, 5)
...
EXPECTED_OCCUPIED_RECORD_DIGEST = (
    "e07eecd95f4d3bdb0be745a5147c34d33115852b90fc449959850bfb9e726a3a"
)
...
EXPECTED_PARENT_INTERSECTION_WIDTH_HEX = "0x1.9d498ce5a3fdcp-32"
EXPECTED_PARENT_CENTER_RELATION = "target_below_product"
EXPECTED_PARENT_CENTER_GAP_HEX = "0x1.3aa03b95e2c0ap-28"
...
EXPECTED_ORDINAL_ZERO_WITNESS_DIGEST = (
    "de9435001e9603c4488945d9077e45abfa30e1c267271a27ae41d01b84993442"
)
EXPECTED_ORDINAL_ONE_RESOLUTION_DIGEST = (
    "57badc5d7675b78362958718cfc68036f87a34d11f88fe4cdd88c7c264932e3c"
)

Q011CF_ARTIFACT_SHA256 = "e12fd42292a42bb3203884fdedfacc8ac2a9027f3b621665573c74af77431806"
Q011CF_RUNNER_SHA256 = "d6c7af8fa781d80ac0dfc5a446a810ed45c5d6981b64a9aa0eb33d0d6efdb94b"
Q011CF_DIGEST_NAMES = (
    "input_digest_sha256",
...
EXPECTED_Q011CF_COMPARISON_STREAM_DIGEST = (
    "e96de25a6f164de2af6b78be68217dfb33deba343594621d550a0ec6dd2a24eb"
)

INERT_CLASSIFICATION = (
    "the individual-disc partition is interval-inert for the third Q011cb witness"
)
RESOLVED_CLASSIFICATION = "the third Q011cb witness is resolved by individual partition"
EFFECTIVE_PERSISTENT_CLASSIFICATION = (
    "the individual-disc partition changes intervals but the third Q011cb witness persists"
)
INCONCLUSIVE_CLASSIFICATION = "the Q011cg individual-disc partition audit is inconclusive"
...
def _interval_record(lower: Fraction, upper: Fraction) -> dict[str, Any]:
    if not lower <= upper:
        raise RuntimeError("Q011cg constructed an unordered interval")
    return {
...
def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011cf._sealed_input_audit()
    artifact_path = _artifact_directory() / (
        "q011cf_degree34_next_component_safe_phase_discs.json"
    )
    ...
    digests = tuple(cycle[name] for name in Q011CF_DIGEST_NAMES)
    checks = {
        "q011cf_sixty_two_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["artifact_count"] == 62
            and prior["direct_digest_count"] == 291
            and len(artifacts) == 62
            and all(prior["checks"].values())
        ),
        "q011cf_artifact_and_runner_sha256_match": bool(
            _file_sha256(artifact_path) == Q011CF_ARTIFACT_SHA256
            and _file_sha256(runner_path) == Q011CF_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011CF_RUNNER_SHA256
        ),
        "q011cf_section_digests_match": digests == Q011CF_DIGESTS,
        "q011cf_valid_phase_resolution_and_boundary_reproduce": bool(
            ...
            and theorem[
                "component_safe_complex_phase_discs_resolve_next_q011cb_witness"
            ]
            and theorem["q011cd_first_witness_phase_resolution_is_preserved"]
        ...
        "q011cf_registered_ordinal_one_resolution_reproduces": bool(
        ...
        "q011cf_artifact_is_finite_strict_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "two_hundred_ninety_six_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 296
        ),
    }
    artifacts["q011cf"] = artifact
    return (
        {
            "prior_q011cf_sealed_input_audit": prior,
            "q011cf": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011CF_DIGEST_NAMES),
...
def _third_parent_witness_audit(
    ...
    ordinal_zero_theorem = artifacts["q011cd"]["cycle"]["theorem_consequence"]
    ordinal_one_cycle = artifacts["q011cf"]["cycle"]
    ordinal_one_theorem = ordinal_one_cycle["theorem_consequence"]
    ordinal_one_comparison = ordinal_one_cycle["complex_phase_product_disc_audit"]
    ...
    flat_index = int(
        np.ravel_multi_index((PARENT_LEFT_INDEX, PARENT_RIGHT_INDEX), active.shape)
    )
    raw = {
    ...
    checks = {
        ...
        "ordinal_zero_and_one_are_sealed_phase_resolutions": bool(
            stored_first["left_index"] == stored_first["right_index"] == 0
            and stored_first["witness_digest_sha256"]
            == EXPECTED_ORDINAL_ZERO_WITNESS_DIGEST
            and ordinal_zero_theorem[
                "component_safe_complex_phase_discs_resolve_first_q011cb_witness"
            ]
            and ordinal_one_theorem[
                "component_safe_complex_phase_discs_resolve_next_q011cb_witness"
            ]
            and ordinal_one_comparison["global_minimum_margin_witness"][
                "witness_digest_sha256"
            ]
            == EXPECTED_ORDINAL_ONE_RESOLUTION_DIGEST
        ),
        "registered_ordinal_two_parent_witness_reproduces": bool(
            flat_index == PARENT_FLAT_ORDINAL
            and active[PARENT_LEFT_INDEX, PARENT_RIGHT_INDEX]
            and tuple(tuple(group) for group in parent["class_counts"])
            == PARENT_CLASS_COUNTS
            and parent["wave_multiplicity"] == EXPECTED_PARENT_WAVE_MULTIPLICITY
            and parent["block_zero_multiplicity"]
            == EXPECTED_PARENT_BLOCK_ZERO_MULTIPLICITY
            and parent["intersection_interval"]["width_hex"]
            == EXPECTED_PARENT_INTERSECTION_WIDTH_HEX
            and parent["center_only_diagnostic"]["relation"]
            == EXPECTED_PARENT_CENTER_RELATION
            and parent["center_only_diagnostic"]["gap_hex"]
    ...
    audit = {
        ...
        "previous_phase_resolved_ordinals": [0, 1],
        "ordinal_zero_witness_digest_sha256": stored_first["witness_digest_sha256"],
        "ordinal_one_resolution_digest_sha256": (
            EXPECTED_ORDINAL_ONE_RESOLUTION_DIGEST
        ),
        "third_parent_witness": parent,
        "third_parent_witness_digest_sha256": parent["witness_digest_sha256"],
        "maximum_fourier_crude_int64_bound": crude_bound,
        "previous_q011cb_refined_signatures_classified_again": False,
...
def _fixed_individual_input_audit(
    ...
    selection, classes, lookup, parent = _third_parent_witness_audit(artifacts)
    occupied_records = []
    ...
    parent_counts = tuple(
        source_frequencies[identifier] for identifier in identifier_order_tuple
    )
    ...
    checks = {
        "registered_third_parent_selection_reproduces": bool(
        ...
        "occupied_classes_and_common_intervals_reproduce": bool(
            tuple(
                record["common_interval_digest_sha256"] for record in occupied_records
            )
            == EXPECTED_OCCUPIED_INTERVAL_DIGESTS
    ...
    audit = {
        ...
        "target_identifier": TARGET_IDENTIFIER,
        "output_block": OUTPUT_BLOCK,
        "third_parent_witness_selection_audit": selection,
...
def run_degree_thirty_four_third_individual_partition_audit() -> dict[str, Any]:
    ...
    validity_gates = {
        "q011cf_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "63 artifacts and 296 direct digests reproduce",
            "value": sealed["checks"],
        },
        "registered_third_parent_witness_reproduces": {
            "passed": selection["passed"],
            "threshold": "ordinal 2, class counts, 852 multiplicity and exact witness",
            "value": selection["third_parent_witness_digest_sha256"],
        },
    ...
    diagnostic_gates = {
        "only_the_registered_third_q011cb_signature_is_refined": {
            "passed": bool(
            ...
            "threshold": "ordinal 2 only, one target and no other aggregate",
    ...
    cycle = {
        "question": (
            "Does splitting Q011cb flatten ordinal 2 into ten singleton identifiers "
    ...
    cycle["theorem_consequence"] = {
        "individual_partition_is_interval_inert_for_third_q011cb_witness": inert,
        "third_q011cb_witness_is_resolved_by_individual_partition": resolved,
    ...
    cycle["claim_boundary"] = (
        "This diagnostic concerns only Q011cb flatten ordinal 2, its 852 "
        ...
        "It uses ordinals 0 and 1 only as sealed selection boundaries and does not "
        "reevaluate them. It does not evaluate complex phase, the later 44797 Q011cb "
        "refined signatures, the other 31 parent coalesced overlaps, other targets, "
        "aggregate 2340 as a whole or aggregate 972. It leaves prior rejection, "
        "persistence, partition-inert and phase-resolution results, certified degrees "
        "2--33 and 91+, and missing degrees 34--90 unchanged, and makes no claim "
        "about an actual resonance, degree-34 or all-order nonresonance, higher graph "
        "smoothness, SSM existence or uniqueness, normal attraction, a basin, other "
        "grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Preregister Q011ch to expand only the 852 registered wave allocations into "
        "component-safe complex phase discs."
        if inert
        else "Preregister Q011ch for the next Q011cb refined overlap."
        if resolved
        else "Preregister Q011ch to apply component-safe complex phase discs only to "
        "the first remaining individual overlap."
        if effective_persistent
        else "Repair only the first Q011cg validity failure."
    )
    if not (
        ...
        raise RuntimeError("Q011cg cycle failed strict serialization or digest")
    return cycle


def run_q011cg_study() -> dict[str, Any]:
    cycle = run_degree_thirty_four_third_individual_partition_audit()
    ...
    return {
        ...
        "mathematical_scope": {
            "diagnostic": "third Q011cb witness individual-disc partition invariance",
            "grid": [SIZE, SIZE],
