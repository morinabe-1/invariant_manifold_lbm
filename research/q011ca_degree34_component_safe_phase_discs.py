"""Q011ca component-safe complex phase-disc audit for the first Q011by witness."""
...
q011by = q011bz.q011by
q011b = q011bz.q011b
q011z = q011bz.q011z
q011ap = q011by.q011bw.q011ap
q011u = q011ap.q011u
q011o = q011u.q011o
...
EXPECTED_TARGET_RADIUS_HEX = "0x1.dfdeb7124fa27p-26"
...
EXPECTED_FULL_ALLOCATION_COUNT = 81_200
EXPECTED_COMPATIBLE_ALLOCATION_COUNT = 5_140
...
EXPECTED_FIRST_COMPATIBLE_COUNTS = (0, 4, 0, 8, 0, 19, 3, 0)
EXPECTED_LAST_COMPATIBLE_COUNTS = (4, 0, 24, 0, 3, 0, 0, 3)
...
PERSISTENT_CLASSIFICATION = (
    "the first Q011by witness family persists under component-safe complex phase discs"
)
INCONCLUSIVE_CLASSIFICATION = (
    "the Q011ca component-safe complex phase-disc audit is inconclusive"
)
...
class _FramedRecordDigest:
    """Hash an ordered exact-derived record stream without retaining its payload."""
...
def _fraction_record(value: Fraction) -> dict[str, str]:
    return q011z._exact_fraction_record(value)
...
def _interval_record(value: RationalInterval) -> dict[str, Any]:
    return {
        ...
        "lower_binary64_hex": float(value.lower).hex(),
        "upper_binary64_hex": float(value.upper).hex(),
    }


def _signed_fraction_record(value: Fraction) -> dict[str, Any]:
    return {
        "exact": _fraction_record(value),
...
def _exact_fraction_sha256(value: Fraction) -> str:
    digest = sha256(b"q011ca-exact-fraction-v1")
...
def _exact_complex_sha256(value: ExactComplex) -> str:
    digest = sha256(b"q011ca-exact-complex-v1")
    for part in value:
...
def _exact_interval_sha256(value: RationalInterval) -> str:
    digest = sha256(b"q011ca-exact-interval-v1")
...
def _exact_fraction_sequence_sha256(
    domain: bytes, values: tuple[Fraction, ...]
) -> str:
...
def _product_modulus_expression_sha256(
    center_modulus: RationalInterval, radius: Fraction
) -> str:
    return _exact_fraction_sequence_sha256(
        b"q011ca-product-modulus-expression-v1",
        (center_modulus.lower, center_modulus.upper, radius),
    )


def _sealed_input_audit() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    prior, artifacts = q011bz._sealed_input_audit()
    ...
    digests = tuple(cycle[name] for name in Q011BZ_DIGEST_NAMES)
    checks = {
        "q011bz_fifty_six_prior_artifacts_and_helpers_reproduce": bool(
            prior["passed"]
            and prior["artifact_count"] == 56
            and prior["direct_digest_count"] == 265
            and len(artifacts) == 56
            and all(prior["checks"].values())
        ),
        "q011bz_artifact_and_runner_sha256_match": bool(
            _file_sha256(artifact_path) == Q011BZ_ARTIFACT_SHA256
            and _file_sha256(runner_path) == Q011BZ_RUNNER_SHA256
            and artifact["runner_source"]["sha256"] == Q011BZ_RUNNER_SHA256
        ),
        "q011bz_section_digests_match": digests == Q011BZ_DIGESTS,
        "q011bz_valid_interval_inert_outcome_and_boundary_reproduce": bool(
            ...
            and theorem[
                "individual_partition_is_interval_inert_for_first_q011by_witness"
            ]
        ...
        "q011bz_parent_partition_and_relations_reproduce": bool(
            cycle["fixed_individual_partition_input_audit"][
                "compatible_allocation_count"
            ]
            == 39
            and cycle["fixed_individual_partition_input_audit"][
                "compatible_allocation_digest_sha256"
            ]
            == q011bz.EXPECTED_COMPATIBLE_DIGEST
            and partition["compatible_allocation_count"] == 39
            and partition["exact_relation_counts"]
            == {"product_below_target": 0, "target_below_product": 0, "overlap": 39}
        ...
        "q011bz_artifact_is_finite_strict_json": bool(
            _all_numeric_values_finite(artifact)
            and _strict_json_serializable(artifact)
            and json.dumps(artifact, allow_nan=False)
        ),
        "two_hundred_sixty_nine_direct_digests_are_sealed": (
            prior["direct_digest_count"] + len(digests) == 269
        ),
    }
    artifacts["q011bz"] = artifact
    return (
        {
            "prior_q011bz_sealed_input_audit": prior,
            "q011bz": {
                "artifact_filename": artifact_path.name,
                "artifact_sha256": _file_sha256(artifact_path),
                "runner_filename": runner_path.name,
                "runner_sha256": _file_sha256(runner_path),
                "digest_names": list(Q011BZ_DIGEST_NAMES),
...
def _radius_by_block(artifacts: dict[str, dict[str, Any]]) -> dict[int, Fraction]:
    records = artifacts["q011ak"]["cycle"][
    ...
    for record in records:
        ...
        if block in result and result[block] != radius:
            raise RuntimeError("Q011ak block radius is not constant within a block")
        result[block] = radius
    return result


def _component_memberships(artifact: dict[str, Any]) -> dict[int, tuple[tuple[int, ...], ...]]:
    structured = artifact["cycle"]["active_block_structured_row_audit"]
    memberships: dict[int, tuple[tuple[int, ...], ...]] = {}
    for record in structured["representative_block_records"].values():
        for key in ("representative_component", "transported_component"):
            component = record[key]
            memberships[int(component["block_index"])] = tuple(
                tuple(int(index) for index in group)
                for group in component["active_component_memberships"]
            )
    return memberships
...
def _fixed_phase_input_audit(
    ...
    fixed, lookup, wave_compatible, parent = q011bz._fixed_individual_input_audit(
        artifacts
    )
    partition = q011bz._individual_partition_audit(lookup, wave_compatible, parent)
    stored = artifacts["q011bz"]["cycle"]
    ...
    radii = _radius_by_block(artifacts)
    source_discs = []
    source_records = []
    ...
    memberships = _component_memberships(artifacts["q011an"])
    expected_components = ((145,), (150, 151))
    row_discs = source_discs[:6]
    block_discs = source_discs[6:]
    ...
    conjugate_pairs = ((0, 1), (2, 4), (3, 5), (6, 7))
    checks = {
        "q011bz_fixed_parent_and_interval_partition_replay_bitwise": bool(
        ...
        "active_singleton_and_two_row_components_have_exact_multiplicity": bool(
            ...
            and all(
                disc.component == ((disc.center_index,) if disc.center_index == 145 else (150, 151))
    ...
    audit = {
        ...
        "q011bz_compatible_wave_allocation_count": len(wave_compatible),
        "q011bz_compatible_wave_allocation_digest_sha256": fixed[
            "compatible_allocation_digest_sha256"
        ],
        "label_semantics": (
...
def _allocation_inventory(
    ...
    for count_16_145 in range(5):
        for count_16_150 in range(28):
            for count_16_151 in range(28 - count_16_150):
                for count_1_150 in range(28 - count_16_150 - count_16_151):
                    count_1_151 = (
                        27 - count_16_150 - count_16_151 - count_1_150
                    )
    ...
    checks = {
        ...
        "all_count_degree_and_wave_constraints_close": all(
            ...
            and record["counts"][0] + record["counts"][1] == 4
            and sum(record["counts"][2:6]) == 27
            and record["counts"][6] + record["counts"][7] == 3
            and all(count >= 0 for count in record["counts"])
            and record["output_block"] == OUTPUT_BLOCK
            for record in compatible
        ),
        "projection_matches_all_thirty_nine_q011bz_wave_allocations": bool(
            set(projection) == wave_counts
            and len(projection) == len(wave_counts) == 39
            and projection_digest == EXPECTED_WAVE_PROJECTION_DIGEST
        ),
        "all_phase_fibers_are_nonempty_and_registered": bool(
            min(projection.values()) == 28
            and max(projection.values()) == 204
...
def _complex_multiply(left: ExactComplex, right: ExactComplex) -> ExactComplex:
    return (
...
def _complex_power(value: ExactComplex, exponent: int) -> ExactComplex:
    result = (Fraction(1), Fraction(0))
    factor = value
    power = exponent
    while power:
        if power & 1:
            result = _complex_multiply(result, factor)
        factor = _complex_multiply(factor, factor)
        power //= 2
    return result
...
def _power_tables(
    ...
    maximum_counts = (4, 4, 27, 27, 27, 27, 3, 3)
    center_powers = []
    upper_powers = []
    full_powers = []
    radius_powers = []
    ...
    for upper, disc, maximum in zip(
        center_uppers, source_discs, maximum_counts, strict=True
    ):
        disc_center_powers = []
        disc_upper_powers = []
        disc_full_powers = []
        ...
        for count in range(maximum + 1):
            center_power = _complex_power(disc.center, count)
...
def _phase_product_audit(
    ...
    stream = _FramedRecordDigest("q011ca-component-safe-phase-comparisons-v1")
    ...
    radius_cache: dict[tuple[int, int, int, int], tuple[Fraction, Fraction]] = {}
    radius_digest_cache: dict[tuple[int, int, int, int], str] = {}
    radius_bound_cache: dict[
        tuple[int, int, int, int], tuple[Fraction, Fraction, Fraction]
    ] = {}
    first_unresolved: tuple[Any, ...] | None = None
    minimum_distance_by_radius: dict[
        tuple[int, int, int, int], tuple[tuple[Any, ...], tuple[Any, ...]]
    ] = {}
    ...
    for index, allocation in enumerate(compatible):
        ...
        all_square_root_enclosures_hold = bool(
            ...
            and center_distance.lower**2
            <= squared_distance
            <= center_distance.upper**2
        )
        ...
        exact_digests = {
            ...
            "complex_margin_expression_digest_sha256": (
                _exact_fraction_sequence_sha256(
                    b"q011ca-complex-margin-expression-v1",
    ...
    if not minimum_distance_by_radius:
        raise RuntimeError("Q011ca has no compatible phase allocation")
    ...
    minimum_separated_data = (
        min(
            (
                record[1]
                for record in minimum_separated_distance_by_radius.values()
            ),
            key=lambda data: (
                _comparison_margin(data),
                tuple(data[1]["counts"]),
    ...
    first_unresolved_witness = (
        _materialize_comparison_witness(first_unresolved)
    ...
    checks = {
        ...
        "closed_product_radius_equals_one_factor_recurrence_exactly": (
            power_table_recurrences_exact
            and all_radius_recurrences_equal
            and len(radius_cache) == 28
        ),
    ...
    return {
        ...
        "comparison_stream_domain": "q011ca-component-safe-phase-comparisons-v1",
        ...
        "other_q011by_class_signatures_recomputed": False,
...
def run_degree_thirty_four_component_safe_phase_audit() -> dict[str, Any]:
    ...
    validity_gates = {
        "q011bz_and_all_prior_proof_objects_are_sealed": {
            "passed": sealed["passed"],
            "threshold": "57 artifacts and 269 direct digests reproduce",
            "value": sealed["checks"],
        },
        "q011bz_parent_partition_and_interval_inert_result_replay": {
            "passed": phase_input["checks"][
                "q011bz_fixed_parent_and_interval_partition_replay_bitwise"
            ],
            "threshold": "the parent witness, 39 wave allocations and inert result replay",
            "value": phase_input["q011bz_compatible_wave_allocation_digest_sha256"],
        },
        "exact_centers_radii_components_and_target_reproduce": {
            ...
            "threshold": "eight source discs, one target and isolated components reproduce",
            "value": {
        ...
        "containment_conjugacy_and_label_free_semantics_pass": {
            ...
            "threshold": "safe disc containment, exact conjugacy and no internal labels",
            "value": phase_input["checks"],
        },
        "component_safe_allocation_and_wave_projection_reproduce": {
            "passed": allocation["passed"],
            "threshold": "81200 total, 5140 compatible and 39 nonempty wave fibers",
            "value": allocation["checks"],
        },
        "exact_product_radius_distance_and_categories_close": {
            "passed": comparison["passed"],
            "threshold": (
                "5140 exact products, recurrence equality, sqrt enclosures and "
    ...
    diagnostic_gates = {
        "only_the_registered_first_witness_family_is_refined": {
            "passed": bool(
                not comparison["other_q011by_class_signatures_recomputed"]
                and not comparison["other_parent_targets_recomputed"]
                and not comparison["second_q011bx_overlap_recomputed"]
            ),
            "threshold": "one Q011by signature, one target and no second aggregate",
        ...
        "all_component_safe_phase_product_discs_are_processed": {
            ...
            "threshold": "all 5140 compatible phase allocations have one category",
    ...
    cycle = {
        "question": (
            ...
            "strictly separate every complex product in the first Q011by witness family?"
        ),
        **input_sections,
    ...
    cycle["claim_boundary"] = (
        "This certificate concerns only the fixed 17x17 repaired exact map on one fixed "
        "conservation leaf, degree-34 parent aggregate 972, the first Q011by class "
        "signature, its 5140 registered component-safe phase allocations and target "
        "block=12;center=124. It does not classify the other 9799 Q011by signatures, "
        "other targets, aggregate 2340 or the full degree-34 sweep. Product-disc overlap "
        "does not establish an actual resonance. The audit leaves the Q011bx rejection, "
        "Q011by persistence, Q011bz interval-inert result, certified degrees 2--33 and "
        "91+, and missing degrees 34--90 unchanged, and makes no claim about degree-34 "
        "nonresonance, all-order nonresonance, higher graph smoothness, SSM existence or "
        "uniqueness, normal attraction, a basin, other grids, forcing, walls or D3Q27."
    )
    cycle["next_change"] = (
        "Preregister Q011cb to audit the second Q011bx unresolved aggregate 2340."
        if resolved
        else (
            "Preregister Q011cb to decompose the radius contribution of the first "
            "unresolved component-safe phase witness."
        )
        if persistent
        else "Repair only the first Q011ca validity failure."
    )
    if not (
        ...
        raise RuntimeError("Q011ca cycle failed strict serialization or digest")
    return cycle


def run_q011ca_study() -> dict[str, Any]:
    started = perf_counter()
    cycle = run_degree_thirty_four_component_safe_phase_audit()
    ...
    return {
        ...
        "mathematical_scope": {
            "diagnostic": "first Q011by witness component-safe complex phase discs",
            "grid": [SIZE, SIZE],
