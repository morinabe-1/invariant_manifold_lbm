...
def q011fj_structure() -> dict[str, Any]:
    sealed, artifacts = q011fj._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011fj._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011fj._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011fj_study() -> dict[str, Any]:
    return q011fj.run_q011fj_study()


@pytest.fixture(scope="module")
def q011fj_cycle(q011fj_study: dict[str, Any]) -> dict[str, Any]:
    return q011fj_study["cycle"]


def test_q011fj_seals_q011fi_and_all_prior_inputs(
    q011fj_structure: dict[str, Any],
) -> None:
    sealed = q011fj_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 144
    assert sealed["direct_digest_count"] == 660
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011fi"]["digests"]) == q011fj.Q011FI_DIGESTS
    assert sealed["q011fi"]["artifact_sha256"] == q011fj.Q011FI_ARTIFACT_SHA256
    assert sealed["q011fi"]["runner_sha256"] == q011fj.Q011FI_RUNNER_SHA256


def test_q011fj_reconstructs_component_safe_phase_discs(
    q011fj_structure: dict[str, Any],
) -> None:
    fixed = q011fj_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 42
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011fj.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011fj.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011fj.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011fj.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011fi_compatible_wave_allocation_count"] == 2_553
    assert fixed["q011fi_compatible_wave_allocation_digest_sha256"] == (
        q011fj.q011fi.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011fj._protocol_globals_are_restored()


def test_q011fj_enumerates_registered_label_free_phase_inventory(
    q011fj_structure: dict[str, Any],
) -> None:
    allocation = q011fj_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 332_640
    assert allocation["full_allocation_digest_sha256"] == (q011fj.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 18_718
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011fj.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011fj.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011fj.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 852
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011fj.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_forty_two_totals"] == [13, 9, 5, 2, 5]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fj result is not sealed")
def test_q011fj_certifies_all_complex_phase_product_discs(
    q011fj_cycle: dict[str, Any],
) -> None:
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fj result is not sealed")
def test_q011fj_minimum_exact_phase_margin_is_fixed(
    q011fj_cycle: dict[str, Any],
) -> None:
    comparison = q011fj_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011fj minimum phase witness has not been sealed yet")
    ...
    assert q011fj.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fj result is not sealed")
def test_q011fj_records_scoped_resolution(q011fj_cycle: dict[str, Any]) -> None:
    assert q011fj_cycle["study_validity"] == "passed"
    assert q011fj_cycle["failed_validity_order"] == []
    assert q011fj_cycle["failed_diagnostic_order"] == []
    assert len(q011fj_cycle["validity_gates"]) == 7
    assert len(q011fj_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011fj_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011fj_cycle["diagnostic_gates"].values())
    assert q011fj_cycle["refinement_outcome"] == "component_safe_phase_resolved"
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fj result is not sealed")
def test_q011fj_preserves_scientific_boundary(q011fj_cycle: dict[str, Any]) -> None:
    theorem = q011fj_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_forty_third_q011cb_witness"]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fj result is not sealed")
def test_q011fj_cycle_has_strict_reproducible_digests(
    q011fj_cycle: dict[str, Any],
) -> None:
    json.dumps(q011fj_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011fj_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011fj_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011fj_cycle["result_digest_sha256"] == (
        q011fj.q011b._canonical_json_sha256(q011fj._result_digest_sections(q011fj_cycle))
    )
    assert q011fj._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fj result is not sealed")
def test_q011fj_study_metadata_and_optional_artifact_are_scoped(
    q011fj_study: dict[str, Any],
) -> None:
    assert q011fj_study["schema_version"] == 1
    assert q011fj_study["source"] == source_metadata()
    ...
    json.dumps(q011fj_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011fj runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011fj artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011fj.q011b._canonical_json_sha256(q011fj._result_digest_sections(artifact["cycle"]))
    )
