...
EXPECTED_SECTION_DIGESTS: dict[str, str] | None = {
    "input_digest_sha256": "36236c8ba96104616423bc3733286ecf3fcc603443c1e8ab5de6bd233febb4ca",
    "phase_input_digest_sha256": "e20b2529b22d258a47a50385761b2dbbc0b6570abda7acf3875f3f8252288f87",
    "allocation_digest_sha256": "414c652bc76732fc9f6b250de651f74982ddb44bfebd2390643f7a49264871a2",
    "phase_comparison_digest_sha256": "b15e9a82c78c84b1f5e5a436d9b35dfac94e04a26aa8a5965c30b90f4931fa1e",
    "result_digest_sha256": "7b1b82c72b306cdff63bd01c2bc806fe436aeb3dfd5b605f5ab83d2a8ab34faa",
}
...
def q011gf_structure() -> dict[str, Any]:
    sealed, artifacts = q011gf._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011gf._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011gf._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011gf_study() -> dict[str, Any]:
    return q011gf.run_q011gf_study()


@pytest.fixture(scope="module")
def q011gf_cycle(q011gf_study: dict[str, Any]) -> dict[str, Any]:
    return q011gf_study["cycle"]


def test_q011gf_seals_q011ge_and_all_prior_inputs(
    q011gf_structure: dict[str, Any],
) -> None:
    sealed = q011gf_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 166
    assert sealed["direct_digest_count"] == 759
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011ge"]["digests"]) == q011gf.Q011GE_DIGESTS
    assert sealed["q011ge"]["artifact_sha256"] == q011gf.Q011GE_ARTIFACT_SHA256
    assert sealed["q011ge"]["runner_sha256"] == q011gf.Q011GE_RUNNER_SHA256


def test_q011gf_reconstructs_component_safe_phase_discs(
    q011gf_structure: dict[str, Any],
) -> None:
    fixed = q011gf_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 53
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011gf.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011gf.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011gf.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011gf.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011ge_compatible_wave_allocation_count"] == 2_382
    assert fixed["q011ge_compatible_wave_allocation_digest_sha256"] == (
        q011gf.q011ge.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011gf._protocol_globals_are_restored()


def test_q011gf_enumerates_registered_label_free_phase_inventory(
    q011gf_structure: dict[str, Any],
) -> None:
    allocation = q011gf_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 332_640
    assert allocation["full_allocation_digest_sha256"] == (q011gf.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 18_718
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011gf.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011gf.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011gf.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 852
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011gf.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_fifty_three_totals"] == [13, 9, 5, 5, 2]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gf result is not sealed")
def test_q011gf_certifies_all_complex_phase_product_discs(
    q011gf_cycle: dict[str, Any],
) -> None:
    comparison = q011gf_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 53
    ...
    assert comparison["comparison_stream_domain"] == ("q011gf-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011gf comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gf result is not sealed")
def test_q011gf_minimum_exact_phase_margin_is_fixed(
    q011gf_cycle: dict[str, Any],
) -> None:
    comparison = q011gf_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011gf minimum phase witness has not been sealed yet")
    ...
    assert q011gf.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gf result is not sealed")
def test_q011gf_records_scoped_resolution(q011gf_cycle: dict[str, Any]) -> None:
    assert q011gf_cycle["study_validity"] == "passed"
    assert q011gf_cycle["failed_validity_order"] == []
    assert q011gf_cycle["failed_diagnostic_order"] == []
    assert len(q011gf_cycle["validity_gates"]) == 7
    assert len(q011gf_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011gf_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011gf_cycle["diagnostic_gates"].values())
    assert q011gf_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011gf_cycle["diagnostic_classification"] == q011gf.RESOLVED_CLASSIFICATION
    assert q011gf_cycle["scientific_outcome"] == "not_evaluated"
    assert q011gf_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011gg" in q011gf_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011gf_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gf result is not sealed")
def test_q011gf_preserves_scientific_boundary(q011gf_cycle: dict[str, Any]) -> None:
    theorem = q011gf_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_fifty_fourth_q011cb_witness"]
    assert not theorem["fifty_fourth_q011cb_witness_persists_under_component_safe_phase_discs"]
    ...
    assert "flatten ordinal 53" in q011gf_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 52" in q011gf_cycle["claim_boundary"]
    assert "later 44746 Q011cb refined signatures" in q011gf_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011gf_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gf result is not sealed")
def test_q011gf_cycle_has_strict_reproducible_digests(
    q011gf_cycle: dict[str, Any],
) -> None:
    json.dumps(q011gf_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011gf_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011gf_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011gf_cycle["result_digest_sha256"] == (
        q011gf.q011b._canonical_json_sha256(q011gf._result_digest_sections(q011gf_cycle))
    )
    assert q011gf._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gf result is not sealed")
def test_q011gf_study_metadata_and_optional_artifact_are_scoped(
    q011gf_study: dict[str, Any],
) -> None:
    assert q011gf_study["schema_version"] == 1
    assert q011gf_study["source"] == source_metadata()
    assert q011gf_study["study_gate"] == "passed"
    assert q011gf_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011gf_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 18_718
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011gf_study["mathematical_scope"]
    ...
    json.dumps(q011gf_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011gf runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011gf artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011gf.q011b._canonical_json_sha256(q011gf._result_digest_sections(artifact["cycle"]))
    )
