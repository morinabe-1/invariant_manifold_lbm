...
import research.q011fn_degree34_forty_fifth_component_safe_phase_discs as q011fn
...
EXPECTED_SECTION_DIGESTS: dict[str, str] | None = {
    "input_digest_sha256": "f7de611741e0af2efe9e4fb013e5cf5b51a71814a40a8b8ae3ba152ed82089da",
    "phase_input_digest_sha256": "7bd9fd868e76f91fa13337d0cd8293908c43fe8ec8e374bf46a9797f62e2a037",
    "allocation_digest_sha256": "c87f7d726dfe7d9a9598385d91b26ca2c72b48655ead2c25f9423f12dfff3594",
    "phase_comparison_digest_sha256": "df204ff635cb5c2acff4d327e8188d1696cc9124835896f7d38252081e4275e8",
    "result_digest_sha256": "b1ae3bc65e106c4f312302087724f254149a0bb59f906d92098caa034df125d4",
}
...
def q011fn_structure() -> dict[str, Any]:
    sealed, artifacts = q011fn._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011fn._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011fn._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011fn_study() -> dict[str, Any]:
    return q011fn.run_q011fn_study()


@pytest.fixture(scope="module")
def q011fn_cycle(q011fn_study: dict[str, Any]) -> dict[str, Any]:
    return q011fn_study["cycle"]


def test_q011fn_seals_q011fm_and_all_prior_inputs(
    q011fn_structure: dict[str, Any],
) -> None:
    sealed = q011fn_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 148
    assert sealed["direct_digest_count"] == 678
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011fm"]["digests"]) == q011fn.Q011FM_DIGESTS
    assert sealed["q011fm"]["artifact_sha256"] == q011fn.Q011FM_ARTIFACT_SHA256
    assert sealed["q011fm"]["runner_sha256"] == q011fn.Q011FM_RUNNER_SHA256


def test_q011fn_reconstructs_component_safe_phase_discs(
    q011fn_structure: dict[str, Any],
) -> None:
    fixed = q011fn_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 44
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011fn.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011fn.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011fn.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011fn.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011fm_compatible_wave_allocation_count"] == 2_837
    assert fixed["q011fm_compatible_wave_allocation_digest_sha256"] == (
        q011fn.q011fm.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011fn._protocol_globals_are_restored()


def test_q011fn_enumerates_registered_label_free_phase_inventory(
    q011fn_structure: dict[str, Any],
) -> None:
    allocation = q011fn_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 369_600
    assert allocation["full_allocation_digest_sha256"] == (q011fn.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 20_786
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011fn.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011fn.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011fn.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 945
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011fn.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_forty_four_totals"] == [13, 9, 5, 4, 3]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fn result is not sealed")
def test_q011fn_certifies_all_complex_phase_product_discs(
    q011fn_cycle: dict[str, Any],
) -> None:
    comparison = q011fn_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 44
    ...
    assert comparison["comparison_stream_domain"] == ("q011fn-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011fn comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fn result is not sealed")
def test_q011fn_minimum_exact_phase_margin_is_fixed(
    q011fn_cycle: dict[str, Any],
) -> None:
    comparison = q011fn_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011fn minimum phase witness has not been sealed yet")
    ...
    assert q011fn.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fn result is not sealed")
def test_q011fn_records_scoped_resolution(q011fn_cycle: dict[str, Any]) -> None:
    assert q011fn_cycle["study_validity"] == "passed"
    assert q011fn_cycle["failed_validity_order"] == []
    assert q011fn_cycle["failed_diagnostic_order"] == []
    assert len(q011fn_cycle["validity_gates"]) == 7
    assert len(q011fn_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011fn_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011fn_cycle["diagnostic_gates"].values())
    assert q011fn_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011fn_cycle["diagnostic_classification"] == q011fn.RESOLVED_CLASSIFICATION
    assert q011fn_cycle["scientific_outcome"] == "not_evaluated"
    assert q011fn_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011fo" in q011fn_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011fn_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fn result is not sealed")
def test_q011fn_preserves_scientific_boundary(q011fn_cycle: dict[str, Any]) -> None:
    theorem = q011fn_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_forty_fifth_q011cb_witness"]
    assert not theorem["forty_fifth_q011cb_witness_persists_under_component_safe_phase_discs"]
    assert theorem["q011an_component_internal_eigenvalue_labels_are_assumed"] is False
    assert theorem["q011fm_interval_inert_diagnostic_is_preserved"]
    ...
    assert "flatten ordinal 44" in q011fn_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 43" in q011fn_cycle["claim_boundary"]
    assert "later 44755 Q011cb refined signatures" in q011fn_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011fn_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fn result is not sealed")
def test_q011fn_cycle_has_strict_reproducible_digests(
    q011fn_cycle: dict[str, Any],
) -> None:
    json.dumps(q011fn_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011fn_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011fn_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011fn_cycle["result_digest_sha256"] == (
        q011fn.q011b._canonical_json_sha256(q011fn._result_digest_sections(q011fn_cycle))
    )
    assert q011fn._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fn result is not sealed")
def test_q011fn_study_metadata_and_optional_artifact_are_scoped(
    q011fn_study: dict[str, Any],
) -> None:
    assert q011fn_study["schema_version"] == 1
    assert q011fn_study["source"] == source_metadata()
    assert q011fn_study["study_gate"] == "passed"
    assert q011fn_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011fn_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 20_786
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011fn_study["mathematical_scope"]
    assert scope["degree"] == 34
    assert scope["parent_aggregate_index"] == 2340
    assert scope["parent_flat_ordinal"] == 44
    assert scope["target_identifier"] == "block=7;center=44"
    assert scope["component_internal_eigenvalue_labels_assumed"] is False
    assert scope["degree_thirty_four_nonresonance_claim"] is False
    assert scope["actual_resonance_claim"] is False
    json.dumps(q011fn_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011fn runner hash has not been sealed yet")
    runner_path = Path(q011fn.__file__).resolve()
    assert _file_sha256(runner_path) == EXPECTED_RUNNER_SHA256
    artifact_path = (
        runner_path.parent
        / "artifacts"
        / "q011fn_degree34_forty_fifth_component_safe_phase_discs.json"
    )
    if not artifact_path.exists():
        pytest.skip("Q011fn artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011fn.q011b._canonical_json_sha256(q011fn._result_digest_sections(artifact["cycle"]))
    )
