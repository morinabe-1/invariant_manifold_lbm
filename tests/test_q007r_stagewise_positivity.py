...
import research.q007r_stagewise_positivity as q007r
...
def q007r_artifact() -> dict:
    artifact_path = (
        Path(q007r.__file__).resolve().parent
        / "artifacts"
...
def test_q007r_accepts_exact_stagewise_population_positivity(
    q007r_artifact,
) -> None:
    cycle = q007r_artifact["cycle"]

    assert cycle == q007r.run_stagewise_positivity_audit()
...
def test_q007r_reconstructs_the_exact_linear_operator_norms(
    q007r_artifact,
) -> None:
    audit = q007r_artifact["cycle"]["linear_operator_audit"]
    projector_columns = [
        _fraction(record)
...
def test_q007r_reconstructs_each_stage_bound_as_an_exact_fraction(
    q007r_artifact,
) -> None:
    cycle = q007r_artifact["cycle"]
...
def test_q007r_audits_streaming_permutations_and_the_convex_filter(
    q007r_artifact,
) -> None:
    audit = q007r_artifact["cycle"]["stage_structure_audit"]
    coefficients = [
...
def test_q007r_records_provenance_and_the_exact_arithmetic_boundary(
    q007r_artifact,
) -> None:
    cycle = q007r_artifact["cycle"]
    runner_path = Path(q007r.__file__).resolve()

    assert q007r_artifact["schema_version"] == 1
    assert q007r_artifact["source"] == source_metadata()
    assert q007r_artifact["runner_source"] == {
        "filename": "q007r_stagewise_positivity.py",
        "sha256": _file_sha256(runner_path),
        "sha256_newline_normalization": "UTF-8 text with universal newlines",
    }
    assert cycle["input_artifact"]["sha256"] == (
        q007r.REGISTERED_Q007Q_ARTIFACT_SHA256
    )
    assert cycle["input_artifact"]["observed_runner_sha256"] == (
        q007r.REGISTERED_Q007Q_RUNNER_SHA256
    )
    assert cycle["input_artifact"]["passed"]
    assert cycle["registered_tube_reuse"]["q007p_forward_invariance"]
    assert cycle["registered_tube_reuse"]["passed"]
    ...
    json.dumps(q007r_artifact, allow_nan=False)
