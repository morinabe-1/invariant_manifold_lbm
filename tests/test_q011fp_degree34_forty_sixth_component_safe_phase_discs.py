...
EXPECTED_SECTION_DIGESTS: dict[str, str] | None = {
    "input_digest_sha256": "ce20602cd488ff3fc048dd56c0ddf59cdc36dddf29756c6fc2e8f65d39782b5e",
    "phase_input_digest_sha256": "8ca18de38550173d5cdf92206bfdd02aece036ff2b2dfc7375a4edb198d3e014",
    "allocation_digest_sha256": "4d588948cf93ff2095d1a8e3691ad86ab3522d950e74f0412f53f4e865d7ca03",
    "phase_comparison_digest_sha256": "5fffa90c4430e10e050090cc38ab5caea4dd117cf8fb37c6002b3108a6c273be",
    "result_digest_sha256": "a382c69573770d52fb131220e7c29bb3e0990dc7afde70fbb3253ea8b63f65ba",
}
...
def q011fp_structure() -> dict[str, Any]:
    sealed, artifacts = q011fp._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011fp._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011fp._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011fp_study() -> dict[str, Any]:
    return q011fp.run_q011fp_study()


@pytest.fixture(scope="module")
def q011fp_cycle(q011fp_study: dict[str, Any]) -> dict[str, Any]:
    return q011fp_study["cycle"]


def test_q011fp_seals_q011fo_and_all_prior_inputs(
    q011fp_structure: dict[str, Any],
) -> None:
    sealed = q011fp_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 150
    assert sealed["direct_digest_count"] == 687
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011fo"]["digests"]) == q011fp.Q011FO_DIGESTS
    assert sealed["q011fo"]["artifact_sha256"] == q011fp.Q011FO_ARTIFACT_SHA256
    assert sealed["q011fo"]["runner_sha256"] == q011fp.Q011FO_RUNNER_SHA256


def test_q011fp_reconstructs_component_safe_phase_discs(
    q011fp_structure: dict[str, Any],
) -> None:
    fixed = q011fp_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 45
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011fp.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011fp.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011fp.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011fp.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011fo_compatible_wave_allocation_count"] == 2_553
    assert fixed["q011fo_compatible_wave_allocation_digest_sha256"] == (
        q011fp.q011fo.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011fp._protocol_globals_are_restored()


def test_q011fp_enumerates_registered_label_free_phase_inventory(
    q011fp_structure: dict[str, Any],
) -> None:
    allocation = q011fp_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 332_640
    assert allocation["full_allocation_digest_sha256"] == (q011fp.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 18_718
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011fp.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011fp.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011fp.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 852
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011fp.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_forty_five_totals"] == [13, 9, 5, 5, 2]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fp result is not sealed")
def test_q011fp_certifies_all_complex_phase_product_discs(
    q011fp_cycle: dict[str, Any],
) -> None:
    comparison = q011fp_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 45
    ...
    assert comparison["comparison_stream_domain"] == ("q011fp-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011fp comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fp result is not sealed")
def test_q011fp_minimum_exact_phase_margin_is_fixed(
    q011fp_cycle: dict[str, Any],
) -> None:
    comparison = q011fp_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011fp minimum phase witness has not been sealed yet")
    ...
    assert q011fp.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fp result is not sealed")
def test_q011fp_records_scoped_resolution(q011fp_cycle: dict[str, Any]) -> None:
    assert q011fp_cycle["study_validity"] == "passed"
    assert q011fp_cycle["failed_validity_order"] == []
    assert q011fp_cycle["failed_diagnostic_order"] == []
    assert len(q011fp_cycle["validity_gates"]) == 7
    assert len(q011fp_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011fp_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011fp_cycle["diagnostic_gates"].values())
    assert q011fp_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011fp_cycle["diagnostic_classification"] == q011fp.RESOLVED_CLASSIFICATION
    assert q011fp_cycle["scientific_outcome"] == "not_evaluated"
    assert q011fp_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011fq" in q011fp_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011fp_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fp result is not sealed")
def test_q011fp_preserves_scientific_boundary(q011fp_cycle: dict[str, Any]) -> None:
    theorem = q011fp_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_forty_sixth_q011cb_witness"]
    assert not theorem["forty_sixth_q011cb_witness_persists_under_component_safe_phase_discs"]
    assert theorem["q011an_component_internal_eigenvalue_labels_are_assumed"] is False
    assert theorem["q011fo_interval_inert_diagnostic_is_preserved"]
    ...
    assert "flatten ordinal 45" in q011fp_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 44" in q011fp_cycle["claim_boundary"]
    assert "later 44754 Q011cb refined signatures" in q011fp_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011fp_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fp result is not sealed")
def test_q011fp_cycle_has_strict_reproducible_digests(
    q011fp_cycle: dict[str, Any],
) -> None:
    json.dumps(q011fp_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011fp_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011fp_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011fp_cycle["result_digest_sha256"] == (
        q011fp.q011b._canonical_json_sha256(q011fp._result_digest_sections(q011fp_cycle))
    )
    assert q011fp._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fp result is not sealed")
def test_q011fp_study_metadata_and_optional_artifact_are_scoped(
    q011fp_study: dict[str, Any],
) -> None:
    assert q011fp_study["schema_version"] == 1
    assert q011fp_study["source"] == source_metadata()
    assert q011fp_study["study_gate"] == "passed"
    assert q011fp_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011fp_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 18_718
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011fp_study["mathematical_scope"]
    ...
    json.dumps(q011fp_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011fp runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011fp artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011fp.q011b._canonical_json_sha256(q011fp._result_digest_sections(artifact["cycle"]))
    )
