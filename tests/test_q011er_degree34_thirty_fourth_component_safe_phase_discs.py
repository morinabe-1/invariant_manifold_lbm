...
EXPECTED_SECTION_DIGESTS = {
    "input_digest_sha256": "1545503d51bb3d379b73473a315469b1b1074acd4cc387411f340e3499b2849a",
    "phase_input_digest_sha256": "e22d92dc366ce9b8749ca60b71f003bf2df6cb72b9524170fc0a2174d746a087",
    "allocation_digest_sha256": "013a5d6e374e841adae9f2e6535b8bc024b983d662cdb4a855d163c6e94354c4",
    "phase_comparison_digest_sha256": "f31d1abd2794e9f9fb0be10364bed7a5f72fe7b04cb5570c9f5539740fcac83f",
    "result_digest_sha256": "6f63c6f26580ae7f39a77d919c09b7c43ae2cfe56dcbfe2aee878107de644dcb",
}
EXPECTED_STREAM_DIGEST = "ed93343a036c0c37991d4e36986bde98cde6d960b68c0ce3dc636b4f81fd0187"
...
EXPECTED_MINIMUM_MARGIN_HEX = "0x1.a8f10a6dc84e1p-6"


@pytest.fixture(scope="module")
def q011er_study() -> dict[str, Any]:
    return q011er.run_q011er_study()


@pytest.fixture(scope="module")
def q011er_cycle(q011er_study: dict[str, Any]) -> dict[str, Any]:
    return q011er_study["cycle"]


def test_q011er_seals_q011eq_and_all_prior_inputs(
    q011er_cycle: dict[str, Any],
) -> None:
    sealed = q011er_cycle["sealed_input_audit"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 126
    assert sealed["direct_digest_count"] == 579
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011eq"]["digests"]) == q011er.Q011EQ_DIGESTS
    assert sealed["q011eq"]["artifact_sha256"] == q011er.Q011EQ_ARTIFACT_SHA256
    assert sealed["q011eq"]["runner_sha256"] == q011er.Q011EQ_RUNNER_SHA256


def test_q011er_reconstructs_component_safe_phase_discs(
    q011er_cycle: dict[str, Any],
) -> None:
    fixed = q011er_cycle["fixed_component_safe_phase_input_audit"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 33
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011er.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011er.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011er.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011er.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011eq_compatible_wave_allocation_count"] == 1_986
    assert fixed["q011eq_compatible_wave_allocation_digest_sha256"] == (
        q011er.q011eq.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011er._protocol_globals_are_restored()


def test_q011er_enumerates_registered_label_free_phase_inventory(
    q011er_cycle: dict[str, Any],
) -> None:
    allocation = q011er_cycle["component_safe_phase_allocation_audit"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 258_720
    assert allocation["full_allocation_digest_sha256"] == (q011er.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 14_578
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011er.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011er.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011er.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 665
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011er.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_thirty_three_totals"] == [13, 9, 5, 1, 6]
...
def test_q011er_certifies_all_complex_phase_product_discs(
    q011er_cycle: dict[str, Any],
) -> None:
    comparison = q011er_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 33
    ...
    assert comparison["comparison_stream_domain"] == ("q011er-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011er comparison stream digest has not been sealed yet")
...
def test_q011er_minimum_exact_phase_margin_is_fixed(
    q011er_cycle: dict[str, Any],
) -> None:
    comparison = q011er_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011er minimum phase witness has not been sealed yet")
    ...
    assert q011er.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


def test_q011er_records_scoped_resolution(q011er_cycle: dict[str, Any]) -> None:
    assert q011er_cycle["study_validity"] == "passed"
    assert q011er_cycle["failed_validity_order"] == []
    assert q011er_cycle["failed_diagnostic_order"] == []
    assert len(q011er_cycle["validity_gates"]) == 7
    assert len(q011er_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011er_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011er_cycle["diagnostic_gates"].values())
    assert q011er_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011er_cycle["diagnostic_classification"] == q011er.RESOLVED_CLASSIFICATION
    assert q011er_cycle["scientific_outcome"] == "not_evaluated"
    assert q011er_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011es" in q011er_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011er_cycle["next_change"]


def test_q011er_preserves_scientific_boundary(q011er_cycle: dict[str, Any]) -> None:
    theorem = q011er_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_thirty_fourth_q011cb_witness"]
    assert not theorem["thirty_fourth_q011cb_witness_persists_under_component_safe_phase_discs"]
    assert theorem["q011an_component_internal_eigenvalue_labels_are_assumed"] is False
    assert theorem["q011eq_interval_inert_diagnostic_is_preserved"]
    ...
    assert "flatten ordinal 33" in q011er_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 32" in q011er_cycle["claim_boundary"]
    assert "later 44766 Q011cb refined signatures" in q011er_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011er_cycle["claim_boundary"]


def test_q011er_cycle_has_strict_reproducible_digests(
    q011er_cycle: dict[str, Any],
) -> None:
    json.dumps(q011er_cycle, allow_nan=False)
    if not all(EXPECTED_SECTION_DIGESTS.values()):
        pytest.skip("Q011er section digests have not been sealed yet")
    assert {name: q011er_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011er_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011er_cycle["result_digest_sha256"] == (
        q011er.q011b._canonical_json_sha256(q011er._result_digest_sections(q011er_cycle))
    )
    assert q011er._protocol_globals_are_restored()


def test_q011er_study_metadata_and_optional_artifact_are_scoped(
    q011er_study: dict[str, Any],
) -> None:
    assert q011er_study["schema_version"] == 1
    assert q011er_study["source"] == source_metadata()
    assert q011er_study["study_gate"] == "passed"
    assert q011er_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011er_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 14_578
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011er_study["mathematical_scope"]
    ...
    json.dumps(q011er_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011er runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011er artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011er.q011b._canonical_json_sha256(q011er._result_digest_sections(artifact["cycle"]))
    )
