...
EXPECTED_SECTION_DIGESTS: dict[str, str] | None = {
    "input_digest_sha256": "49dcf059890130f340817b83ab7bc5232c97d5a3c06469c40548892a31ca86ae",
    "phase_input_digest_sha256": "196844f29ef91b36762300a1016ebf5de5e2183d4449092e7e572def7d9a6609",
    "allocation_digest_sha256": "5fe91401ead1b3d20572f2174a2e1b6bbde949882fbdf0115afd4d7d8c01b4ab",
    "phase_comparison_digest_sha256": "301dfb4b4376fd41000883d0c154655ee9ca168b508d336f769e7665cbb962c4",
    "result_digest_sha256": "e68a6f5b82f8b6fd7c674f4f4906770e9433fe342d2eb48c17b7bdbd35a72e4b",
}
...
def q011gh_structure() -> dict[str, Any]:
    sealed, artifacts = q011gh._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011gh._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011gh._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011gh_study() -> dict[str, Any]:
    return q011gh.run_q011gh_study()


@pytest.fixture(scope="module")
def q011gh_cycle(q011gh_study: dict[str, Any]) -> dict[str, Any]:
    return q011gh_study["cycle"]


def test_q011gh_seals_q011gg_and_all_prior_inputs(
    q011gh_structure: dict[str, Any],
) -> None:
    sealed = q011gh_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 168
    assert sealed["direct_digest_count"] == 768
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011gg"]["digests"]) == q011gh.Q011GG_DIGESTS
    assert sealed["q011gg"]["artifact_sha256"] == q011gh.Q011GG_ARTIFACT_SHA256
    assert sealed["q011gg"]["runner_sha256"] == q011gh.Q011GG_RUNNER_SHA256


def test_q011gh_reconstructs_component_safe_phase_discs(
    q011gh_structure: dict[str, Any],
) -> None:
    fixed = q011gh_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 54
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011gh.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011gh.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011gh.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011gh.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011gg_compatible_wave_allocation_count"] == 1_854
    assert fixed["q011gg_compatible_wave_allocation_digest_sha256"] == (
        q011gh.q011gg.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011gh._protocol_globals_are_restored()


def test_q011gh_enumerates_registered_label_free_phase_inventory(
    q011gh_structure: dict[str, Any],
) -> None:
    allocation = q011gh_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 258_720
    assert allocation["full_allocation_digest_sha256"] == (q011gh.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 14_578
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011gh.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011gh.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011gh.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 665
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011gh.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_fifty_four_totals"] == [13, 9, 5, 6, 1]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gh result is not sealed")
def test_q011gh_certifies_all_complex_phase_product_discs(
    q011gh_cycle: dict[str, Any],
) -> None:
    comparison = q011gh_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 54
    ...
    assert comparison["comparison_stream_domain"] == ("q011gh-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011gh comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gh result is not sealed")
def test_q011gh_minimum_exact_phase_margin_is_fixed(
    q011gh_cycle: dict[str, Any],
) -> None:
    comparison = q011gh_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011gh minimum phase witness has not been sealed yet")
    ...
    assert q011gh.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gh result is not sealed")
def test_q011gh_records_scoped_resolution(q011gh_cycle: dict[str, Any]) -> None:
    assert q011gh_cycle["study_validity"] == "passed"
    assert q011gh_cycle["failed_validity_order"] == []
    assert q011gh_cycle["failed_diagnostic_order"] == []
    assert len(q011gh_cycle["validity_gates"]) == 7
    assert len(q011gh_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011gh_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011gh_cycle["diagnostic_gates"].values())
    assert q011gh_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011gh_cycle["diagnostic_classification"] == q011gh.RESOLVED_CLASSIFICATION
    assert q011gh_cycle["scientific_outcome"] == "not_evaluated"
    assert q011gh_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011gi" in q011gh_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011gh_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gh result is not sealed")
def test_q011gh_preserves_scientific_boundary(q011gh_cycle: dict[str, Any]) -> None:
    theorem = q011gh_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_fifty_fifth_q011cb_witness"]
    assert not theorem["fifty_fifth_q011cb_witness_persists_under_component_safe_phase_discs"]
    ...
    assert "flatten ordinal 54" in q011gh_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 53" in q011gh_cycle["claim_boundary"]
    assert "later 44745 Q011cb refined signatures" in q011gh_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011gh_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gh result is not sealed")
def test_q011gh_cycle_has_strict_reproducible_digests(
    q011gh_cycle: dict[str, Any],
) -> None:
    json.dumps(q011gh_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011gh_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011gh_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011gh_cycle["result_digest_sha256"] == (
        q011gh.q011b._canonical_json_sha256(q011gh._result_digest_sections(q011gh_cycle))
    )
    assert q011gh._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gh result is not sealed")
def test_q011gh_study_metadata_and_optional_artifact_are_scoped(
    q011gh_study: dict[str, Any],
) -> None:
    assert q011gh_study["schema_version"] == 1
    assert q011gh_study["source"] == source_metadata()
    assert q011gh_study["study_gate"] == "passed"
    assert q011gh_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011gh_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 14_578
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011gh_study["mathematical_scope"]
    ...
    json.dumps(q011gh_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011gh runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011gh artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011gh.q011b._canonical_json_sha256(q011gh._result_digest_sections(artifact["cycle"]))
    )
