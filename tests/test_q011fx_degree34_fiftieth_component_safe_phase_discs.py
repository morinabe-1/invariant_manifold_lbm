...
EXPECTED_SECTION_DIGESTS: dict[str, str] | None = {
    "input_digest_sha256": "b536f7304263ede7d22e2095836012312bd783fe17b04ef02d59a7447035457c",
    "phase_input_digest_sha256": "de4a9e934fa457925e22064253889181a1fc6cb8ec198bd36aae96a19604213c",
    "allocation_digest_sha256": "314a92c5330ae43259f40961b800b703ffff1c8396be344820e0f52122b8ff1c",
    "phase_comparison_digest_sha256": "9138fb23066c310fdc5a311cfd3aa62f7a5c92747442e659a17756997cfac135",
    "result_digest_sha256": "8242d8941147dbfbd24bfabea94e7d8eb4b9b1c14ce57c32df1377d681f78d64",
}
...
def q011fx_structure() -> dict[str, Any]:
    sealed, artifacts = q011fx._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011fx._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011fx._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011fx_study() -> dict[str, Any]:
    return q011fx.run_q011fx_study()


@pytest.fixture(scope="module")
def q011fx_cycle(q011fx_study: dict[str, Any]) -> dict[str, Any]:
    return q011fx_study["cycle"]


def test_q011fx_seals_q011fw_and_all_prior_inputs(
    q011fx_structure: dict[str, Any],
) -> None:
    sealed = q011fx_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 158
    assert sealed["direct_digest_count"] == 723
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011fw"]["digests"]) == q011fx.Q011FW_DIGESTS
    assert sealed["q011fw"]["artifact_sha256"] == q011fx.Q011FW_ARTIFACT_SHA256
    assert sealed["q011fw"]["runner_sha256"] == q011fx.Q011FW_RUNNER_SHA256


def test_q011fx_reconstructs_component_safe_phase_discs(
    q011fx_structure: dict[str, Any],
) -> None:
    fixed = q011fx_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 49
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011fx.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011fx.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011fx.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011fx.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011fw_compatible_wave_allocation_count"] == 1_854
    assert fixed["q011fw_compatible_wave_allocation_digest_sha256"] == (
        q011fx.q011fw.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011fx._protocol_globals_are_restored()


def test_q011fx_enumerates_registered_label_free_phase_inventory(
    q011fx_structure: dict[str, Any],
) -> None:
    allocation = q011fx_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 258_720
    assert allocation["full_allocation_digest_sha256"] == (q011fx.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 14_578
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011fx.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011fx.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011fx.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 665
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011fx.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_forty_nine_totals"] == [13, 9, 5, 1, 6]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fx result is not sealed")
def test_q011fx_certifies_all_complex_phase_product_discs(
    q011fx_cycle: dict[str, Any],
) -> None:
    comparison = q011fx_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 49
    ...
    assert comparison["comparison_stream_domain"] == ("q011fx-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011fx comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fx result is not sealed")
def test_q011fx_minimum_exact_phase_margin_is_fixed(
    q011fx_cycle: dict[str, Any],
) -> None:
    comparison = q011fx_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011fx minimum phase witness has not been sealed yet")
    ...
    assert q011fx.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fx result is not sealed")
def test_q011fx_records_scoped_resolution(q011fx_cycle: dict[str, Any]) -> None:
    assert q011fx_cycle["study_validity"] == "passed"
    assert q011fx_cycle["failed_validity_order"] == []
    assert q011fx_cycle["failed_diagnostic_order"] == []
    assert len(q011fx_cycle["validity_gates"]) == 7
    assert len(q011fx_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011fx_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011fx_cycle["diagnostic_gates"].values())
    assert q011fx_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011fx_cycle["diagnostic_classification"] == q011fx.RESOLVED_CLASSIFICATION
    assert q011fx_cycle["scientific_outcome"] == "not_evaluated"
    assert q011fx_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011fy" in q011fx_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011fx_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fx result is not sealed")
def test_q011fx_preserves_scientific_boundary(q011fx_cycle: dict[str, Any]) -> None:
    theorem = q011fx_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_fiftieth_q011cb_witness"]
    assert not theorem["fiftieth_q011cb_witness_persists_under_component_safe_phase_discs"]
    ...
    assert "flatten ordinal 49" in q011fx_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 48" in q011fx_cycle["claim_boundary"]
    assert "later 44750 Q011cb refined signatures" in q011fx_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011fx_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fx result is not sealed")
def test_q011fx_cycle_has_strict_reproducible_digests(
    q011fx_cycle: dict[str, Any],
) -> None:
    json.dumps(q011fx_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011fx_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011fx_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011fx_cycle["result_digest_sha256"] == (
        q011fx.q011b._canonical_json_sha256(q011fx._result_digest_sections(q011fx_cycle))
    )
    assert q011fx._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fx result is not sealed")
def test_q011fx_study_metadata_and_optional_artifact_are_scoped(
    q011fx_study: dict[str, Any],
) -> None:
    assert q011fx_study["schema_version"] == 1
    assert q011fx_study["source"] == source_metadata()
    assert q011fx_study["study_gate"] == "passed"
    assert q011fx_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011fx_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 14_578
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011fx_study["mathematical_scope"]
    ...
    json.dumps(q011fx_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011fx runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011fx artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011fx.q011b._canonical_json_sha256(q011fx._result_digest_sections(artifact["cycle"]))
    )
