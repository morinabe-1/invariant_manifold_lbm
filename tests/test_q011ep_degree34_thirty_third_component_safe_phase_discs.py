...
EXPECTED_SECTION_DIGESTS = {
    "input_digest_sha256": "959928bfc1c256b5b10a72e0001783a4f623d63cd9f8c556cd71c4c20f047b7b",
    "phase_input_digest_sha256": "6a069ccfc34608033e9c86b7fb2659fc40e686d537a4c8ab4b7ebb6bca822f30",
    "allocation_digest_sha256": "319c0aa97c592e6e363c234d1d73f53cfe1d5e72b9c81c137240fba97ad362aa",
    "phase_comparison_digest_sha256": "597748fe387d8ea771cdf5373910d7c83485edbef3634dbfb49c4697c03fbff8",
    "result_digest_sha256": "83a0050d13e11eefcdfaa358b76f50acac5c7deda95db576296ade5a97d54372",
}
EXPECTED_STREAM_DIGEST = "88e4aa8188c98eec04aa38be9feaf313bb3585b35357fc12f008e8df4cc40833"
...
EXPECTED_MINIMUM_MARGIN_HEX = "0x1.a8f10a6dc8463p-6"


@pytest.fixture(scope="module")
def q011ep_study() -> dict[str, Any]:
    return q011ep.run_q011ep_study()


@pytest.fixture(scope="module")
def q011ep_cycle(q011ep_study: dict[str, Any]) -> dict[str, Any]:
    return q011ep_study["cycle"]


def test_q011ep_seals_q011eo_and_all_prior_inputs(
    q011ep_cycle: dict[str, Any],
) -> None:
    sealed = q011ep_cycle["sealed_input_audit"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 124
    assert sealed["direct_digest_count"] == 570
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011eo"]["digests"]) == q011ep.Q011EO_DIGESTS
    assert sealed["q011eo"]["artifact_sha256"] == q011ep.Q011EO_ARTIFACT_SHA256
    assert sealed["q011eo"]["runner_sha256"] == q011ep.Q011EO_RUNNER_SHA256


def test_q011ep_reconstructs_component_safe_phase_discs(
    q011ep_cycle: dict[str, Any],
) -> None:
    fixed = q011ep_cycle["fixed_component_safe_phase_input_audit"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 32
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011ep.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011ep.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011ep.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011ep.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011eo_compatible_wave_allocation_count"] == 1_136
    assert fixed["q011eo_compatible_wave_allocation_digest_sha256"] == (
        q011ep.q011eo.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011ep._protocol_globals_are_restored()


def test_q011ep_enumerates_registered_label_free_phase_inventory(
    q011ep_cycle: dict[str, Any],
) -> None:
    allocation = q011ep_cycle["component_safe_phase_allocation_audit"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 147_840
    assert allocation["full_allocation_digest_sha256"] == (q011ep.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 8_350
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011ep.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011ep.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011ep.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 382
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011ep.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_thirty_two_totals"] == [13, 9, 5, 0, 7]
...
def test_q011ep_certifies_all_complex_phase_product_discs(
    q011ep_cycle: dict[str, Any],
) -> None:
    comparison = q011ep_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 32
    ...
    assert comparison["comparison_stream_domain"] == ("q011ep-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011ep comparison stream digest has not been sealed yet")
...
def test_q011ep_minimum_exact_phase_margin_is_fixed(
    q011ep_cycle: dict[str, Any],
) -> None:
    comparison = q011ep_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011ep minimum phase witness has not been sealed yet")
    ...
    assert q011ep.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


def test_q011ep_records_scoped_resolution(q011ep_cycle: dict[str, Any]) -> None:
    assert q011ep_cycle["study_validity"] == "passed"
    assert q011ep_cycle["failed_validity_order"] == []
    assert q011ep_cycle["failed_diagnostic_order"] == []
    assert len(q011ep_cycle["validity_gates"]) == 7
    assert len(q011ep_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011ep_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011ep_cycle["diagnostic_gates"].values())
    assert q011ep_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011ep_cycle["diagnostic_classification"] == q011ep.RESOLVED_CLASSIFICATION
    assert q011ep_cycle["scientific_outcome"] == "not_evaluated"
    assert q011ep_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011eq" in q011ep_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011ep_cycle["next_change"]


def test_q011ep_preserves_scientific_boundary(q011ep_cycle: dict[str, Any]) -> None:
    theorem = q011ep_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_thirty_third_q011cb_witness"]
    assert not theorem["thirty_third_q011cb_witness_persists_under_component_safe_phase_discs"]
    assert theorem["q011an_component_internal_eigenvalue_labels_are_assumed"] is False
    assert theorem["q011eo_interval_inert_diagnostic_is_preserved"]
    ...
    assert "flatten ordinal 32" in q011ep_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 31" in q011ep_cycle["claim_boundary"]
    assert "later 44767 Q011cb refined signatures" in q011ep_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011ep_cycle["claim_boundary"]


def test_q011ep_cycle_has_strict_reproducible_digests(
    q011ep_cycle: dict[str, Any],
) -> None:
    json.dumps(q011ep_cycle, allow_nan=False)
    if not all(EXPECTED_SECTION_DIGESTS.values()):
        pytest.skip("Q011ep section digests have not been sealed yet")
    assert {name: q011ep_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011ep_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011ep_cycle["result_digest_sha256"] == (
        q011ep.q011b._canonical_json_sha256(q011ep._result_digest_sections(q011ep_cycle))
    )
    assert q011ep._protocol_globals_are_restored()


def test_q011ep_study_metadata_and_optional_artifact_are_scoped(
    q011ep_study: dict[str, Any],
) -> None:
    assert q011ep_study["schema_version"] == 1
    assert q011ep_study["source"] == source_metadata()
    assert q011ep_study["study_gate"] == "passed"
    assert q011ep_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011ep_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 8_350
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011ep_study["mathematical_scope"]
    ...
    json.dumps(q011ep_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011ep runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011ep artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011ep.q011b._canonical_json_sha256(q011ep._result_digest_sections(artifact["cycle"]))
    )
