...
EXPECTED_SECTION_DIGESTS: dict[str, str] | None = {
    "input_digest_sha256": "589210c06d7b0c80feb1c76eae645f92a322d1674e70f8432d43d4a143d58606",
    "phase_input_digest_sha256": "30f600ad2b326f3ae377aa09614ce70e858b938310f47f850117bf3cba5cda4a",
    "allocation_digest_sha256": "44f7e37e32f4470e16e621a947e24f51c43047ad0f2692273687d6f575e91ade",
    "phase_comparison_digest_sha256": "98f21b7a204ec83574edc20b94189b0bfed5d75210d34072a80668aba4587f9d",
    "result_digest_sha256": "f21ae43121baf90cc191bbb2184b281e90308f1c8e24142dddee8146b871b403",
}
...
def q011fz_structure() -> dict[str, Any]:
    sealed, artifacts = q011fz._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011fz._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011fz._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011fz_study() -> dict[str, Any]:
    return q011fz.run_q011fz_study()


@pytest.fixture(scope="module")
def q011fz_cycle(q011fz_study: dict[str, Any]) -> dict[str, Any]:
    return q011fz_study["cycle"]


def test_q011fz_seals_q011fy_and_all_prior_inputs(
    q011fz_structure: dict[str, Any],
) -> None:
    sealed = q011fz_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 160
    assert sealed["direct_digest_count"] == 732
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011fy"]["digests"]) == q011fz.Q011FY_DIGESTS
    assert sealed["q011fy"]["artifact_sha256"] == q011fz.Q011FY_ARTIFACT_SHA256
    assert sealed["q011fy"]["runner_sha256"] == q011fz.Q011FY_RUNNER_SHA256


def test_q011fz_reconstructs_component_safe_phase_discs(
    q011fz_structure: dict[str, Any],
) -> None:
    fixed = q011fz_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 50
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011fz.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011fz.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011fz.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011fz.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011fy_compatible_wave_allocation_count"] == 2_382
    assert fixed["q011fy_compatible_wave_allocation_digest_sha256"] == (
        q011fz.q011fy.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011fz._protocol_globals_are_restored()


def test_q011fz_enumerates_registered_label_free_phase_inventory(
    q011fz_structure: dict[str, Any],
) -> None:
    allocation = q011fz_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 332_640
    assert allocation["full_allocation_digest_sha256"] == (q011fz.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 18_718
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011fz.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011fz.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011fz.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 852
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011fz.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_fifty_totals"] == [13, 9, 5, 2, 5]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fz result is not sealed")
def test_q011fz_certifies_all_complex_phase_product_discs(
    q011fz_cycle: dict[str, Any],
) -> None:
    comparison = q011fz_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 50
    ...
    assert comparison["comparison_stream_domain"] == ("q011fz-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011fz comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fz result is not sealed")
def test_q011fz_minimum_exact_phase_margin_is_fixed(
    q011fz_cycle: dict[str, Any],
) -> None:
    comparison = q011fz_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011fz minimum phase witness has not been sealed yet")
    ...
    assert q011fz.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fz result is not sealed")
def test_q011fz_records_scoped_resolution(q011fz_cycle: dict[str, Any]) -> None:
    assert q011fz_cycle["study_validity"] == "passed"
    assert q011fz_cycle["failed_validity_order"] == []
    assert q011fz_cycle["failed_diagnostic_order"] == []
    assert len(q011fz_cycle["validity_gates"]) == 7
    assert len(q011fz_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011fz_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011fz_cycle["diagnostic_gates"].values())
    assert q011fz_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011fz_cycle["diagnostic_classification"] == q011fz.RESOLVED_CLASSIFICATION
    assert q011fz_cycle["scientific_outcome"] == "not_evaluated"
    assert q011fz_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011ga" in q011fz_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011fz_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fz result is not sealed")
def test_q011fz_preserves_scientific_boundary(q011fz_cycle: dict[str, Any]) -> None:
    theorem = q011fz_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_fifty_first_q011cb_witness"]
    assert not theorem["fifty_first_q011cb_witness_persists_under_component_safe_phase_discs"]
    ...
    assert "flatten ordinal 50" in q011fz_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 49" in q011fz_cycle["claim_boundary"]
    assert "later 44749 Q011cb refined signatures" in q011fz_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011fz_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fz result is not sealed")
def test_q011fz_cycle_has_strict_reproducible_digests(
    q011fz_cycle: dict[str, Any],
) -> None:
    json.dumps(q011fz_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011fz_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011fz_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011fz_cycle["result_digest_sha256"] == (
        q011fz.q011b._canonical_json_sha256(q011fz._result_digest_sections(q011fz_cycle))
    )
    assert q011fz._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fz result is not sealed")
def test_q011fz_study_metadata_and_optional_artifact_are_scoped(
    q011fz_study: dict[str, Any],
) -> None:
    assert q011fz_study["schema_version"] == 1
    assert q011fz_study["source"] == source_metadata()
    assert q011fz_study["study_gate"] == "passed"
    assert q011fz_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011fz_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 18_718
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011fz_study["mathematical_scope"]
    ...
    json.dumps(q011fz_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011fz runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011fz artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011fz.q011b._canonical_json_sha256(q011fz._result_digest_sections(artifact["cycle"]))
    )
