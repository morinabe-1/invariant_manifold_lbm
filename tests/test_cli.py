...
def test_q007c1_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007c1", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007c1_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q007c1"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007c1_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007c1"])
...
def test_q007c2_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007c2", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007c2_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q007c2"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007c2_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007c2"])
...
def test_q007d_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007d", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007d_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q007d"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007d_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007d"])
...
def test_q007e_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007e", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007e_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q007e"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007e_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007e"])
...
def test_q007f_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007f", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007f_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q007f"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007f_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007f"])
...
def test_q007g_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007g", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007g_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q007g"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007g_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007g"])
...
def test_q007h_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007h", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007h_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "failed", "cycle": {"question": "q007h"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007h_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007h"])
...
def test_q007h1_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007h1", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007h1_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q007h1"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007h1_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007h1"])
...
def test_q007i_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q007i", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q007i_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q007i"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q007i_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q007i"])
...
def test_q008a_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q008a", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q008a_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q008a"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q008a_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q008a"])
...
def test_q008c_rejects_an_omega_override(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["ttim_lbm", "--study", "q008c", "--omega", "1.5"],
    )
    with pytest.raises(SystemExit, match="2"):
        main()
    assert "--omega is only valid with --study baseline" in capsys.readouterr().err
...
def test_q008c_dispatches_the_registered_study(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    expected = {"study_gate": "passed", "cycle": {"question": "q008c"}}
    monkeypatch.setattr("ttim_lbm.__main__.run_q008c_study", lambda: expected)
    monkeypatch.setattr(sys, "argv", ["ttim_lbm", "--study", "q008c"])
