...
import research.q011ft_degree34_forty_eighth_component_safe_phase_discs as q011ft
...
EXPECTED_SECTION_DIGESTS: dict[str, str] | None = {
    "input_digest_sha256": "367d530a438985e773c4478b04caaff586615952570c7772bdde741cb2c11a40",
    "phase_input_digest_sha256": "c45abeea2f6021d03ca582a28c50e0a156660c5375399ea269046828431dc433",
    "allocation_digest_sha256": "fa87a49fd2a8bbb069bf6d83be2d26e88a8ac27c7ae196abb040cfb3b9333d82",
    "phase_comparison_digest_sha256": "0cd0914189e020cdd89752a47dbadcf4c4b59ea1063bee37249a89a85e7e9d48",
    "result_digest_sha256": "9025e08c21ef91535e2015cbfd97fb3c89ea5a43ddd66878ab0f3522c5590eb6",
}
...
def q011ft_structure() -> dict[str, Any]:
    sealed, artifacts = q011ft._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011ft._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011ft._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011ft_study() -> dict[str, Any]:
    return q011ft.run_q011ft_study()


@pytest.fixture(scope="module")
def q011ft_cycle(q011ft_study: dict[str, Any]) -> dict[str, Any]:
    return q011ft_study["cycle"]


def test_q011ft_seals_q011fs_and_all_prior_inputs(
    q011ft_structure: dict[str, Any],
) -> None:
    sealed = q011ft_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 154
    assert sealed["direct_digest_count"] == 705
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011fs"]["digests"]) == q011ft.Q011FS_DIGESTS
    assert sealed["q011fs"]["artifact_sha256"] == q011ft.Q011FS_ARTIFACT_SHA256
    assert sealed["q011fs"]["runner_sha256"] == q011ft.Q011FS_RUNNER_SHA256


def test_q011ft_reconstructs_component_safe_phase_discs(
    q011ft_structure: dict[str, Any],
) -> None:
    fixed = q011ft_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 47
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011ft.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011ft.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011ft.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011ft.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011fs_compatible_wave_allocation_count"] == 1_136
    assert fixed["q011fs_compatible_wave_allocation_digest_sha256"] == (
        q011ft.q011fs.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011ft._protocol_globals_are_restored()


def test_q011ft_enumerates_registered_label_free_phase_inventory(
    q011ft_structure: dict[str, Any],
) -> None:
    allocation = q011ft_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 147_840
    assert allocation["full_allocation_digest_sha256"] == (q011ft.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 8_350
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011ft.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011ft.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011ft.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 382
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011ft.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_forty_seven_totals"] == [13, 9, 5, 7, 0]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011ft result is not sealed")
def test_q011ft_certifies_all_complex_phase_product_discs(
    q011ft_cycle: dict[str, Any],
) -> None:
    comparison = q011ft_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 47
    ...
    assert comparison["comparison_stream_domain"] == ("q011ft-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011ft comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011ft result is not sealed")
def test_q011ft_minimum_exact_phase_margin_is_fixed(
    q011ft_cycle: dict[str, Any],
) -> None:
    comparison = q011ft_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011ft minimum phase witness has not been sealed yet")
    ...
    assert q011ft.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011ft result is not sealed")
def test_q011ft_records_scoped_resolution(q011ft_cycle: dict[str, Any]) -> None:
    assert q011ft_cycle["study_validity"] == "passed"
    assert q011ft_cycle["failed_validity_order"] == []
    assert q011ft_cycle["failed_diagnostic_order"] == []
    assert len(q011ft_cycle["validity_gates"]) == 7
    assert len(q011ft_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011ft_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011ft_cycle["diagnostic_gates"].values())
    assert q011ft_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011ft_cycle["diagnostic_classification"] == q011ft.RESOLVED_CLASSIFICATION
    assert q011ft_cycle["scientific_outcome"] == "not_evaluated"
    assert q011ft_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011fu" in q011ft_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011ft_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011ft result is not sealed")
def test_q011ft_preserves_scientific_boundary(q011ft_cycle: dict[str, Any]) -> None:
    theorem = q011ft_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_forty_eighth_q011cb_witness"]
    assert not theorem["forty_eighth_q011cb_witness_persists_under_component_safe_phase_discs"]
    assert theorem["q011an_component_internal_eigenvalue_labels_are_assumed"] is False
    assert theorem["q011fs_interval_inert_diagnostic_is_preserved"]
    ...
    assert "flatten ordinal 47" in q011ft_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 46" in q011ft_cycle["claim_boundary"]
    assert "later 44752 Q011cb refined signatures" in q011ft_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011ft_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011ft result is not sealed")
def test_q011ft_cycle_has_strict_reproducible_digests(
    q011ft_cycle: dict[str, Any],
) -> None:
    json.dumps(q011ft_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011ft_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011ft_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011ft_cycle["result_digest_sha256"] == (
        q011ft.q011b._canonical_json_sha256(q011ft._result_digest_sections(q011ft_cycle))
    )
    assert q011ft._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011ft result is not sealed")
def test_q011ft_study_metadata_and_optional_artifact_are_scoped(
    q011ft_study: dict[str, Any],
) -> None:
    assert q011ft_study["schema_version"] == 1
    assert q011ft_study["source"] == source_metadata()
    assert q011ft_study["study_gate"] == "passed"
    assert q011ft_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011ft_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 8_350
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011ft_study["mathematical_scope"]
    assert scope["degree"] == 34
    assert scope["parent_aggregate_index"] == 2340
    assert scope["parent_flat_ordinal"] == 47
    assert scope["target_identifier"] == "block=7;center=44"
    assert scope["component_internal_eigenvalue_labels_assumed"] is False
    assert scope["degree_thirty_four_nonresonance_claim"] is False
    assert scope["actual_resonance_claim"] is False
    json.dumps(q011ft_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011ft runner hash has not been sealed yet")
    runner_path = Path(q011ft.__file__).resolve()
    assert _file_sha256(runner_path) == EXPECTED_RUNNER_SHA256
    artifact_path = (
        runner_path.parent
        / "artifacts"
        / "q011ft_degree34_forty_eighth_component_safe_phase_discs.json"
    )
    if not artifact_path.exists():
        pytest.skip("Q011ft artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011ft.q011b._canonical_json_sha256(q011ft._result_digest_sections(artifact["cycle"]))
    )
