...
def q011lw_structure() -> dict[str, Any]:
    sealed, artifacts = q011lw._sealed_input_audit()
    fixed, _lookup, compatible, parent = q011lw._fixed_individual_input_audit(artifacts)
    return {
        "sealed": sealed,
        "fixed": fixed,
...
@pytest.fixture(scope="module")
def q011lw_study() -> dict[str, Any]:
    return q011lw.run_q011lw_study()


@pytest.fixture(scope="module")
def q011lw_cycle(q011lw_study: dict[str, Any]) -> dict[str, Any]:
    return q011lw_study["cycle"]


def test_q011lw_seals_q011lv_and_all_prior_inputs(
    q011lw_structure: dict[str, Any],
) -> None:
    sealed = q011lw_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 313
    assert sealed["direct_digest_count"] == 1_421
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011lv"]["digests"]) == q011lw.Q011LV_DIGESTS
    assert sealed["q011lv"]["artifact_sha256"] == q011lw.Q011LV_ARTIFACT_SHA256
    assert sealed["q011lv"]["runner_sha256"] == q011lw.Q011LV_RUNNER_SHA256
    assert sealed["q011lv"]["resolved_witness_digest_sha256"] == (
        q011lw.EXPECTED_ORDINAL_ONE_HUNDRED_TWENTY_SIX_RESOLUTION_DIGEST
    )


def test_q011lw_selects_exactly_flatten_ordinal_one_hundred_twenty_seven(
    q011lw_structure: dict[str, Any],
) -> None:
    fixed = q011lw_structure["fixed"]
    selection = fixed["one_hundred_twenty_eighth_parent_witness_selection_audit"]
    assert selection["passed"]
    assert all(selection["checks"].values())
    assert selection["flatten_shape"] == [5_600, 8]
    assert selection["selected_flat_ordinal"] == 127
    assert selection["selected_left_index"] == 15
    assert selection["selected_right_index"] == 7
    assert selection["previous_phase_resolved_ordinals"] == list(range(127))
    assert selection["ordinal_one_hundred_twenty_six_resolution_digest_sha256"] == (
        q011lw.EXPECTED_ORDINAL_ONE_HUNDRED_TWENTY_SIX_RESOLUTION_DIGEST
    )
    parent = selection["one_hundred_twenty_eighth_parent_witness"]
    assert parent["class_counts"] == [[0, 0, 1, 12], [5, 4], [5], [7, 0]]
    assert parent["wave_multiplicity"] == 2_076
    assert parent["block_zero_multiplicity"] == 0
    assert parent["intersection_interval"]["width_hex"] == (
        q011lw.EXPECTED_PARENT_INTERSECTION_WIDTH_HEX
    )
    assert parent["center_only_diagnostic"]["relation"] == "target_below_product"
    assert parent["center_only_diagnostic"]["gap_hex"] == (
        q011lw.EXPECTED_PARENT_CENTER_GAP_HEX
    )
    assert parent["witness_digest_sha256"] == q011lw.EXPECTED_PARENT_WITNESS_DIGEST


def test_q011lw_reconstructs_registered_partition_and_inventory(
    q011lw_structure: dict[str, Any],
) -> None:
    fixed = q011lw_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["parent_class_counts"] == [[0, 0, 1, 12], [5, 4], [5], [7, 0]]
    ...
    assert fixed["occupied_class_record_digest_sha256"] == q011lw.EXPECTED_OCCUPIED_RECORD_DIGEST
    assert tuple(fixed["singleton_identifier_order"]) == q011lw.EXPECTED_IDENTIFIER_ORDER
    assert fixed["full_allocation_count"] == 37_440
    assert fixed["full_allocation_digest_sha256"] == q011lw.EXPECTED_ALLOCATION_DIGEST
    assert fixed["compatible_allocation_count"] == 2_076
    assert q011lw_structure["compatible_count"] == 2_076
    assert fixed["compatible_allocation_digest_sha256"] == q011lw.EXPECTED_COMPATIBLE_DIGEST
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011lw result is not sealed")
def test_q011lw_classifies_every_registered_exact_interval(
    q011lw_cycle: dict[str, Any],
) -> None:
    assert EXPECTED_PARTITION_DIGESTS is not None
    partition = q011lw_cycle["individual_allocation_interval_audit"]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011lw result is not sealed")
def test_q011lw_applies_registered_stopping_rule(
    q011lw_cycle: dict[str, Any],
) -> None:
    assert EXPECTED_REFINEMENT_OUTCOME is not None
    assert q011lw_cycle["study_validity"] == "passed"
    assert q011lw_cycle["failed_validity_order"] == []
    assert q011lw_cycle["failed_diagnostic_order"] == []
    assert all(gate["passed"] for gate in q011lw_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011lw_cycle["diagnostic_gates"].values())
    assert q011lw_cycle["scientific_outcome"] == "not_evaluated"
    assert q011lw_cycle["actual_resonance_outcome"] == "not_established"
    assert q011lw_cycle["refinement_outcome"] == EXPECTED_REFINEMENT_OUTCOME
    expected = {
        "partition_inert_persistent": q011lw.INERT_CLASSIFICATION,
        "resolved_by_individual_partition": q011lw.RESOLVED_CLASSIFICATION,
        "partition_effective_but_persistent": q011lw.EFFECTIVE_PERSISTENT_CLASSIFICATION,
    }[EXPECTED_REFINEMENT_OUTCOME]
    assert q011lw_cycle["diagnostic_classification"] == expected
    assert not q011lw_cycle["individual_allocation_interval_audit"][
        "complex_phase_product_evaluated"
    ]
    theorem = q011lw_cycle["theorem_consequence"]
    flags = (
        theorem[
            "individual_partition_is_interval_inert_for_one_hundred_twenty_eighth_q011cb_witness"
        ],
        theorem["one_hundred_twenty_eighth_q011cb_witness_is_resolved_by_individual_partition"],
        theorem[
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011lw result is not sealed")
def test_q011lw_preserves_boundary_and_reproducible_digests(
    q011lw_cycle: dict[str, Any],
) -> None:
    ...
    assert "flatten ordinal 127" in q011lw_cycle["claim_boundary"]
    assert "ordinals 0 through 126" in q011lw_cycle["claim_boundary"]
    assert "later 44672 Q011cb refined signatures" in q011lw_cycle["claim_boundary"]
    assert "Q011lx" in q011lw_cycle["next_change"]
    assert {name: q011lw_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    assert q011lw_cycle["result_digest_sha256"] == (
        q011lw.q011b._canonical_json_sha256(q011lw._result_digest_sections(q011lw_cycle))
    )
    assert q011lw._protocol_globals_are_restored()
    json.dumps(q011lw_cycle, allow_nan=False)


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011lw result is not sealed")
def test_q011lw_study_metadata_and_optional_artifact_are_scoped(
    q011lw_study: dict[str, Any],
) -> None:
    assert q011lw_study["schema_version"] == 1
    assert q011lw_study["source"] == source_metadata()
    assert q011lw_study["study_gate"] == "passed"
    assert q011lw_study["refinement_outcome"] == EXPECTED_REFINEMENT_OUTCOME
    runtime = q011lw_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 2_076
    assert runtime["complex_phase_product_evaluated"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011lw_study["mathematical_scope"]
    ...
    json.dumps(q011lw_study, allow_nan=False)
    ...
    if not artifact_path.exists():
        pytest.skip("Q011lw artifact has not been generated yet")
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    if EXPECTED_RUNNER_SHA256 is not None:
        assert artifact["runner_source"]["sha256"] == EXPECTED_RUNNER_SHA256
    assert artifact["study_gate"] == "passed"
    assert artifact["refinement_outcome"] == q011lw_study["refinement_outcome"]
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011lw.q011b._canonical_json_sha256(q011lw._result_digest_sections(artifact["cycle"]))
    )
