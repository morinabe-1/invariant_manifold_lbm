...
def q011fr_structure() -> dict[str, Any]:
    sealed, artifacts = q011fr._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011fr._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011fr._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011fr_study() -> dict[str, Any]:
    return q011fr.run_q011fr_study()


@pytest.fixture(scope="module")
def q011fr_cycle(q011fr_study: dict[str, Any]) -> dict[str, Any]:
    return q011fr_study["cycle"]


def test_q011fr_seals_q011fq_and_all_prior_inputs(
    q011fr_structure: dict[str, Any],
) -> None:
    sealed = q011fr_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 152
    assert sealed["direct_digest_count"] == 696
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011fq"]["digests"]) == q011fr.Q011FQ_DIGESTS
    assert sealed["q011fq"]["artifact_sha256"] == q011fr.Q011FQ_ARTIFACT_SHA256
    assert sealed["q011fq"]["runner_sha256"] == q011fr.Q011FQ_RUNNER_SHA256


def test_q011fr_reconstructs_component_safe_phase_discs(
    q011fr_structure: dict[str, Any],
) -> None:
    fixed = q011fr_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 46
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011fr.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011fr.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011fr.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011fr.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011fq_compatible_wave_allocation_count"] == 1_986
    assert fixed["q011fq_compatible_wave_allocation_digest_sha256"] == (
        q011fr.q011fq.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011fr._protocol_globals_are_restored()


def test_q011fr_enumerates_registered_label_free_phase_inventory(
    q011fr_structure: dict[str, Any],
) -> None:
    allocation = q011fr_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 258_720
    assert allocation["full_allocation_digest_sha256"] == (q011fr.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 14_578
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011fr.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011fr.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011fr.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 665
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011fr.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_forty_six_totals"] == [13, 9, 5, 6, 1]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fr result is not sealed")
def test_q011fr_certifies_all_complex_phase_product_discs(
    q011fr_cycle: dict[str, Any],
) -> None:
    comparison = q011fr_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 46
    ...
    assert comparison["comparison_stream_domain"] == ("q011fr-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011fr comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fr result is not sealed")
def test_q011fr_minimum_exact_phase_margin_is_fixed(
    q011fr_cycle: dict[str, Any],
) -> None:
    comparison = q011fr_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011fr minimum phase witness has not been sealed yet")
    ...
    assert q011fr.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fr result is not sealed")
def test_q011fr_records_scoped_resolution(q011fr_cycle: dict[str, Any]) -> None:
    assert q011fr_cycle["study_validity"] == "passed"
    assert q011fr_cycle["failed_validity_order"] == []
    assert q011fr_cycle["failed_diagnostic_order"] == []
    assert len(q011fr_cycle["validity_gates"]) == 7
    assert len(q011fr_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011fr_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011fr_cycle["diagnostic_gates"].values())
    assert q011fr_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011fr_cycle["diagnostic_classification"] == q011fr.RESOLVED_CLASSIFICATION
    assert q011fr_cycle["scientific_outcome"] == "not_evaluated"
    assert q011fr_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011fs" in q011fr_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011fr_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fr result is not sealed")
def test_q011fr_preserves_scientific_boundary(q011fr_cycle: dict[str, Any]) -> None:
    theorem = q011fr_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_forty_seventh_q011cb_witness"]
    assert not theorem["forty_seventh_q011cb_witness_persists_under_component_safe_phase_discs"]
    assert theorem["q011an_component_internal_eigenvalue_labels_are_assumed"] is False
    assert theorem["q011fq_interval_inert_diagnostic_is_preserved"]
    ...
    assert "flatten ordinal 46" in q011fr_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 45" in q011fr_cycle["claim_boundary"]
    assert "later 44753 Q011cb refined signatures" in q011fr_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011fr_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fr result is not sealed")
def test_q011fr_cycle_has_strict_reproducible_digests(
    q011fr_cycle: dict[str, Any],
) -> None:
    json.dumps(q011fr_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011fr_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011fr_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011fr_cycle["result_digest_sha256"] == (
        q011fr.q011b._canonical_json_sha256(q011fr._result_digest_sections(q011fr_cycle))
    )
    assert q011fr._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011fr result is not sealed")
def test_q011fr_study_metadata_and_optional_artifact_are_scoped(
    q011fr_study: dict[str, Any],
) -> None:
    assert q011fr_study["schema_version"] == 1
    assert q011fr_study["source"] == source_metadata()
    assert q011fr_study["study_gate"] == "passed"
    assert q011fr_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011fr_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 14_578
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011fr_study["mathematical_scope"]
    ...
    json.dumps(q011fr_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011fr runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011fr artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011fr.q011b._canonical_json_sha256(q011fr._result_digest_sections(artifact["cycle"]))
    )
