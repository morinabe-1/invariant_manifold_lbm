...
EXPECTED_SECTION_DIGESTS: dict[str, str] | None = {
    "input_digest_sha256": "09cf1865fd16f691c3db31b5009e9c0de330d548ea7e9ccd300bda4ed6851705",
    "phase_input_digest_sha256": "12f490d0d61531d5c5e6e90b15b0dea970479d4aa669412a5d8af0685858736f",
    "allocation_digest_sha256": "8b7ee5dac8a442c1b38b086f3c7a31c251f0c19b05e33005f240d109cd36cd14",
    "phase_comparison_digest_sha256": "6fbcd162e8e0623cb1e2804e3ed294a122835735dbc50d7e6ca207e899f682f4",
    "result_digest_sha256": "549292ba84fb6cb4984c9abe06915a55466dbf4698239e1d59760467457baf0b",
}
...
def q011gd_structure() -> dict[str, Any]:
    sealed, artifacts = q011gd._sealed_input_audit()
    fixed, sources, target, wave_compatible = q011gd._fixed_phase_input_audit(artifacts)
    allocation, compatible = q011gd._allocation_inventory(sources, wave_compatible)
    return {
        "sealed": sealed,
        "fixed": fixed,
        "target": target,
...
@pytest.fixture(scope="module")
def q011gd_study() -> dict[str, Any]:
    return q011gd.run_q011gd_study()


@pytest.fixture(scope="module")
def q011gd_cycle(q011gd_study: dict[str, Any]) -> dict[str, Any]:
    return q011gd_study["cycle"]


def test_q011gd_seals_q011gc_and_all_prior_inputs(
    q011gd_structure: dict[str, Any],
) -> None:
    sealed = q011gd_structure["sealed"]
    assert sealed["passed"]
    assert sealed["artifact_count"] == 164
    assert sealed["direct_digest_count"] == 750
    assert all(sealed["checks"].values())
    assert tuple(sealed["q011gc"]["digests"]) == q011gd.Q011GC_DIGESTS
    assert sealed["q011gc"]["artifact_sha256"] == q011gd.Q011GC_ARTIFACT_SHA256
    assert sealed["q011gc"]["runner_sha256"] == q011gd.Q011GC_RUNNER_SHA256


def test_q011gd_reconstructs_component_safe_phase_discs(
    q011gd_structure: dict[str, Any],
) -> None:
    fixed = q011gd_structure["fixed"]
    assert fixed["passed"]
    assert all(fixed["checks"].values())
    assert fixed["degree"] == 34
    assert fixed["parent_q011bx_aggregate_index"] == 2340
    assert fixed["parent_q011cb_local_aggregate_index"] == 0
    assert fixed["parent_flat_ordinal"] == 52
    assert fixed["target_identifier"] == "block=7;center=44"
    assert fixed["source_variant_count"] == 12
    assert fixed["source_phase_disc_record_digest_sha256"] == (q011gd.EXPECTED_SOURCE_RECORD_DIGEST)
    assert fixed["target_phase_disc_record_digest_sha256"] == (q011gd.EXPECTED_TARGET_RECORD_DIGEST)
    assert tuple(fixed["source_radius_binary64_hex"]) == (q011gd.EXPECTED_SOURCE_RADIUS_HEX)
    assert fixed["target_radius_binary64_hex"] == q011gd.EXPECTED_TARGET_RADIUS_HEX
    ...
    assert fixed["q011gc_compatible_wave_allocation_count"] == 2_646
    assert fixed["q011gc_compatible_wave_allocation_digest_sha256"] == (
        q011gd.q011gc.EXPECTED_COMPATIBLE_DIGEST
    )
    assert "no component-internal eigenvalue label" in fixed["label_semantics"]
    assert q011gd._protocol_globals_are_restored()


def test_q011gd_enumerates_registered_label_free_phase_inventory(
    q011gd_structure: dict[str, Any],
) -> None:
    allocation = q011gd_structure["allocation"]
    assert allocation["passed"]
    assert all(allocation["checks"].values())
    assert allocation["full_allocation_count"] == 369_600
    assert allocation["full_allocation_digest_sha256"] == (q011gd.EXPECTED_FULL_ALLOCATION_DIGEST)
    assert allocation["compatible_allocation_count"] == 20_786
    assert allocation["compatible_allocation_digest_sha256"] == (
        q011gd.EXPECTED_COMPATIBLE_ALLOCATION_DIGEST
    )
    ...
    assert allocation["individual_to_component_bridge_record_digest_sha256"] == (
        q011gd.EXPECTED_BRIDGE_RECORD_DIGEST
    )
    ...
    assert allocation["bridge_phase_paired_record_digest_sha256"] == (
        q011gd.EXPECTED_PAIRED_RECORD_DIGEST
    )
    assert allocation["wave_projection_count"] == 945
    assert allocation["wave_projection_record_digest_sha256"] == (
        q011gd.EXPECTED_WAVE_PROJECTION_DIGEST
    )
    ...
    assert adapter["ordinal_fifty_two_totals"] == [13, 9, 5, 4, 3]
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gd result is not sealed")
def test_q011gd_certifies_all_complex_phase_product_discs(
    q011gd_cycle: dict[str, Any],
) -> None:
    comparison = q011gd_cycle["complex_phase_product_disc_audit"]
    assert comparison["passed"]
    assert all(comparison["checks"].values())
    assert comparison["parent_flat_ordinal"] == 52
    ...
    assert comparison["comparison_stream_domain"] == ("q011gd-component-safe-phase-comparisons-v1")
    if not EXPECTED_STREAM_DIGEST:
        pytest.skip("Q011gd comparison stream digest has not been sealed yet")
...
@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gd result is not sealed")
def test_q011gd_minimum_exact_phase_margin_is_fixed(
    q011gd_cycle: dict[str, Any],
) -> None:
    comparison = q011gd_cycle["complex_phase_product_disc_audit"]
    ...
    if EXPECTED_MINIMUM_INDEX is None or EXPECTED_MINIMUM_COUNTS is None:
        pytest.skip("Q011gd minimum phase witness has not been sealed yet")
    ...
    assert q011gd.q011z._fraction(witness["complex_separation_margin_lower"]["exact"]) > 0
    assert witness["witness_digest_sha256"] == EXPECTED_MINIMUM_WITNESS_DIGEST


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gd result is not sealed")
def test_q011gd_records_scoped_resolution(q011gd_cycle: dict[str, Any]) -> None:
    assert q011gd_cycle["study_validity"] == "passed"
    assert q011gd_cycle["failed_validity_order"] == []
    assert q011gd_cycle["failed_diagnostic_order"] == []
    assert len(q011gd_cycle["validity_gates"]) == 7
    assert len(q011gd_cycle["diagnostic_gates"]) == 4
    assert all(gate["passed"] for gate in q011gd_cycle["validity_gates"].values())
    assert all(gate["passed"] for gate in q011gd_cycle["diagnostic_gates"].values())
    assert q011gd_cycle["refinement_outcome"] == "component_safe_phase_resolved"
    assert q011gd_cycle["diagnostic_classification"] == q011gd.RESOLVED_CLASSIFICATION
    assert q011gd_cycle["scientific_outcome"] == "not_evaluated"
    assert q011gd_cycle["actual_resonance_outcome"] == "not_established"
    assert "Q011ge" in q011gd_cycle["next_change"]
    assert "next Q011cb refined overlap" in q011gd_cycle["next_change"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gd result is not sealed")
def test_q011gd_preserves_scientific_boundary(q011gd_cycle: dict[str, Any]) -> None:
    theorem = q011gd_cycle["theorem_consequence"]
    assert theorem["component_safe_complex_phase_discs_resolve_fifty_third_q011cb_witness"]
    assert not theorem["fifty_third_q011cb_witness_persists_under_component_safe_phase_discs"]
    ...
    assert "flatten ordinal 52" in q011gd_cycle["claim_boundary"]
    assert "does not reevaluate ordinals 0 through 51" in q011gd_cycle["claim_boundary"]
    assert "later 44747 Q011cb refined signatures" in q011gd_cycle["claim_boundary"]
    assert "does not establish an actual resonance" in q011gd_cycle["claim_boundary"]


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gd result is not sealed")
def test_q011gd_cycle_has_strict_reproducible_digests(
    q011gd_cycle: dict[str, Any],
) -> None:
    json.dumps(q011gd_cycle, allow_nan=False)
    assert EXPECTED_SECTION_DIGESTS is not None
    assert {name: q011gd_cycle[name] for name in EXPECTED_SECTION_DIGESTS} == (
        EXPECTED_SECTION_DIGESTS
    )
    comparison = q011gd_cycle["complex_phase_product_disc_audit"]
    assert comparison["comparison_stream_digest_sha256"] == EXPECTED_STREAM_DIGEST
    assert q011gd_cycle["result_digest_sha256"] == (
        q011gd.q011b._canonical_json_sha256(q011gd._result_digest_sections(q011gd_cycle))
    )
    assert q011gd._protocol_globals_are_restored()


@pytest.mark.skipif(not RESULT_EXPECTATIONS_FIXED, reason="Q011gd result is not sealed")
def test_q011gd_study_metadata_and_optional_artifact_are_scoped(
    q011gd_study: dict[str, Any],
) -> None:
    assert q011gd_study["schema_version"] == 1
    assert q011gd_study["source"] == source_metadata()
    assert q011gd_study["study_gate"] == "passed"
    assert q011gd_study["refinement_outcome"] == "component_safe_phase_resolved"
    runtime = q011gd_study["arithmetic_runtime"]
    assert runtime["target_comparisons"] == 20_786
    assert runtime["full_comparison_records_retained"] is False
    assert runtime["floating_point_used_for_gate_decisions"] is False
    assert runtime["protocol_globals_restored_after_use"] is True
    scope = q011gd_study["mathematical_scope"]
    ...
    json.dumps(q011gd_study, allow_nan=False)

    if EXPECTED_RUNNER_SHA256 is None:
        pytest.skip("Q011gd runner hash has not been sealed yet")
    ...
    if not artifact_path.exists():
        pytest.skip("Q011gd artifact has not been generated yet")
    ...
    assert artifact["cycle"]["result_digest_sha256"] == (
        q011gd.q011b._canonical_json_sha256(q011gd._result_digest_sections(artifact["cycle"]))
    )
